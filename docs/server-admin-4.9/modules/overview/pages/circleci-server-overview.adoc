= CircleCI Server 4.9 overview
:page-platform: server
:page-vcs: github, github-enterprise
:page-server-min-version: "4.9"
:page-audience: admin
:page-description: CircleCI Server 4.9 is a continuous integration and continuous delivery (CI/CD) platform that you can install on your GCP or AWS Kubernetes cluster.
:experimental:
...
.CircleCI Server 4.9 Architecture
image::guides:ROOT:server-4-architecture-diagram-2024.png[server 4.9 services architecture]
...
CircleCI Server 4.9 consists of the following services. Find their descriptions and failure implications below:
...
These metrics can also be configured to be exported to other places (such as Datadog or Prometheus).
|
|
...
* xref:release-notes.adoc#[CircleCI Server 4.9 Release Notes].
* server 4.9 installation prerequisites for xref:installation:phase-1-aws-prerequisites.adoc#[AWS] and xref:installation:phase-1-gcp-prerequisites.adoc#[GCP].
endif::pdf[]
