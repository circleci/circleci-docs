...
- A SeaweedFS (S3 compatible storage) instance running at seaweedfs.internal.example.com, with its API listening on port 9000, and no TLS
...
* xref:installation:phase-2-aws-core-services.adoc[Phase 2 AWS - Core Services]
* xref:installation:phase-2-gcp-core-services.adoc[Phase 2 GCP - Core Services]
...
object_storage:
  bucketName: "circleci-data" # Update to the name of the bucket created in SeaweedFS
  expireAfter: 0
  s3:
    enabled: true
    endpoint: "http://seaweedfs.internal.example.com:8333"
    accessKey: "<<seaweedfs-username>>"
    secretKey: "<<seaweedfs-password>>"

# Distributor using CircleCI Agent in SeaweedFS
distributor:
  agent_base_url: http://seaweedfs.internal.example.com:8888/buckets/circleci-data
  launch_agent_base_url: http://seaweedfs.internal.example.com:8888/buckets/circleci-data
