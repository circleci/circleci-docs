...
:page-audience: admin
:page-description: This page presents some items that should be considered when starting an air-gapped installation of CircleCI Server 4.9.
:experimental:
...
----
+
The address pool can be named something other than "default", but the annotations in `values.yaml` will need to be updated. If there is only one k3s node, the address range must have the same IP repeated (for example, `10.0.0.5-10.0.0.5`).
...
. After installing the Helm chart, patch the `circleci-proxy` service to use the internal IP of the desired k3s node to act as the load balancer. This IP must be in the range entered in the ConfigMap above. The example below uses the IP address `10.0.0.5`.
+
[source, bash]
----
