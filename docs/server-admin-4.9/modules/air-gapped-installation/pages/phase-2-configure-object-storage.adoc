...
:page-description: How to configure object storage through SeaweedFS to run CircleCI Server 4.9 in an air-gapped environment.
:experimental:

pass:[<!-- vale off -->]
[#create-buckets-in-seaweedfs]
== 1. Create buckets in SeaweedFS
CircleCI Server supports S3 compatible storage for air-gapped object storage. For our use-case we have used [SeaweedFS](https://github.com/seaweedfs/seaweedfs). SeaweedFS is used in the air-gapped environment as a replacement for cloud-based object storage, such as GCP GCS or AWS S3. With an air-gapped SeaweedFS instance installed, complete the steps outlined in the next sections.

image::guides:ROOT:seaweedfs/seaweedfs_install_0.png[Create a bucket in SeaweedFS]

[#creates-data-bucket]
=== a. Create a `circleci-data` bucket.
Create a new bucket in SeaweedFS named `circleci-data`.

NOTE: This bucket can be given a different name, but references to it will need to be updated in the Helm `values.yaml` file.

image::guides:ROOT:seaweedfs/seaweedfs_created_buckets.png[Buckets created in SeaweedFS]

[#copy-build-agent]
== 2. Copy the CircleCI build agent
Follow the steps in this section to copy the pinned CircleCI build agent into SeaweedFS within your air-gapped environment.
...
[#copy-release-txt]
=== b. Create a release.txt file and copy to SeaweedFS
Create a `release.txt` file with the value of `$CIRCLE_AGENT_VERSION`. Copy this file to your air-gapped environment and place it in the root of the `circleci-data` bucket in SeaweedFS.
...
Using the version specified by `release.txt`, create a new directory in the root of the `circleci-data` bucket with the name of that release. In the following examples, a directory is created at the root of `circleci-data` in SeaweedFS with the name `$CIRCLE_AGENT_VERSION`. Note `$CIRCLE_AGENT_VERSION` is not a literal string, but the value of the environment variable we set earlier.

[#upload-checksums-file]
=== e. Upload the checksums.txt file to the newly created directory.
Copy the downloaded `checksums.txt` file (step c) to your virtual environment, and place it in SeaweedFS nested under the newly created release directory (step d).
...
image::guides:ROOT:seaweedfs/seaweedfs_checksum.png[Uploading `checksums.txt` into the release directory]
...
image::guides:ROOT:seaweedfs/seaweedfs_create_linux_dir.png[Creating a Linux dir]

image::guides:ROOT:seaweedfs/seaweedfs_create_amd_dir.png[Creating an AMD dir]
...
image::guides:ROOT:seaweedfs/seaweedfs_upload_cci_agent.png[Uploading the CircleCI agent]
...
image::guides:ROOT:seaweedfs/seaweedfs_provisioner_agent.png[Docker/Machine Agents]


[#copy-other-miscellaneous-files]
== 4. Copy other miscellaneous files
Follow the steps in this section to copy the required files into SeaweedFS within your air-gapped environment.
...
image::guides:ROOT:seaweedfs/seaweedfs_canary.png[Docker/Machine Agents]

== 5. S3 bucket data tree
After uploading all required binaries and files, S3 bucket structure will be as below - 

image::guides:ROOT:seaweedfs/seaweedfs_dir_tree.png[Docker/Machine Agents]
