...
:page-audience: admin
:page-description: Learn how to install CircleCI Server 4.9 behind a proxy.
:experimental:
...
Configuring a proxy happens during phase 2, core services (xref:phase-2-aws-core-services.adoc#l-installing-behind-a-proxy[AWS], xref:phase-2-gcp-core-services.adoc#l-installing-behind-a-proxy[GCP]).

[#known-limitations]
== Known limitations

* Some additional configuration is required to import orbs when installed behind a proxy. See xref:operator:managing-orbs.adoc#using-orbs-behind-a-proxy[Managing Orbs on Server] docs for more information.
...
* If your GitHub instance is running outside of the proxied environment (either GitHub.com or GitHub Enterprise), you must ensure SSH traffic can reach your instance. This traffic comes from CircleCI (inside the Kubernetes cluster) and from our Nomad node. Otherwise, the default `checkout` step in a CircleCI job will fail to clone code, and our `ssh-keyscan` of GitHub Enterprise will not work. While you may configure an SSH proxy, `ssh-keyscan` can NOT be proxied. Instead, you must provide `github.fingerprint` when using GHE.
