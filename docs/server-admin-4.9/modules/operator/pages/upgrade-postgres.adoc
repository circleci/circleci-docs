...
:page-description: Learn how to upgrade the internal PostgreSQL database from version 12 to 14 in an installation of CircleCI Server 4.9.
:experimental:

PostgreSQL is a database service used by CircleCI Server. Server 4.9 ships PostgreSQL `12`. This page describes how to upgrade the internal PostgreSQL instance from version `12` to `14`.

NOTE: PostgreSQL 12 reached end of life in November 2024 and no longer receives security or bug fixes. Upgrade to PostgreSQL 14 to stay on a supported version.

This is an in-place major-version upgrade that uses `pg_upgrade` to migrate the on-disk data format. The upgrade requires downtime, typically 30 to 60 minutes depending on your database size and storage performance. All CircleCI services are unavailable during the upgrade, so schedule it during a maintenance window.
...
* The cluster has network access to the PostgreSQL 14 image (`14.22.x`) in your Azure Container Registry (ACR).
...
----
helm upgrade circleci-server oci://cciserver.azurecr.io/circleci-server -n <namespace> --version {serverversion49} -f <path-to-values.yaml> --username $USERNAME --password $PASSWORD
----
