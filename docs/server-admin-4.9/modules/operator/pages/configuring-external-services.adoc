...
NOTE: Your externalized PostgreSQL instance needs to be version 12 or higher. The current version we use for testing is 12, which is also the version we ship when using an internalized PostgreSQL. You can also use services that are fully compatible with PostgreSQL, such as Amazon RDS for PostgreSQL.
