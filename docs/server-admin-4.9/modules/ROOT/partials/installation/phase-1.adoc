...
| Install and configure link:AWS CLI for your S3 compatible object storage provider if you choose not to use AWS or GCP object storage.
| TBC
...
CircleCI Server installs into an existing Kubernetes cluster. If you have not already created a cluster, you should do so next. Instructions for creating a cluster are provided below, or if you are installing locally, first consider the sizing, version, and permissions requirements.
...
* link:https://github.com/seaweedfs/seaweedfs[SeaweedFS]

While any S3 compatible object storage may work, we test and support AWS S3 and SeaweedFS. Follow the instructions below to create a bucket and access method for AWS S3 or GCS.
