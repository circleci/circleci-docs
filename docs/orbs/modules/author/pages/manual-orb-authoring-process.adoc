= Registry orb authoring process (manual)
:page-platform: Cloud, Server v4+
:page-description: Authoring simple orbs manually without the orb development kit.
:experimental:
:page-aliases: orb-author-validate-publish

This guide covers the steps required to create a simple registry xref:use:orb-intro.adoc[Orb], manually, without using the orb development kit. We recommend the xref:orb-author.adoc#orb-development-kit[Orb Development Kit] for most registry orb projects.

[NOTE]
====
This guide applies to **registry orbs only**. For inline orbs, see xref:create-an-inline-orb.adoc[Create an Inline Orb]. For URL orbs, see xref:create-test-and-use-url-orbs.adoc[Create, Test, and Use URL Orbs].
====
