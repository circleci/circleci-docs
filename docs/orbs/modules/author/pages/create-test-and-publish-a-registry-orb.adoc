...
* Install the link:https://cli.circleci.com/[CircleCI CLI] on your local machine with a link:https://app.circleci.com/settings/user/tokens[personal access token]. More information is available for the CLI on the xref:guides:toolkit:circleci-cli.adoc[CircleCI CLI] page.
