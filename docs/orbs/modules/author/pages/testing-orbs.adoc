...
[#testing-by-orb-type]
=== Testing approaches by orb type

The testing methods available depend on which type of orb you are authoring:

[.table-scroll]
--
[cols="2,1,1,1", options="header"]
|===
| Testing Method | Inline Orbs | URL Orbs | Registry Orbs
...
^1^ _Inline orbs are tested directly in the config where they are defined._

^2^ _Available via the orb development kit._

NOTE: The majority of this guide focuses on **registry orb** testing using the orb development kit. For inline orbs, validation and integration testing are the primary methods. For URL orbs, most testing methods apply except for the automated orb development kit pipeline.

For registry orbs, the orb development kit provides a simple path to implementing a full range of robust tests.

video::kTeRJrwxShI[youtube]

[#orb-tools-pipeline-overview]
== Orb-tools pipeline overview (registry orbs only)

NOTE: This section applies to **registry orbs** created with the orb development kit. Inline orbs and URL orbs do not use this automated pipeline.

If you are following this guide and have created your registry orb using the orb development kit, your orb project will follow the same structure as the link:https://github.com/CircleCI-Public/Orb-Template[Orb Template]. If you look inside your `.circleci/` directory, you will find two config files, `config.yml` and `test-deploy.yml`, each of which contains a set of tests to run.
...
NOTE: Validation applies to **all orb types** (inline, URL, and registry orbs). However, the automated pipeline is only available for registry orbs using the orb development kit.

The most basic forms of testing for orbs are configuration validation and code linting. All orbs must be both valid YAML and valid CircleCI syntax.

For registry orbs, these checks are automatically applied when using the orb development kit, through the CI/CD pipeline set out in the project's config file at `.circleci/config.yml`. For inline orbs and URL orbs, validation can be performed manually using the CircleCI CLI.
...
In addition to YAML Linting, we must validate the "packed" `orb.yml` file to ensure it properly conforms to orb schema. We first xref:use:orb-concepts.adoc#orb-packing[pack] the orb to combine the multiple source files into an `orb.yml`. Then we run the `circleci orb validate` command for schema checking.
...
NOTE: ShellCheck applies to **URL orbs** and **registry orbs** that use the xref:use:orb-concepts.adoc#file-include-syntax[file include syntax] to import external scripts. Inline orbs typically don't use external scripts, so ShellCheck is less relevant.

One of the major benefits of using the orb development kit is the ability to xref:use:orb-concepts.adoc#file-include-syntax[import external Bash scripts] into your final orb. Because you can keep your Bash scripts in the link:https://github.com/CircleCI-Public/Orb-Template/tree/main/src/scripts[src/scripts] directory, you can run additional tests against your scripts.
...
NOTE: The review job is available for **URL orbs** and **registry orbs**. It can be used manually for URL orbs or automatically via the orb development kit for registry orbs.
...
NOTE: Unit testing applies to **URL orbs** and **registry orbs** that use external scripts with the xref:use:orb-concepts.adoc#file-include-syntax[file include syntax]. Inline orbs typically don't use external scripts, so unit testing is less relevant.

If you are taking advantage of the orb development kit's xref:use:orb-concepts.adoc#file-include-syntax[`<<include(file)>>` file inclusion] feature and `src/scripts` directory to store and source your Bash files, you can write true unit tests for your scripts.
...
NOTE: Integration testing applies to **all orb types** (inline, URL, and registry orbs). The approach differs slightly: inline orbs are tested directly in the config where they're defined, URL orbs are tested by referencing them via URL, and registry orbs are tested using development versions in automated pipelines.

After validating, linting, shellchecking, and any other testing that you can perform on the source code is complete, you must test your orb's functionality in a real CircleCI config.

For **registry orbs**, in the second config file (`test-deploy.yml`), you can access the development version of the orb you published in the first config, and attempt to execute your orbs commands and jobs.

For **URL orbs**, you can create a test project that references your URL orb and validates its functionality.

For **inline orbs**, integration testing happens naturally as the orb is used within the same config file where it's defined.
...
* Refer to xref:use:orb-concepts.adoc#orb-types-comparison[Orb types comparison] for understanding the differences between inline, URL, and registry orbs.
* Refer to xref:use:orb-concepts.adoc[Orbs Concepts] for high-level information about CircleCI orbs.
* Refer to xref:creating-orbs.adoc[Orb Publishing Process] for information about publishing registry orbs.
