= Registry orb testing methodologies
...
The first config file publishes a development version of our orb, so that you may run integration tests against it in the second workflow, `test-deploy`. At this point, because the orb is not yet published, you can not test the orb _directly_, but you can still lint, validate, review, and run unit tests against your scripts.
...
Now the development version of the orb has been published, this second configuration file has two main tasks:

* Directly test your orb with integration testing.
* In the event that a tag is created, this config will also publish the orb to the CircleCI orb registry.
...
One of the major benefits of using the orb development kit is the ability to xref:use:orb-concepts.adoc#file-include-syntax[Import External Bash Scripts] into your final orb. Because you can keep your Bash scripts in the `src/scripts` link:https://github.com/CircleCI-Public/Orb-Template/tree/main/src/scripts[directory], you can run additional tests against your scripts.
...
The orb-tools orb includes a job `orb-tools/review` which will run a suite of tests against your orb designed to find opportunities to implement best practices and improve quality. The "review" job was modeled closely after _ShellCheck_, and operates based on a list of rules called "RC" Review Checks. Each "RC" code corresponds to a specific rule, which can optionally be ignored using the `exclude` parameter in your `config.yaml` file.
...
.Example of orb-tools review check RC008
image::guides:ROOT:orbtools-rc008.png[orb-tools review check RC008]

When you click into the error you will receive more information including the file and line number in the code where the error was found, and a suggestion for resolution.

CAUTION: The `orb-tools/review` job currently cannot be run locally due to the fact that the results are output as JUnit XML and uploaded to CircleCI, which is not supported by the local execute command at this time.

[#unit-testing]
== Unit testing

If you are using the ++`<<include(file)>>`++ xref:use:orb-concepts.adoc#file-include-syntax[File Inclusion] feature and `src/scripts` directory to store and source your Bash files, you can write true integration tests for your scripts.

.Example of unit testing Bash with BATS-Core
...
You will see a job in your `test-deploy.yml` file named `command-tests`. This example job will run one or all of your commands as an integration test.
...
By default you will see the included "greet" command is being tested. Because the greet command only outputs a message to stdout, you cannot do any additional validation checks.
...
In this example, we are testing the `github-cli/install` command. This command may pass or fail, but we can also validate in the next step that the GitHub CLI is installed and is available on the command line. If the `gh` binary is not found in the path, the job will fail.
...
In your `test-deploy` workflow, as we mentioned with testing commands above, there is ultimately an `orb-tools/publish` job which requires every job before it in the workflow to have completed. To test the jobs of your orb, add them to this workflow and ensure they are required in the `orb-tools/publish` job.
  ...
  test-deploy:
    jobs:
      - aws-ecr/build-and-push-image:
          ...
          filters: pipeline.git.tag matches /^v[0-9]+\.[0-9]+\.[0-9]+$/
# ...
      - orb-tools/publish:
          ...
          filters: pipeline.git.branch matches /^v[0-9]+\.[0-9]+\.[0-9]+$/
----

The AWS ECR orb contains a job named "build-and-push-image", which builds and pushes an image to the AWS ECR repository. We run this job and others with multiple parameter options to test their functionality with each code change.

Similar to how we use additional steps to test our commands, we can take advantage of link:https://circleci.com/docs/configuration-reference/#pre-steps-and-post-steps[post-steps] to validate in the job environment. Alternatively, as shown in this example, we can "clean up" anything we may have created in the job. Post-Steps are additional steps that can be injected at the end of an existing job.
