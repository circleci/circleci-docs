...
Orbs utilize a special config key xref:author:create-test-and-publish-a-registry-orb.adoc#orbyml[`display`] that can hold a `source_url` for linking to your Git repository, which in turn holds the orb source code and `home_url` to link to the product or service home page if applicable.
