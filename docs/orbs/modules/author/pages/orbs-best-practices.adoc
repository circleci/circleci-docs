= Registry orb authoring best practices
...
[#general]
== General
This guide covers best practices for creating registry orbs.

[#give-your-orb-a-descriptive-name]
=== Give your orb a descriptive name

An orb "slug" is made up of a _namespace_ and _orb_ name separated by a forward slash. The namespace represents the individual, company, or organization that owns and maintains the orb. The orb name itself should describe the product, service, or action provided by the individual orb.
...
When orbs are published to the Orb Registry they are searchable by their name and description. Good descriptions give your users a better idea of the purpose and functionality of your orb. They are also important for search optimization.

[#include-display-links]
==== Include display links

Orbs utilize a special config key `display` that can hold a `source_url` for linking to your Git repository. This repository holds the orb source code. The `home_url` links to the product or service home page if applicable.
...
As you can see, this job utilizes an executor named `default` which accepts a `version` parameter. To enable the user of this _job_ to set the `version` parameter in the _executor_, we must create the parameter in our job and pass it to our other orb components.
...
In orb development, executors are often used to provide or utilize a specific execution environment when we have multiple jobs which can only run in that environment. For example, if your orb relies on a specific Docker container and includes two jobs and no commands, it makes sense to abstract the execution environment. You can use a single xref:reference:ROOT:reusing-config.adoc#authoring-reusable-executors[Authoring Reusable Executors] for both jobs.
...
Each usage example must present a full example including showing the orb being imported. The version number displayed in the usage-example should match the currently published orb. For example, if your orb is currently on version `0.1.0` and you open a pull request to publish version `1.0.0`, update your usage examples to reflect the new version.
...
CircleCI ships with the xref:guides:security:env-vars.adoc#environment-variable-substitution[Environment Variable Substitution] CLI (`envsubst`) built in, which allows us to evaluate environment variables in strings.
...
When building orbs, it is typical to xref:use:orb-concepts.adoc#file-include-syntax[File Include Syntax] to avoid some of the pitfalls of YAML. This also gives you access to syntax highlighting and other features in your IDE. In this example, we are including a script named `your_script.sh` which contains the following code:
