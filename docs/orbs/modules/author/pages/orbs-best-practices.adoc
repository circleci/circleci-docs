...
An orb "slug" is made up of a _namespace_ and _orb_ name separated by a forward slash. The namespace represents the individual, company, or organization that owns and maintains the orb. The orb name itself describes the product, service, or action provided by the individual orb.
...
Commands, Jobs, Executors, Examples, and Parameters can all accept descriptions. Ensure each and every component of your orb has a helpful description and provides any additional documentation that may be needed.
...
The `@orb.yml` file acts as the "root" of our project. It contains much of the meta-data for our orb, which will appear on the Orb Registry as well as the CLI.

[#all-orbs-should-include-a-description]
==== Include a description for every orb
...
Orbs utilize a special config key xref:author:create-test-and-publish-a-registry-orb.adoc#orbyml[`display`] that can hold a `source_url` for linking to your Git repository, which in turn holds the orb source code. It can also hold a `home_url` to link to the product or service home page, if applicable.
...
As you can see, this job utilizes an executor named `default` which accepts a `version` parameter. To let the user of this _job_ set the `version` parameter in the _executor_, create the parameter in your job. Then, pass the parameter to your other orb components.
...
In orb development, executors are often used to either provide or utilize a specific execution environment when we have multiple jobs which can only run in that environment. For example, say your orb relies on a specific Docker container and includes two jobs and no commands. In that case, it makes sense to abstract the execution environment into a single xref:reference:ROOT:reusing-config.adoc#authoring-reusable-executors[Reusable Executor] to be used for both jobs.
...
Orb xref:use:orb-concepts.adoc#usage-examples[Usage Examples] provide an excellent way for orb developers to share use-cases and best practices with the community. Usage examples act as the main source of documentation users will reference when utilizing an orb, so it is important to include clear and useful examples.
...
==== Include at least one usage example in public orbs

Orbs intended for consumption by other organizations must include at least one usage example, with a description.

[#use-case-based-examples]
==== Use-case based examples

Name each included usage example for a specific use-case, to instruct the user in how to accomplish a task. Example: `install_cli_and_deploy`, `scan_docker_container`, or `test_application_with_this-tool`

[#show-correct-orb-version]
==== Show correct orb version

Each usage example must present a full example, including the orb import. The version number displayed in the usage example must match the currently published orb. For example, if your orb is currently on version `0.1.0` and you open a pull request to publish version `1.0.0`, update your usage examples to reflect version `1.0.0` of the orb in use.

[#parameters]
=== Parameters

[#secrets-should-never-be-directly-entered]
==== Never directly enter secrets

Never enter any information that could be considered "secret", such as API keys, auth tokens, and passwords, directly as a parameter value. Instead, use the xref:reference:ROOT:reusing-config.adoc#environment-variable-name[`env_var_name` parameter] type, which expects the string value of the name of the environment variable that contains the secret information. The `env_var_name` parameter type will fail validation if the parameter value is not a POSIX-compliant environment variable name. This validation rule is designed to assist in preventing users from accidentally entering the secret value directly as a string.

If your orb requires an "API key", create a parameter named `api_key` of type `env_var_name`. The developer using your orb would then pass the name of the environment variable containing the API key as the value of the `api_key` parameter.
...
When building orbs, it is typical to xref:use:orb-concepts.adoc#file-include-syntax[Write Your Code in a Separate File] to avoid some of the pitfalls of YAML. This also gives you access to syntax highlighting and other features in your IDE. In this example, we are including a script named `your_script.sh` which contains the following code:
...
Going through this substitution process allows us to accept a string value, or an environment variable, and use it in our script. Keep in mind, it is recommended to <<secrets-should-never-be-directly-entered,use the `env_var_name` parameter type for secrets>>, which must never be entered directly.

[#parameterize-the-installation-path]
==== Parameterize the installation path

When installing any binary into a potentially unknown user-defined Docker image, it is hard to know what permissions will be available. Create an `install-path` parameter, ideally with a default value of `/usr/local/bin`, and install binaries to this location (if possible). This often avoids the issue of requiring "root" privileges in environments where that may not be possible.
...
Semantic versioning is a critical update and release practice in which version numbers communicate either bug fixes and patches, new functionality, or breaking changes. Introducing a breaking change as a patch update, for example, can lead to users of that orb automatically receiving updates that block their CI process. Before updating your orbs, make sure you have read over and understood xref:use:orb-concepts.adoc#semantic-versioning[Semantic Versioning].

[#keep-a-changelog]
=== Keep a changelog

Keeping a concise changelog allows users of an orb to quickly see what has changed in a particular version. While git does provide a log of changes, it can be difficult to discover the difference between two versions, especially when commits do not necessarily align to a release. Changelogs must conform to the link:https://keepachangelog.com/en/1.0.0/[Keep a Changelog] guidelines.
