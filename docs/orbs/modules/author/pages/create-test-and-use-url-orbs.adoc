...
[NOTE]
====
This guide covers **URL orbs**. For information on creating inline orbs, see xref:create-an-inline-orb.adoc[Create an inline orb]. For information on creating registry orbs, see xref:orb-author.adoc[Orb authoring process].
====

== When to use URL orbs

Use URL orbs instead of inline orbs when:

* You need to share configuration across multiple projects in your organization.
* You want centralized configuration management.

Use URL orbs instead of registry orbs when:

* You need to use proprietary or confidential configurations.
* You need direct control over updates and distribution.
* You don't need public discoverability or community sharing.
* You want to bypass the orb publishing process and semantic versioning.
...
* Ensure your organization has configured the URL orb allow-list. See xref:use:managing-url-orbs-allow-lists.adoc[Managing URL orbs allow-lists] for steps.
