...
CircleCI orbs are packages of xref:use:orb-concepts.adoc#orb-configuration-elements[Reusable Configuration] that you can import into your CircleCI configuration files. Orbs are available in three types:
...
Before authoring an orb, we recommend that you become familiar with the xref:guides:getting-started:config-intro.adoc[Configuration Reference] and xref:reference:ROOT:reusing-config.adoc[Reusable Configuration Reference] pages.

Orbs consist of three main elements. You can follow the links for each to learn more:

* xref:use:orb-concepts.adoc#commands[Commands]. A CircleCI command is a reusable block of configuration that can be called multiple times in your configuration.
* xref:use:orb-concepts.adoc#jobs[Jobs]. A CircleCI job is a collection of steps that run commands or scripts inside a specified executor (like a Docker container or virtual machine).
* xref:use:orb-concepts.adoc#executors[Executors]. A CircleCI executor is a specified environment in which to run a job.
...
Alternatively, as orbs are packages of xref:reference:ROOT:reusing-config.adoc[Reusable Configuration], they can be written manually too, as singular `yaml` files, and published using the link:https://cli.circleci.com[CircleCI CLI]. For a guide, see the  xref:manual-orb-authoring-process.adoc[Manual Orb Authoring Process] page.
...
* link:https://cli.circleci.com/[CircleCI CLI].
** link:https://cli.circleci.com/reference/[Orb Pack Command].
** link:https://cli.circleci.com/reference/[Orb Init Command].
...
To begin creating orbs, first install the link:https://cli.circleci.com/[CircleCI CLI] on your local machine, with a link:https://app.circleci.com/settings/user/tokens[personal access token]. More information is available for the CLI on the xref:guides:toolkit:circleci-cli.adoc[CircleCI CLI] page.
...
For more information on how to use the `circleci orb` command, see the CLI link:https://cli.circleci.com/reference/[documentation].
...
The list of categories can also be obtained by running the `circleci orb list-categories` CLI command. You can view the detailed docs for this command link:https://cli.circleci.com/reference/[in the CLI documentation].
...
Add your orb to your chosen category by running `circleci orb add-to-category <namespace>/<orb> "<category-name>"`. You can view the detailed docs for this command link:https://cli.circleci.com/reference/[in the CLI documentation].
...
Remove an orb from a category by running `circleci orb remove-from-category <namespace>/<orb> "<category-name>"`. You can view the detailed docs for this command link:https://cli.circleci.com/reference/[in the CLI documentation].
...
To see which categorizations have been applied to an orb, check the output of `circleci orb info <namespace>/<orb>` for a list. You can view the detailed docs for this command link:https://cli.circleci.com/reference/[in the CLI documentation].
