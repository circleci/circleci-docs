...
Before authoring an orb, we recommend that you become familiar with the xref:guides:getting-started:config-intro.adoc[CircleCI config] and authoring xref:reference:ROOT:reusing-config.adoc[parameterized reusable config elements] pages.
...
NOTE: If you find your inline orb would benefit other projects in your organization, you can convert it to a URL orb. If you want to share it publicly, consider converting it to a registry orb.
...
Whether you are writing your first orb or getting ready for production level, we recommend using our <<orb-development-kit>> to get started. A fuill guide is available in the xref:create-test-and-publish-a-registry-orb.adoc[Create, test, and publish a registry orb] guide.
...
[#orb-development-kit]
=== Orb development kit (registry orbs only)
...
[#permissions-matrix]
=== Permissions matrix (registry orbs only)
