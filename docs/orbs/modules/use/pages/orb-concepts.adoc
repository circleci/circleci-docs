...
In order to _pack_ an orb, an xref:author:create-test-and-publish-a-registry-orb.adoc#orbyml[@orb.yml] file must be present. The `@` signifies the _root_ of our orb project. Within the same directory, you can include additional directories for each orb component's type, such as xref:reference:ROOT:reusing-config.adoc#authoring-reusable-commands[Authoring Reusable Commands], xref:reference:ROOT:reusing-config.adoc#authoring-parameterized-jobs[Authoring Parameterized Jobs], xref:reference:ROOT:reusing-config.adoc#authoring-reusable-executors[Authoring Reusable Executors], and <<usage-examples,Usage Examples>>. Any additional files or folders will be safely ignored.
