...
[#usage-examples]
=== Usage examples for registry orbs
...
== Registry orb versions (development vs production vs inline)

Registry orbs can be in one of three states as follows:
...
Registry orbs in _production_ are:

* Immutable, they cannot be deleted or edited, and updates must be provided in a new SemVer release.
* Have a version string that is in SemVer format, for example, `<namespace>/<orb>@1.2.3`.
...
Registry orbs in _development_ are temporary, mutable orb tag versions, useful for rapid development and testing prior to deploying a SemVer deployed production change. A registry orb in _development_:

* Can only be published if the orb has an initial SemVer deployed production version.
* Are mutable, can be overwritten, and automatically expire 90 days after they are published.
* Have a version string that begins with `dev:` followed by any string, for example, `<namespace>/<orb>@dev:my-feature-branch`.
...
Inline orbs are defined directly within the user's configuration file, are completely local and scoped to the individual project.
...
* Exist only locally within the user's configuration file.
* Are not accessible outside of the repository.
* Are not public.
* Are not accessible via CircleCI CLI.
...
All CircleCI orbs are singular YAML files, typically named `orb.yml`. However, for development, it is often easier to break the code up into more manageable chunks. The `circleci orb pack` command, a component of the xref:author:orb-author.adoc#orb-development-kit[orb development kit], is used to "pack" or condense the separate YAML files together.
