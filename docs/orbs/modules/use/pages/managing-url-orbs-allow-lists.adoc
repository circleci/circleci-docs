...
The URL orb allow-list is a security mechanism that restricts orb URL references to only approved URL prefixes. It provides security and access control for organizations using private or external orbs.
...
. Send a *POST* request to the link:https://circleci.com/docs/api/v2/#tag/Organization/operation/createURLOrbAllowListEntry[Create a new URL orb allow-list entry] endpoint. Add an allow-list entry with a JSON map containing name, prefix, and auth keys, as the request body.
...
To use URL orbs from both public and private repositories in the same version control system, create multiple allow-list entries with the same URL prefix but different authentication types.
...
* Once an orb has been fetched the content is cached for 5 minutes. This reduces the load on the remote system hosting the orb, and improves build times for busy projects.
