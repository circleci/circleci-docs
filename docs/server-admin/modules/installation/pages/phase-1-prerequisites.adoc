...
CircleCI server v4.6 is installed as a Helm chart. The installation process is broken down into four phases. Each phase ends with a validation step, allowing you to confirm success before moving to the next phase. Depending on your requirements, phases 3 and 4 may include multiple steps. This installation guide assumes you have already read the xref:overview:circleci-server-overview.adoc[CircleCI server v4.6 overview].
...
CircleCI server installs into an existing Kubernetes cluster. If you have not already created a cluster, you should do so next. The following section covers creating an EKS or GKE cluster, or if you are installing locally, first consider the sizing, version, and permissions requirements.
...
Registering and setting up a new GitHub OAuth app for CircleCI server enables the following:

* Authorization control for your server installation using GitHub OAuth.
...
Once you have created your certificates using one of the methods described below, you can use these commands to retrieve the certificates when you need them during installation:
...
You may have a requirement to terminate TLS for CircleCI server outside the application. Upstream TLS termination is an alternate method to using ACM or supplying the certificate chain during Helm deployment. An example would be a proxy running in front of the CircleCI installation providing TLS termination for your domain name. In this case the CircleCI application acts as the backend for your load balancer or proxy.
...
CircleCI server v4.6 hosts build artifacts, test results, and other state object storage. The following storage options are supported:
...
If your server installation runs in a GKE cluster, ensure that your current IAM user is cluster admin for this cluster. This permissions level is required to create RBAC (role-based access control) objects. More information can be found in the link:https://cloud.google.com/kubernetes-engine/docs/how-to/role-based-access-control[GKE documentation].
