...
* If AWS S3 is used for xref:air-gapped-installation:phase-1-prerequisites.adoc#s3-storage[object storage], the use of Docker layer caching (DLC) requires link:https://docs.aws.amazon.com/AmazonS3/latest/userguide/manage-versioning-examples.html[versioning to be enabled] on the bucket.
