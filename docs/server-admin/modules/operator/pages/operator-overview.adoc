...
This guide contains information for CircleCI server operators, or those responsible for ensuring CircleCI server is running properly through maintenance and monitoring. Before reading this operator guide, ensure you have read the xref:overview:circleci-server-overview.adoc[CircleCI server v4.6 overview].
...
CircleCI server v4.6 uses Nomad as the primary job scheduler. Refer to our xref:introduction-to-nomad-cluster-operation.adoc[Introduction to Nomad Cluster Operation] to learn more about the job scheduler and how to perform basic client and cluster operations.
