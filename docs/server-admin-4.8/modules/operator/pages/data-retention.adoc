...
----
+
By default, the interval is set to `0`. This value must be updated to a number greater than `0` for WFC deletion to run every *n* milliseconds. For example, the following command sets it to 1000 milliseconds or 1 second:
+
[source,clojure]
----
...
If a retention policy of `n` days is configured for both MongoDB and PostgreSQL data, you can set `n+1` for **all objects** in your S3/GCS buckets. This means all objects expire at `n+1` days, aligning database retention with object storage retention.

CAUTION: **Audit Logs** +
Be sure to configure exceptions for critical paths, such as `audit-logs/*`, in accordance with your organization's compliance or audit requirements. By default, do not expire objects under these paths.
