...
[#script-upgrade]
== Scripted upgrade
We have created a shell script which may be used to upgrade your cluster's MongoDB instance. See the link:https://github.com/CircleCI-Public/server-scripts/tree/main/upgrade-mongo-to-4.4[MongoDB upgrade script].
...
. Your `values.yaml` must contain the following snippet:
+
```yaml
mongodb:
  image:
...
. Once the `helm upgrade` has completed and MongoDB has rolled, `exec` into the pod (with the root password handy) to modify the compatibility version. Be sure to replace `<password>` with your MongoDB root password.
+
```bash
...
. You will get a `{ "ok" : 1 }` response from Mongo. Exit out of the MongoDB shell and pod.
...
. You will get `{ "ok" : 1 }` again. Exit out of the shell and pod.
