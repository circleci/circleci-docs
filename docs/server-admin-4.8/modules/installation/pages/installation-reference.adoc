...
|nomad.server.limits.http_max_conns_per_client |string |`""` |Maximum number of concurrent HTTP connections per client IP. When not set, Nomad applies its default of 100.

|nomad.server.limits.rpc_max_conns_per_client |string |`""` |Maximum number of concurrent RPC connections per client IP. When not set, Nomad applies its default of 100. Increase for large clusters with many Nomad clients.
...
|nomad.server.resources.limits.cpu |int |`1` |CPU limit configuration for the Nomad Server deployment.

|nomad.server.resources.limits.memory |string |`"1G"` |Memory limit configuration for the Nomad Server deployment.

|nomad.server.resources.requests.cpu |string |`"50m"` |CPU request configuration for the Nomad Server deployment.

|nomad.server.resources.requests.memory |string |`"100M"` |Memory request configuration for the Nomad Server deployment.
...
|rabbitmq.memoryHighWatermark.type |string |`"relative"` |

|rabbitmq.memoryHighWatermark.value |float |`0.4` |
