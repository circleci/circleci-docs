...
Instances building jobs (that is, the Nomad clients and external VMs) also will likely need to pull code from your VCS using SSH (TCP port 22). SSH is also used to communicate with external VMs. Allow SSH for all instances with the VM subnet and your VCS as the destination.
...
[#intra-node-traffic]
=== Intra-node traffic
By default, networking policies regulate the traffic within your Kubernetes cluster, and this is sufficient to regulate the traffic between pods. No additional requirement are needed to reduce traffic between Kubernetes nodes any further (it is fine to allow all traffic between Kubernetes nodes).
...
[#ingress-kubernetes]
=== Ingress
If you are using a managed service, you can check the rules created for the traffic coming from the load balancers and the allowed port range. The standard port range for Kubernetes load balancers (30000-32767) is all that is needed here for ingress. If you are using transparent load balancers, you need to apply the ingress rules listed for load balancers above.
...
ifndef::pdf[]
## Next steps
* Server v4.8: xref:operator:operator-overview.adoc[Operator Overview]
endif::[]
