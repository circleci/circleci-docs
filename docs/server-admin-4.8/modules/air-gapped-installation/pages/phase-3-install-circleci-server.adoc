...
Set the `hostname` value to the internal hostname of your GitHub enterprise installation, and set the `enterprise` value to true.
...
When following these instructions to re-host agents in a partially air-gapped environment, you can configure the machine provisioner for MinIO as follows:
