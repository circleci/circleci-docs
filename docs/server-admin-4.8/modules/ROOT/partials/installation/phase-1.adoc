CircleCI server {serverversion48} is installed as a Helm chart. The installation process is broken down into four phases. There is a validation step at the end of each phase, allowing you to confirm success before moving to the next phase. Depending on your requirements, phases 3 and 4 may include multiple steps. This installation guide assumes you have already read the xref:server-admin:overview:circleci-server-overview.adoc#[CircleCI server v4.8 overview].
...
We use RabbitMQ for workflow messaging, test results, usage, crons, output, notifications, and scheduler.
