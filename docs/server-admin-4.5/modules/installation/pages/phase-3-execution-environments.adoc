...
Once you have completed the server installation process you can further configure machine provisioner. Config options include:

* Building and specifying a Windows image to give developers access to the Windows execution environment
* Specifying an alternative Linux machine image.
* Specifying a number of preallocated instances to remain spun up at all times.

For more information, see the xref:operator:manage-virtual-machines-with-machine-provisioner.adoc[Manage Virtual Machines with machine provisioner] page.
