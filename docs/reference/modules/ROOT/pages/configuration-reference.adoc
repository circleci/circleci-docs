...
| N
| String
| The delay between reruns of the step if it fails. This delay can only be set along with `max_auto_reruns`. The string is a decimal with unit suffix using either seconds or minutes up to a maximum of 10 minutes, such as "10s", "1m20s".
...
The `max_auto_reruns` and `auto_rerun_delay` attributes can be used to automatically rerun a step if it fails. This is useful for steps that are expected to fail intermittently, such as tests.

*Example:*

.CircleCI job showing a run step set to automatically rerun up to 3 times if it fails with a 10 second delay between attempts
[,yml]
----
version: 2.1
