...
Use `teardown` for work that must happen however the job ends, such as saving a cache or storing diagnostic output. Because the steps are declared on the step they belong to, a `save_cache` can sit alongside its matching `restore_cache` instead of at the end of the job.
...
`restore_cache` and `attach_workspace` are not supported. Use them as ordinary steps in the body of the job instead.
