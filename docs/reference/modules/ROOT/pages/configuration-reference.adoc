...
* xref:orbs:author:orb-author-intro.adoc#[Authoring Orbs].
...
Pipeline parameters declared for use in the configuration. See xref:guides:orchestrate:pipeline-variables.adoc#pipeline-parameters-in-configuration[Pipeline Values and Parameters] for usage details.
...
If you are working on CircleCI server v3.1 and up, you can access the macOS execution environment using xref:guides:execution-runner:runner-overview.adoc#[self-hosted runner].
...
Generates and stores a cache of a file or directory of files such as dependencies or source code in our object storage. Later jobs can <<restorecache,restore this cache>>. Learn more on the xref:guides:optimize:caching.adoc#[Caching Dependencies] page.
...
CAUTION: The scheduled workflows feature is set to be deprecated. Using *scheduled pipelines* rather than scheduled workflows offers several benefits. Visit the scheduled pipelines xref:guides:orchestrate:migrate-scheduled-workflows-to-scheduled-pipelines.adoc#[migration guide] to find out how to migrate existing scheduled workflows to scheduled pipelines. If you would like to set up scheduled pipelines from scratch, visit the xref:guides:orchestrate:scheduled-pipelines.adoc#[Scheduled pipelines] page.
