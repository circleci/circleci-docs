...
`with_tool_cache` is supported on all executors (`docker`, `machine`, `macos`, and `windows`). Cached data is stored in CircleCI's object storage, the same as `save_cache`/`restore_cache`.
