...
workflows:
   nightly:
     triggers:
       - schedule:
           cron: "0 0 * * *"
           filters: pipeline.git.branch == "main" or pipeline.git.branch == "beta" # only run on main or beta branches
----
...
workflows:
  ...
  nightly:
    triggers:
      - schedule:
          cron: "0 0 * * *"
          filters: pipeline.git.branch == "main" or pipeline.git.branch starts_with "release/" # only run on main or release branches
    jobs:
      - coverage
----
...
workflows:
  build-deploy:
    jobs:
      - build:
          filters: pipeline.git.branch != "develop" and not(pipeline.git.branch matches /feature-.*/) # only run on branches that are not develop or feature- prefixed
      - deploy-stage:
          requires:
            - build
          filters: pipeline.git.branch == "staging" # only run on the staging branch
      - deploy-prod:
          requires:
            - build
          filters: pipeline.git.branch == "main" # only run on the main branch
----
