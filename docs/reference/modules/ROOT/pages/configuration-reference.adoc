...
The example configuration below uses a pipeline value, `pipeline.git.branch` to drive the `integration_tests` workflow.

This example prevents the workflow `integration_tests` from running unless the pipeline is triggered on the `main` branch.

.Use when to only run the workflow on the `main` branch
[,yaml]
----
workflows:
  integration_tests:
...
You might want to combine pipeline values to refine logic for when to run a workflow. In the following example the workflow will only run when:

* The trigger is set to one of the GitHub PR triggers.
* the branch has an open pull request that is _ready for review_, that is, not in _draft_.

NOTE: These options are only available for GitHub App pipelines. For more information, see the xref:guides:orchestrate:github-trigger-event-options.adoc[GitHub trigger event options] page.

.Use when to only run the workflow when a PR is open and ready for review
[,yaml]
----
workflows:
  integration_tests:
    when: pipeline.event.name == "pull_request" and pipeline.event.github.pull_request.draft == false
    jobs:
      - mytestjob
