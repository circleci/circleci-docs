...
* <<parallelism,Test splitting>> across four parallel execution environments, and a <<matrix,matrix>> that runs the test job once per suite.
...
jobs:
  ...
  test:
    executor: node-service
    # Run this job in four execution environments at the same time.
    parallelism: 4
    ...
    steps:
      ...
      - run:
          ...
          command: |
            circleci tests glob "test/<< parameters.suite >>/**/*.test.js" |
              circleci tests run --command="xargs npm run test:ci --" --split-by=timings
      - store_test_results:
          path: test-results
      - store_artifacts:
          path: coverage
