...
NOTE: Build tool caching is in beta on CircleCI Cloud. The feature is still evolving, so you may encounter bugs or changes to behavior. There is no additional cost to use it beyond standard xref:guides:optimize:persist-data.adoc#managing-network-and-storage-usage[Cache Storage Usage].
