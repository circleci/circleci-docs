...
CircleCI uses the `version` field to issue warnings for deprecation or breaking changes.
...
Jobs with the `release` type are used to xref:guides:deploy:configure-your-kubernetes-components.adoc#link-release[Connect Your Pipeline Configuration] to a deployment in the CircleCI deploys UI. A release job names the release plan created by a `circleci run release plan` step, using the `plan_name` key. For full details, see the xref:guides:deploy:deployment-overview.adoc#[Deploys Overview] page.
...
Add a `validation` key to a release job to turn on the release validation engine. The engine gates the release on signals from your monitoring tool, and triggers your rollback pipeline when a release fails. For an overview of the feature, see the xref:guides:deploy:smart-deployments-overview.adoc[Smart Deployments Overview] page.
...
| Y
| Boolean
| Turns on release validation for this release job.
...
| N
| Boolean
| When `true`, the engine triggers your rollback pipeline if validation fails during the evaluation window.

| `webhooks`
| N
| List
| Webhook validation checks, where each entry defines one named signal from a monitoring provider. When omitted, CircleCI uses a catch-all check that accepts any signal.
|===
--

Each entry under `webhooks` takes the following keys:
...
| Y
| String
| Identifier for this check. Your monitoring tool must send a matching `signal_name` so CircleCI can route alerts to it.
...
| N
| Map
| Maps fields in the incoming webhook payload to the values CircleCI matches on. Provider defaults apply to any key you omit.

| `fail_when`
| N
| String
| Expression that decides when a webhook event counts as a failure. Provider defaults apply when you omit it.

| `max_failures`
| N
| Integer
| Number of matching failures before this check fails.
|===
--

**Example** of a release job with release validation enabled:
...
For the `data_points` keys, the provider defaults, and how the engine matches an incoming signal to a check, see the xref:guides:deploy:release-validation-reference.adoc[Release Validation Configuration Reference] page.

Use the `no-op` type to configure a job that performs no actions and consumes no credits. Teams commonly use `no-op` to organize the order of operations within a workflow and make it easier to maintain. A `no-op` type job needs only the `type` key, and requires no further job configuration. For some examples of using `no-op` jobs, see the xref:guides:orchestrate:orchestration-cookbook.adoc#use-no-op-jobs-to-create-a-cleaner-workflow-graph[Orchestration Cookbook].
...
On CircleCI Cloud, use `circleci testsuite` to run and split your test suite across parallel containers; on CircleCI Server, use the CircleCI CLI's `circleci tests` commands.
...
jobs:
  build:
    ...
    working_directory: ~/my-app
    steps:
      - run: go test -v $(go list ./... | circleci tests split --split-by=timings --timings-type=name)
----
--
====

`parallelism` also accepts a pipeline value expression instead of a plain integer, for example to vary the number of parallel executors by branch:
...
| N
| String or List
| The command to use as PID 1 (or as arguments for entrypoint) when launching the container. `command` overrides the image's `COMMAND`. If the image has an `ENTRYPOINT`, Docker uses `command` as arguments to it. If the image has no `ENTRYPOINT`, Docker uses `command` as the executable.
...
| N
| Map
| Authentication for GCP Artifact Registry using OIDC
|===
--
...
For steps to get set up with OIDC to pull images from AWS ECR, see the xref:guides:permissions-authentication:pull-an-image-from-aws-ecr-with-oidc.adoc#[Pull an Image From AWS ECR With OIDC] page.
...
Using an image hosted on link:https://cloud.google.com/artifact-registry[GCP Artifact Registry] requires authentication using GCP credentials, either via OIDC, or by setting environment variables. The two configuration options are described in the following sections:
...
For steps to get set up with OIDC to pull images from GCP Artifact Registry, see the xref:guides:permissions-authentication:pull-an-image-from-gcp-gar-with-oidc.adoc#[Pull an Image From GCP Artifact Registry With OIDC] page.
...
jobs:
  build: # name of your job
    machine: # executor type
      image: ubuntu-2604:current # recommended linux image
...
* https://circleci.com/developer/machine/image/ubuntu-2404[Ubuntu-2404]
* https://circleci.com/developer/machine/image/ubuntu-2604[Ubuntu-2604]
...
CAUTION: Java, Erlang, and any other languages that introspect the `/proc` directory for CPU count information may require additional configuration. Without it, they can slow down when using the CircleCI resource class feature. Programs with this issue may request 32 CPU cores and run slower than they would when requesting one core. Users of languages with this issue must pin their CPU count to their guaranteed CPU resources.
...
include::guides:ROOT:partial$execution-resources/docker-gen2-resource-table.adoc[]
...
include::guides:ROOT:partial$execution-resources/docker-arm-gen2-resource-table.adoc[]

.Set a resource class for an Arm Docker job
[,yaml]
----
jobs:
  build:
...
include::guides:ROOT:partial$execution-resources/arm-gen2-resource-table.adoc[]
...
Here `run` is a step type. The UI uses the `name` attribute for display purposes. The `command` attribute is specific to the `run` step and defines the command to execute.

Some steps support a shorthand syntax. For example, you can also call `run` like this:
...
In this case, the `checkout` step checks out project source code into the job's <<jobs,`working_directory`>>.
...
| N
| Boolean
| Whether or not this step runs in the background (default: false)

| `working_directory`
| N
| String
| The directory where this step runs. CircleCI interprets this relative to the <<jobs,`working_directory`>> of the job. (default: `.`)
...
For more information about which files Bash executes when it starts, link:https://linux.die.net/man/1/bash[see the `INVOCATION` section of the `bash` manpage].

The following sections describe the `-eo pipefail` options.

==== `-e`

Exit as soon as any of the following exits with a non-zero status:
...
NOTE: If `make test` fails, the rest of the pipeline is executed.
...
`on_success`:: The step runs only if all previous steps succeeded (returned exit code 0). `on_success` is the default value.

`always`:: The step runs regardless of the exit status of previous steps. Use `always` when you have a task that must run whether previous steps succeed or fail. For example, you need to upload logs or code-coverage data.

`on_fail`:: The step runs only if one of the preceding steps failed (returns a non-zero exit code). Common uses of `on_fail` include storing diagnostic data to debug test failures, or running custom notifications about the failure, such as sending emails or triggering alerts.

NOTE: Some steps, such as `store_artifacts` and `store_test_results`, always run, even if a *step has failed* (returned a non-zero exit code) previously. However, `store_artifacts`, `store_test_results`, and the `when` attribute are not run if the job has been *killed* by a cancel request or has reached the runtime timeout limit.
...
| N
| String
| The delay between reruns of the step if it fails. This delay can only be set along with `max_auto_reruns`. The string is a decimal with unit suffix using either seconds `s` or minutes `m` up to a maximum of 10 minutes, such as "10s", "2m". If you do not supply a delay, the rerun starts directly after the step fails.
|===
--
...
NOTE: `version: 2.1` configuration supports the `when` and `unless` steps.

A conditional step consists of a step with the key `when` or `unless`. Under the `when` key are the subkeys `condition` and `steps`. Use the `when` step to run commands or apply job configuration conditionally. CircleCI determines conditions at config compilation time, which means it checks `when` step conditions before your workflow runs. See the xref:reusing-config.adoc#defining-conditional-steps[Conditional Steps Section of the Reusable Configuration Reference] for more details.
...
| Y
| Logic
| xref:configuration-reference.adoc#logic-statements[A Logic Statement]. Expressions are not supported for the `when` step.
...
| N
| String
| The directory where CircleCI checks out code. CircleCI interprets this relative to the <<jobs,`working_directory`>> of the job. (default: `.`)
|===
--
...
NOTE: The `checkout` step configures Git to skip automatic garbage collection. If you are caching your `.git` directory with <<restorecache>> and want to use garbage collection to reduce its size, use a <<run>> step with `git gc` before doing so.
...
| N
| boolean
| Set this to `true` to pull images from the same region as the VM where possible. If the pull fails, CircleCI falls back to the image's original region (default: `false`)

| `version`
| N
| String
| Version string of Docker you would like to use (default: `24.0.9`). View the list of xref:guides:execution-managed:building-docker-images.adoc#docker-version[Supported Docker Versions].
|===
--
...
| Y
| List
| List of directories to add to the cache
...
| `{{ .CheckoutKey }}`
| The SSH key used to check out the repository.
...
NOTE: When multiple matches exist, CircleCI uses the *most recent match*, even if a more precise match exists.
...
In this case, CircleCI restores cache `v1-myapp-cache-new`, the most recent match with the `v1-myapp-cache` prefix. CircleCI restores it even though the first key (`v1-myapp-cache`) is an exact match.
...
NOTE: Build tool caching is in beta on CircleCI Cloud. The feature is still evolving, so you may encounter bugs or changes to behavior. Using it costs no more than standard xref:guides:optimize:persist-data.adoc#managing-network-and-storage-usage[Cache Storage Usage].

Wraps a sequence of steps and configures the named build tool to use a CircleCI-managed remote cache for its own build outputs. See <<savecache,`save_cache`>> and <<restorecache,`restore_cache`>> for the default caching steps. For more information, see the xref:guides:optimize:build-tool-cache.adoc#[Build Tool Cache] page.
...
| Y
| String
| The build tool to cache for. One of: `gradle`, `bazel`, `turborepo`, `xcode`.

| `steps`
| Y
| List
| The steps to run with tool caching enabled.
|===
--

The `with_tool_cache` step is supported on all executors (`docker`, `machine`, `macos`, and `windows`). Cached data is stored in CircleCI's object storage, the same as when using the default caching steps `save_cache` and `restore_cache`.
...
Special step that persists a temporary file for another job in the workflow to use. For more information on using workspaces, see the xref:guides:orchestrate:workspaces.adoc#[Using Workspaces to Share Data Between Jobs] page.

`persist_to_workspace` adopts the storage settings from the storage customization controls on the CircleCI web app. If you do not provide a custom setting, `persist_to_workspace` defaults to 15 days.

You can customize workspace storage retention on the https://app.circleci.com/[CircleCI web app] by navigating to menu:Plan[Usage Controls].
...
NOTE: Everything must be relative to the workspace root directory.
...
Special step that attaches the workflow's workspace to the current container. CircleCI downloads the full contents of the workspace and copies them into the directory where it attaches the workspace. For more information on using workspaces, see the xref:guides:orchestrate:workspaces.adoc#[Using Workspaces] page.
...
Special step that adds SSH keys from a project's settings to a container. Also configures SSH to use these keys. For more information on SSH keys see the xref:guides:integration:add-ssh-key.adoc[Add Additional SSH Keys] page.

CAUTION: *Using server?* CircleCI Server supports only MD5 fingerprints. You can see the MD5 fingerprint in CircleCI under menu:Project Settings[SSH keys > Additional SSH keys]. An upcoming server release is planned to include SHA256 support.
...
NOTE: Scheduled workflows are not available for projects integrated through the GitHub App or GitLab.
...
The `branches` key controls whether the _current_ branch has a schedule trigger created for it, where _current_ branch is the branch containing the `config.yml` file with the `trigger` stanza. That is, a push on the `main` branch only schedules a workflow using xref:guides:orchestrate:workflows.adoc#using-filters-in-your-workflows[Filters in Your Workflows] for the `main` branch.
...
NOTE: `version: 2.1` configuration supports using `when` or `unless` under `workflows`. Workflows always run unless a `when` or `unless` filter prevents them from running. If you want a workflow to run in every pipeline, do not add a `when` or `unless` filter.

You may use a `when` clause (the inverse clause `unless` is also supported) under a workflow declaration with a <<logic-statements,logic statement>> to determine whether to run that workflow.
...
The `integration_tests` workflow runs only when the pipeline is triggered on the `main` branch.

For more examples and conceptual information, see the xref:guides:orchestrate:workflows.adoc#[Workflows] page.
...
In the example above, the orb job `my-orb/my-test` overrides the `test` job in the workflow. The `my-orb/my-test` job can define a different resource class or execution steps.

If the orb does not define the `my-orb/my-test` job, the `test` job compiles using the local job definition.
...
* Serial groups wait for five hours. After this time, CircleCI cancels any jobs still waiting in the group. This does not affect the standard limits that apply to a <<jobs,job's runtime>>.
...
* This immediate skip process maintains order integrity, preventing unintended work in a build, such as a deployment job running a previous version.
...
| N
| List
a| A list of jobs that must succeed a specified status before the job starts. *Note*: When jobs you list as dependencies do not execute (due to filters, for example), CircleCI ignores them as dependencies for other jobs. However, if all dependencies of a job are filtered, that job doesn't execute either.
...
* Map of job name to a list of statuses (a required job that must attain one of the specified statuses for the job to start).

The possible *status* values are: `success`, `failed`, `canceled`, `unauthorized`, `not_run`, and `terminal`.
|===
--
...
You can use the `name` key to invoke reusable jobs across any number of workflows. Using the name key ensures CircleCI does not append numbers to your job name (for example, sayhello-1, sayhello-2, etc.). The name you assign to the `name` key needs to be unique; otherwise, CircleCI still appends numbers to the job name.
...
| N
| String/List
| The name of the context(s). The initial default name is `org-global`. Each context name must be unique. CircleCI Server supports only a single context per workflow. *Note:* CircleCI allows a maximum of 100 unique contexts across all workflows.
|===
--
  ...
  build-deploy:
    jobs:
      ...
      - build_server_pdfs: # << the job to conditionally run based on the filter-by-branch-name.
          filters: pipeline.git.branch starts-with "server/" # only run on branches starting with server/
----
...
WARNING: CircleCI has deprecated job filters. Use expression-based job filters instead. See <<expression-based-job-filters>> for more information.
...
Tags can have the keys `only` and `ignore`. You may also use regular expressions to match against tags by enclosing them with slashes, or map to a list of such strings. Regular expressions must match the *entire* string. CircleCI supports both lightweight and annotated tags.

* Any tags that match `only` runs the job.
* Any tags that match `ignore` do not run the job.
* If you specify neither `only` nor `ignore`, CircleCI skips the job for all tags.
* If you specify both `only` and `ignore`, CircleCI considers `only` before `ignore`.
...
| Y
| Map
| A map of parameter names to every value the job is called with

| `exclude`
| N
| List
| A list of argument maps to exclude from the matrix
...
.Basic example of using matrix jobs which creates nine different `build` jobs
[,yaml]
----
workflows:
  workflow:
...
.Equivalent configuration without using matrix jobs
[,yaml]
----
workflows:
  workflow:
...
NOTE: `version: 2.1` configuration supports pre-steps and post-steps.

Every job invocation in a workflow may optionally accept two special arguments: `pre-steps` and `post-steps`.

CircleCI executes steps under `pre-steps` before any other steps in the job, and executes steps under `post-steps` after all other steps.
...
[#logic-statements]
== Logic statement syntax

NOTE: CircleCI recommends using xref:#using-expressions-in-your-configuration[Expressions] for conditional logic in your configuration. Expressions can reference pipeline values and pipeline parameters and use comparison and logic operators.
...
All other values are truthy. Also note that using logic with an empty list causes a validation error.

Logic statements always resolve to a boolean value at the top level, and coerce as necessary. You can nest them in an arbitrary fashion, according to their argument specifications, to a maximum depth of 100 levels.

`matches` uses link:https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html[Java regular expressions] for its `pattern`. You must provide a full match pattern; prefix matching is not an option. We recommend enclosing a pattern in `^` and `$` to avoid accidental partial matches.
...
Compare this with the expression syntax equivalent:
...
The following configuration file builds, tests, and deploys a Node.js service. It combines keys from across this page into one pipeline, including:

* Reusable <<orbs,orbs>>, <<executors,executors>>, and <<commands,commands>>.
* A <<parameters-pipeline,pipeline parameter>> that sets the deployment target when you trigger the pipeline.
* Service containers, dependency caching, workspaces, artifacts, and test results.
* <<parallelism,Parallelism>> with Smarter Testing to split each suite across four execution environments, and a <<matrix,matrix>> that runs the test job once per suite. See the xref:guides:test:getting-started-with-smarter-testing.adoc[Getting Started With Smarter Testing] guide.
* <<expression-based-job-filters,Expression-based job filters>>, an <<type,approval job>>, and a <<serial-group,serial group>> so only one deployment runs at a time across your organization.
* Deploy markers that log each deployment in the CircleCI web app. See the xref:guides:deploy:configure-deploy-markers.adoc[Configure Deploy Markers] guide.
...
# Orbs are shareable packages of configuration.
orbs:
  node: circleci/node@7.2.1

# Set a pipeline parameter when you trigger a pipeline from the web app or the API.
parameters:
  deploy-target:
    type: enum
...
# A reusable executor defines an execution environment once for several jobs to share.
executors:
  node-service:
    docker:
      # The first image in the list is the primary container, where job steps run.
      - image: cimg/node:24.21.0
      # Any further images run as service containers, reachable on localhost.
      - image: cimg/postgres:17.11
        environment:
          POSTGRES_USER: circleci
          POSTGRES_DB: app_test
      - image: cimg/redis:8.4
    resource_class: large
...
# A reusable command defines a sequence of steps once for several jobs to share.
commands:
  install-dependencies:
    description: Check out the code and install Node packages.
    steps:
      - checkout
      # The node orb caches your packages between runs.
...
jobs:
  build:
    ...
    steps:
      ...
      # Pass the build output to the jobs that run later in the workflow.
      - persist_to_workspace:
          root: ~/app
          paths:
            - dist
  ...
  test:
    executor: node-service
    # Smarter Testing splits the suite across these four execution environments.
    parallelism: 4
    # Delete this job's cache data after seven days.
    retention:
      caches: 7d
    parameters:
      suite:
    ...
    steps:
      ...
      - attach_workspace:
          at: ~/app
      # Resolves to a suite named in .circleci/test-suites.yml, such as "unit tests".
      - run:
          name: Run the << parameters.suite >> tests
          command: circleci testsuite run "<< parameters.suite >> tests"
      - store_test_results:
          # This path must match the outputs.junit path in test-suites.yml.
          path: test-reports
      - store_artifacts:
          path: coverage
  ...
  publish-image:
    machine:
      image: ubuntu-2604:current
      # Reuse the image layers built by previous runs of this job.
  ...
  deploy:
    ...
    steps:
      - checkout
      # A deploy marker logs this deployment in the CircleCI web app.
      - run:
          name: Plan the deployment
          command: |
            circleci run release plan my-service \
              --environment-name=<< pipeline.parameters.deploy-target >> \
              --component-name=my-service \
              --target-version=<< pipeline.number >>
      - run:
          name: Deploy to << pipeline.parameters.deploy-target >>
          command: ./scripts/deploy.sh << pipeline.parameters.deploy-target >>
      - run:
          name: Mark the deployment as running
          command: circleci run release update my-service --status=running
      # The when attribute reports the outcome back to the deploy marker.
      - run:
          name: Mark the deployment as successful
          command: circleci run release update my-service --status=SUCCESS
          when: on_success
      - run:
          name: Mark the deployment as failed
          command: circleci run release update my-service --status=FAILED
          when: on_fail
  ...
  cancel-deploy:
    ...
    steps:
      - run:
          name: Mark the deployment as canceled
          command: circleci run release update my-service --status=CANCELED

workflows:
  build-test-deploy:
    # Rerun failed jobs in this workflow up to two times.
    max_auto_reruns: 2
    jobs:
      - build
      - test:
          # The matrix runs the test job once for each suite listed.
          matrix:
            parameters:
          ...
          requires:
            - build
      - publish-image:
          # A context holds credentials shared across your organization.
          context:
            - docker-hub
          requires:
            - build
          filters: pipeline.git.branch == "main" or pipeline.git.tag starts-with "v"
      # An approval job holds the workflow until someone approves it in the web app.
      - hold:
          type: approval
          requires:
            - test
      ...
      - deploy:
          context:
            - deploy-credentials
          # Only one job in this serial group runs at a time across your organization.
          serial-group: << pipeline.project.slug >>/deploy-<< pipeline.parameters.deploy-target >>
          requires:
            - hold
          filters: pipeline.git.branch == "main"
      # Requiring a status runs this job only when the deploy job is canceled.
      - cancel-deploy:
          requires:
            - deploy:
                - canceled
...
The `test` job runs the two suites named in a companion `.circleci/test-suites.yml` file:

[,yaml]
----
---
name: unit tests
discover: vitest list --filesOnly test/unit
...
discover: vitest list --filesOnly test/integration
run: vitest run --reporter=junit --outputFile="<< outputs.junit >>" << test.atoms >>
outputs:
  junit: test-reports/junit.xml
options:
  impact-key: integration
  dynamic-test-splitting: true
  # Rerun a failed test up to twice before the job fails.
  max-auto-rerun: 2
----

For every key you can set in this file, see the xref:testsuite-configuration-reference.adoc[Testsuite Configuration Reference] page.
