...
| Y
| Logic
| xref:configuration-reference.adoc#legacy-logic-statements[A logic statement] (or use xref:configuration-reference.adoc#using-expressions-in-your-configuration[expressions] in workflow/job filters instead)
...
The `branches` key controls whether the _current_ branch should have a schedule trigger created for it, where _current_ branch is the branch containing the `config.yml` file with the `trigger` stanza. That is, a push on the `main` branch will only schedule a workflow using xref:guides:orchestrate:workflows.adoc#using-filters-in-your-workflows[Filters in Your Workflows] for the `main` branch.
...
| N
| List
a| A list of jobs that must succeed a specified status before the job starts. *Note*: When jobs you list as dependencies don't execute (due to filters, for example), CircleCI ignores them as dependencies for other jobs. However, if all dependencies of a job are filtered, that job doesn't execute either.
...
[#legacy-logic-statements]
== Legacy logic statement syntax

NOTE: CircleCI recommends using xref:#using-expressions-in-your-configuration[expressions] for conditional logic in your configuration. The legacy logic statement syntax described in this section still works, but expressions provide a more intuitive and flexible approach.
...
[#legacy-logic-statement-example]
=== Legacy logic statement example
