...
Some configuration fields accept expressions directly as well as string values. When you provide a string value, you can use `<< >>` delimiters to mark a section of the string that should be interpreted as an expression.

In fields that accept expressions, you can write the expression directly:
...
=== Direct expressions and templated strings
  ...
  my-workflow:
    jobs:
      - build:
          filters:
            branches:
              only: pipeline.git.branch == "main" or pipeline.git.branch starts-with "release/"
----
...
For all other expressions, you must use the `<< >>` delimiters to mark the expression as an expression, as shown in other examples in this section.
