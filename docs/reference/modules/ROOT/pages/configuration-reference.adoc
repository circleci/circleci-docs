...
**Example** with validation enabled and explicit webhook checks:
...
jobs:
  release-my-service:
    ...
    agents:
      - name: generic_check
        prompt: Check error rate stayed below 1% after deploy
----
...
| N
| Boolean
| Whether or not this step run in the background (default: false)
...
NOTE: If `make test` fails the rest of pipeline is executed.
...
* Any tags that match `ignore` does not run the job.
