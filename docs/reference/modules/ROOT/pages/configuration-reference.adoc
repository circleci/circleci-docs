...
| N
| List
| <<teardown,Steps to run when the job ends>>, including on failure, cancellation, or timeout.
...
For more information about which files are executed when Bash is invoked, link:https://linux.die.net/man/1/bash[see the `INVOCATION` section of the `bash` manpage].
...
NOTE: Some steps, such as `store_artifacts` and `store_test_results`, always run, even if a *step has failed* (returned a non-zero exit code) previously. However, `store_artifacts`, `store_test_results`, and the `when` attribute are not run if the job has been *killed* by a cancel request or has reached the runtime timeout limit. To run steps when a job is canceled or times out, use the <<teardown,`teardown` attribute>>.
...
The `teardown` attribute attaches a list of steps to a `run` step. CircleCI runs these steps when the job ends, including when the job fails, is canceled, or reaches its runtime timeout.

Use `teardown` for work that must happen whether the job succeeds or fails, such as saving a cache or storing diagnostic output. Because the steps are declared on the step they belong to, a `save_cache` can sit alongside its matching `restore_cache` instead of at the end of the job.

The following steps are supported inside `teardown`:
...
`restore_cache` and `attach_workspace` are not supported. Use them as ordinary steps in the job's `steps` list instead.

A teardown step cannot run in the background, cannot set `max_auto_reruns` or `auto_rerun_delay`, and cannot nest another `teardown` key, which is rejected when your config is validated.

The `teardown` attribute works on a `run` step inside a xref:reusing-config.adoc#authoring-reusable-commands[Reusable Command] definition.

NOTE: `teardown` takes a list of steps. Use list syntax even for a single step.

.Save a build cache even if the job is canceled
[,yml]
----
version: 2.1
  ...
  build:
    ...
    steps:
      ...
      - run:
          ...
          command: make build
          teardown:
            - save_cache: # runs even if the job is canceled or times out
...
A step registers its teardown steps when it starts, before its command runs, so the teardown steps still run if the job is canceled part way through the step. A step that CircleCI skips, for example because of its <<the-when-attribute,`when` attribute>>, never reaches registration and registers nothing. A step that is retried through `max_auto_reruns` registers its teardown steps once, not once per attempt.

Teardown steps run in reverse order of registration across the whole job, in the manner of Go's `defer`: the most recently registered teardown step runs first. A teardown step can register additional teardown steps at runtime; those run before the remaining ones.

CircleCI allows 10 minutes in total for a job's teardown steps, not 10 minutes each. Teardown steps that have not run when that time is up do not run.

[#teardown-when]
===== Conditions and failures

Steps inside `teardown` accept the <<the-when-attribute,`when` attribute>>, and default to `always` rather than `on_success`. This includes `store_artifacts` and `store_test_results`, which do not accept `when` as ordinary steps.

Inside `teardown`, `when` is evaluated against the state of the job, not the result of the step the teardown is attached to. A teardown step with `when: on_fail` runs when the job has failed, even if its own parent step succeeded.

If a teardown step fails, the job fails.
...
A conditional step consists of a step with the key `when` or `unless`. Under the `when` key are the subkeys `condition` and `steps`. Use the `when` step to customize commands and job configuration to run on custom conditions. Conditions are determined at config compilation time. This means `when` step conditions are checked before your workflow runs. See the xref:reusing-config.adoc#defining-conditional-steps[Conditional Steps Section of the Reusable Configuration Reference] for more details.
...
`matches` uses link:https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html[Java regular expressions] for its `pattern`. A full match pattern must be provided, prefix matching is not an option. Though, it is recommended to enclose a pattern in `^` and `$` to avoid accidental partial matches.
