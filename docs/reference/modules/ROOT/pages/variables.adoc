...
#TODO Confirm whether CIRCLE_OIDC_TOKEN and CIRCLE_OIDC_TOKEN_V2 are populated for Cursor Origin. They appear to be set in the job, but this is not confirmed.#
...
#TODO Confirm whether the `pipeline.deploy.*` values are populated for Cursor Origin. Deploy and rollback pipelines are unsupported for Origin, so these are expected to be unpopulated.#
