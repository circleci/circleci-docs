= Project values and variables
:page-platform: Cloud, Server v4+
:page-description: A list of all built-in variables for your CircleCI projects.
:experimental:

This page is a reference for all built-in values available for use in your CircleCI projects.

[#built-in-environment-variables]
== Built-in environment variables

The following built-in environment variables are available for CircleCI projects. Some environment variables are available for GitHub OAuth and Bitbucket Cloud projects and are deprecated for GitLab, GitHub App, GitHub Enterprise Server, and Bitbucket Data Center support. If you must continue to use those as environment variables in your GitLab pipelines, refer to the workaround described after the list below.

Environment variables are scoped at the job level and can be used within the context of a job. Environment variables do not exist at a pipeline level, therefore they cannot be used for any logic at the pipeline or workflow level.

NOTE: You cannot use a built-in environment variable to define another environment variable. Instead, you must use a `run` step to export the new environment variables using `BASH_ENV`. For more details, see xref:guides:security:set-environment-variable.adoc#set-an-environment-variable-in-a-shell-command[Setting an Environment Variable in a Shell Command].

NOTE: Pull request variables are not populated for Cursor Origin pipelines. `CIRCLE_REPOSITORY_URL` and `CIRCLE_USERNAME` are set but empty for Cursor Origin pipelines.

#TODO Confirm whether these variables are populated for Cursor Origin: CIRCLE_TAG, CIRCLE_OIDC_TOKEN, CIRCLE_OIDC_TOKEN_V2, CIRCLE_PREVIOUS_BUILD_NUM, CIRCLE_INTERNAL_TASK_DATA. A push build does not exercise them, so the sample output could not confirm them.#

[.table-scroll]
--
[cols="2,2,1,2,1,1", options="header"]
|===
| Variable
| VCS
| Type
| Value
| Cloud
| Server

| `CI`
| GitHub, Bitbucket, GitLab, Cursor Origin
| Boolean
| `true` (represents whether the current environment is a CI environment)
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLECI`
| GitHub, Bitbucket, GitLab, Cursor Origin
| Boolean
| `true` (represents whether the current environment is a CircleCI environment)
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_BRANCH`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| The name of the Git branch currently being built.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_BUILD_NUM`
| GitHub, Bitbucket, GitLab, Cursor Origin
| Integer
| The number of the current job. Job numbers are unique for each job.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_BUILD_URL`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| The URL for the current job on CircleCI.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_JOB`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| The name of the current job.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_NODE_INDEX`
| GitHub, Bitbucket, GitLab, Cursor Origin
| Integer
| For jobs that run with parallelism enabled, this is the index of the current parallel run. The value ranges from 0 to (`CIRCLE_NODE_TOTAL` - 1).
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_NODE_TOTAL`
| GitHub, Bitbucket, GitLab, Cursor Origin
| Integer
| For jobs that run with parallelism enabled, this is the number of parallel runs. The value is equivalent to the value of `parallelism` in your config file.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_OIDC_TOKEN`
| GitHub, Bitbucket, GitLab
| String
| An OpenID Connect token signed by CircleCI which includes details about the current job.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**# from v4.4

| `CIRCLE_OIDC_TOKEN_V2`
| GitHub, Bitbucket, GitLab
| String
| An OpenID Connect token signed by CircleCI which includes details about the current job.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**# from v4.4

| `CIRCLE_ORGANIZATION_ID`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| A unique identifier for the CircleCI organization.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_PIPELINE_ID`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| A unique identifier for the current pipeline.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_PR_NUMBER`
| GitHub OAuth, Bitbucket Cloud
| Integer
| The number of the associated GitHub or Bitbucket pull request. Only available on forked PRs.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_PR_REPONAME`
| GitHub OAuth, Bitbucket Cloud
| String
| The name of the GitHub or Bitbucket repository where the pull request was created. Only available on forked PRs.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_PR_USERNAME`
| GitHub OAuth, Bitbucket Cloud
| String
| The GitHub or Bitbucket username of the user who created the pull request. Only available on forked PRs.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_PROJECT_ID`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| A unique identifier for the current project.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_PROJECT_REPONAME`
| GitHub, GitLab, Bitbucket, Cursor Origin
| String
| The name of the repository of the current project.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_PROJECT_USERNAME`
| GitHub, GitLab, Bitbucket, Cursor Origin
| String
| The GitHub or Bitbucket username of the current project.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_PULL_REQUEST`
| GitHub, Bitbucket Cloud
| String
| The URL of the associated pull request. If there are multiple associated pull requests, one URL is randomly chosen.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_PULL_REQUESTS`
| GitHub OAuth, Bitbucket Cloud
| List
| Comma-separated list of URLs of the current build's associated pull requests.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_REPOSITORY_URL`
| GitHub OAuth, Bitbucket Cloud
| String
| The URL of your GitHub or Bitbucket repository.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_SHA1`
| GitHub, GitLab, Bitbucket, Cursor Origin
| String
| The SHA1 hash of the last commit of the current build.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_TAG`
| GitHub, Bitbucket, GitLab
| String
| The name of the git tag, if the current build is tagged. For more information, see the Git tag job execution section of the xref:guides:orchestrate:workflows.adoc#executing-workflows-for-a-git-tag[Workflows] page.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_USERNAME`
| GitHub, Bitbucket, GitLab
| String
| The GitHub or Bitbucket username of the user who triggered the pipeline (only if the user has a CircleCI account).
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_WORKFLOW_ID`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| A unique identifier for the workflow instance of the current job. This identifier is the same for every job in a given workflow instance.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_WORKFLOW_JOB_ID`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| A unique identifier for the current job.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_WORKFLOW_WORKSPACE_ID`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| An identifier for the <<glossary#workspace,workspace>> of the current job. This identifier is the same for every job in a given workflow.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `CIRCLE_WORKING_DIRECTORY`
| GitHub, Bitbucket, GitLab, Cursor Origin
| String
| The value of the `working_directory` key of the current job.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#
|===
--

If you must use the environment variables that are deprecated for GitLab SaaS in your GitLab pipelines, you can recreate this functionality using pipeline values in your configuration file. The following example shows how to set an environment variable `CIRCLE_PROJECT_REPONAME` using the xref:guides:security:env-vars.adoc#environment-variable-usage-options[`environment` key] and populating it with the `pipeline.trigger_parameters.gitlab.repo_name` value:

```yaml
build:
  docker:
    - image: cimg/node:17.0
      auth:
        username: mydockerhub-user
        password: $DOCKERHUB_PASSWORD  # context / project UI env-var reference
  environment:
    CIRCLE_PROJECT_REPONAME: << pipeline.trigger_parameters.gitlab.repo_name >>
  steps:
    - run: echo $CIRCLE_PROJECT_REPONAME
```


[#pipeline-values]
== Pipeline values

Pipeline values are available to all pipeline configurations and can be used without previous declaration. Pipeline values are scoped at the pipeline level. They are interpolated at compilation time, not workflow/job runtime.

The pipeline values available to you depend on your pipeline/integration type, as shown in the "Source" column of each table below. The tables below group values by their purpose and namespace.

#TODO Confirm for Cursor Origin: `pipeline.git.tag`, `pipeline.git.base_revision`, `pipeline.schedule.*`, and `pipeline.deploy.*`. A push build does not exercise them.#

include::guides:ROOT:partial$pipelines-and-triggers/pipeline-values.adoc[]

=== Usage example

```yaml
version: 2.1

jobs:
  build:
    docker:
      - image: cimg/node:20.3.0
    environment:
      CIRCLE_COMPARE_URL: << pipeline.project.git_url >>/compare/<< pipeline.git.base_revision >>..<<pipeline.git.revision>>
    working_directory: ~/main
    steps:
      - run: echo "This is pipeline ID << pipeline.id >>"
      - run: echo $CIRCLE_COMPARE_URL
```

When using the above method to set the variables in the `environment` key, note that if the pipeline variable is empty it will be set to `<nil>`. If you need an empty string instead, set the environment variable in a xref:guides:security:set-environment-variable.adoc#set-an-environment-variable-in-a-shell-command[Shell Command].
