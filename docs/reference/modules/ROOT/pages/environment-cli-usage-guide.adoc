...
:page-aliases: guides:toolkit:environment-cli-usage-guide.adoc
:experimental:
...
The environment CLI is different from the xref:guides:toolkit:circleci-cli.adoc[CircleCI Local CLI] tool that you may already know. Understanding the difference between these two CLI interfaces is essential for effective usage during your build processes.
...
For more information on using the `env` command, see the xref:guides:security:env-vars.adoc#environment-variable-substitution[Introduction to Environment Variables] page.
...
For more information, including an example, see the xref:guides:permissions-authentication:oidc-tokens-with-custom-claims.adoc[OpenID Connect Tokens With Custom Claims] page.
...
| Keep the target version marked live when the deploy reports `FAILED` or `CANCELED`. By default, CircleCI restores the last successful version as live. See xref:guides:deploy:configure-deploy-markers.adoc#live-version-on-failed-or-canceled-deploys[Live Version on Failed or Canceled Deploys].
|===
--
...
For more information on using the `circleci run release update` command, see the xref:guides:deploy:configure-deploy-markers.adoc[Configure Deploy Markers] page.
...
For more information on using the `circleci run release log` command, see the xref:guides:deploy:configure-deploy-markers.adoc[Configure Deploy Markers] page.
...
Collect and split tests so they can run in parallel. For full details of splitting and running tests using the environment CLI, see the xref:guides:optimize:use-the-circleci-cli-to-split-tests.adoc[Use the CircleCI Environment CLI to Split Tests] page.
...
NOTE: `circleci tests run` is available on CircleCI Cloud only. It is *not* supported on CircleCI Server. If you are on CircleCI Server, use `circleci tests split` (above) instead.
...
* xref:guides:optimize:parallelism-faster-jobs.adoc#how-test-splitting-works[Guide to Test Splitting and Parallelism]
* xref:guides:optimize:use-the-circleci-cli-to-split-tests.adoc[Use the CircleCI CLI to Split Tests]
