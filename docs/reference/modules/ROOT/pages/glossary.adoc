...
A <<workflow>> configuration option that controls which pushed tags run a given <<job>>. Tag filters use `only` and `ignore` keys with exact tag names or regular expressions, and jobs are excluded from tag-triggered workflows by default unless a tag filter is specified. See the xref:guides:orchestrate:workflows.adoc#using-regular-expressions-to-filter-tags-and-branches[Using Regular Expressions to Filter Tags and Branches] section of the Workflows page. Compare with <<branch-filter>>.
