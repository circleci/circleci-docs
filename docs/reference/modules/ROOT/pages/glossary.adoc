...
The CircleCI Model Context Protocol (MCP) server, which lets AI coding assistants diagnose build failures, fix issues, and manage runs through natural language without leaving the development environment. See the xref:guides:toolkit:circleci-mcp-overview.adoc[CircleCI MCP Overview] page.
...
A version of a <<component>> becoming available to an audience of users and other services. A release occurs when an existing component is updated or when the first version of a new component is deployed. Release history is retained according to your plan's data retention policy, then automatically deleted; this does not affect the currently live version of a component. See the xref:guides:deploy:deployment-overview.adoc[Deployment Overview] page. Compare with <<deployment>>.
...
The instance produced when an <<event>> matches a <<trigger>>. A run records the outcome of the attempt and groups any <<workflow,workflows>> that executed, including errors that occurred before workflows could start. Labeled as "Pipeline run" in the CircleCI web app.
...
The platform where your code is stored and that CircleCI integrates with, such as GitHub, GitHub Enterprise, GitLab, GitLab self-managed, or Bitbucket Cloud. See the xref:guides:integration:version-control-system-integration-overview.adoc[VCS Integration Overview] page.
