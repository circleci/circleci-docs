...
Executors define the environment in which the steps of a job will be run. When declaring a `job` in CircleCI configuration, you define the type of execution environment (`docker`, `machine`, `macos`, etc.) to run in. You also define other parameters for that environment, including which environment variables to populate, which shell to use, and what size `resource_class` to use.
