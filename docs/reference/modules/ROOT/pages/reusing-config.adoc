...
* Install the CircleCI CLI so that you have access to the `circleci config process` command (optional). This command lets you see the expanded configuration with all reusable keys processed. Follow the xref:guides:toolkit:local-cli.adoc[Using the CircleCI CLI] documentation for installation instructions and tips.
...
In all other instances, quotes are optional.
...
| N
| Map
| A map of parameter keys. See the xref:reusing-config.adoc#parameter-syntax[Parameter Syntax] section for details.
...
To use parameters in executors, define the parameters under the given executor. When you invoke the executor, pass the keys of the parameters as a map of keys under the `executor:` declaration. Each key has the value of the parameter to pass in.
...
Conditional steps run only if a condition is met at config-compile time, before a workflow runs. You cannot use a condition to check an environment variable, as they are not injected until your steps are running in the shell of your execution environment.
...
For a guide to creating inline orbs, see the xref:orbs:author:create-an-inline-orb.adoc[Create an Inline Orb] guide.

[#see-also]
== See also

* Refer to xref:guides:toolkit:sample-config.adoc[Sample Configurations] for some sample configurations that you can use in your own CircleCI configuration.
* Refer to xref:guides:toolkit:postgres-config.adoc[Database Examples] for database examples you can use in your CircleCI configuration.
