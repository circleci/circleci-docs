...
* Use the template variable `<< test.atoms >>` in your run command. This variable is replaced with a space-separated list of test atoms to run.
...
* *Go coverage format* `<< outputs.go-coverage >>` -- analyses one test atom at a time.
