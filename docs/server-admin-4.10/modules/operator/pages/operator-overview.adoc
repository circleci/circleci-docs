...
:page-audience: admin
:page-description: Learn about the various tasks and tools involved in administering an installation of CircleCI Server 4.10.
:experimental:

This guide contains information for CircleCI Server operators, or those responsible for ensuring CircleCI Server is running properly through maintenance and monitoring. Before reading this operator guide, ensure you have read the xref:overview:circleci-server-overview.adoc[CircleCI Server 4.10 Overview].
...
CircleCI Server 4.10 uses Nomad as the primary job scheduler. Refer to our xref:introduction-to-nomad-cluster-operation.adoc#[Introduction to Nomad Cluster Operation] to learn more about the job scheduler and how to perform basic client and cluster operations.
...
See the link:https://developer.hashicorp.com/nomad/docs/deploy/production/requirements#resources[Nomad Documentation] for options for optimizing the resource usage of Nomad clients.
