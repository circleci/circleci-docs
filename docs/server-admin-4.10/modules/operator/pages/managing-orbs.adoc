...
Orbs are accessed via the xref:guides:toolkit:local-cli.adoc#[CircleCI CLI]. Orbs require your CircleCI user to be an admin. They also require a xref:guides:toolkit:managing-api-tokens.adoc#[personal API token].
...
When importing orbs, the CLI must be able to talk to the server installation and to `circleci.com`. To import orbs for a server installation behind a proxy, configure the CLI to use the proxy to make requests to `circleci.com`, rather than proxying requests to the server installation. For example:
