...
If a retention policy of `n` days is configured for both MongoDB and PostgreSQL data, set `n+1` for **all objects** in your S3/GCS buckets. These objects then expire at `n+1` days. This ensures alignment between database retention and object storage retention.
