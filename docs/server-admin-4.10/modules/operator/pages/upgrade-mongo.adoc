...
:page-audience: admin
:page-description: Learn how to upgrade MongoDB up to v7.0.15 in an installation of CircleCI Server 4.10.
:experimental:
...
MongoDB `3.6.22` is shipped with CircleCI Server 4.10.
...
[#script-upgrade-4.4]
=== Scripted upgrade
We have created a link:https://github.com/CircleCI-Public/server-scripts/tree/main/upgrade-mongo-to-4.4[shell script] that you can use to upgrade your cluster's MongoDB instance.
...
. Your `values.yaml` contains the following snippet:
+
[source,yaml]
----
mongodb:
...
. You will receive a `{ "ok" : 1 }` response from MongoDB. Exit out of the MongoDB shell and pod.
...
. You will receive a `{ "ok" : 1 }` response from MongoDB again. Exit out of the shell and pod.
...
[#script-upgrade-7.0]
=== Scripted upgrade
We have created a link:https://github.com/CircleCI-Public/server-scripts/tree/main/upgrade-mongo-to-7.0[shell script] that you can use to upgrade your cluster's MongoDB instance.
