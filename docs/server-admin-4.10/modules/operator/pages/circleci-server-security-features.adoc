...
:page-audience: admin
:page-description: This document outlines security features built into CircleCI Server 4.10 and related integrations.
:experimental:
...
| `id`
| A UUID that uniquely identifies this event. The UUID allows consumers of events to identify duplicate deliveries.
...
| `scope`
| If the target is owned by an account in the CircleCI domain model, the account field is filled in with the account name and ID. This data is a JSON blob that will always contain `id` and `type` and will likely contain `name`.
...
* Minimize the number of secrets (private keys / environment variables) your build needs and rotate secrets regularly.
  ...
  ** Ensure the secrets you _do_ use are of limited scope, with only enough permissions for the purposes of your build. Consider carefully adjudicating the role and permission systems of other platforms you use outside of CircleCI. Examples include IAM permissions on AWS, and GitHub's link:https://developer.github.com/v3/guides/managing-deploy-keys/#machine-users[Machine User] feature.
...
* If your project is open source and public, make note of whether you want to share your environment variables. On CircleCI, you can change a project's settings to control whether your environment variables can pass on to _forked versions of your repository_. That setting is **not enabled** by default. You can read more about these settings and open source security in our xref:guides:integration:oss.adoc#security[Open Source Projects Document].
