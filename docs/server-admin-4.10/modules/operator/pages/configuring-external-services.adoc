...
* Redis Cluster mode is not supported. A single-shard configuration with replicas (for example, link:https://aws.amazon.com/elasticache/[Amazon ElastiCache] in *Cluster Mode Disabled*) is supported.
...
If your Redis provider exposes a separate reader endpoint (for example, the ElastiCache reader endpoint), set `redis.readerHost` and `redis.readerPort` to that endpoint so read traffic is routed to the replica. If you do not set them, CircleCI Server uses `redis.host` and `redis.port` for both reads and writes.
...
ElastiCache TLS endpoints use AWS-issued certificates that chain to a public CA, so they work without further configuration.
