...
:page-audience: admin
:page-description: This page presents some items that should be considered when starting an air-gapped installation of CircleCI Server 4.10.
:experimental:
...
. After installing the Helm chart, patch the `circleci-proxy` service to use the internal IP of the desired k3s node as the load balancer. This IP must be in the range entered in the ConfigMap above. The example below uses the IP address `10.0.0.5`.
+
[source, bash]
----
