...
:page-audience: admin
:page-description: How to install the CircleCI Server 4.10 Helm deployment to an air-gapped environment.
:experimental:
...
Configure your `values.yaml` file for installing CircleCI Server by following the Create Helm Values section of the regular installation guide (xref:installation:phase-2-aws-core-services.adoc#create-helm-values[AWS], xref:installation:phase-2-gcp-core-services.adoc#create-helm-values[GCP]). Once this is complete, you will modify the fields listed in the steps below for air-gapped installation compatibility.

=== a. Configure global values

NOTE: For a full example of a `values.yaml` file for an air-gapped environment, see the xref:example-values.adoc[Example Values] page.
...
Set the `hostname` value to the internal hostname of your GitHub enterprise installation. Set the `enterprise` value to true.
...
Set an OAuth application and a personal access token in GitHub Enterprise and values provided as shown below. For more information about this process, see the documentation for creating a GitHub OAuth application. For details, see the xref:installation:phase-1-aws-prerequisites.adoc#create-a-new-github-oauth-app[AWS] or xref:installation:phase-1-gcp-prerequisites.adoc#create-a-new-github-oauth-app[GCP] installation guide.
...
object_storage:
  ...
  s3:
    enabled: true
    endpoint: http://<s3-internal-hostname>:<port>
    accessKey: <s3-root-user>
    secretKey: <s3-root-password>
----

NOTE: Update the endpoint protocol to `http` or `https` depending on your Object Store installation.


=== d. Configure build agent
In the distributor section of the `values.yaml` file, point `agent_base_url` to the `circleci-data` bucket you created in your Object Store installation.

[source, yaml]
----
distributor:
  agent_base_url: http://<s3-internal-hostname>:<port>/circleci-data/
----

NOTE: Update the protocol to `http` or `https` depending on your Object Store installation.
...
When following these instructions to re-host agents in a partially air-gapped environment, you can configure the machine provisioner for object Store as follows:

[source, yaml]
----
machine_provisioner:
  agent_base_url: http://<s3-internal-hostname>:<port>/circleci-data/machine-provisioner
----

[#configure-docker-provisioner]
=== g. Configure Docker provisioner

Update the `docker_provisioner` section of the `values.yaml` file to point `agent_base_url` to the Object Store bucket:

[source, yaml]
----
docker_provisioner:
  agent_base_url: http://<s3-internal-hostname>:<port>/circleci-data/docker-provisioner
----
...
----
helm install circleci-server ./circleci-server/ -n <kubernetes-namespace> --version {serverversion410} -f <path-to-values.yaml>
----
