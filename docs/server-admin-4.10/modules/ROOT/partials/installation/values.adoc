...
| string
| `""`
| Name of existing Kubernetes secret containing Redis password. The secret must contain the key specified by auth.existingSecretKey (default: "redis-password").
...
| string
| `""`
| Redis password. Used when redis.internal is false and auth.existingSecret is empty.

| redis.auth.username
| string
| `""`
| Redis ACL username (Redis 6+). Leave empty for password-only auth.
...
| bool
| `true`
| Set to false if you want to use an externalized Redis instance.
...
| string
| `""`
| Redis reader/replica host. Defaults to redis.host if empty.

| redis.readerPort
| string
| `""`
| Redis reader/replica port. Defaults to redis.port if empty.
...
| bool
| `false`
| Set to true if Redis connection uses TLS/SSL.
