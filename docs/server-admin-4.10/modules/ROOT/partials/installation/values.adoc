...
| string
| `""`
| SSH key algorithm for CircleCI-generated deploy keys. Leave empty for the default (Ed25519). Set to "ssh-rsa" for FIPS 140 environments
...
| docker_provisioner.use_podman
| bool
| `false`
| Enable podman as the container runtime instead of docker. Defaults to false.
...
| int
| `0`
| EBS volume IOPS value. Backend default is 5000 if not specified. +
IOPS can range from 3,000 to 16,000 for gp3 volumes. +
Maximum of 500 IOPS per GiB of volume size.

| machine_provisioner.providers.ec2.throughput
| int
| `0`
| EBS volume throughput in MB/s. Backend default is 500 if not specified. +
...
| mongodb.customLivenessProbe.exec.command[0]
| string
| `"mongosh"`
|

| mongodb.customLivenessProbe.exec.command[1]
| string
| `"--eval"`
|

| mongodb.customLivenessProbe.exec.command[2]
| string
| `"db.adminCommand('ping')"`
|

| mongodb.customLivenessProbe.failureThreshold
| int
| `6`
|

| mongodb.customLivenessProbe.initialDelaySeconds
| int
| `30`
|

| mongodb.customLivenessProbe.periodSeconds
| int
| `10`
|

| mongodb.customLivenessProbe.successThreshold
| int
| `1`
|

| mongodb.customLivenessProbe.timeoutSeconds
| int
| `5`
|

| mongodb.customReadinessProbe.exec.command[0]
| string
| `"bash"`
|

| mongodb.customReadinessProbe.exec.command[1]
| string
| `"-ec"`
|

| mongodb.customReadinessProbe.exec.command[2]
| string
| `"mongosh --eval 'db.hello().isWritablePrimary \|\| db.hello().secondary' \| grep -q 'true'\n"`
|

| mongodb.customReadinessProbe.failureThreshold
| int
| `6`
|

| mongodb.customReadinessProbe.initialDelaySeconds
| int
| `5`
|

| mongodb.customReadinessProbe.periodSeconds
| int
| `10`
|

| mongodb.customReadinessProbe.successThreshold
| int
| `1`
|

| mongodb.customReadinessProbe.timeoutSeconds
| int
| `5`
|
...
| string
| `"8Gi"`
| To increase PVC size, follow this guide: xref:../operator/expanding-internal-database-volumes#[Expanding internal database volumes]
...
| mongodb.readinessProbe.enabled
| bool
| `false`
|
...
| Nomad mTLS (2 Options), strongly suggested for RPC communication +
Encrypts traffic and authenticates clients to ensure no unauthenticated clients can join the cluster. +
*Option 1:* Provide the CACertificate, certificate, and privateKey values - CircleCI will create the secret for you. +
*Option 2:* Leave the value blank (default) and create the secret yourself. If the values are populated, CircleCI will use them. +
...
| notifications.audit_log_batcher.replicas
| int
| `1`
| Number of replicas to deploy for the audit-log-batcher deployment.

| notifications.audit_log_batcher.resources.limits.cpu
| string
| `"2000m"`
| CPU limit for the audit-log-batcher deployment.

| notifications.audit_log_batcher.resources.limits.memory
| string
| `"2Gi"`
| Memory limit for the audit-log-batcher deployment.

| notifications.audit_log_enricher.replicas
| int
| `1`
| Number of replicas to deploy for the audit-log-enricher deployment.

| notifications.audit_log_enricher.resources.limits.cpu
| string
| `"2000m"`
| CPU limit for the audit-log-enricher deployment.

| notifications.audit_log_enricher.resources.limits.memory
| string
| `"256Mi"`
| Memory limit for the audit-log-enricher deployment.
...
| string
| `""`
| The json web key (JWK) or key set (JWKS) used for signing ID tokens. Value should be base64 encoded. Not required if json_web_keys_secret_name is set.

| oidc_service.json_web_keys_secret_name
| string
| `""`
| Name of a pre-existing Kubernetes Secret containing the JWKS in a key named "json-web-keys". When set, the chart will not create its own Secret and will reference this one instead. Use this to integrate with external secret managers (e.g. External Secrets Operator, CSI Secret Store Driver) that sync secrets from AWS Secrets Manager, HashiCorp Vault, etc.
...
| opentelemetry-collector.config.receivers.statsd.endpoint
| string
| `"0.0.0.0:8125"`
|
...
| opentelemetry-collector.config.service.pipelines.metrics.exporters[0]
| string
| `"debug"`
|

| opentelemetry-collector.config.service.pipelines.metrics.receivers[0]
| string
| `"statsd"`
|
...
| opentelemetry-collector.ports.statsd.containerPort
| int
| `8125`
|

| opentelemetry-collector.ports.statsd.enabled
| bool
| `true`
|

| opentelemetry-collector.ports.statsd.protocol
| string
| `"UDP"`
|

| opentelemetry-collector.ports.statsd.servicePort
| int
| `8125`
|
...
| string
| `""`
| Name of an existing Kubernetes secret containing the Redis password. The secret must contain the key specified by `redis.auth.existingSecretKey`. Used when `redis.internal` is `false`.

| redis.auth.existingSecretKey
| string
| `"redis-password"`
| Key within `redis.auth.existingSecret` that holds the Redis password.

| redis.auth.password
| string
| `""`
| Redis password. Used when `redis.internal` is `false` and `redis.auth.existingSecret` is empty. CircleCI Server creates the secret from this value.
...
| string
| `"redis-master"`
| Redis primary host. Used when `redis.internal` is `false`.
...
| bool
| `true`
| Set to `false` to use an externalized Redis instance.
...
| int
| `6379`
| Redis primary port. Used when `redis.internal` is `false`.

| redis.readerHost
| string
| `""`
| Redis reader/replica host. Used when `redis.internal` is `false`. Defaults to `redis.host` if empty.

| redis.readerPort
| string
| `""`
| Redis reader/replica port. Used when `redis.internal` is `false`. Defaults to `redis.port` if empty.
...
| bool
| `false`
| Set to `true` if the Redis connection uses TLS/SSL. Used when `redis.internal` is `false`.

| redis.sslSkipVerify
| bool
| `false`
| Set to `true` to skip TLS certificate verification, for example when the Redis endpoint uses a self-signed certificate. Only applies when `redis.ssl` is `true`. Custom CA certificates are not supported.
...
| Tink Configuration +
Tink is given precedence over vault. If tink.enabled is true, vault will not be deployed. Tink or vault must be set once at install and cannot be changed. +
