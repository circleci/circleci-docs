...
| Configure Nomad clients mTLS only when using External Nomad Servers. Required when `nomad.server.internal: false`. All three fields must be provided: `CACertificate` (base64 encoded CA cert to verify server), `certificate` (base64 encoded client cert for authentication), and `privateKey` (base64 encoded private key).
...
| Tink Configuration +
Tink is given precedence over vault. If tink.enabled is true, vault will not be deployed. Tink keys can be rotated (see link:https://support.circleci.com/hc/en-us/articles/39931197265691-Rotating-Tink-keys-in-CircleCI-Server[Rotating Tink keys in CircleCI Server]). To migrate from Vault to Tink, see link:https://github.com/CircleCI-Public/server-scripts/tree/main/vault-to-tink[server-scripts vault-to-tink]. +
