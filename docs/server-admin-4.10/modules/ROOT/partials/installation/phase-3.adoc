Before you begin with the CircleCI Server 4.10 execution environment installation phase, ensure you have run through the following:
...
nomad:
  ...
  clients:
    mTLS:
      ...
      certificate: <base64 encoded Client Certificate from your terraform output>
      privateKey: <base64 encoded Client Private Key from your terraform output>
----
.. If you externalize your Nomad servers, remove the following blocks from your `values.yaml`: `nomad.server.replicas`, `nomad.server.gossip` and `nomad.server.rpc`.
...
kubectl -n <namespace> create secret generic nomad-mtls \
  --from-literal=cert.pem="<base64-encoded-certificate>" \
  --from-literal=key.pem="<base64-encoded-private-key>" \
  --from-literal=ca.pem="<base64-encoded-ca-certificate>"
...
Once you have configured the environment variables and contexts, rerun the Reality Check tests. You will see the features and resource jobs complete successfully. Your test results will look something like the following:
...
+
NOTE: If deploying in a specific zone, you need to specify a subnetwork for AWS.
+
[source,yaml]
----
machine_provisioner:
...
. *Create policy:*
+
Create a `policy.json` file with the following content. Fill in the ID of the machine provisioner security group created in step 2 (`MachineProvisionerSecurityGroupID`) and the VPC ID (`vpcID`) below.
+
[source,json]
----
{
...
----
+
NOTE: If you are deploying CircleCI Server in a shared VPC, create this user in the project in which you intend to run your VM jobs.
...
After running the following command, you will have a file named `circleci-server-vm-keyfile` in your local working directory. You will need this when you configure your server installation.
...
Once you have configured and deployed CircleCI Server, the next step is to validate that machine provisioner is operational. You can rerun the Reality Check project within your CircleCI installation to see the machine provisioner jobs complete. At this point, all tests will pass.
