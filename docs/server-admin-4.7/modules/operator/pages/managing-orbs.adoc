...
When importing orbs, the CLI must be able to talk to the server installation and to `circleci.com`. If your server installation is behind a proxy, configure the CLI to use the proxy for requests to `circleci.com`, rather than proxying requests to the server install. For example:
