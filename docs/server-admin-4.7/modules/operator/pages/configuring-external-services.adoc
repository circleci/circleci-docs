...
This page describes how to configure external services for use with a new CircleCI Server v4.7 installation. It also covers migrating internal PostgreSQL and MongoDB data from an existing CircleCI Server v4.7 installation to your externalized datastores.
...
You can now connect to your external PostgreSQL. If not, resolve any issues before proceeding.
...
The changes will take effect upon running `helm install/upgrade`. If you are completing a migration to an externalized PostgreSQL instance, run `helm upgrade`. The scaled down pods will then be scaled back to their replica numbers, as defined by your `values.yaml`.


[#backing-up-postgresql]
=== Back up PostgreSQL
PostgreSQL provides official documentation for backing up and restoring your PostgreSQL 12 install. See the link:https://www.postgresql.org/docs/12/backup.html[PostgreSQL backup and restore documentation].
...
You can now connect to your external MongoDB. If not, resolve any issues before proceeding.
...
The changes will take effect upon running `helm install/upgrade`. If you are completing a migration to an externalized MongoDB instance, run `helm upgrade`. The scaled down pods will then be scaled back to their replica numbers, as defined by your `values.yaml`.
