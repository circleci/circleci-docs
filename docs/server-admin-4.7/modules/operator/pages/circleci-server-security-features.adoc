...
:page-audience: admin
:page-description: This document outlines security features built into CircleCI server v4.7 and related integrations.
:icons: font
:toc: macro
:toc-title:
...
| `scope`
| If the target is owned by an account in the CircleCI domain model, fill in the account field with the account name and ID. This data is a JSON blob that will always contain `id` and `type` and will likely contain `name`.
