...
:page-audience: admin
:page-description: This document outlines recommendations for how to back up and restore your CircleCI server v4.7 instance data and state.
:icons: font
:toc: macro
:toc-title:
...
While operating and administering CircleCI Server, you need to consider how to maintain backups and recover your installation. You may need to migrate it to another cluster, or recover from a critical event.

CircleCI recommends link:https://velero.io/[Velero] for backup and restore. This approach restores your application's data. It also restores the state of the Kubernetes cluster and its resources at the time of the backup. CircleCI Server supports backup and restore with Velero `1.12`. This document outlines recommendations for how to back up and restore your CircleCI Server instance data and state using link:https://velero.io/[Velero].
