...
----
+
The address pool can be named something other than "default", but the annotations in `values.yaml` will need to be updated. If there is only one k3s node, use the same IP repeated for the address range (for example, `10.0.0.5-10.0.0.5`).
...
. After installing the Helm chart, the `circleci-proxy` service must be patched to use the internal IP of the desired k3s node to act as the load balancer. This IP must be in the range entered in the ConfigMap above. The example below uses the IP address `10.0.0.5`.
+
[source, bash]
----
...
=== Configuring the TLS certificates array
Instead of providing a list of `hostname:port` combinations for certificates to import, provide the public certificate chains of the corresponding TLS certificates to trust in the `values.yaml` file. Add these to the `tls.certificates` value as a list of base64 encoded certificates strings.
