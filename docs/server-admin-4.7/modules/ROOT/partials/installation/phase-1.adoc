CircleCI Server v4.7 is installed as a Helm chart. The installation process is broken down into four phases. Each phase has a validation step at the end, allowing you to confirm success before moving to the next phase. Depending on your requirements, phases 3 and 4 may include multiple steps. This installation guide assumes you have already read the xref:server-admin:overview:circleci-server-overview.adoc[CircleCI Server 4.7 Overview].
...
NOTE: **Installing server locally?** You can skip this step. Still follow the resourcing recommendation of using /18 CIDR blocks. That gives you adequate capacity for both your Kubernetes cluster and the Nomad clients that run your jobs.
...
CircleCI Server installs into an existing Kubernetes cluster. If you have not already created a cluster, create one next. Instructions for creating a cluster are provided below. If you are installing locally, first consider the sizing, version, and permissions requirements.
...
----
+
NOTE: Your kube-context is updated with the new cluster credentials automatically.
+
...
Registering a new GitHub OAuth app for CircleCI Server gives you authorization control over your server installation. It also lets CircleCI update GitHub projects and repositories with build status information. The following steps apply for both GitHub.com and GitHub Enterprise.
...
By default, CircleCI Server creates self-signed certificates to get you started. In production, supply a certificate from a trusted certificate authority. The link:https://letsencrypt.org/[Let's Encrypt] certificate authority, for example, can issue a free certificate using their link:https://certbot.eff.org/[Certbot] tool. The sections below cover using Google Cloud DNS and AWS Route 53.
...
Once you have created your certificates using one of the methods described below, use the following commands to retrieve them later during this installation:
...
You may have a requirement to terminate TLS for CircleCI Server outside the application. Terminating TLS outside the application is an alternate method to using ACM or supplying the certificate chain during Helm deployment. An example would be a proxy running in front of the CircleCI installation providing TLS termination for your domain name. In this case the CircleCI application acts as the backend for your load balancer or proxy.
...
If you are installing locally rather than in AWS or GCP, follow the MinIO instructions in the xref:server-admin:air-gapped-installation:phase-2-configure-object-storage.adoc[Air-gapped Installation Docs].

NOTE: If you are installing behind a proxy, put object storage behind the same proxy. Otherwise, proxy details will need to be supplied at the job level within every project `.circleci/config.yml` to allow artifacts, test results, cache save and restore, and workspaces to work. For more information see the xref:server-admin:installation:installing-server-behind-a-proxy.adoc[Installing Server Behind a Proxy] guide.
...
If your server installation runs in a GKE cluster, ensure your current IAM user is cluster admin. CircleCI needs to create RBAC (role-based access control) objects. More information can be found in the link:https://cloud.google.com/kubernetes-engine/docs/how-to/role-based-access-control[GKE documentation].
