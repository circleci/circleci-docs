...
NOTE: During the installation process, you may use the following command to generate a random alphanumeric value as required: `cat /dev/urandom | LC_ALL=C tr -dc 'a-zA-Z0-9' | head -c <num-of-chars>`. This command works on any *nix based system.

[#api-token]
=== a. API token

The application requires a Kubernetes Secret containing an API token. This API token is used to facilitate internal API communication to `api-service`. Use a random string and store it securely. CircleCI will not be able to recover this value if lost. You can create the Kubernetes Secret yourself, or CircleCI can create it for you.
...
The application requires a session cookie key Kubernetes Secret, which CircleCI uses to sign session cookies. The Secret must be exactly 16 characters long. Use a random string and store it securely. CircleCI will not be able to recover this value if lost. You can create the Kubernetes Secret, or CircleCI can create it for you.
...
The application requires a Kubernetes Secret containing signing and encryption keysets. These keysets are used to encrypt and sign artifacts generated by CircleCI. You created these keysets during the prerequisites phase (xref:server-admin:installation:phase-1-gcp-prerequisites.adoc#encryption-signing-keys[GCP Prerequisites], xref:server-admin:installation:phase-1-aws-prerequisites.adoc#encryption-signing-keys[AWS Prerequisites]). CircleCI will not be able to recover the values if lost. Depending on how you prefer to manage Kubernetes Secrets, there are two options.
...
Enter the domain name you specified when creating your Frontend TLS key and certificate (xref:server-admin:installation:phase-1-aws-prerequisites.adoc#frontend-tls-certificates[AWS], xref:server-admin:installation:phase-1-gcp-prerequisites.adoc#frontend-tls-certificates[GCP]).
...
[#github]
==== i. GitHub
These instructions are for the GitHub.com, **not** GitHub Enterprise. Use the client ID and secret you created with your GitHub OAuth application in the prerequisites phase (xref:server-admin:installation:phase-1-aws-prerequisites.adoc#create-a-new-github-oauth-app[AWS], xref:server-admin:installation:phase-1-gcp-prerequisites.adoc#create-a-new-github-oauth-app[GCP]).
...
In the case of GitHub Enterprise add the `defaultToken` created in the prerequisites phase (xref:server-admin:installation:phase-1-aws-prerequisites.adoc#create-a-new-github-oauth-app[AWS], xref:server-admin:installation:phase-1-gcp-prerequisites.adoc#create-a-new-github-oauth-app[GCP]) to the `GitHub` section. The hostname must not include the protocol, ex: `github.exampleorg.com`.
...
ifndef::env-gcp[]
Regardless of your storage provider, the bucket name you created during the xref:server-admin:installation:phase-1-aws-prerequisites.adoc#object-storage-and-permissions[Prerequisites Phase] will need to be included.
endif::env-gcp[]

ifndef::env-aws[]
Regardless of your storage provider, the bucket name you created during the xref:server-admin:installation:phase-1-gcp-prerequisites.adoc#object-storage-and-permissions[Prerequisites Phase] will need to be included.
endif::env-aws[]
...
Under `object_storage.s3`, you may provide the `accessKey` and `secretKey`, the `irsaRole`, or nothing. You created these during the prerequisites steps.
...
If you wish to store artifacts larger than 5GB, you will need to update your trust policy for your xref:phase-1-aws-prerequisites.adoc#s3-storage[IRSA Role]. Then disable presigned mode by adding the following to the `object_storage.s3` section:
...
Under `object_storage.gcs` you may add `service_account`,  `workloadIdentity`, or neither. You created these keys and roles during the prerequisites steps.
...
* configure the `no_proxy` hosts and subnets. This will include localhost, your GitHub Enterprise host (optional), the hostname of your CircleCI installation (see xref:server-admin:installation:installing-server-behind-a-proxy.adoc#known-limitations[Known Limitations] for an explanation), and the CIDR of Nomad.
...
+
Tink keys can be rotated after deployment. See link:https://support.circleci.com/hc/en-us/articles/39931197265691-Rotating-Tink-keys-in-CircleCI-Server[Rotating Tink keys in CircleCI Server] for the procedure. To migrate from Vault to Tink (Vault is deprecated), see link:https://github.com/CircleCI-Public/server-scripts/tree/main/vault-to-tink[Vault to Tink server script].
...
. CircleCI Server stores your generated keyset in a Kubernetes Secret. You may generate this Secret in either of the following ways:
+
[tabs]
====
You create Secret::
...
CircleCI Server optionally supports xref:guides:permissions-authentication:openid-connect-tokens.adoc[OpenID Connect Tokens] (OIDC) in jobs.
...
[#create-dns-entry]
== 5. Create DNS entry
Create a DNS entry for your nginx load balancer, for example, `circleci.your.domain.com` and `app.circleci.your.domain.com`. The DNS entry must align with the DNS names used when creating your TLS certificate and GitHub OAuth app during the prerequisites steps. All traffic will be routed through this DNS record.
...
You can now navigate to your CircleCI Server installation and log in to the application successfully.

Now we will move on to build services. It may take a while for all your services to be up. You can periodically check by running the following command (you are looking for the `frontend` pod to show a status of `running` and **ready** will show `1/1` in the output):
