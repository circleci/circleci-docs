...
To add an icon to the flow of text, use the `icon` inline macro. This automatically handles light and dark theme switching.
...
The icon macro automatically includes both light and dark versions of the icon. You can optionally provide custom alt text:

.Example with custom alt text
[source,adoc,subs="-macros"]
----
Select the icon:more[More options icon] to see additional settings.
----

=== How icon theming works

When you use pass:[icon:more[]], the extension automatically expands it to include both versions:

* `more.svg` - Light theme version (dark icon on light background).
* `more-dark.svg` - Dark theme version (light icon on dark background).

The appropriate version is shown based on the user's theme preference.

=== Creating dark mode icons

When adding a new icon, create two versions:

1. **Light theme version** (++`name.svg`++): Dark icon color (typically `#161616`)
2. **Dark theme version** (++`name-dark.svg`++): Light icon color (typically `#D7DBE3`)

Both files should be placed in:

[,shell]
----
docs/guides/modules/ROOT/images/icons/
├── more.svg          # Light theme (dark icon)
└── more-dark.svg     # Dark theme (light icon)
----

=== Available icons

You can find available icons by looking here:
...
NOTE: If you use the older `image:` macro syntax (for example, pass:[`image:guides:ROOT:icons/example.svg[alt text, role="no-border"`]), it will not automatically switch themes. Use the pass:[icon:name[Icon alt text]] macro instead for theme-aware icons.

== Diagrams

You can add mermaid diagrams to a page using Beautiful Mermaid. The diagrams automatically adapt to light and dark themes.
...
The diagram renders as SVG at build time and automatically switches between light and dark variants based on the user's theme preference. All standard mermaid syntax is supported, including flowcharts, state diagrams, sequence diagrams, class diagrams, ER diagrams, and XY charts.
pass:[<!-- vale on -->]