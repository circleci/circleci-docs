...
Consider how a team might use credits. In this example, your team is divided into several groups working on different projects. Some projects are larger while others are smaller and need fewer resources from a CI/CD configuration. With credits, it is possible to specify exactly where and when you need to maximize machine resources.
...
For comprehensive cost tracking and analysis, you can integrate with Vantage or Finout to monitor and manage your CI/CD costs alongside other infrastructure spending. See the xref:integrate-with-vantage-for-cost-management.adoc[Integrate CircleCI With Vantage] and xref:integrate-with-finout-for-cost-management.adoc[Integrate CircleCI With Finout] guides for more information.
