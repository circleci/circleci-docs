...
:experimental:
:page-aliases: test-splitting-tutorial.adoc
...
NOTE: This page covers test splitting with the legacy `circleci tests` command family. CircleCI's preferred way to split tests is now `circleci testsuite`, which supports the same splitting capabilities plus dynamic test splitting, and is available on CircleCI Cloud only. See xref:test:test.adoc#smarter-testing[Get Started With Smarter Testing]. If you are on CircleCI Server, continue to use `circleci tests glob` and `circleci tests split`. `circleci tests run`, including its examples on this page, is available on CircleCI Cloud only and is *not* supported on CircleCI Server. See <<the-tests-split-command,The tests split command>> below.
...
The following how-to guide walks you through the following:

* Steps required to glob your test files.
* Split your tests evenly across parallel containers or VMs.
...
NOTE: `circleci tests run`, used in the examples below, is available on CircleCI Cloud only. If you are on CircleCI Server, use `circleci tests split` instead. See <<the-tests-split-command,The tests split command>>.
...
.Diagram showing how parallelism works for different executor types
image::guides:ROOT:executor_types_plus_parallelism.png[Parallelism]

[#use-parallelism-with-self-hosted-runners]
=== Use parallelism with self-hosted runners

To use parallelism with jobs that use xref:execution-runner:runner-overview.adoc[Self-Hosted Runners], ensure that you have at least two runners associated with the resource class that your job uses. If you set the parallelism value to be greater than the number of active runners, the excess parallel tasks will queue until a runner is available.
...
CircleCI's intelligent test splitting allows you to:

* Specify a number of identical execution environments for a job.
* Split your test suite into buckets so that the tests can be run in parallel on your configured execution environments.

Test splitting reduces the length of time taken for testing in your pipelines.

You can split your tests up in a few different ways:

* Alphabetically, by name, which is the default:
+
[,yaml]
----
# ~/.circleci/config.yml
...
CircleCI attempts to detect the granularity of the test split (for example, to split by file name, or down to class name) based on the input to the split command. You may need to choose a different timing type depending on how your test coverage output is formatted, using the `--timings-type` option. Valid timing types are:
...
Using *timing-based* test splitting, timing data from the _previous_ test run is used to split a test suite as evenly as possible over a specified number of test environments. This delivers the lowest possible test time for the compute power in use.

.Diagram showing the impact of using timing-based test splitting
...
NOTE: `circleci tests run` is available on CircleCI Cloud only. It is *not* supported on CircleCI Server. If you are on CircleCI Server, use `circleci tests split` instead. See <<the-tests-split-command,The tests split command>> below.
...
The following table shows a full list of option flags available when using `circleci tests run`.
...
| `--timings-type`
| string
| name of the field to use from historical test results. CircleCI matches it against the test names given to the command to determine historical timings, previous status, and flakiness. Available values: `autodetect`, `classname`, `testname`, `filename` (default).
| No
...
On CircleCI Cloud, `circleci tests split` has been superseded by `circleci tests run`, as this command also gives you access to the xref:test:rerun-failed-tests.adoc[Rerun Failed Tests] options. `circleci tests run` is *not* available on CircleCI Server, so if you are on CircleCI Server, continue to use `circleci tests split`.
...
* xref:parallelism-faster-jobs.adoc[Parallelism and Test Splitting]
