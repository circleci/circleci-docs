...
Artifacts persist data after a job completes and store the outputs of your build process.
...
.Artifacts data flow
image::guides:ROOT:Diagram-v3-Artifact.png[Artifacts data flow]
...
Navigate to a pipeline's *Job* page on the link:https://app.circleci.com/[CircleCI web app] to find the *Artifacts* tab. Artifacts are stored on Amazon S3 and are protected with your CircleCI account for private projects. Individual artifact files have a maximum size limit of 3GB.

.Artifacts tab in job page
image::guides:ROOT:artifacts.png[Artifacts tab screenshot]
...
For information on managing network and storage usage, see the xref:persist-data.adoc[Persisting Data] page.
...
If you need to restrict the connections allowed in your jobs, consider enabling xref:security:ip-ranges.adoc[IP Ranges]. This enables CircleCI jobs to go through a set of well-defined IP address ranges.
...
. Create a `Makefile` with the following lines. Note that the line starting with `gcc` must be indented with a tab character, not spaces:
+
[,make]
----
all:
...
The following shows a full `config.yml` that compiles the example C abort program, and collects the core dumps as artifacts.

.Collecting core dumps as artifacts
[,yaml]
----
version: 2.1
jobs:
...
.Core dump file in Artifacts page
image::guides:ROOT:core_dumps.png[Core dump file in Artifacts page]
...
== Downloading all artifacts for a build

include::ROOT:partial$notes/server-api-examples.adoc[]

To download your artifacts with `curl`, follow the steps below.

. xref:toolkit:managing-api-tokens.adoc#creating-a-personal-api-token[Create a Personal API Token] and copy it to the clipboard.
...
To download all artifacts from a workflow at the same time, use the following method. You will need your workflow ID. Copy your workflow ID from the end of the URL when viewing the workflow page in the CircleCI web app:
...
You can read more about using CircleCI's API to interact with artifacts in the link:https://circleci.com/docs/api/v1/#artifacts[API v1.1 reference guide].
...
| `:vcs-type`
| The version control system (VCS) you are using. Either `github`, `bitbucket`, or `gitlab`.
...
When using self-hosted runners, there is a network and storage usage limit included in your plan. Actions related to artifacts accrue network and storage usage. Once your usage exceeds your limit, charges will apply.

Retaining artifacts for long periods increases storage costs. Determine why you are retaining artifacts before setting retention periods. For example, you might retain artifacts to troubleshoot failing builds. Once the build passes, the artifact is likely not needed. Set a low storage retention period for artifacts if this suits your needs.

You can customize storage usage retention periods for artifacts on the https://app.circleci.com/[CircleCI web app] by navigating to menu:Plan[Usage Controls]. For information on managing network and storage usage, see the xref:persist-data.adoc#managing-network-and-storage-usage[Persisting Data] page.
...
The `store_artifacts` step is often used on a large directory when only a few files are really needed. Check which artifacts are being uploaded and why.
...
Artifacts that are text can be compressed at very little cost. If you must upload a large artifact you can upload them to your own bucket at no cost.
...
If your pipelines build a binary or uberJAR, consider whether these are necessary for every commit. Consider uploading artifacts only on failure, only on success, or only on specific branches using a filter.

[discrete#only-upload-test-results-on-failure]
==== Only upload test results on failure

xref:reference:ROOT:configuration-reference.adoc#the-when-attribute[The When Attribute] lets you filter what happens within a step in your configuration. The `when` attribute can be set to `on_success`, `on_fail` or `always`. To only upload artifacts when tests fail, add the `when: on_fail` attribute to the `store_artifacts` step:
