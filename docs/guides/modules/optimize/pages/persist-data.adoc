...
Storage usage is calculated once, at the moment an object is saved, rather than accrued day by day while it remains in storage. CircleCI multiplies the object's size by its configured retention period to get its total GB-months, and adds that amount to your storage usage for the billing period in which it was saved.
