...
Data stored at the end of a billing cycle is restored when the cycle restarts, using the storage period set in your usage controls. For example, suppose you restore and save a cache on day 25 with a 10-day storage period. If no changes are made by day 30, a new cache is built on day 31 and saved for a new 10-day period.
