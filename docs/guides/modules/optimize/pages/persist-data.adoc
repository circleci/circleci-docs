...
Dependency caching and build tool cache both use CircleCI's cache storage, but they cache different things. Dependency caching (above) saves what your package manager downloads. Build tool cache saves what your build tool itself produces: task or build outputs for tools such as Gradle, Bazel, and Turborepo. Use the `with_tool_cache` step instead of a `save_cache`/`restore_cache` pair when you want to cache one of these tools' own build outputs.
