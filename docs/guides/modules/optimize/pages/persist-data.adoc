...
:page-description: An introductory guide to the various ways to persist data in CircleCI
:experimental:
...
Because caches are global within a project, a cache saved on one branch will be used by jobs run on other branches. Caches are only suitable for data that can be shared across branches.
...
Dependency caching and build tool cache both use CircleCI's cache storage, but they cache different things. Dependency caching (above) saves what your package manager downloads. Build tool cache saves what your build tool itself produces: task or build outputs for tools such as Gradle, Bazel, Turborepo, and Xcode. Use the `with_tool_cache` step instead of a `save_cache`/`restore_cache` pair when you want to cache one of these tools' own build outputs.

* For more information, see the xref:build-tool-cache.adoc[Build Tool Cache] page.

[#cache-optimization]
=== Caching optimization

Optimize your pipelines to ensure you are getting the most out of your network and storage usage. For example, when looking for opportunities to reduce data usage, consider whether specific usage is providing enough value to be kept. In the case of caches, consider whether the developer or compute time-saving from the cache outweighs the cost of the download and upload.
...
* For more information on caching optimization and other caching strategies, like partial dependency caching, caching tradeoffs, and using multiple caches, see the xref:caching.adoc[Caching] page.
...
If you notice your workspace usage is high, search for the `persist_to_workspace` command in your `.circleci/config.yml` file. This helps identify all jobs using workspaces and determine if all items in the path are necessary.

You also might find that you are only using workspaces to be able to re-run builds from failure. Once the failing build passes, the workspace might not be needed. Setting a low storage period of, for example, one day, might be suitable for your projects. A low storage retention period for workspaces will save costs by not keeping unnecessary data in storage.
...
Optimization goes beyond speeding up your builds and improving efficiency. Optimization can also help reduce costs. The information below describes how your network and storage usage is accumulating, and will help you find ways to optimize and implement cost saving measures.
...
For more questions about your network and storage usage, contact link:https://support.circleci.com/hc/en-us/requests/new[support] and open a ticket for *Accounts / Billing*.
...
.Save cache step output
image::guides:ROOT:job-output-save-cache.png[save-cache-job-output]

[#custom-storage-usage]
=== Custom storage usage

If you are on a paid plan you can customize storage usage retention periods for the following:

* Workspaces (default and maximum is 15 days).
* Caches (default and maximum is 15 days).
* Artifacts (default and maximum is 30 days).

To access custom storage controls from the link:https://app.circleci.com/[CircleCI web app], select menu:Plan[Usage Controls]. Here you can set custom storage periods by adjusting the sliders for each object type.

Once you save your preferences, they take effect immediately. Previously created objects that are stored with a different retention period will continue to persist for the retention period set when the object was created.
...
Storage retention periods run independently of your billing cycle. When you save a cache, its retention countdown begins immediately for the configured storage period (for example, 10 days). Storage retention is not affected by when your billing cycle starts or ends. For example, if you save a cache on day 25 with a 10-day storage period, that cache remains in storage through day 35. This period spans the end of one billing cycle and the start of the next.
...
Storage usage is calculated once, at the moment an object is saved. Storage usage is not accrued day by day while it remains in storage. CircleCI multiplies the object's size by its configured retention period to get its total GB-days. This amount is added to your storage usage for the billing period in which it was saved.
...
Network transfer to self-hosted runners can be reduced by using custom local storage, such as a persistent volume, instead of the built-in caches and workspaces provided by CircleCI.

To minimize storage billing, evaluate your needs and set custom retention periods for artifacts, workspaces, and caches. Navigate to menu:Plan[Usage Controls] on the link:https://app.circleci.com/[CircleCI web app].
