...
[#concurrency]
== Concurrency
Within CircleCI, concurrent jobs can be classified as one of two categories:
...
To keep the system stable for all CircleCI customers, we implement different soft concurrency limits on each of the xref:reference:ROOT:configuration-reference.adoc#resourceclass[Resource Classes] for different executors.
...
=== Fan-out/fan-in workflow example
A more complex example runs a common build job, then fans out to run a set of acceptance test jobs concurrently. It then fans in to run a common deploy job. This flow is illustrated below.
...
[#see-also]
== See also
- xref:orchestrate:workflows.adoc[Workflows]
