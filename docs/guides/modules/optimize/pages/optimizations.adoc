...
Choosing the right Docker image for your project can have a huge impact on build time. For example, a basic language image requires downloading dependencies and tools on every pipeline run. An image with those dependencies pre-installed saves that time on every build run. When configuring your projects and specifying images, consider the following options:
...
Some build tools (Gradle, Bazel, Turborepo, and Xcode) have their own built-in remote caching mechanisms for build outputs, separate from dependency caching. Use the `with_tool_cache` step to back these with a CircleCI-managed remote cache instead of hand-rolling `save_cache`/`restore_cache` steps for tool-specific build outputs.

* Read more on the xref:build-tool-cache.adoc[Build Tool Cache] page.
...
If your project has a large test suite, configure your build to use xref:reference:ROOT:configuration-reference.adoc#parallelism[`parallelism`] with xref:parallelism-faster-jobs.adoc[CircleCI's test splitting functionality] or a xref:parallelism-faster-jobs.adoc#other-ways-to-split-tests[third party application or library]. This splits your tests across multiple machines. CircleCI supports automatic test allocation across machines/containers on a file basis. You can also manually customize how tests are allocated.
...
* Follow the xref:parallelism-faster-jobs.adoc[Parallelism and Test Splitting] page.
...
* Simplify configuration (`.circleci/config.yml`)
* Automate repeated processes
* Accelerate project setup
* Simplify integration with third-party tools.

Read more about orbs on the xref:orbs:use:orb-intro.adoc[Orbs Overview] page.
...
* For information about how Yarn can potentially speed up builds and reduce errors, view the xref:caching.adoc#yarn-node[Caching Dependencies] page.
