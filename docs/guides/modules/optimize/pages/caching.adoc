...
*Example 2:* `Job1 -> Job2`. Job2 reads the cache written by Job1. If Job2 starts before Job1 has finished saving, it may find no cache. The race condition is resolved by the explicit workflow dependency, but verify the dependency link is in place if you see intermittent misses on a dependent job.
...
A cache key can include *templates* — dynamic values evaluated at runtime and interpolated into the key string:
...
The most reliable thing to cache is the *download cache* maintained by your package manager — the directory where it stores downloaded packages before installing them. The download cache differs from the installed environment (`node_modules`, `venv`, `vendor/bundle`).
...
For most dependency caches (under 300 MB) restore time is well within that threshold. For large caches — Conda environments, large Gradle repositories, binary assets — it is worth measuring:
...
Lock files — `Gemfile.lock`, `package-lock.json`, `yarn.lock`, `Pipfile.lock`, `poetry.lock`, `uv.lock` — are the preferred checksum source for cache keys. They capture the exact resolved dependency tree. The cache key therefore changes only when your dependencies change, not when an unpinned version range resolves differently.
...
For compilation caches, use a key that reflects what determines the output — typically a checksum of source files or a version string — rather than a dependency manifest.
...
Dependency managers sometimes leave unnecessary files in their directories — documentation, type stubs, test fixtures. Tools like link:https://www.npmjs.com/package/node-prune[node-prune] can remove these before the `save_cache` step, reducing cache size and expansion time on restore.
