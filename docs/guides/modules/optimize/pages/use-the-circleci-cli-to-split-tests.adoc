= Use the CircleCI task agent CLI to split tests
:page-platform: Cloud, Server v4+
:page-description: How-to guide for using the task agent CLI to split tests across parallel compute environments in a pipeline.
:experimental:
...
Test splitting requires the CircleCI task agent CLI together with parallelism. The task agent commands `circleci tests glob` and `circleci tests run` are used to define your test suite and allocate tests across multiple environments. The task agent CLI is automatically injected into your job at run-time, so there is no further setup required to use the `circleci tests` commands.

NOTE: The `circleci tests` commands are part of the task agent CLI as they require information that only exists within a CircleCI container. `circleci tests` commands are not available in the xref:toolkit:local-cli.adoc[CircleCI local CLI].
...
The task agent CLI supports globbing test files using the following patterns:
...
The task agent CLI attempts to auto detect the granularity of the test split (for example, whether to split by filename, or down to class name) based on the input to the `split` command. You may need to choose a different timing type depending on how your test coverage output is formatted, using the `--timings-type` option. Valid timing types are:
...
When provided with file paths, the task agent CLI can also split by file size. Use the `--split-by` flag with the `filesize` split type:
