...
NOTE: The `circleci tests` commands are part of the environment CLI as they require information that only exists within a CircleCI container. `circleci tests` commands are not available in the xref:toolkit:local-cli.adoc[CircleCI local CLI].
