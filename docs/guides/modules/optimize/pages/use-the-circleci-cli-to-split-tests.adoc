= Use the CircleCI environment CLI to split tests
...
NOTE: The `circleci tests` commands are part of the environment CLI as they require information that only exists within a CircleCI container. `circleci tests` commands are not available in the xref:toolkit:local-cli.adoc[CircleCI CLI].

NOTE: xref:execution-runner:runner-overview.adoc[Self-Hosted Runners] can invoke `circleci-agent` directly instead of using the CLI to split tests. The xref:execution-runner:runner-overview.adoc#circleci-self-hosted-runner-operation[task-agent] already exists on the `$PATH`, which removes an additional dependency when splitting tests.

NOTE: `circleci tests glob`/`run`/`split` are legacy commands. CircleCI's preferred way to run and split tests is now `circleci testsuite`, which is available on CircleCI Cloud only. See xref:test:test.adoc#smarter-testing[Get Started With Smarter Testing]. If you are on CircleCI Server, continue to use `circleci tests glob` and `circleci tests split`. `circleci tests run` is *not* supported on CircleCI Server, even though it is part of the same command family. See <<tests-split-examples,Using tests split>> below.
...
== Use the tests run command

NOTE: `circleci tests run`, covered in this section, is available on CircleCI Cloud only, for existing configurations. If you are on CircleCI Server, skip ahead to <<tests-split-examples,Using tests split>>.
...
Two commands are available for you to split your tests: `circleci tests run` and `circleci tests split`. On CircleCI Cloud, we recommend using `circleci tests run` as this enables you to also take advantage of the xref:test:rerun-failed-tests.adoc[Rerun Failed Tests] feature. The examples in this section use the `circleci tests run` command.

`circleci tests run` is *not* supported on CircleCI Server. If you are on CircleCI Server, use `circleci tests split` examples <<tests-split-examples,here>> instead.

[#tests-run-examples]
== Test framework examples using tests run

NOTE: `circleci tests run` is available on CircleCI Cloud only. If you are on CircleCI Server, use the <<tests-split-examples,`circleci tests split` examples>> below instead.

The following set of examples show how to use the `circleci tests run` command to split and run your tests across parallel execution environments. Using this command also allows you to use the xref:test:rerun-failed-tests.adoc[Rerun Failed Tests] feature.
...
* Each example below shows configuring your tests to be split by timing data. Splitting by timing data is the recommended approach, but other options include splitting by name or by file size. Read more about these options in the xref:parallelism-faster-jobs.adoc#introduction[Test Splitting and Parallelism Overview].
...
# Pass those tests as stdin to tests run
jobs:
  build:
    docker:
      - image: cimg/base:2023.11
...
By default, if you do not specify a method using the `--split-by` flag, `circleci tests split` expects a list of file names or class names and splits tests alphabetically by test name. You can provide this list in the following ways:
