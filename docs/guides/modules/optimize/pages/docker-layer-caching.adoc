...
DLC caches the individual layers of any Docker images built during your CircleCI jobs. Unchanged image layers are reused on subsequent job runs, rather than rebuilding the entire image every time. In short, the less your Dockerfiles change from commit to commit, the less time your image-building jobs will take to run.
...
For example, if building a Docker image takes over two minutes on the first run, and the Dockerfile is unchanged, subsequent builds happen instantly.
