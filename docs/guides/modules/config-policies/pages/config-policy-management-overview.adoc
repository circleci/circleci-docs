...
CircleCI config policies support two Rego language syntax versions: **v0** (the legacy syntax) and **v1** (the syntax introduced with link:https://www.openpolicyagent.org/docs/v0-upgrade[OPA 1.0]). Policy files are parsed as v0 by default, which is why the examples on this page (such as `policy_name["unique_policy_name"]`) use the legacy bracket syntax.
...
NOTE: If you use `if`, `contains`, or other v1 keywords without adding `import rego.v1` to your file, the policy will fail to parse with an error similar to `` `contains` keyword is required for partial set rules ``.
