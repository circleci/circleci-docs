...
# needed to use keywords like `in`.
...
Adding a new policy to the bundle added a new rule, which led to the failures. The decision was affected in two ways:
...
To illustrate this, the following steps show how to create a _helper_ function including some OPA tests, and run the `circleci policy test` command to see the results of those tests.
