...
The CLI provides a language-agnostic way of evaluating local policies against arbitrary config inputs. Using the CLI is the recommended way of developing and testing policies. It is similar to the previous command except that it provides a path to the local policies directory.
...
Policies that use `data.meta...` values like `vcs.branch` or `project_id` must also provide a JSON file mocking those values with `--metafile $PATH_TO_JSON`
...
Audit logs provide documentary evidence for a policy decision being made at a certain point in time.
...
* The command accepts the following filters for logs: `--after`, `--before`, `--branch`, `--status`, and `--project-id`. These filters are optional, and any combination of filters can be used to suit your auditing needs.
