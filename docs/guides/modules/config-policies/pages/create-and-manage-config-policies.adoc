...
CircleCI policies are managed by pushing directories of policies to CircleCI via the CLI. Following are recommended best practices:

* Store your policies in a repository in your VCS, within your organization. Storing policies in your VCS is how policies are managed internally at CircleCI. Pushing a policy bundle is done by triggering a CircleCI pipeline.

* Create a bot account for pushing policies, and use its associated CircleCI personal API token for authentication. For maximum security the token is be stored as an environment variable within a context, and that context must be restricted to groups that are responsible for managing policies. For more information, see the xref:security:contexts.adoc[Using Contexts] page.
...
. Select **Add Environment Variable**. Enter `CIRCLECI_CLI_TOKEN` for the name, and your personal API token string for the value. For more information, see the xref:toolkit:circleci-cli.adoc[Installing the CircleCI CLI] page. Then click **Add Environment Variable** again to complete.
...
workflows:
  main-workflow:
    jobs:
      - diff-policy-bundle:
          context: my-context
          filters: pipeline.git.branch != "main" # run the diff-policy-bundle job on every branch except main
      - push-policy-bundle:
          context: my-context
          filters: pipeline.git.branch == "main" # only run the push-policy-bundle job on the main branch
...
+
--
Push your changes to `main` and head to the CircleCI web app to see your policy pipeline. This assumes you set up your config policies repository with the sample configuration from the xref:create-and-manage-config-policies.adoc#manage-policies-with-your-vcs[Config Policies] guide.

For policy development, you can push to a development branch. You get a diff of your policy bundle when you push your changes, rather than your new policy being pushed to your CircleCI organization.
--
====
