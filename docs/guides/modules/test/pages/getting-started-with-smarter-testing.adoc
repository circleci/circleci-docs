...
. Install the latest `testsuite` CLI plugin. The `testsuite` CLI plugin is distributed through a Homebrew tap.
+
[source,console]
----
...
Smarter Testing operates on _test atoms_.

A _test atom_ is a single runnable unit that exercises one or more tests, depending on the specific test runner being used. For example:
...
. Run the test suite and confirm that the `discover` command finds the test atoms you expect:
+
[source,console]
----
...
. Add the `run` command.
. Define the JUnit output path in `outputs.junit`. This path determines where JUnit results are written. Later on, when you configure your CI job to run `circleci run testsuite`, `store_test_results` should point to the directory of this path.
+
[tabs]
====
Vitest::
...
TIP: Running the test suite locally creates a reports folder (matching your `outputs.junit` directory). Consider adding this directory to `.gitignore` to avoid committing test reports to your repository.
...
* Enable xref:use-dynamic-test-splitting.adoc[Dynamic Test Splitting] to evenly split tests across parallel nodes.
* Enable xref:auto-rerun-failed-tests.adoc[Auto Rerun Failed Tests] to automatically retry flaky tests.
