...
#Insert here - onboarded to closed preview? Plan? VCS? Org type?#
...
#Should we offer the option to not get started locally at this point? If not we should just say to do it rather than just "recommend" it#
...
. Install the latest `testsuite` CLI plugin. Steps to install via Homebrew are below. If you need to install the plugin on other platforms (Windows, Linux ARM, etc.) or prefer manual installation, reach out to the CircleCI team via the private Slack channel for preview customers or email mailto:minjun.seong@circleci.com[minjun.seong@circleci.com].
...
#TODO insert here: What is the discover command? What does it do? Why are we using it? How does it work?#
...
At this point, you are running your tests using the new `testsuite` command. Now you can adopt the Smarter Testing features to optimize your test execution:
