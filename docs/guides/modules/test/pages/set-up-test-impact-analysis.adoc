...
The behavior of the analysis and selection phases varies by branch and can be configured using command line flags. Learn more in the <<cli-flags,test suite flag options>> section.
...
Make sure that the command produces code coverage results, and that it is passed a test atom to run. For example, if your normal command to run a test with coverage is:

.Example test command using `go test` with coverage enabled
[source,console]
----
$ go test -coverprofile="cover.out" -cover -coverpkg ./... cmd/plugin
----
...
The coverage report output location can be specified with the template variable `<< outputs.go-coverage >>`. CircleCI replaces `<< outputs.go-coverage >>` with the file path specified in `outputs.go-coverage` in your test suite configuration.

NOTE: You do not need to define `outputs.go-coverage` in your configuration. If not defined, Smarter Testing automatically creates a temporary path in your directory to store coverage data during analysis. The coverage data is used only during the analysis phase and does not need to be uploaded or persisted.

Some test runners only let you specify a _directory_ for coverage output and may write one or more coverage files into that directory. In these cases, your analysis command must **concatenate those files into the single path** given by the template variable.
...
.Example analysis command using `go test` with coverage enabled with placeholders
[source,console]
----
...
+
--
Use the link:https://github.com/circleci/vitest-circleci-coverage[vitest-circleci-coverage] plugin as a dev dependency before configuring the analysis command. This plugin outputs the CircleCI coverage format and enables much faster analysis than using general code coverage tooling.
[source,console]
----
$ pnpm add --dev jsr:@circleci/vitest-circleci-coverage
----
...
analysis: |
  CIRCLECI_COVERAGE=<< outputs.circleci-coverage >>
  vitest run --silent \
             --bail 0 \
             << test.atoms >>
outputs:
...
+
--
Use the link:https://github.com/circleci/jest-circleci-coverage[jest-circleci-coverage] plugin as a dev dependency before configuring the analysis command. This plugin outputs the CircleCI coverage format and enables much faster analysis than using general code coverage tooling.
[source,console]
----
$ pnpm add --dev jsr:@circleci/jest-circleci-coverage
----
...
+
--
Use the link:https://github.com/circleci/mocha-circleci-coverage[mocha-circleci-coverage] plugin as a dev dependency before configuring the analysis command. This plugin outputs the CircleCI coverage format and enables much faster analysis than using general code coverage tooling.
[source,console]
----
$ pnpm add --dev jsr:@circleci/mocha-circleci-coverage
----

Add the custom hook to your `.mocharc.js`
[source,javascript]
----
module.exports = {
...
run: JEST_JUNIT_OUTPUT_FILE="<< outputs.junit >>" jest --runInBand --reporters=jest-junit --bail << test.atoms >>
analysis: |
  CIRCLECI_COVERAGE=<< outputs.circleci-coverage >> mocha << test.atoms >>
outputs:
  junit: test-reports/tests.xml
options:
  # Enable test impact analysis.
...
+
--
Use the link:https://github.com/circleci/pytest-circleci-coverage[pytest-circleci-coverage] plugin as a dev dependency before configuring the analysis command. This plugin outputs the CircleCI coverage format and enables fast analysis runs.
...
+
--
Use the link:https://github.com/circleci/rspec-circleci-coverage[rspec-circleci-coverage] plugin as a dev dependency before configuring the analysis command. This plugin outputs the CircleCI coverage format and enables much faster analysis than using general code coverage tooling.
...
---
name: ci tests
discover: find spec -type f -name '*_spec.tb'
...
+
NOTE: Javascript/Typescript projects should use `pnpm`/`yarn`/`npm` to launch the command, for example, `pnpm circleci run testsuite "ci tests"`.
+
...
. Remove `options.test-analysis-duration` from your `.circleci/test-suites.yml`. This allows analysis to run on without time limits.
...
See <<set-up-examples>> for configuration examples of each approach.
...
run: my-test-runner --junit=<< outputs.junit >> --run << test.atoms >>
analysis: my-test-runner --coverage=<< outputs.coverage >> --run << test.atoms >>
file-mapper: my-test-runner --list-files-for << test.atoms >>
----
====
