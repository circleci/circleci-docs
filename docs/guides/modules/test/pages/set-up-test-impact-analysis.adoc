...
Next, run the testsuite command with flags `--test-selection="none"` and `--test-analysis="impacted"` (learn more about the flags in the <<cli-flags,test suite flag options>> section). Confirm that the analysis command analyzes the test atoms you expect. Look for the output lines `Found N files impacting TEST`, which show that analysis is discovering the source files covered by the test.
