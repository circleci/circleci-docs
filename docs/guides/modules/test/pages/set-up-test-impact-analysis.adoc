...
=== Understand the `--analyze-tests` and `--select-tests` flags

The `testsuite` command has two independent phases you control with CLI flags:

* `--analyze-tests` controls whether to *build or update impact data*. This runs tests with coverage instrumentation and stores the mapping of test atoms to source files. Analysis must run on your default branch to keep impact data up to date.
* `--select-tests` controls whether to *choose and run tests based on impact data*. This compares changed files against the stored impact data and runs only the affected test atoms.

You can enable or disable each phase independently. For example, you can analyze without running tests (`--select-tests=none`), or run impacted tests without re-analyzing (`--analyze-tests=none`).
...
This means by default, feature branches run only impacted tests, and the default branch runs all tests while also updating impact data.
====
...
Use the `--select-tests` and `--analyze-tests` flags to verify analysis on your feature branch before merging into the default branch.
