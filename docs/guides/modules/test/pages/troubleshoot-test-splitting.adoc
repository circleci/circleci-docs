...
To use test splitting with CircleCI, you must pass in a list of tests to run. However, due to how Django executes tests, you cannot simply glob the tests and pass them in.
...
Some third party applications and libraries might help you to split your test suite. These applications are not developed or supported by CircleCI. Check with the owner if you have issues using it with CircleCI. If you're unable to resolve the issue, you can search and ask on our link:https://discuss.circleci.com/[Discuss] forum.
