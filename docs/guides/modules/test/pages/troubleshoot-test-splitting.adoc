...
NOTE: This page covers troubleshooting the legacy `circleci tests` command family. CircleCI's preferred way to run and split tests is now `circleci testsuite`. See xref:test.adoc#smarter-testing[Get Started With Smarter Testing]. If you are on CircleCI Server, continue to use `circleci tests` -- `circleci testsuite` is currently Cloud-only.

[#using-test-splitting-with-python-django-tests]
== Using test splitting with Python Django tests

To use test splitting with CircleCI, you must pass in a list of tests to run. However, due to how Django executes tests, you cannot glob the tests and pass them in directly.
...
If so, you may need to adjust the file paths that are saving to your test metadata XML file. Alternatively, try working out of the standard working directory to see if that resolves the issue. Remove any `working_directory` instances from your test run job.
...
The `.circleci/resources/pytest_build_config.ini` path may need to be replaced to point to where it is located in your project.
...
NOTE: To follow along with the commands in the video below you will need to be xref:execution-managed:ssh-access-jobs.adoc[`SSH-ed into a job`].
...
Some third party applications and libraries might help you to split your test suite. These applications are not developed or supported by CircleCI. Check with the owner if you have issues using it with CircleCI. If you are unable to resolve the issue, you can search and ask on our link:https://discuss.circleci.com/[Discuss] forum.

* **link:https://knapsackpro.com[Knapsack Pro]** - Enables allocating tests dynamically across parallel CI nodes, allowing your test suite execution to run faster. See the link:https://docs.knapsackpro.com/2018/improve-circleci-parallelisation-for-rspec-minitest-cypress[CI build time graph examples] for details.

* **link:https://github.com/previousnext/phpunit-finder[`phpunit-finder`]** - A helper CLI tool that queries `phpunit.xml` files to get a list of test filenames and print them, useful for splitting tests in parallel based on CI timings.
...
* xref:collect-test-data.adoc[Collect Test Data]
