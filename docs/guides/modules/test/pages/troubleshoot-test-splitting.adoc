...
NOTE: This page covers troubleshooting the legacy `circleci tests` command family. CircleCI's preferred way to run and split tests is now `circleci testsuite`. See xref:test.adoc#smarter-testing[Get Started With Smarter Testing]. If you are on CircleCI Server, continue to use `circleci tests`.
