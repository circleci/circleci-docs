...
:page-description: A guide to collecting test data in your CircleCI projects.
:experimental:
...
See the xref:insights:insights-tests.adoc[Test Insights] page for information on using the Insights feature to gather information about your tests.
This includes flaky test detection, viewing tests that failed most often, slowest tests, and an overall performance summary.

Also, see the link:https://circleci.com/docs/api/v2/#tag/Insights[API v2 Insights endpoints] to find test failure information.
...
* Configure Django using the link:https://github.com/django-nose/django-nose[`django-nose`] test runner.
...
For more details on `--runInBand`, refer to the link:https://jestjs.io/docs/cli#--runinband[Jest CLI] documentation. For more information on these issues, see link:https://github.com/jestjs/jest/issues/1524#issuecomment-262366820[Issue 1524] and link:https://github.com/jestjs/jest/issues/5239#issuecomment-355867359[Issue 5239] of the official Jest repository.
...
For custom Cucumber steps, generate a file using the JUnit formatter and write it to the `cucumber` directory. The following is an example of the addition to your `.circleci/config.yml` file:
...
For PHPUnit tests, generate a file using the `--log-junit` command line option and write it to the `/phpunit` directory. Your `.circleci/config.yml` might be:
...
Assuming that you are already using kaocha as your test runner, do the following to produce and store test results:
...
* xref:insights:insights.adoc[Using Insights]
