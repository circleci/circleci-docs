...
Auto rerun failed tests identifies which test units to rerun from JUnit XML output. Your test runner must be configured to emit JUnit XML results for the feature to work.
...
All test units are run as normal with the `discover` and `run` commands.

When auto rerun failed tests is enabled:

* Any test units that failed in the JUnit XML output are rerun.
* The process repeats until all test units pass, or the maximum number of rerun attempts is reached.
* If all test units pass, the errors are suppressed and the job succeeds.
...
|`max-auto-rerun`
|0
|The maximum number of times test units are rerun if they fail. Accepts a value in the range 0 to 10 inclusive.

When `auto-rerun-duration` is also set, test units rerun until one of the two conditions is reached.

|`auto-rerun-duration`
|null
|The duration or percentage of total runtime to spend rerunning failed test units, for example, `5m`, `1h`, `20%`.

When `max-auto-rerun` is also set, test units rerun until one of the two conditions is reached.
|===
--
