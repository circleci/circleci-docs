...
Test splitting is a technique in which tests are executed simultaneously across multiple execution environments. Test splitting takes advantage of a feature called xref:optimize:parallelism-faster-jobs.adoc[_parallelism_], which allows you to run a job in CircleCI on different nodes at the same time.
...
+
image::guides:ROOT:test-splitting-parallel-runs.png[Five parallel runs with run times displayed]
+
