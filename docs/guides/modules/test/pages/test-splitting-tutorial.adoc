...
Test splitting is a technique in which tests are executed simultaneously across multiple execution environments. Test splitting takes advantage of a xref:optimize:parallelism-faster-jobs.adoc[Parallelism], which allows you to run a job in CircleCI on different nodes at the same time.
...
* A CircleCI account - if you do not have an account, xref:getting-started:first-steps.adoc[you can sign up for free].
* A Version Control System (VCS) provider, such as GitHub, Bitbucket, or GitLab, connected to your CircleCI account. If you have not already done so, follow the steps in the xref:integration:github-integration.adoc[GitHub], xref:integration:bitbucket-integration.adoc[Bitbucket], or xref:integration:gitlab-integration.adoc[GitLab] integration pages to connect your VCS provider.
...
* **Line 1:** This project uses the configuration `version: 2.1`. This enables CircleCI features such as orbs and reusable configuration. More information on config version 2.1 can be found in the xref:reference:ROOT:configuration-reference.adoc[Configuration reference] page.
* **Lines 3-4:** This project uses the CircleCI Node xref:orbs:use:orb-intro.adoc[orb]. The Node orb is a package of reusable configuration elements that allow you to execute tasks common to Node.js apps and help reduce complexity in your config.yml file. In this particular example, the orb's `install-packages` command is used to install Node packages and configure Yarn as the default package manager using the `pkg-manager` parameter. More details on the Node orb can be found on the link:https://circleci.com/developer/orbs/orb/circleci/node[Developer Hub].
...
The steps are a little different depending on which organization type you have: `circleci`, `github` or `bitbucket`. See the xref:permissions-authentication:users-organizations-and-integrations-guide.adoc[Users, organizations, and integrations guide] for more information.
...
+
image::guides:ROOT:test-splitting-sample-configs.png[Sample config for Node]
...
+
image::guides:ROOT:test-splitting-first-setup-steps.png[Steps run successfully within the job]
--
`bitbucket` type organization::
+
--
...
* The `--split-by=timings` flag indicates that the tests should be split according to timing data. For other test splitting options, see the xref:optimize:parallelism-faster-jobs.adoc[Test splitting and parallelism] page.
+
...
----
+
This step uploads the test data to CircleCI and is **required** to split tests by timing data. This step allows test data to be accessible on the Tests tab of the job in the CircleCI web app, and can be helpful for debugging if tests fail. To read more about the Tests tab and test insights in CircleCI, visit the xref:collect-test-data.adoc[Collect test data] page.
...
. In any of the parallel runs, open the **Test application** step. You will see which test suites and how many individual tests executed in this particular run. You will also see this message in the output:
+
[,shell]
----
...
+
On subsequent runs, you will see an additional step labelled **Downloading previous test results**.
+
The presence of this step confirms that, during this run, test results stored by a previous run (with the `store_test_results` step) were successfully retrieved. Provided that the data is properly formatted () xref:optimize:parallelism-faster-jobs.adoc#junit-xml-reports[JUnit XML]), the test suite will be split as evenly as possible over the specified number of test environments.
...
image::guides:ROOT:test-splitting-timing-tab.png[Parallel runs visualization in Timings tab]
+
...
In the previous step, you saw that test splitting defaulted to splitting tests based on name. Now that test data has been saved, CircleCI can split your tests by timing the next time the pipeline runs.
...
. Open the pipeline in the web app, and view the **Test application** step. This time, you will see `Autodetected filename timings.` in the output. This means that CircleCI is now splitting tests based on available timing data from preceding runs.
+
.Testing step showing split by timing
image::guides:ROOT:test-splitting-by-timing.png[Testing step showing split by timing]
...
* Learn about xref:insights:insights-tests.adoc[Test Insights] available in CircleCI.
