...
Test Impact Analysis intelligently selects which tests to run based on the files that changed in your code. This reduces test execution time while maintaining test confidence.
...
== 1. Configure the analysis command
...
Run the test suite with flags `--test-selection="none"` and `--test-analysis="impacted"`, and confirm that the analysis command analyzes the test atoms you expect. Learn more in xref:configuration-options.adoc#cli-flags[CLI flags]. Look for the output lines `Found N files impacting TEST`, which show analysis discovering the source files covered by the test.
...
== 3. Run your test suite in CircleCI
