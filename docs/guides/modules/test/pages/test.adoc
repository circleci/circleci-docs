...
NOTE: `circleci tests glob` and `circleci tests split` are the test tooling for CircleCI Server projects. `circleci tests run` is *not* supported on CircleCI Server, even though it is part of the same command family; see <<tests-command-rerun,Re-run failed tests only>> below. On Cloud, `circleci testsuite` is the preferred option. See <<smarter-testing,Get started with Smarter Testing>>.
...
NOTE: `circleci tests run` is available on CircleCI Cloud only, for existing configurations. It is *not* supported on CircleCI Server. If you are on CircleCI Server, use `circleci tests split` (see <<tests-command-splitting,Test splitting and parallelism>> above) instead.
