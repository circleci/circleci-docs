...
<<basics,Basics>> below covers running your test suite using your native test library and the `run` step, which works with any test runner. `circleci testsuite` is CircleCI's preferred way to run that test command, for all customers. It supports the same test discovery, running, and splitting capabilities as the legacy `circleci tests` command family, plus additional commands, such as `list-tests` for introspecting which tests will run. It also adds three features unique to link:https://circleci.com/blog/smarter-testing-stop-running-tests-that-dont-matter/[Smarter Testing], each usable on its own:
...
See <<smarter-testing,Get started with Smarter Testing>> below to switch, or <<legacy-test-command,Legacy: `circleci tests` command>> if you are on Server or not ready to move yet.
...
NOTE: `circleci testsuite` is currently available on Cloud only. If you are on CircleCI Server, use the <<legacy-test-command,legacy `circleci tests` commands>> instead.
