...
Refer to the xref:execution-managed:circleci-images.adoc[Pre-Built CircleCI Convenience Images] and add `-browsers:` to the image name. This will give you a variant that includes Java 8, geckodriver, Firefox, and Chrome. Add  `-browsers-legacy` to the image name for a variant which includes PhantomJS.
...
LambdaTest has developed a link:https://circleci.com/developer/orbs/orb/lambdatest/lambda-tunnel[CircleCI orb] for browser compatibility testing. It enables you to open a Lambda Tunnel before performing any browser testing, easing the process of integrating LambdaTest with CircleCI. Use the orb to set up a Lambda tunnel and then define your test steps.
...
If you are using JavaScript to test your web application, you can still take advantage of the Sauce Labs platform. Use link:https://docs.saucelabs.com/testrunner-toolkit[`saucectl`] with the JS framework of your choice, and integrate the link:https://circleci.com/developer/orbs/orb/saucelabs/saucectl-run[saucectl-run orb] in your CircleCI workflow.

. Add your `SAUCE_USERNAME` and `SAUCE_ACCESS_KEY` as xref:security:env-vars.adoc[Environment Variables] in your Circle CI project.
...
Following the example above, you can replace the installation of Sauce Labs with another cross-browser testing platform, such as BrowserStack. Then, set the `USERNAME` and `ACCESS_KEY` xref:security:env-vars.adoc[Environment Variables] to those associated with your BrowserStack account.

For mobile applications, you can use Appium or an equivalent platform that uses the WebDriver protocol. To do this, install Appium in your job, and use CircleCI xref:security:env-vars.adoc[Environment Variables] for the `USERNAME` and `ACCESS_KEY`.
...
. To add port-forwarding to the command, use the `-L` flag. The following example forwards requests to `+http://localhost:3000+` on your local browser to port `8080` on the CircleCI container. An example use case for this would be if your job runs a debug Ruby on Rails app, which listens on port 8080. After you run this, if you go to your local browser and request `\http://localhost:3000`, you will see whatever is served on port 8080 of the container.
+
...
. Then, open your browser on your local machine and navigate to `+http://localhost:3000+` to send requests directly to the server running on port `8080` on the CircleCI container. You can also manually start the test server on the CircleCI container (if it is not already running), and you can access the running test server from the browser on your development machine.
...
. Open a Terminal window, xref:execution-managed:ssh-access-jobs.adoc[Start an SSH Run] to a CircleCI container and forward the remote port 5901 to the local port 5902.
+
[,shell]
----
...
. You will see a display containing a terminal window. Since your connection is secured through the SSH tunnel, ignore any warnings about an insecure or unencrypted connection.
...
Now, you can run integration tests from the command line and watch the browser for unexpected behavior. You can even interact with the browser as if your tests run on your local machine.
...
. Now when you xref:execution-managed:ssh-access-jobs.adoc[Start an SSH Build], you'll be able to connect to the VNC server while your default test steps run. You can either use a VNC viewer that is capable of SSH tunneling, or set up a tunnel on your own:
+
[,shell]
----
...
. With X set up on your system, xref:execution-managed:ssh-access-jobs.adoc[Start an SSH Build] to a CircleCI VM, using the `-X` flag to set up forwarding:
+
[,shell]
----
...
Now you can run your integration tests from the command line and watch the browser for unexpected behavior. You can even interact with the browser as if your tests run on your local machine.

[#see-also]
== See also

* xref:security:env-vars.adoc[Environment Variables]
