...
The coverage report output location can be specified with the template variable `<< outputs.lcov >>`. CircleCI replaces `<< outputs.lcov >>` with the file path specified in `outputs.lcov` in your test suite configuration. If not defined, a temporary path is created by Smarter Testing. CircleCI requires coverage data in a single file to interpret results - if your test runner generates multiple coverage files, your analysis command must consolidate them into `<< outputs.lcov >>`.
...
The next step is to run your test suite in CI. Also feel free to remove any files that got created from running the command locally (ex. impact.json, coverage files, JUnit results).
