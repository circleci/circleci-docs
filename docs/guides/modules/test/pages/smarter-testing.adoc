...
*Selection phase*:: Compares the current repository state against the most recent impact analysis data. Tests covering modified files are selected to run (for clarity, new or modified tests will also be selected). Additionally, the entire test suite runs if any `full-test-run-paths` files were modified since the last analysis (these file states are stored in impact data even if not covered by tests). See the <<test-suite-configuration-options>> for details. By default, test selection is applied on feature branches and all tests are run on your default branch. You can customize this behavior in your CircleCI configuration. 
