...
*Selection phase*:: Compares the current repository state against the most recent impact analysis data. Tests covering modified files are selected to run (including new or modified tests). Additionally, the entire test suite runs if any `full-test-run-paths` files were modified since the last analysis. See the <<test-suite-configuration-options>> for details. By default, test selection is applied on feature branches and all tests are run on your default branch. You can customize this behavior in your CircleCI configuration. 

TIP: You can also select tests with Smarter Testing in your local test runs on your machine. See <<New section needed>> for details.
...
Start with your existing test command. For example, you might currently run:
[source,console]
----
...
Not updating test impact data, analysis not enabled
----
NOTE: Running the test suite locally will create a test-reports/ folder that you can safely delete after validating your configuration.
...
For example, with Vitest this would look like:
[source,console]
----
...
The `analysis` command needs to be modified to use placeholders for the coverage report output location, and the test atom that should be run. Some test runners (including `vitest` and `jest`) generate multiple coverage files that must be concatenated into a single file for Smarter Testing to parse.
...
When running the test suite locally, test selection is enabled by default but analysis is disabled. Run the test suite with `--test-selection=none` and `--test-analysis=impacted` to override the default behaviour. Confirm that the `analysis` command analyzes the test atoms you expect. Look for the output lines `Found N files impacting TEST` which show analysis discovering the source files covered by the test.

The `--local` flag is required for analysis to run on your machine - without it, analysis is skipped locally. This is for validation during onboarding and setup only. Actual analysis should run in CI where impact data is stored by Smarter Testing, which can then be used to select tests in CI or locally.
...
NOTE: When running with `--local`, you'll see `.circleci/impact.json` created - this stores the impact data used for test selection. Keep this file for the next step to validate test selection. You can delete it afterwards.
