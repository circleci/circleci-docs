...
:page-description: Docs page on setting up LLM evaluations in your CI/CD pipeline
:experimental:
...
* Job step details can include direct links to your evaluation results stored on your third-party evaluation provider. When you need to further review the evaluation results in detail, this link enables you to navigate to and view your evaluation results.
* Your evaluation results can be saved to a `.json` file, for evaluation testing.

.Evaluations Job Step Details: Task Evals
...
Assertions are expressed using the link:https://cel.dev/[Common Expression Language] (CEL). For more information, see the link:https://github.com/google/cel-spec/blob/master/doc/langdef.md[specification]. All values from the results `.json` file are available as global variables. Try out CEL using the link:https://playcel.undistro.io/[CEL playground].

.Evaluation testing: diagram
...
Test cases are composed of a name and an assertion. In the first example above, the name is `correctness` and the assertion is that correctness is above 0.9. When configuring test cases, we suggest assigning a name based on the test scenario. The examples above use names based on input metrics: `correctness` , `toxicity` and `labels`.

Evaluation testing results determine whether a job stops or continues, as follows:
...
* In the step details
+
.Evaluation testing: Job Step Details
...
+
.Evaluation testing: Test Failure Details
...
.Project Settings > LLMOps: Create Context Modal Window in CircleCI
image::guides:ROOT:llmops/create-context.png[Project Settings > LLMOps: Create Context Modal Window in CircleCI]
.Project Settings > LLMOps: View contexts in CircleCI
