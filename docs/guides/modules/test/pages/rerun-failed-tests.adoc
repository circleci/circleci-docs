...
NOTE: This page covers rerunning failed tests with the legacy `circleci tests run` command. CircleCI's preferred way to rerun failed tests is now the xref:auto-rerun-failed-tests.adoc[Auto Rerun Failed Tests] feature of `circleci testsuite`.
...
.Screenshot of the Rerun menu with the Rerun failed tests option highlighted
...
* Your testing job must be configured to upload test results to CircleCI using the `store_test_results` step. See the xref:collect-test-data.adoc[Collecting Test Data] page and the xref:reference:ROOT:configuration-reference.adoc#storetestresults[Configuration Reference] for more details.

* `file` or `classname` attributes **must be present** in the JUnit XML output. See the xref:optimize:use-the-circleci-cli-to-split-tests.adoc[Parallelism and Test Splitting] page for more details.
...
+
If your current job is using xref:optimize:parallelism-faster-jobs.adoc[Intelligent Test Splitting], you must *change* the `circleci tests split` command to `circleci tests run` combined with a `--split-by=` flag.

[#quickstart]
== Quickstart

To use the rerun failed tests feature, run your tests using the `circleci tests run` command. The basic structure of your testing command is:

* Create a directory for your results.
* Glob your test files.
* Make your test files available as standard input (stdin).
* Run tests with `circleci tests run`. Run your tests with the `--command` flag, using `xargs` to get the tests from stdin.
...
* Splits tests by previous timing results.
...
** Invoke `circleci tests run` and specify the original command (`pytest`) used to run tests as part of the `--command=` parameter. **This parameter is required**. `xargs` must be present as well.
...
Then, the next time you encounter a test failure on that job, select btn:[Rerun failed tests].  If the `--verbose` setting is enabled, you will see output similar to the following the next time you select btn:[Rerun failed tests] with this job on CircleCI:
...
. Update the `glob` command to match your specific use case.  **If your current job is using xref:optimize:parallelism-faster-jobs.adoc[Test Splitting to Speed Up Your Pipelines], you must change the `circleci tests split` command to `circleci tests run` with the `--split-by=timings` parameter.** If you are not using test splitting, `--split-by=timings` can be omitted.
...
. **If your current job is using xref:optimize:parallelism-faster-jobs.adoc[Test Splitting to Speed Up Your Pipelines], you must change the `circleci tests split` command to `circleci tests run` with the `--split-by=timings` parameter.** If you are not using test splitting, `--split-by=timings` can be omitted.
...
. Update the `glob` command to match your use case. **If your current job is using xref:optimize:parallelism-faster-jobs.adoc[Test Splitting to Speed Up Your Pipelines], you must change the `circleci tests split` command to `circleci tests run` with the `--split-by=timings` parameter.**. If you are not using test splitting, `--split-by=timings` can be omitted.
...
. Note that this example uses a utility named link:https://github.com/previousnext/phpunit-finder[`phpunit-finder`] which is a third party tool that is not supported by CircleCI, use at your own risk. If your current job is using xref:optimize:parallelism-faster-jobs.adoc[Test Splitting to Speed Up Your Pipelines], you must change the `circleci tests split` command to `circleci tests run` with the `--split-by=timings` parameter. If you are not using test splitting, `--split-by=timings` can be omitted.
...
. Update the `glob` command to match your use case. **If your current job is using xref:optimize:parallelism-faster-jobs.adoc[Test Splitting to Speed Up Your Pipelines], you must change the `circleci tests split` command to `circleci tests run` with the `--split-by=timings` parameter.**. If you are not using test splitting, `--split-by=timings` can be omitted. Note: you may also use link:https://playwright.dev/docs/test-reporters#junit-reporter[Playwright's built-in flag] (`PLAYWRIGHT_JUNIT_OUTPUT_NAME`) to specify the JUnit XML output directory.
+
...
. Ensure you are using `xargs` in your `circleci tests run` command to pass the list of test files/classnames via stdin to `--command`.
. Update the `glob` command to match your use case. See the RSpec section in the xref:collect-test-data.adoc#rspec[Collect Test Data] document for details on how to output test results in an acceptable format for `rspec`. **If your current job is using xref:optimize:parallelism-faster-jobs.adoc[Test Splitting to Speed Up Your Pipelines], you must change the `circleci tests split` command to `circleci tests run` with the `--split-by=timings` parameter.** If you are not using test splitting, `--split-by=timings` can be omitted.
...
. You may also enable link:https://docs.knapsackpro.com/ruby/split-by-test-examples/[Split by Test Examples] to parallelize tests across CI nodes by individual `it`/`specify`. Splitting by test examples helps when you have slow test files but do not want to split them into smaller files by hand.
...
. Update the `glob` command to match your use case. See the Cucumber section in the xref:collect-test-data.adoc#cucumber[Collect Test Data] document for details on how to output test results in an acceptable format for `Cucumber`. **If your current job is using xref:optimize:parallelism-faster-jobs.adoc[Test Splitting to Speed Up Your Pipelines], you must change the `circleci tests split` command to `circleci tests run` with the `--split-by=timings` parameter.** If you are not using test splitting, `--split-by=timings` can be omitted.
...
If your testing setup on CircleCI is not compatible with invoking your test runner in the `circleci tests run` command, you can use an alternative approach. You can opt to use `circleci tests run` to receive the file names and save them to a temporary location. You can then invoke your test runner using the generated file names.
...
* If your testing job uses parallelism and test splitting, the job will spin up the number of containers/virtual machines (VMs) that are specified with the `parallelism` key. However, the tests are split across the total number of parallel containers/VMs. Each container or VM therefore runs only a subset of the tests, and some run no tests at all.
+
...
+
If you would like to maximize credit savings, check whether the parallel container/VM will execute tests as the first step in a job. If there are no tests to run, terminate job execution. With this approach, ensure that the logic to run tests happens in a subsequent step. For example:
+
```yml
steps:
  - checkout
...
* If a shell script is invoked to run tests, `circleci tests run` must be placed **in the shell script** itself, and not `.circleci/config.yml`. Alternatively, pipe the list of test files to a `.txt` file, then pass those file names to your shell script. See the <<output-test-files-only,section above>>.
...
+
Code coverage reports from the original job run need to be persisted alongside reports from re-runs. See the following example for a sample Go project:
+
[source,yaml]
----
jobs:
...
. Use xref:reference:ROOT:configuration-reference.adoc#storeartifacts[`store_artifacts`] to upload the JUnit XML that contains test results to CircleCI.  The JUnit XML is the same file or files uploaded to CircleCI with `store_test_results`.
...
If your job runs tests in parallel and persists files to a workspace, you may see a failure on a rerun. The failure occurs when the `persist_to_workspace` step cannot find contents in the specified directory. This happens because parallel runs do not always execute tests on a rerun when there are not enough tests to distribute.
...
If your workflow has an approval job and a failed job with failed tests, you cannot select btn:[Rerun failed tests] until the workflow terminates. You must cancel the approval job before you can select btn:[Rerun failed tests].
...
**Question:** I do not see my test framework on this page, can I still use the functionality?
...
**Answer:** You may need to add the `-DfailIfNoTests=false` flag. This flag ensures the testing framework ignores skipped tests instead of reporting a failure when it encounters a skipped test on a dependent module.
...
**Question:** I use Cypress and my job uploads test results, but the failed test re-run still runs all tests instead of just the failed tests
