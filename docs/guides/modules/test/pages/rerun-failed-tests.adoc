...
* If a shell script is invoked to run tests, `circleci tests run` must be placed **in the shell script** itself, and not `.circleci/config.yml`. Alternatively, see the <<output-test-files-only,section above>> to pipe the list of test files to a `.txt` file. Then pass the list of test file names to your shell script.
