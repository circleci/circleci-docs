...
Simple projects run with minimal configuration.
...
We recommend adding a `Gemfile` to your repository to ensure that the same version of Fastlane is used both locally and on CircleCI, and that all dependencies are installed. Below is a sample of a simple `Gemfile`:
...
workflows:
  build-test-adhoc:
    jobs:
      - build-and-test
      - adhoc:
          filters: pipeline.git.branch == "development" # only deploy on the development branch
          requires:
            - build-and-test
----

The environment variable `FL_OUTPUT_DIR` is the artifact directory where Fastlane logs and signed `.ipa` file are stored. Use this to set the path in the `store_artifacts` step to automatically save logs and build artifacts from Fastlane.
...
Our Xcode images ship with multiple versions of Ruby installed. The versions installed are the latest stable versions of Ruby at the time the image is built. See the link:https://www.ruby-lang.org/en/downloads/[Ruby-Lang.org downloads page] for reference. Each image's software manifest lists the installed Ruby versions and the default Ruby (see <<supported-xcode-versions,supported Xcode versions>>).
...
To use the same version of CocoaPods locally and in CircleCI, create a Gemfile in your iOS project and add the CocoaPods version to it:
...
These images have NodeJS managed by `nvm` and ship with the latest `current` and `lts` releases at the time the image is built. The `lts` version is set as the default.
...
link:https://brew.sh/[Homebrew] is pre-installed on CircleCI, so you can use `brew install` to add nearly any dependency you require to complete your build. For example:
