...
[WARNING]
====
Each Slack workspace can only be connected to one CircleCI organization at a time. This is a Slack platform constraint — Slack issues a single bot token per workspace, and a new installation overwrites the existing one. If a second CircleCI organization installs the app into an already-connected workspace, it will invalidate the first organization's token and break their integration. Connecting multiple organizations to the same Slack workspace is not currently supported.
====
...
.Integration stops working unexpectedly
[NOTE]
====
If your Slack integration stops working, it may be because another organization has installed the CircleCI app into the same Slack workspace, which invalidates the existing token. To restore the connection, an org admin can reinstall the app from menu:Org[Slack Integration].
====
