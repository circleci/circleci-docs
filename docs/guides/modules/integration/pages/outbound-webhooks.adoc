...
. Find your project in the list, select the ellipsis icon:more[Ellipsis menu icon], and select **Project Settings**.
...
| Y
| Ensure the receiving host has a valid SSL certificate before sending an event. Only leave this unchecked for testing purposes.
...
Currently, the latest (and only) signature version is v1. _Only_ check the latest signature type to prevent downgrade attacks.
