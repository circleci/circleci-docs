...
NOTE: Webhook payload may be different depending on the integration used by the pipeline that emits the webhook. Check out the xref:reference:ROOT:outbound-webhooks-reference.adoc#sample-webhook-payloads[sample webhook payloads] for each integration for more information.
