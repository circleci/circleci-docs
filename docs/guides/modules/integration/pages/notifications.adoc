...
If you link your Slack identity, CircleCI also delivers My Work notifications as Slack direct messages. You can set different status filters for Slack and email. See xref:integration:slack-integration.adoc#receive-dms-for-workflows-you-trigger[Receive DMs for Workflows You Trigger] in the Slack Integration guide to set this up.
...
CircleCI emails you 30 days, 15 days, seven days, and one day before a personal API token expires.

Expiry notifications are mandatory communications rather than a notification category, so you cannot opt out of them. To stop the emails for a token you no longer need, delete the token. For help creating, rotating, and deleting tokens, see the xref:toolkit:managing-api-tokens.adoc[Managing API Tokens] guide.
...
* *Slack direct message* to the user who triggered the pipeline, if they have linked their Slack identity. See xref:integration:slack-integration.adoc#link-your-slack-identity[Link Your Slack Identity] and xref:integration:slack-integration.adoc#receive-dms-for-workflows-you-trigger[Receive DMs for Workflows You Trigger] for details.
