...
+
NOTE: The test ping event has an abbreviated payload for ease of testing. See full examples for xref:reference:ROOT:webhooks-reference.adoc#sample-webhook-payloads[sample webhook payloads] section of the webhooks reference.
...
Webhook requests may be duplicated. To deduplicate (prevent requests from being duplicated for a specific event), use the xref:reference:ROOT:webhooks-reference.adoc#common-top-level-keys[`id` property] in the webhook payload for identification.
...
* See the xref:reference:ROOT:webhooks-reference.adoc#[Webhooks reference] page for key definitions and sample payloads.
