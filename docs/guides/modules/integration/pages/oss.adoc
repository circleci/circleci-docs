...
:page-platform: cloud
:page-vcs: github, github-enterprise, gitlab, bitbucket
:page-audience: developer
:page-description: Best practices for building open source projects
:experimental:
...
See the xref:plans-pricing:credits.adoc#open-source-credit-usage[Using Credits] page for up-to-date information on free credits available for open source projects.
...
For more information, see the xref:security:set-environment-variable.adoc#set-an-environment-variable-in-a-project[Set an Environment Variable] document.
...
+
--
include::ROOT:partial$app-navigation/steps-to-project-settings.adoc[Insert steps from selecting an organization up to the Project Settings page]
...
image::guides:ROOT:triggers/run-on-pr-opened-pushed-to.png[Trigger option to build pull requests]
--
...
image::guides:ROOT:triggers/advanced-only-build-prs.png[Enable the Only build pull requests option]
...
image::guides:ROOT:triggers/gitlab-cloud-trigger-filter-only-mr.png[Set the Only Build Merge Requests option in GitLab Cloud trigger]

.Trigger page for GitLab self-managed
image::guides:ROOT:triggers/gitlab-self-managed-trigger-filter-only-mr.png[Set the Only Build Merge Requests option in GitLab self-managed trigger]
--
====
...
Sometimes a user submits a pull request to your repository from a fork, but no pipeline is triggered. This usually happens because the user is following a project fork on their personal account rather than the project itself. As a result, jobs trigger under the user's personal account instead of the organization account. To resolve this issue, have the user unfollow their fork of the project on CircleCI and instead follow the source project. This causes their jobs to run under the organization when they submit pull requests.
...
* xref:permissions-authentication:users-organizations-and-integrations-guide.adoc#create-a-user-key[Deployment Keys and User Keys].
* Passphraseless private SSH keys you have xref:add-ssh-key.adoc[Added to CircleCI] to access arbitrary hosts during a build.
* xref:deploy:deploy-to-aws.adoc[AWS Permissions] and configuration files.
...
If you wish to use OpenID Connect in your project, the OIDC tokens will _only_ be generated if you have this setting turned on. This prevents your token data being shared with forked builds unless you require it. If you use OIDC with open source projects and enable the *Pass secrets to builds from forked pull requests* option, you *must* check the `oidc.circleci.com/vcs-origin` claims in your policies. This avoids forked builds having access to resources outside those that you require. For more information on OIDC, see the xref:permissions-authentication:openid-connect-tokens.adoc[Using OpenID Connect Tokens in Jobs] page.
...
* *https://github.com/fastlane/fastlane[Fastlane]* - A build automation tool for Android and iOS.

== See also

Refer to the xref:toolkit:example-configs.adoc[Examples] document for more public and open source project configuration links organized by CircleCI features and by programming language.
