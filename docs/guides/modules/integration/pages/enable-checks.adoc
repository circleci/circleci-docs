...
* GitHub *status updates* are the default way status updates from your builds are reported in the GitHub UI, and they are reported per _job_. FOr more information on status updates, see the xref:guides:getting-started:status-updates.adoc[Status updates overview] guide.
