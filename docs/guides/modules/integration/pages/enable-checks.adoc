...
.Example of the Checks tab on a GitHub PR view showing workflow status.
image::guides:ROOT:checks_tab.png[CircleCI checks on GitHub]

NOTE: GitHub does not currently provide a granular way to rerun workflows. Selecting the Re-run checks button reruns all checks, regardless of whether you chose "re-run failed checks" or "rerun all checks".
...
If you are using GitHub Checks or GitHub status updates with the xref:orchestrate:skip-build.adoc#skip-jobs[Skip Jobs] feature, the status of the skipped builds are not reported. However, the GitHub Checks are still created in GitHub.
