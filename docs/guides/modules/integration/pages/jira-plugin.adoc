...
:page-platform: Cloud
:page-description: Connecting Jira with CircleCI
:experimental:
...
.. For the appropriate Jira instance, select the ellipsis (icon:more[Ellipsis menu icon]) icon.
...
====
+
.Set organization ID on the app Configure page
...
+
.Submit changes on the app Configure page
image::guides:ROOT:jira_submit_changes.png[Submit changes on the app Configure page]
. Now copy your *Web Trigger URL* for use in the next section.
+
.Copy the Web Trigger URL
image::guides:ROOT:jira_copy_webtrigger_url.png[Copy the Web Trigger URL]
