...
TIP: Looking for information about connecting a self-hosted GitHub instance? See the xref:github-enterprise-server-integration.adoc[GitHub Enterprise Server Integration] page.
...
* Uses proprietary _timetable_ interface (Repeats, Start time, Repeats per hour, ...).
...
* Extensive *event name* options available, including all pushes, pull request opened, pull request merged, pull request marked ready for review, tag pushes, and many more (see the full list of xref:orchestrate:github-trigger-event-options.adoc[GitHub Trigger Event Options]).
...
* Trigger pipelines from the xref:orchestrate:triggers-overview.adoc#run-a-pipeline-from-the-circleci-web-app[CircleCI Web App].
...
=== Why can I not see the GitHub App options?
