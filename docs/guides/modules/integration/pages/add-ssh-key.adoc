...
TIP: Do you have a `github` or `bitbucket` organization, and want to check out code from additional repositories in GitHub or Bitbucket Cloud? If so, see the xref:permissions-authentication:users-organizations-and-integrations-guide.adoc#create-a-user-key[Users, Organizations, and Integrations Guide] for steps to add a user key. Otherwise, follow the steps on this page to add additional SSH keys to your project.
...
+
If you have a GitLab integration, you will find that an additional SSH key already exists. This was created automatically by CircleCI during the project creation process. This key is used to check out your code. Do not remove it.
. Select *Add SSH Key*.
...
To check out additional repositories from within your job, run the `checkout` command *before* `add_ssh_keys`; otherwise, `CIRCLE_CI_REPOSITORY_URL` will be empty. Also make sure the private key is added to the CircleCI project. The public key must also be added to the additional repositories that you want to check out from within your job.
...
In this scenario, the first key is used regardless of whether it is the correct key. If you have added the SSH key to a container, either set `IdentitiesOnly yes` in the appropriate block, or remove all keys from the `ssh-agent` for the job using `ssh-add -D`. Then, read the added key using `ssh-add /path/to/key`.
