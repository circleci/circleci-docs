...
* `twine` and a recent Python in the Docker image (or executor) used by your publishing job. <!-- [tktktk] confirm minimum twine version once the mint-token flow is finalized -->
...
Add a publishing job to your `.circleci/config.yml`. The job retrieves an OIDC token for the `pypi` audience, exchanges it with PyPI for a short-lived API token, and uses that token to upload your distributions with `twine`.
...
<!-- [tktktk] simplify this step if twine adds native CircleCI trusted publishing support, so the mint-token exchange is no longer needed -->
...
Trusted publishing from CircleCI self-hosted runners works. <!-- [tktktk] confirm -->

[#notes]
== Notes

* You can test the full flow against link:https://test.pypi.org[TestPyPI] before publishing to production PyPI. Use the `testpypi` audience and the `https://test.pypi.org/_/oidc/mint-token` endpoint, and configure a separate trusted publisher on TestPyPI.
