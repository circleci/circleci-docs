...
:page-description: Roll back a deployment to a previous stable version using a CircleCI rollback pipeline or workflow re-run.
:experimental:
...
CircleCI rollbacks support two methods of rolling back manually:
...
CircleCI can also roll back for you, without anyone triggering it. See the <<automatic-rollback-on-validation-failure,Automatic Rollback on Validation Failure>> section.

Each manual method has appropriate use cases, advantages, and disadvantages, as follows:
...
+
*Component Name*:: The name of the component you wish to roll back. If your project deploys multiple components, this helps you choose a specific component you want to roll back.
*Environment Name*:: The environment in which you wish to perform the rollback.
*Current Version*:: Once you choose the component name and environment name, "Current version" displays all possible current versions. The most common scenario is just one current version available. There may be two if, for example, a new progressive release is ongoing. Choose the current version of your component. To help, the relevant commit information is displayed alongside the version. The "Current version" value is used to verify that no new release has happened since the rollback was triggered. If after triggering the rollback, the current version changes, the rollback will be aborted and you will be notified of the mismatch. You can then decide whether to re-trigger the rollback or not.
*Target Version*:: Choose the version you wish to roll back to. To help you out, the relevant commit information is also displayed alongside the version.
...
* *Confirm rollback*. Select btn:[Next] and confirm rollback to proceed.
...
. Select the rollback icon icon:rebuild[rollback icon].
...
Both methods above are manual: someone notices a problem and triggers the rollback. With release validation configured, CircleCI can detect the problem and roll back on its own.

After a deploy completes, CircleCI opens an evaluation window and listens for alerts from your monitoring tool. If a validation check fails, CircleCI marks the release as failed and triggers your rollback pipeline to return the component to the last healthy version. No manual step is involved.

Automatic rollback needs a configured rollback pipeline, so set that up first using the xref:set-up-rollbacks.adoc[Set Up Rollbacks] guide. Note the following behavior:

* A rollback that fails does not trigger another automatic rollback, even when your rollback pipeline includes validation of its own.
* CircleCI skips the rollback when there is no prior successful release to return to.

To get set up, see the xref:set-up-smart-deployments.adoc[Set Up Smart Deployments] guide.
