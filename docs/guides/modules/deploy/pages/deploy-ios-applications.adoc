...
workflows:
  build-test-adhoc:
    jobs:
      - build-and-test
      - adhoc:
          filters: pipeline.git.branch == "development" # run the job on the development branch only
          requires:
            - build-and-test
----
...
To set up Fastlane to automatically upload iOS binaries to App Store Connect and/or TestFlight, follow these steps to allow Fastlane access to your App Store Connect account.
...
Firebase requires a token to be used during authentication. To generate the token, use the Firebase CLI and a browser. As CircleCI is a headless environment, generate this token locally, rather than at runtime, then add it to CircleCI as an environment variable.
...
NOTE: The Firebase plugin may cause errors when run with the macOS system Ruby. It is therefore advisable to switch to a different xref:test:testing-ios.adoc#using-ruby[Ruby Version].

[#deploy-to-visual-studio-app-center]
== Deploy to Visual Studio App Center

link:https://appcenter.ms/[Visual Studio App Center] (formerly HockeyApp), is a distribution service from Microsoft. App Center integration with Fastlane is enabled by installing the link:https://github.com/microsoft/fastlane-plugin-appcenter[App Center plugin].
...
.Open the TestFairy dashboard and navigate to the Preferences page
...
Deploy markers provide a way to track and manage your iOS app deployments in the CircleCI web app. Deploy markers work with all iOS distribution methods including App Store Connect, TestFlight, Visual Studio App Center, and TestFairy. When you add deploy markers to your deployment job, you can view a timeline of all deployments, track their status, and enable rollback and deploy pipelines.
