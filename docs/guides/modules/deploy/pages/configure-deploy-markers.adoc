...
Configure deploy markers with or without status updates as follows:

* To display updated statuses depending of the outcome of your deployments. Steps for this are provided in the <<set-up-deploy-markers-with-status-updates>> section.
* Or, you can choose to configure deploy markers to log deployments without status updates. Steps for this are provided in the <<set-up-deploy-marker-logs-without-status-updates>> section.
...
* You can choose to rollback (image:guides:ROOT:icons/rebuild.svg[rollback icon, role="no-border"]) to a version.
* You can promote (deploy) a version (image:guides:ROOT:icons/promote.svg[promote icon, role="no-border"]), once you also have a deploy pipeline set up. For more information on deploys, see the xref:set-up-deploys.adoc[Set up Deploys] guide.
...
This guide details how to set up deploy markers *manually by editing your CircleCI configuration file*.

Alternatively, you can begin by setting up a rollback pipeline. Through the rollback pipeline setup process you will be guided in the CircleCI web app to set up deploy markers. See the xref:set-up-rollbacks.adoc[Set up Rollbacks] guide for more details.
...
== Set up deploy markers with status updates
...
== Set up deploy marker logs without status updates
