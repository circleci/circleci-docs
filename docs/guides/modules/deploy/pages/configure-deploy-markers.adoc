...
CAUTION: If you configure the `circleci run release plan` and `circleci run release update` commands in _different_ jobs, you will need to ensure any reruns are made using the btn:[Rerun workflow from start] option, not btn:[Rerun workflow from failed]. This is because rerunning from _failed_ in this scenario runs the risk of not running the `plan` or `update` commands correctly. For this reason, we recommend configuring `plan` and `update` in the same job.
