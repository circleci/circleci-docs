...
Unless you opt out of the feature, CircleCI autodetects potential deployments and creates deploy markers for you. This guide is for you if you would rather create deploy markers yourself. For more information see the xref:deploys-overview.adoc#[Deploys overview] page.

Deploy markers provide a lightweight way to log your deployments without requiring a full CircleCI deploys setup. You can use deploy markers independently, without installing the release agent. If you are interested in comprehensive deployment visibility and management features, refer to the xref:set-up-circleci-deploys.adoc#[Set up CircleCI deploys] guide for more information.
