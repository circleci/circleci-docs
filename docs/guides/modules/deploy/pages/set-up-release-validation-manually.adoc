...
TIP: You can use the xref:guides:toolkit:circleci-agent-skills.adoc[CircleCI Agent Skills] to write this configuration in your editor. Install the link:https://github.com/CircleCI-Public/skills[CircleCI Public Skills] plugin to your AI coding agent, then ask your agent to set up release validation for your project. The `circleci-deploy-setup` skill adds the `validation` block with a matching release job, then checks the result against your configuration file.
...
|The extracted `component_name` must match the component on the release exactly. Confirm your monitor sends `component_name`, or a Datadog alias such as `app`, with the same value as `--component-name` in your deploy marker.
