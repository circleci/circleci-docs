...
:page-description: This document provides examples of deploying to Azure Container Registry.
:experimental:
...
Whether you require a user or service principal login, you will need to provide environment variables for username, password and tenant to CircleCI. For user logins use environment variable names as follows:
  ...
  build-deploy:
    jobs:
      - azure-acr/build-and-push-image:
          ...
          filters: pipeline.git.branch == "main" # only run the build-and-push-image job on the main branch
```
