...
:page-description: This document provides examples of deploying to Firebase.
:experimental:
...
. Set the JSON file path as the value of an environment variable `$GOOGLE_APPLICATION_CREDENTIALS` declared in the shell (See xref:security:env-vars.adoc#order-of-precedence[Environment Variables Order of Precedence]) guide. Then, run the Firebase deploy command.
