...
:page-description: This document provides examples of deploying to Cloud Foundry.
:experimental:
  ...
  build-deploy:
    jobs:
      ...
      - dark-deploy:
          requires:
            - test
          filters: pipeline.git.branch == "main" # only run the dark-deploy job on the main branch
      - hold:
          type: approval
          requires:
            - dark-deploy
          filters: pipeline.git.branch == "main" # only run the hold approval job on the main branch
      - live-deploy:
          requires:
            - hold # manual approval required via the CircleCI UI to run the live-deploy job
          filters: pipeline.git.branch == "main" # only run the live-deploy job on the main branch
```
