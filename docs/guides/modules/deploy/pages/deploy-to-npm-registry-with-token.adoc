...
TIP: Trusted publishing is the recommended way to publish to npm from CircleCI. It removes the need for a long-lived `NPM_TOKEN` environment variable. See xref:deploy:deploy-to-npm-registry.adoc[Publish to npm]. Use this guide only if trusted publishing is not an option for your package or registry.
