...
CircleCI provides deployment tracking and management features through two main features:
...
|All CircleCI projects can use deploy markers. More information is available in the xref:configure-deploy-markers.adoc[Configure Deploy Markers] guide.

|CircleCI Release agent
|The release agent is a tool that you install in your Kubernetes cluster to enable you to manage your deployments through CircleCI. The release agent supports Argo Rollouts for progressive delivery.
...
*Deploy markers* provide a lightweight way to log your deployments. Deploy markers generate a log of all deployments in one place, with links back to the CI/CD pipelines that triggered them, for a clear overview of what has changed, without the need to search through your CI/CD pipelines.

To configure deploy markers you will add commands to your pipeline configuration to enable CircleCI to log and track deployment status in the CircleCI UI. With deploy markers set up you can also set up and use *rollback pipelines* to rollback your application to a previous version from the CircleCI web app. More information is available in the xref:set-up-rollbacks.adoc[Set up Rollbacks] guide.
