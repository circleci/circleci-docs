...
With deploy markers set up you can also use the *rollback* feature to rollback to a previous version of a component from the CircleCI web app. Set up a rollback pipeline to get back to a working version of your application quickly. More information is available in the xref:set-up-rollbacks.adoc[Set up Rollbacks] guide.
...
If you are deploying to Kubernetes you can also use the CircleCI *Release agent*, which you install in your cluster to enable you to manage your deployments through CircleCI. For more information see the xref:release-agent-overview.adoc[Release Agent Overview].
...
* xref:configure-deploy-markers.adoc[Tutorial 1: Configure CircleCI Deploy Markers] (This is a manual setup guide for deploy markers)
* xref:set-up-rollbacks.adoc[Tutorial 2: Set Up Rollbacks] (Set up a rollback pipeline and through that process you will be guided to set up deploy markers)
