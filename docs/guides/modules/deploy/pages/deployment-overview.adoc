...
*Smart Deployments* is the CircleCI continuous validation feature. It validates each release against the monitoring tools you already run, and rolls back automatically when a signal fails. Smart Deployments needs no Kubernetes cluster and no agent installed. See the xref:smart-deployments-overview.adoc[Smart Deployments Overview] page.

If you deploy to Kubernetes and want cluster-native controls or progressive delivery with Argo Rollouts, you can use the CircleCI release agent instead. For more information, see the xref:release-agent-overview.adoc[Release Agent Overview] guide.
...
Set up deploy markers to track deployments in the CircleCI web app and rollback to previous versions as required. Add release validation to check the health of each release before you let it stand.
...
CircleCI provides deployment tracking and management through the features outlined in the following table and described in more detail in the sections below:
...
|Smart Deployments
|The CircleCI continuous validation feature. The release validation engine listens for signals from your monitoring tools, evaluates them against a policy in your configuration, then marks the release as a success or a failure. A failed release triggers your rollback pipeline automatically.
|Any project that wants to gate releases on health, without adopting Kubernetes, Argo Rollouts, or a service mesh. See the xref:smart-deployments-overview.adoc[Smart Deployments Overview].
...
|All CircleCI projects can use the deploy and rollback features. These are also the baseline that Smart Deployments builds on. See the xref:set-up-rollbacks.adoc[Set Up Rollbacks] and xref:configure-deploy-markers.adoc[Configure Deploy Markers] guides.
...
|Projects that need Kubernetes-native controls, such as scaling and restarting components from the web app, or progressive delivery with Argo Rollouts. More information is available in the xref:release-agent-overview.adoc[Release Agent Overview].
|===
--
...
To configure deploy markers, add steps to your pipeline configuration manually or use the multi-project setup in the CircleCI web app. For more information, see the xref:configure-deploy-markers.adoc[Configure Deploy Markers] guide.
...
* *Release validation* to evaluate signals from your monitoring tool after each deploy, and roll back automatically when a signal fails. See the xref:set-up-smart-deployments.adoc[Set Up Smart Deployments] guide.
* *Environment hierarchy and version promotion* to define ordered promotion paths and promote versions through each stage. See the xref:environment-hierarchy-and-version-promotion.adoc[Environment Hierarchy and Version Promotion] guide.

.Rollback guided setup
image::guides:ROOT:deploy/rollback-guided-setup.png[Rollback guided setup]

==== Get set up with Smart Deployments
...
To add one feature at a time, use the following guides:

* xref:configure-deploy-markers.adoc[Configure Deploy Markers]: Configure deploy markers manually, or use the multi-project setup to generate and commit deploy marker configuration for multiple projects at once.
...
You do not need the release agent to gate releases on health. Smart Deployments does that without cluster access. Choose the release agent when you want Kubernetes-native controls, such as scaling and restarting components from the web app, or progressive delivery with Argo Rollouts.
...
_Continuous validation_ is the practice of checking the health of a release automatically after it ships, and reverting it without human intervention when the checks fail.

CircleCI implements continuous validation as *Smart Deployments*, which works through the release validation engine. After a deploy completes, the engine opens an evaluation window and listens for webhook alerts from your monitoring tool. Each validation check has an expression that decides when an incoming alert counts as a failure, and a threshold for how many failures it tolerates. If a check fails, the engine marks the release as failed and can trigger your rollback pipeline automatically.

For more information, see the xref:smart-deployments-overview.adoc[Smart Deployments Overview] page.
...
An environment integration is a named environment record in CircleCI's Deploys UI that represents a deployment target. An environment integration can be one of two types:

* Custom. This can be used for any deployment target.
* Kubernetes cluster. Using this option you will be guided to set up the CircleCI release agent and configure your Kubernetes components.

The environment integration is used to organize and track deployments in the CircleCI deploys UI.
...
** Access settings for each environment icon:settings[Settings icon].
...
** Set up a new component by selecting btn:[Create Component]. From here you can select an environment that has a successful environment integration set up, and from there you can add a new component.
** Get straight to the component's project building on CircleCI by selecting the project name.
** Access setting for each component icon:settings[Settings icon].
...
. You are now on the component view page. Select the edit button icon:edit-solid[Edit icon] in the labels panel.
...
. Select the cog icon icon:settings[Settings icon] for your environment.
. You are now on the environment settings page. Select the edit button icon:edit-solid[Edit icon] in the labels panel.
. Enter or edit your label(s) and select btn:[Done].
...
When you use release validation, each deployment also has a validation status of its own, visible in the Release Validation panel on the deployment details page. For more information, see the xref:smart-deployments-overview.adoc#view-validation-results[View Validation Results] section.

[#data-retention]
== Data retention

Deployment and release data shown in the deploys UI, including the timeline, component and environment details pages, and release agent commands, is retained according to your plan's data retention policy:
...
Once a deployment or release exceeds your plan's retention period, its historical record is automatically and permanently deleted. This does not affect the currently live version of any component; only historical deployment and release records are removed. For more information, see the link:https://support.circleci.com/hc/en-us/articles/5645222646939-Cloud-Data-Retention-Policy-and-Settings[Cloud Data Retention Policy and Settings] support article.
