...
*Deploy markers* provide a lightweight way to log your deployments. Deploy markers are configured as steps in your pipeline configuration files. Deploy markers generate a log of all deployments in one place, with links back to the CI/CD pipelines that triggered them, for a clear overview of what has changed, without the need to search through your CI/CD pipelines.

To configure deploy markers you will add commands to your pipeline configuration to enable CircleCI to log and track deployment status in the CircleCI UI. With deploy markers set up you can also set up and use *rollback pipelines* to rollback your application to a previous version from the CircleCI web app. More information is available in the xref:set-up-rollbacks.adoc[Set up a Rollback Pipeline] guide.
...
* xref:configure-deploy-markers.adoc[Configure CircleCI Deploy Markers] - This is a guide to configuing deploy markers manually.
* xref:set-up-rollbacks.adoc[Set up Rollbacks] – Set up a rollback pipeline and through that process you will be guided to also set up deploy markers if you have not already done so.
