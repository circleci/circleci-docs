...
Track deployments using deploy markers. Deploy and rollback to a stable version using preconfigured deploy and rollback pipelines. Validate each release against your monitoring tool and roll back automatically when a signal fails.

The quickest way to get all of this set up is the Smart Deployments guided setup in the web app. See the xref:set-up-smart-deployments.adoc[Set up Smart Deployments] guide.
...
Set up deploy markers to track deployments in the CircleCI web app and rollback to previous versions as required. Add deploy validation to check the health of each release before you let it stand.
...
|CircleCI deploy markers generate a log of all deployments in one place. This deploy log provides a clear overview of what has changed, without the need to search through your CI/CD pipelines. Deploy markers enable rollback and deploy pipelines so you can deploy and rollback your projects from the CircleCI web app. Deploy validation adds automatic health checks and automatic rollback on top.
|All CircleCI projects can use the deploy and rollback features. To set them all up at once, see the xref:set-up-smart-deployments.adoc[Set up Smart Deployments] guide. For individual setup, see the xref:set-up-rollbacks.adoc[Set up Rollbacks] and xref:configure-deploy-markers.adoc[Configure Deploy Markers] guides.
...
* *Deploy validation* to evaluate signals from your monitoring tool after each deploy, and roll back automatically when a signal fails. See the xref:set-up-smart-deployments.adoc[Set up Smart Deployments] guide.
...
Smart Deployments is a guided setup in the web app that configures deploy markers, rollback and deploy pipelines, deploy validation, and automatic rollback in one flow. It generates the configuration with AI and opens a pull request for you to review and merge.

Start with the xref:set-up-smart-deployments.adoc[Set up Smart Deployments] guide. Smart Deployments requires a GitHub repository and AI features enabled for your organization.

==== Get set up feature by feature

If Smart Deployments is unavailable for your organization, or you want to add one feature at a time, use the following guides:
...
* xref:set-up-deploy-validation-manually.adoc[Set up Deploy Validation Manually]: Configure deploy validation by editing your configuration file directly, then connect your monitoring tool. Use this route if your code is not in a GitHub repository.
...
CircleCI implements continuous validation as *deploy validation*. After a deploy completes, CircleCI opens an evaluation window and listens for webhook alerts from your monitoring tool. Each validation check has an expression that decides when an incoming alert counts as a failure, and a threshold for how many failures it tolerates. If a check fails, CircleCI marks the release as failed and can trigger your rollback pipeline automatically.

Set up deploy validation with the xref:set-up-smart-deployments.adoc[Set up Smart Deployments] guide.
...
|INCONCLUSIVE
|Deploy validation reached its evaluation deadline without a decisive result. See the xref:set-up-smart-deployments.adoc#how-deploy-validation-works[How Deploy Validation Works] section.
|===
--

When you use deploy validation, each deployment also has a validation status of its own, visible in the Release Validation panel on the deployment details page. For more information, see the xref:set-up-smart-deployments.adoc#view-validation-results[View Validation Results] section.
