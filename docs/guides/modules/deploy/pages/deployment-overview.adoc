...
To configure deploy markers you can add steps to your pipeline configuration manually, or use the multi-project setup in the CircleCI web app to generate and commit deploy marker configuration across multiple projects at once using AI. For more information, see the xref:configure-deploy-markers.adoc[Configure Deploy Markers] guide.
...
* *Environment hierarchy and version promotion* to define ordered promotion paths (for example, dev → staging → production) and promote versions through each stage with a single click. See the xref:environment-hierarchy-and-version-promotion.adoc[Environment Hierarchy and Version Promotion] guide.
