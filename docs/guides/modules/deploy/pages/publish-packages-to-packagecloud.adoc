...
The following is a full example `.circleci/config.yml` that will:

* Checkout a git repository.
* Run a `make` task (this command can be anything configured to build your package).
* Deploy the package to a packagecloud repo.
...
Deploy markers provide a way to track and manage your Packagecloud package publications in the CircleCI web app. When you add deploy markers to your deployment job, you can view a timeline of all deployments, track their status, and enable rollback and deploy pipelines.
