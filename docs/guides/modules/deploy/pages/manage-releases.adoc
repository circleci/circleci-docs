...
This guide is for the CircleCI release agent. If you are using the CircleCI deploys feature, see the xref:deployment-overview.adoc[Deployment and Deploy Management] guide.
...
. Scroll to locate the version you want to restore, and select the "Restore this version" icon in the **Actions** column.
. Read through the recommendations in the popup. If you want to proceed, type `RESTORE`, then select btn:[Restore]. We recommend reverting the associated changes in your source repository as soon as possible after using this action.
...
. Scroll or search to find and select your component to enter the component view.
. Select the "Active Versions" tab.
. Find the active version that you would like to scale and select the "Scale Component" icon.
. Read through the notes associated with this action in the popup. To proceed, type `SCALE` and select btn:[Scale].
...
. Scroll or search to find and select your component to enter the component view.
. Select the "Active Versions" tab.
. Find the active version that you would like to restart and select the "Restart Component" icon.
. Read the note associated with this action in the popup. To proceed, type `RESTART` and select btn:[RESTART].
...
Use the btn:[Cancel Rollout] action to stop a deployment from progressing, and revert all steps. For more information see the link:https://argo-rollouts.readthedocs.io/en/stable/generated/kubectl-argo-rollouts/kubectl-argo-rollouts_abort/[Argo Rollouts docs for Rollouts Abort].

NOTE: You can use this action if you are using link:https://argoproj.github.io/argo-rollouts/[Argo Rollouts]

.Cancel Rollout
image::guides:ROOT:releases/promote-options.png[Screenshot showing the cancel Rollout button]
...
. Select btn:[Cancel Rollout].

[#cancel-notes]
=== Notes

Selecting the btn:[Cancel Rollout] action is the equivalent to running:
