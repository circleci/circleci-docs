...
:page-description: Configure a CircleCI deploy pipeline to support manual deployment of a component to a target environment.
:experimental:
...
TIP: *Smart Deployments sets up a deploy pipeline for you.* The Smart Deployments guided setup configures your deploy pipeline alongside deploy markers, a rollback pipeline, release validation, and automatic rollback, then opens a single pull request for you to merge. See the xref:set-up-smart-deployments.adoc[Set Up Smart Deployments] guide. Use this page to configure a deploy pipeline on its own, or to customize the pipeline that Smart Deployments generated for you.
...
- Deploy markers *must* be configured in your project to use a deploy pipeline. You will be guided to configure deploy markers through the steps on this page or you can get set up manually by following the xref:configure-deploy-markers.adoc[Configure Deploy Markers] guide.
...
. Select the btn:[Deploy] button to open the deploy pipeline setup modal.
+
.Deploy button on project overview page
image::guides:ROOT:deploy/project-overview-deploy.png[Deploy button on project overview page]

=== 2. Confirm GitHub App installation

To set up deploy pipelines you must install the CircleCI GitHub App into your organization. For more information, see the xref:permissions-authentication:users-organizations-and-integrations-guide.adoc[Users, Organizations, and Integrations Guide].

NOTE: Only organization administrators can install the GitHub App. If you are not an organization administrator, you can ask an administrator to install the GitHub App for you.
...
No changes are being pushed to your repository at this point. GitHub pipelines are viewable at menu:Project settings[Project Setup].

. Select the repository that includes the project code you want to deploy with this pipeline.
. Select btn:[Create pipeline definition]. This creates a new pipeline definition for your deploy pipeline in your project.

=== 4. Add deploy markers

Deploy markers *must* be configured in your project to use CircleCI deploy pipelines. If you already have deploy markers set up, select "I'll make the config changes myself" and then select btn:[I've updated my config] to proceed to the next step.

The in-app setup process guides you to set up deploy markers now. Alternatively, manual configuration steps are available in the xref:configure-deploy-markers.adoc[Configure Deploy Markers] guide. If you want to set up deploy markers across multiple projects at once, see the xref:configure-deploy-markers.adoc#set-up-deploy-markers-for-multiple-projects[Set Up deploy markers for multiple projects] section.
...
+
--
NOTE: The Generate with AI option requires AI features to be enabled for your organization. If AI features are not enabled, you are presented with a configuration template that you can copy and use to update your configuration manually. An organization admin can enable AI features by navigating to menu:Organization Settings[Advanced]. For more details, see the xref:toolkit:enable-ai-powered-features.adoc[Enable AI-Powered Features] page.
...
. In the **Component Name** field, select or type the name of the component you want to deploy. The component name sets the name that will be displayed in the Deploys UI.
+
...
. Copy the generated commands using the btn:[Copy] button. Paste them into your existing config file where the deployment occurs, on the branch you selected in the guided setup.
+
...
+
NOTE: The configuration will be committed to a new branch called `deploy-pipeline-setup`.
+
.Commit config button
image::guides:ROOT:deploy/deploys-commit-config-button.png[Commit config button]
+
...
. You can manually navigate to the pull request by selecting btn:[View pull request] under the example config file.
...
. After merging the pull request, return to the setup page in the CircleCI web app and select btn:[Setup deploy pipeline] to activate the deploy pipeline.

You can now deploy your component by selecting btn:[Deploy] in your project overview and running your deploy pipeline. For detailed steps, see the xref:deploy-a-component.adoc[Deploy a Component] guide.

TIP: If you deploy to multiple environments (for example dev, staging, or production), you can set up an environment hierarchy to define your promotion path. Version promotion then moves versions through each stage with one click. See the xref:environment-hierarchy-and-version-promotion.adoc[Environment Hierarchy and Version Promotion] guide.

== Run a deploy pipeline

For steps to run a deploy pipeline, see the xref:deploy-a-component.adoc[Deploy a Component] guide.
...
. In the Deploy Pipeline section, choose the pipeline you want to be selected as the deploy pipeline from the dropdown. You can also choose to _unset_ the deploy pipeline at this point, which will enable you to use the guided setup again as described above.

.Change deploy pipeline
image::guides:ROOT:deploy/change-deploy-pipeline.png[Change deploy pipeline]
    ...
    jobs:
      ...
      - cancel-deploy:
          ...
          requires:
            - deploy-component:
              - canceled
          filters: pipeline.git.branch == "main" # only run the cancel-deploy job on the main branch
----

== Next steps

* Learn how to use environment hierarchy and version promotion to manage your deployments through multiple environments. See the xref:environment-hierarchy-and-version-promotion.adoc[Environment Hierarchy and Version Promotion] guide.
* Learn how to deploy to multiple environments using a deploy pipeline. See the xref:deploy-a-component.adoc[Deploy a Component] guide.
