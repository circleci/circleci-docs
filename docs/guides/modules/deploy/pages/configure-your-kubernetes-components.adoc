...
The steps outlined on this page guide you to configure your Kubernetes resources for integration with your environment integration. This manual installation guide is an alternative to using the in-app deploys set up process as described in the xref:set-up-the-circleci-release-agent.adoc[Set up the CircleCI Release Agent] page.
...
Before following the steps below, refer to the xref:set-up-the-circleci-release-agent.adoc[Set up the CircleCI Release Agent] page and ensure you have completed the following:

* Set up your environment integration.
* Install the CircleCI release agent into your Kubernetes cluster.

[#add-required-labels]
== 1. Add required labels and annotations to your Deployment/Rollout

To enable your deployment to show up on the deploys UI in the CircleCI web app, add the following labels and annotations to your Kubernetes Deployment or Argo Rollout:
...
*Built-in environment variable*: Use the `$CIRCLE_PROJECT_ID` xref:reference:ROOT:variables.adoc#built-in-environment-variables[Built-in Environment Variable] in a CircleCI pipeline. This way, you can use tools like `envsubst` to substitute the placeholder for the real value.
...
+
.The project ID available in project settings
image::guides:ROOT:project-settings-overview.png[Screenshot showing where to find the project ID]
****
