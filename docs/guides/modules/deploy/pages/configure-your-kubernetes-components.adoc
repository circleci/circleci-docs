...
* Set up your environment integration. Refer to the xref:set-up-circleci-deploys.adoc[Set up CircleCI deploys] page.
...
*Built-in env var*: Use the `$CIRCLE_PROJECT_ID` xref:reference:ROOT:variables.adoc#built-in-environment-variables[built-in environment variable] in a CircleCI pipeline. This way, you can use tools like `envsubst` to substitute the placeholder for the real value.
