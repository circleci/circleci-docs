= Set up deploy validation manually
:page-platform: Cloud
:page-description: Configure deploy validation by editing your CircleCI configuration file, for organizations that cannot use the Smart Deployments guided setup.
:experimental:

Deploy validation evaluates webhook signals from your monitoring tool during an evaluation window after each deploy. This guide shows how to configure it by editing your CircleCI configuration file, rather than through the Smart Deployments guided setup.
...
Follow this guide in either case. The result is the same deploy validation feature. The difference is that you write the configuration and connect your monitoring tool yourself.

=== What you can set up manually

Deploy validation and deploy markers are configuration, so they work on any version control system. Rollback and deploy pipelines are a web app feature that currently requires GitHub.
...
[IMPORTANT]
====
Without a rollback pipeline, deploy validation detects failures but cannot revert them.
...
Deploy validation attaches to a _release job_, which is a job with `type: release` that names the release plan created by your `circleci run release plan` step. If your configuration already has one, skip to the next step.
...
== 2. Turn on deploy validation

Add a `validation` block to your release job. Deploy validation needs only one field to turn it on:
...
For every field, the `data_points` keys, and the provider defaults, see the xref:deploy-validation-reference.adoc[Deploy Validation Configuration Reference] page.
...
For what each field means during matching, see the xref:deploy-validation-reference.adoc#data-point-keys[Data Point Keys] section.

== Next steps

* Review every field you can set on the `validation` block. See the xref:deploy-validation-reference.adoc[Deploy Validation Configuration Reference] page.
