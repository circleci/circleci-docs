...
The token is shown only once and cannot be retrieved from CircleCI later. Store it in your organization's secret manager, for example HashiCorp Vault, AWS Secrets Manager, or a CircleCI context variable. Pass the token to Helm at install or upgrade time using an environment variable or a values file that is not committed to source control. For example, use `--set tokenSecret.token="\{CCI_INTEGRATION_TOKEN}"`. After installation, the chart stores the token in a Kubernetes Secret in the `circleci-release-agent-system` namespace. On subsequent `helm upgrade` runs, you do not need to supply the token again. An exception applies if you use `--reset-values`, or apply a values file that omits `tokenSecret.token` without `--reuse-values`.
