...
:page-description: Configure a CircleCI rollback pipeline to revert a deployment to a previous stable version.
:experimental:
...
TIP: *Smart Deployments sets up a rollback pipeline for you.* The Smart Deployments guided setup configures your rollback pipeline alongside deploy markers, a deploy pipeline, release validation, and automatic rollback, then opens a single pull request for you to merge. See the xref:set-up-smart-deployments.adoc[Set Up Smart Deployments] guide. Use this page to configure a rollback pipeline on its own, or to customize the pipeline that Smart Deployments generated for you.
...
A rollback pipeline is also what CircleCI triggers when release validation fails and automatic rollback is enabled. For more information, see the xref:set-up-smart-deployments.adoc[Set Up Smart Deployments] guide.
...
No changes are pushed to your repository at this point. GitHub pipelines are viewable at menu:Project settings[Project Setup].

. Select the repository that includes the project code you want to roll back with this pipeline.
...
. In the **Component Name** field, select or type the name of the component you want to roll back. The component name sets the name that will be displayed in the Deploys UI.
+
...
For steps to run a rollback pipeline, see the xref:rollback-a-deployment.adoc[Rollback a Deployment] guide.
...
. In the Rollback Pipeline section, choose the pipeline you want to be selected as the rollback pipeline from the dropdown. You can also choose _unset_ rollback pipeline at this point, which will enable you to use the guided setup again as described above.

.Change rollback pipeline
image::guides:ROOT:deploy/change-rollback-pipeline.png[Change rollback pipeline]
    ...
    jobs:
      ...
      - cancel-rollback:
          ...
          requires:
            - rollback-component:
              - canceled
          filters: pipeline.git.branch == "main" # only run the cancel-rollback job on the main branch
----
