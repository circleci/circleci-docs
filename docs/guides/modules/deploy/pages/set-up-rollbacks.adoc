...
CircleCI rollbacks allow you to quickly revert your application to a previous stable version when issues are detected in production. This guide covers how to configure your CircleCI pipeline to support rollback operations.
...
. Select the repository tyhat includes your project code. This is the repository that you want to rollback with this rollback pipeline.
...
. Expand the **Add deploy markers to configuration file** section.
. You can select btn:[Generate config change with AI] to auto-generate a new configuration file with deploy markers.
+
Alternatively, select the "I'll make the config changes myself" checkbox and follow the next steps manually:
+
