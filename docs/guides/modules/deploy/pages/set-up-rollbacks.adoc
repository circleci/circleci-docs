...
CircleCI rollbacks support two methods, either rollback by running a defined rollback pipeline that you configure, or rollback by rerunning the workflow that originally deployed the version you want to rollback to. This guide covers setting up a rollback pipeline which is the recommended approach for most applications.

The process of rolling back is described in detail in the xref:rollback-a-deployment.adoc[Rollback a Deployment] guide, including a comparison of the two methods
...
. In the CircleCI dashboard, navigate to *Home* from the sidebar.
...
NOTE: To set up rollback pipelines, you must install the CircleCI GitHub App into your organization. For more information, see the xref:permissions-authentication:users-organizations-and-integrations-guide.adoc[Users, Organizations, and Integrations Guide].
...
. Select the repository tyhat includes your project code, that you want to rollback with this rollback pipeline.
. Select btn:[Create pipeline definition]. This created a new pipeline definition for your rollback pipeline.

=== 4. Add deploy markers

Deploy markers *must* be configured in your project to use CircleCI rollbacks. If you already have deploy markers set up, select btn:[I've updated my config] to proceed to the next step.

This in-app setup process guides you to set up deploy markers now. Alternatively, manual configuration steps are available in the  xref:configure-deploy-markers.adoc[Configure Deploy Markers] guide.
...
. Copy the generated commands using the btn:[Copy] button and paste them into your existing config file where the deployment occurs on the branch you selected in the guided setup.
+
...
+
.Confirm deploy marker values have been added
...
+
NOTE: The configuration will be committed to a new branch called `rollback-pipeline` in the repository you selected earlier in this guided setup.
+
...
You can now rollback deployments by running a rollback pipeline. More information is available in the xref:rollback-a-deployment.adoc[Rollback a Deployment] guide.
