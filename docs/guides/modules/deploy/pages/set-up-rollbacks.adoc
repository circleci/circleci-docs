...
=== Start the rollback pipeline guided setup
...
NOTE: The rollback by pipeline method requires that your organization is connected to the xref:integration:github-apps-integration[*CircleCI GitHub App*].

If the GitHub App is not installed in your organization, the rollback setup process will prompt you to install it.

image::guides:ROOT:deploy/install-github-app.png[Install GitHub App]

If the GitHub App is already installed in your organization, The **GitHub App Installation** step will automatically be marked as completed.
...
. If it is not already expanded, expand the **Add deploy markers to exisiting pipeline** section.
