...
For steps to running a rollback pipeline, see the xref:rollback-a-deployment.adoc[Rollback a Deployment] guide.
...
. In the Rollback Pipeline section, choose the pipeline you want to be selected as the rollback pipeline from the dropdown. You can also choose the _unset_ the rollback pipeline at this point, which will enable you to use the guided setup again as described above.
