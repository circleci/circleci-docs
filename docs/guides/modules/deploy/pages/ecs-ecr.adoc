...
WARNING: This page is outdated. CircleCI is working on a new updated sample project. The information on this page is still relevant, but the sample project will be replaced.
...
NOTE: The sample project described in this section makes use of the CircleCI AWS-ECR and AWS-ECS orbs. You can find them in the CircleCI developer hub using the following links:
...
The `deploy-service-update` job of the AWS-ECS orb completes the following:

* Creates a new task definition that is based on the current task definition, but with the new Docker image specified in the task definition's container definitions.
* Deploys the new task definition to the specified ECS service.

If you would like more information about the CircleCI AWS-ECS orb, go to the link:https://circleci.com/developer/orbs/orb/circleci/aws-ecs[CircleCI AWS-ECS orb] page on the developer hub.
...
Deploy markers provide a way to track and manage your AWS ECR and ECS deployments in the CircleCI web app. When you add deploy markers to your deployment job, you can view a timeline of all deployments, track their status, and enable rollback and deploy pipelines.
...
* Go to the link:https://github.com/CircleCI-Public/circleci-demo-aws-ecs-ecr/tree/orbs[AWS-ECS-ECR Orbs] demo page to review an example that builds, tests, and pushes the Docker image to ECR. It then uses the `aws-ecs` orb to deploy the update.
