...
If you set your environment variables as shown earlier in this tutorial, you will not need to set the following parameters. The default values take the same environment value names:
...
The following snippet is an example with each parameter set as its default value.
...
As with the deploy-to-play-store approach, you will not need to set the parameters `base64-keystore`, `release-key-alias`, `release-key-password`, `release-store-password`, and `google-play-key`, if you have created environment variables as outlined earlier in this tutorial.
...
+
Finally, create your Google Play API key from your `api.json` file, which you have already saved as the environment variable `$GOOGLE_PLAY_KEY`. You can refer to this variable and write the `api.json` file at runtime, instead of uploading it to a remote repository.
+
...
. **Deploy your app**
+
To deploy your app, you need to add your Fastlane steps. To do this you only need to add a command that runs `fastlane my_deployment_lane`; in this case, the command looks like this:
+
```yaml
...
Deploy markers provide a way to track and manage your Android app deployments in the CircleCI web app. When you add deploy markers to your deployment job, you can:

* View a timeline of all deployments to the Google Play Store.
* Track deployment status.
* Enable rollback and deploy pipelines.
