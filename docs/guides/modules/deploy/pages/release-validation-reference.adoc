...
For the release job keys and their accepted types, see the xref:reference:ROOT:configuration-reference.adoc#release-job-validation[Configuration Reference]. This page covers what CircleCI does with those values.
...
NOTE: For Datadog, the engine also accepts the monitor tags `app:<name>` and `service:<name>` as aliases for `component_name`. Prefer the canonical name.
...
NOTE: For Datadog, the engine also accepts the monitor tag `environment:<name>` as an alias for `env`. Prefer the canonical name.
