...
Visualise and control deployments to your Kubernetes cluster with CircleCI deploys. The steps outlined on this page guide you to set up CircleCI deploys using the CircleCI web app onboarding process. Alternatively, once you have set up an environment integration, you can follow the guides linked below to install the release agent and configure components manually.
...
. To install the release agent into your cluster and set up your components, follow the in-app setup steps. If you would rather continue the setup process manually, see the xref:set-up-the-release-agent.adoc[Set up the release agent] page.
