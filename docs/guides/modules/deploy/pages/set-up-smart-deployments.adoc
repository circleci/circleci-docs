...
TIP: You can use the xref:guides:toolkit:circleci-agent-skills.adoc[CircleCI Agent Skills] to run this setup from your editor instead of the web app. Install the link:https://github.com/CircleCI-Public/skills[CircleCI Public Skills] plugin to your AI coding agent, then ask your agent to set up deploy markers, pipelines, and release validation for your project. The `circleci-deploy-setup` skill does not depend on your organization's AI settings, so it can generate configuration and open a pull request when the guided setup cannot.
