...
|Interactive first-time onboarding wizard: auth, org ID, sidecar creation, dependency installation, and snapshotting.
...
|Reads and sets the async validation keys in `.chunk/config.json` to control whether the agent waits for `chunk validate` or is released to continue working.
