...
+
CAUTION: For GitHub App or GitLab pipelines, use the link:https://circleci.com/docs/api/v2/index.html#tag/Pipeline/operation/triggerPipelineRun[Trigger a new pipeline endpoint] rather than the endpoint used in this example.
+
[,shell]
----
...
* `mttr` - The Mean Time to Recovery (MTTR). MTTR is the average time it takes to get a failed CI build back to a "success" status.
