...
NOTE: Currently, xref:managing-api-tokens.adoc#creating-a-personal-api-token[Personal API Tokens] are the only supported tokens on API v2. xref:managing-api-tokens.adoc#creating-a-project-api-token[Project Tokens] are not currently supported on API v2.
...
. xref:toolkit:managing-api-tokens.adoc#creating-a-personal-api-token[Create a Personal API Token] by visiting the link:https://app.circleci.com/settings/user/tokens[Personal API Tokens] page, and follow the steps to add an API token.
...
. Select the ellipsis menu image:guides:ROOT:icons/more.svg[more icon, role="no-border"] next to your project and select **Project Settings**.
. The project slug is listed on the project settings homepage. Use the copy button to copy the slug to your clipboard.

The `project_slug` is also included in the response payload when looking up a link:https://circleci.com/docs/api/v2/#tag/Pipeline/operation/getPipelineById[pipeline] or link:https://circleci.com/docs/api/v2/#tag/Workflow/operation/getWorkflowById[workflow] by ID. The project slug can then be used to get information about the project.

=== GitHub OAuth and Bitbucket Cloud
If you integrate your code via the GitHub OAuth app or Bitbucket Cloud, your project slug is in a human readable format: `vcs_type/org_name/repo_name` (VCS type can be `github`/`bitbucket` or `gh`/`bb`).
...
The following section details the steps you need to make an API call from start to finish. This section includes creating a demo repository called `hello-world`. Alternatively, you can use a pre-existing repository to follow along.

Many of the API calls make use of the +++`{project-slug}`+++ triplet, described <<getting-started-with-the-api,above>>.

You will get a list of pipelines for your project using link:https://circleci.com/docs/api/v2/#tag/Pipeline/operation/listPipelinesForProject[+++`GET /project/{project-slug}/pipeline`+++].
...
+
CAUTION: For GitHub App, GitLab or Bitbucket Data Center pipelines, use the link:https://circleci.com/docs/api/v2/index.html#tag/Pipeline/operation/triggerPipelineRun[Trigger a new pipeline endpoint] rather than the endpoint used in this example.
+
[,shell]
----
...
----
+
While this alone can be useful, you may want to customize parameters of the pipeline when you send this POST request. By including a body parameter in the `curl` request (via the `-d` flag), you can customize specific attributes of the pipeline when it runs. These attributes include pipeline parameters, the branch, or the git tag. Below, the pipeline is set to trigger for "my-branch":
+
[,shell]
----
 curl -X POST https://circleci.com/api/v2/project/{project-slug}/pipeline \
...
. Let us move on to a more complex example: triggering a pipeline and passing a parameter that can be dynamically substituted into your configuration. In this example, you will pass a Docker image tag to the `docker` executor key.
+
First, modify the `.circleci/config.yml` to be a little more complex than the standard "Hello World" sample provided by the onboarding.
+
[,yaml]
----
version: 2.1
...
----
+
You will need to declare the parameters you expect to receive from the API. In this case, under the `parameters` key, define an `image-tag` to be expected in the JSON payload of a POST request to the link:https://circleci.com/docs/api/v2/index.html#operation/triggerPipeline[Trigger a new pipeline] endpoint.
+

. Now run a `curl` request that passes variables in a POST request, similar to the following:
+
[,shell]
----
...
Now that you have a general understanding of how the CircleCI API v2 service works, let us look at a few common tasks and operations. You may perform these on a regular basis when using the API.
...
You may often find it helpful to retrieve information about a specific project. This includes the name of the organization the project belongs to, the version control system (VCS) that hosts the project, and other details. The CircleCI API enables you to return this information by making a single GET request to the +`project/{project-slug}`+ endpoint.
...
[,shell]
----
curl -X GET https://circleci.com/api/v2/project/{project-slug}/job/{job_number} \
  --header "Content-Type: application/json" \
  --header "Accept: application/json" \
  --header "Circle-Token: $CIRCLE_TOKEN" \
----
...
The following section details the steps to download artifacts that are generated when a job is run. First, you will return a list of artifacts for a job. Then, you will download the full set of artifacts. If you want to download the _latest_ artifacts for a pipeline without specifying a job number, see the xref:optimize:artifacts.adoc#downloading-all-artifacts-for-a-build-on-circleci[Downloading All Artifacts for a Build on CircleCI] guide. Keep checking back here as this functionality will be added to API v2 in the future.

[#download-artifacts-steps]
==== Steps

. Ensure your API token is set as an environment variable. You may have already done this during authentication. If not, run the command below in your terminal, substituting your personal API token:
+
[,shell]
----
export CIRCLE_TOKEN={your_api_token}
...
+
.Job number location
image::guides:ROOT:job-number.png[Job number]
...
. Next, extend this API call to download the artifacts. Navigate to the location you would like to download the artifacts to. Run the command below, remembering to substitute your own values in the request:
+
[,shell]
----
 curl -X GET https://circleci.com/api/v2/project/{project-slug}/{job_number}/artifacts \
...
The CircleCI API v2 also includes several endpoints that enable you to retrieve detailed insights into your workflows and individual jobs. Read the xref:insights:insights.adoc[Using Insights] page to learn more about insights data.
...
. After you have defined the +`project-slug`+ and made the API request, you will receive unformatted JSON text. It will look similar to the example shown below.
...
Notice that in this JSON response, you will receive detailed metrics for the set of workflows that ran, including:
...
* `total credits used` - The total number of credits used during the build.
...
Now that you have retrieved aggregated data for up to 250 different jobs, you may want to review specific information about a single job. This helps ensure that your jobs are running efficiently. To review an individual job, follow the steps below.
