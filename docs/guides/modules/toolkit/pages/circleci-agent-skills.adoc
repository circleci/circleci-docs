...
The CircleCI plugin currently includes the following eight skills. Your agent picks a skill from your request, or you can invoke a skill by name.
...
| Sets up Smart Deployments in a repository: deploy markers, deploy and rollback pipelines, release validation, and monitoring webhooks. Your agent first audits what the repository already has, then asks which parts to add or regenerate. The manual setup is described on the xref:guides:deploy:set-up-smart-deployments.adoc[Set Up Smart Deployments] page.
| Set up CircleCI deploy markers, deploy and rollback pipelines, and release validation for this project.
