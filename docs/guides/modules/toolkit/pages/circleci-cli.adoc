...
.Login with a short browser-based auth flow and store tokens in your credential management system.
[,shell]
----
...
* Use `DO_NOT_TRACK=1` to disable telemetry (see also xref:#telemetry[Telemetry]).
* Use TTY detection to automatically adjust output formatting when not connected to a terminal, for example in CI pipelines or when piping to other commands.
