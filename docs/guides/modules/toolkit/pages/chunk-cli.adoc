...
* Run validation commands locally or in CircleCI cloud environments using `chunk validate`
* Install AI agent skills for code review, mutation testing, and CI failure debugging using `chunk skill install`
* Set up pre-commit and session-end hooks that automatically run validation in Claude Code using `chunk init`
...
Confirm the chunk CLI is installed and check the version:
...
To see which credentials are configured:
...
* xref:how-to-use-chunk-cli.adoc#run-validation[Run validation commands]
