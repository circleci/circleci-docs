...
You can use the CircleCI CLI can be used to manage config policies for your projects.
...
- `<policy-file-or-dir-path>` is either the path to the policy file or directory containing the policy file.
...
- `--after`: Filter results to only include logs after a specific date and time.
+
Example: `--after 2025/03/14`
- `--before`: Filter results to only include logs before a specific date and time.
+
Example: `--before 2025/05/22`
- `--branch`: Filter results to only include logs for a specific branch.
+
Example: `--branch feature/fixes`
