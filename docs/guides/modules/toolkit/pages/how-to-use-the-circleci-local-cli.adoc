...
. Navigate to your project directory with a `.circleci/config.yml` file
...
For more information on orb packing, see the xref:orbs:use:orb-concepts.adoc#orb-packing[Orbs Concepts] page.
...
- `<namespace>` is the namespace claimed for your organization. You can see your namespace on the self-hosted runners inventory page in the CircleCI web app, by selecting **Runners** in the sidebar.
...
- `<resource-class>` is the name of the resource class for the specific pool of self-hosted runners. You can find the resource class for all your pools in the link:https://app.circleci.com[CircleCI web app], by selecting **Runners** in the sidebar.
...
NOTE: This command only applies to GitLab or GitHub App organizations (Organization slug starts with `circleci` followed by a UUID. For example, `circleci/34R3kN5RtfEE7v4sa4nWAU`. You can find your organization slug under menu:Org[Overview]).
...
- `<org-id>` is the ID of the organization associated with the project. You can find it from the CircleCI web app under menu:Org[Overview].
...
NOTE: The `create` sub-command of the `pipeline` command only applies to pipeline definitions created with the GitHub App provider Organization slug starts with `circleci` followed by a UUID. For example, `circleci/34R3kN5RtfEE7v4sa4nWAU`. You can find your organization slug under menu:Org[Overview]).
...
- `<org-id>` is the ID of the organization associated with the pipeline definition. You can find it from the CircleCI web app under menu:Org[Overview].
...
- `<org-id>` is the ID of your organization that owns the policy. You can find it from the CircleCI web app under menu:Org[Overview].
