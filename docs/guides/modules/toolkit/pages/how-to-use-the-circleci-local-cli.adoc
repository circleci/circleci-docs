...
The xref:orbs:author:orb-author.adoc#orb-development-kit[Orb Development Kit] refers to a suite of tools that work together to simplify the xref:orbs:use:orb-intro.adoc[registry orb] development process, with automatic testing and deployment on CircleCI. The orb development kit has two commands:
