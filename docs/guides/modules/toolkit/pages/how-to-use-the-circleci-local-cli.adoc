...
The above example shows the minimal usage. You will be prompted for required values if you don't intially provide them using the appropriate flags.

The `trigger create` sub-command requires the 

- The ID of the pipeline associated with the trigger. You can retrieve it from the CircleCI web app by selecting btn:[Copy] for the chosen pipeline, under *Project settings* > *Pipelines*.

- The ID of the GitHub repository associated with the trigger. Refer to the link:https://docs.github.com/en/rest/repos/repos?api#get-a-repository[GitHub documentation] to find this ID.
