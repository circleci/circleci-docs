...
- `--enable`is the flag to either enable or disable a policy evaluation. Set it to `true`to enable, `false`to disable.

At the moment, the `settings` sub-command can only be used to read the settings and to xref:config-policies:create-and-manage-config-policies.adoc#config-policy-management-enablement[enable/disable a policy evaluation].
