...
You can also find IDs in the CircleCI web app, from environment variables in your jobs, or from webhook payloads.
...
For additional ways to find resource IDs, see xref:how-to-find-ids.adoc[How to Find IDs].
...
----
curl -g 'https://circleci.com/api/v3/orgs' \
  --header "Authorization: Bearer $CIRCLE_TOKEN" | jq '.data[] | {id, name, slug}'
----
+
...
----
curl -g 'https://circleci.com/api/v3/projects?filter[org_id]=<your-org-id>' \
  --header "Authorization: Bearer $CIRCLE_TOKEN" | jq '.data[] | {id, name, slug: .attributes.slug}'
----
+
...
----
curl -g 'https://circleci.com/api/v3/runs?filter[project_id]=<your-project-id>&page[limit]=10' \
  --header "Authorization: Bearer $CIRCLE_TOKEN" | jq '.data[] | {id, number, phase: .attributes.phase}'
----
+
...
----
curl -g 'https://circleci.com/api/v3/workflows?filter[run_id]=<your-run-id>' \
  --header "Authorization: Bearer $CIRCLE_TOKEN" | jq '.data[] | {id, name: .attributes.name, status: .attributes.status}'
----
+
