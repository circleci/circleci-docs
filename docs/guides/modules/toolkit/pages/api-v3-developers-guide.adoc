...
:page-platform: Cloud
:page-description: CircleCI API v3 guide for developers. Learn how to use the API to return detailed information about users, pipelines, projects, workflows, and more.
:experimental:

This guide walk through getting set up to use the CircleCI API v3 to make API calls to CircleCI services. Use the API to return detailed information about users, runs, pipelines, projects, workflows, and more. View the API v3 specification in the link:https://circleci.com/docs/api/v3/[API v3 Reference documentation].
...
API v3 uses Bearer Authentication. Pass your token in the `Authorization` header.

==== Create an API token

To add an API token, follow these steps:

include::guides:ROOT:partial$shared-sections/create-a-personal-api-token.adoc[]
...
while IFS=$'\t' read -r artifact_path url; do
  ...
  curl -o "artifacts/$artifact_path" "$url"
done < artifacts.tsv
----
+
Artifact URLs are pre-authenticated, so no `Authorization` header is needed for the download itself. Using each artifact's path under a dedicated `artifacts/` directory avoids collisions when multiple artifacts share a filename in different directories.
+
CAUTION: Don't name the loop variable `path` in zsh — zsh ties the lowercase `path` array to `$PATH`, so assigning to it overwrites your shell's `PATH` and breaks every command until you open a new shell.
...
----
+
CAUTION: The `201` response includes the raw token value once. Save it immediately. The API does not return the raw value again, and you cannot recover it if you lose it.
