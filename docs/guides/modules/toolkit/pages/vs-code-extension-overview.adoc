...
- **Auto completion**. Auto completion is available with both built-in keys and parameters as well as user-defined variables.
+
.Autocomplete
...
. Each project has a button to trigger the config test run. Check boxes for each workflow and job let you choose which parts of your config to run. Select your options, and select the "play" icon **Run local config on branch <your-branch-name>**. Any local change you make to your `.circleci/config.yml` file will be included in the test run.
...
* Organization level settings can be found under menu:Org[Advanced]. The setting default is **Off**. Toggle the "Trigger pipelines with unversioned config" option to **On** to opt-in. Organization level settings override project settings, and they can be changed only by organization admins.
...
The audit log event `trigger-event.create` includes information to distinguish pipelines that are run with unversioned config, in particular the following properties:
...
- `organization.settings.update`: emitted when organization-level settings for this feature are changed
