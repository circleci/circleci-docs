...
NOTE: **Chunk is currently in beta.** Beta has no extra costs. Chunk uses CircleCI credits and your AI model provider tokens. Chunk tasks will be a paid feature when generally available.
...
|No
|CircleCI provides the API key. You receive 400,000 tokens per day. The token limit resets at 12:00 UTC. This option is the default.
...
* **CircleCI (Powered by Anthropic)**: No API key required. This option is the default.
...
* **Claude Sonnet**. Best balance of performance and speed for code tasks. Claude Sonnet is the recommended default.
...
* Next to a failed pipeline row in the pipeline list, to the left of the ellipsis icon (icon:more[Ellipsis icon]).
+
...
. Navigate to menu:Org[Tasks].
. Identify your desired agent task.
. Select the ellipsis icon (icon:more[Ellipsis icon]) and select **Chunk Environment**.
...
The increase happens because traffic is routed through CircleCI's static IP infrastructure.

[#email-digest]
== Email digest

Chunk sends an email digest at 09:00 UTC every Monday summarizing your team's Chunk activity. The digest includes:

* Completed tasks.
* PRs opened.
* Failed tasks.
* Most recent tasks that resulted in an open PR, ready for your review.

.Chunk tasks weekly digest email
image::guides:ROOT:chunk/chunk-weekly-email-digest.png[Chunk tasks weekly digest email]

Select **Review tasks** in the digest email to open Chunk tasks that resulted in a pull request being opened. These tasks may not still have an open pull request.

To disable the digest:

include::ROOT:partial$app-navigation/steps-to-user-settings.adoc[]
. Select **Notifications** from the sidebar.
. Scroll to the Chunk section.
. Toggle off **Digest**.
...
If you do not see Chunk image:guides:ROOT:icons/chunk.svg[Chunk icon, role="no-border"] in the sidebar, you may not have the **Allow Chunk tasks** option enabled. Navigate to menu:Org[Advanced] and toggle on **Allow Chunk tasks**.

.Org > Advanced > Allow Chunk tasks
