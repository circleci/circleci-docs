...
An approval job can have any name. In the example above the approval job is named `hold`. Do not reuse the name you choose for an approval job to define a job in the main configuration. An approval job only exists as a workflow orchestration device.
...
.Fan-in/fan-out workflow example
