...
|`--sidecar-id <id>`
|Validate commands remotely in the specified cloud sidecar. Get a sidecar ID from `chunk sidecar create` output or `chunk sidecar list`.
|===
--

[#run-validation-in-a-sidecar]
=== Validate in a cloud sidecar

To validate in a CircleCI Cloud environment instead of locally, pass a sidecar ID:
...
To get a sidecar ID, run `chunk sidecar create` and copy the ID printed when the environment is ready, or run `chunk sidecar list` to retrieve the ID of an existing sidecar. See the <<manage-sidecars,Manage cloud sidecars>> section for details.

[#manage-sidecars]
== Manage cloud sidecars

Cloud sidecars are CircleCI Cloud environments where chunk validates your changes remotely. This lets you validate changes against the same environment your CI pipeline uses, without running them locally.
...
chunk auto-detects your tech stack, generates a Dockerfile, and provisions a cloud environment on CircleCI. The command prints the sidecar ID when the environment is ready. Use this ID with `chunk validate --sidecar-id` to run validations in that environment.
...
Lists all your sidecars and their IDs. Use this to retrieve a sidecar ID if you need it after creation.
