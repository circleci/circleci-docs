...
This section outlines the abilities that the CircleCI MCP server currently supports. Several tools are described below along with usage examples. For full infomation on each tool see the link:https://github.com/CircleCI-Public/mcp-server-circleci/blob/main/README.md[README] file in the tool's repository.
