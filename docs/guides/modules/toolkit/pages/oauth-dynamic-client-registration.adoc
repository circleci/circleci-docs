...
* Your tool needs a way to open a browser and briefly listen for an HTTP redirect on localhost. The Python listener in <<step-2-start-a-local-redirect-listener,Step 2>> provides a minimal reference implementation.
...
PKCE replaces the client secret. The listener generates three values automatically, but the following table describes what each one is and why it is needed — useful if you are implementing the flow in your own language rather than using the listener directly.
...
Construct the authorization URL using your `client_id` from <<step-1-register-your-client,Step 1>> and the PKCE values from <<step-2-start-a-local-redirect-listener,Step 2>>, then open it in a browser.
