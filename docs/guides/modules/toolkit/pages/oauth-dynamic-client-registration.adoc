...
This guide is for developers building a tool, CLI, or integration that needs to access the CircleCI API on behalf of a user.

CircleCI supports the OAuth 2.0 authorization code flow with mandatory PKCE (Proof Key for Code Exchange) and Dynamic Client Registration (DCR). Use this flow to let your tool get a personal API token on behalf of a user without requiring a pre-shared client secret or manual token creation in the CircleCI web app.

Using this flow gives you the following benefits:
...
The resulting token is a standard 90-day CircleCI personal API token and works with all existing API v2 endpoints.
...
* `curl` installed. Install with `brew install curl` on macOS.
* `python3` installed. Used for the local redirect listener in Step 2. Install with `brew install python3` on macOS.
* `jq` installed (optional). Used in the examples to format JSON output. Install with `brew install jq` on macOS.
...
IMPORTANT: The `redirect_uri` must be a loopback address (`127.0.0.1`, `::1`, or `localhost`) with a port in the range 1025–65535 and no query parameters. It must also match exactly the `redirect_uri` you use in the authorization URL and token exchange.
...
A successful registration returns a JSON object containing your client_id. The JSON object is retuned with a `201 Created` status code:
...
PKCE replaces the client secret. CircleCI requires the S256 challenge method — plain code challenges are not accepted. The listener generates all three values automatically using S256, but the following table describes what each one is and why it is needed — useful if you are implementing the flow in your own language rather than using the listener directly.
...
Construct the authorization URL using your `client_id` from <<step-1-register-your-client,Step 1>> and the PKCE values from <<step-2-start-a-local-redirect-listener,Step 2>>, then open it in a browser. The `state` parameter is required. Omitting it returns a 400 error.
...
NOTE: The URL contains the following parameters. This is for reference only — use the shell command above to generate the correctly encoded URL.
+
...
. Select the access level to grant the tool. Choose the minimum level your tool requires.
+
[.table-scroll]
--
[cols="1,3", options="header"]
...
--
+
These scopes cannot grant more access than the authorizing user already holds in CircleCI. A user with read-only org permissions who selects Admin, for example, receives a token scoped to their actual permissions, not elevated ones.
+
...
. If you belong to SSO-enabled organizations, an SSO organizations section appears below the access level selector. Select *Authorize* next to any orgs you want to include in the token's scope. SSO org access can also be managed later from the OAuth Clients page in user settings.
