...
This page provides reference documentation for the CircleCI task agent CLI, which is the CLI available in your CircleCI pipelines.
...
The task agent CLI is different from the xref:local-cli.adoc[CircleCI local CLI] tool that you may already know. Understanding the difference between these two CLI interfaces is essential for effective usage during your build processes.
