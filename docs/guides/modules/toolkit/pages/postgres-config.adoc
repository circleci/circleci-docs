...
The `cimg/ruby:3.0-node` image does not have `psql` installed by default,
and uses the `pg` gem for database access.
...
Once the DB is up, install the `mysql` client into the primary container. Then run a command to connect and import the dummy data, presumably found at `sql-data/dummy.sql` at the root of your project. In this case, that dummy data contains an example set of SQL commands:
