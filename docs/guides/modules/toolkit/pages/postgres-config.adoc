...
sure that `psql` is installed in your PATH and has the proper permissions.
This is necessary because the `cimg/ruby:3.0-node` image does not have `psql`
installed by default and uses the `pg` gem for database access.
...
its primary container. It waits until MySQL is up and running before performing
any `run` commands involving the database (DB).

Once the DB is up, install the `mysql` client into the primary container so that you can run a command to connect and import the dummy data. This data is presumably found at `sql-data/dummy.sql` at the root of your project. In this case, that dummy data contains an example set of SQL commands:
...
Refer to the xref:orchestrate:databases.adoc[Configuring Databases] document for a walkthrough of conceptual information about using service images and database testing steps.
