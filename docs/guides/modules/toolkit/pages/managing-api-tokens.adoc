...
*Personal API tokens*:: These tokens are used to interact with the CircleCI APIs (v1 and v2). Tokens created manually through the web app grant full read and write permissions. Tokens created through the xref:oauth-dynamic-client-registration.adoc[OAuth 2.0 Dynamic Client Registration] flow are scoped to the access level (Read, Write, or Admin) chosen by the user during authorization.
