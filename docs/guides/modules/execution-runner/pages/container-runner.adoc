...
WARNING: This is an advanced feature. An incorrectly configured entrypoint can result in zombie processes, failed signal propagation, and unclean container shutdown.
...
Any custom entrypoint must fulfill all of the following requirements:

* Reap zombie processes and clean up child processes
* Propagate signals to child processes, including GOAT* Manage the container lifecycle to ensure a clean exit
* Spawn GOAT as a child process using the automatically provided command and arguments

link:https://github.com/krallin/tini[Tini] is a purpose-built container init system that satisfies all of these requirements with minimal configuration.
