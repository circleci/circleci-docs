...
By default, xref:execution-managed:using-docker.adoc#using-multiple-docker-images[Service Containers] inherit the same link:https://kubernetes.io/docs/tasks/configure-pod-container/[container configuration] as defined by the primary container. However, this behavior can be overridden using customizable service containers. Using the available overrides allows fine-tuned control over a service's resource usage on a per-image basis.
...
- `cimg/mongo:5` doesn't match any rule, so it uses the `default` rule at the resource class scope, with 0.4 CPU units and 150Mi of memory.
...
CAUTION: Unlike automatic retries on startup, retrying tasks during runtime can be risky: tasks can have arbitrary steps that produce external side effects that are not idempotent or stateless. This includes steps that could impact production environments or databases. Use this feature with care, knowing the risks of rerunning jobs and workflows that may or may not be idempotent.
...
The following sequence shows how unsafe retries work:
...
Refer to the link:https://github.com/CircleCI-Public/container-runner-helm-chart[container runner Helm chart]. You can find a full link:https://github.com/circleci-public/container-runner-helm-chart#values[chart values reference] section in the README.
...
* link:https://gateway-api.sigs.k8s.io/reference/api-types/gateway/[Gateways], Services
** Get
...
Container runner jobs are eligible for xref:optimize:persist-data.adoc#managing-network-and-storage-usage[Runner Network Egress]. These charges follow the existing pricing model for self-hosted runners. They align with CircleCI's network and storage billing roll-out. If there are questions, reach out to your point of contact at CircleCI.
...
NOTE: Overriding the default container entrypoint requires container runner version 3.1.10 or later.

WARNING: Overriding the default container entrypoint is an advanced operation. An incorrectly configured entrypoint can result in zombie processes, failed signal propagation, and unclean container shutdown.

By default, the task agent (GOAT) runs as PID 1 in the primary task container. You can override this by specifying a custom entrypoint in either the CircleCI job configuration or the resource class pod template spec.

Any custom entrypoint must fulfill the following requirements:

* Reap zombie processes and clean up child processes.
* Propagate signals to child processes, including GOAT.
* Manage the container lifecycle to ensure a clean exit.
* Spawn GOAT as a child process using the automatically provided command and arguments.

link:https://github.com/krallin/tini[Tini] is a purpose-built container init system that satisfies these requirements with minimal configuration.

=== CircleCI job configuration

Set a custom entrypoint using the `entrypoint` field on the Docker image in your CircleCI configuration file. Container runner automatically passes the GOAT command and arguments to your entrypoint:
  ...
  test:
    docker:
      - image: krallin/ubuntu-tini
        entrypoint: ["/usr/bin/tini", "--"] # The GOAT command and arguments are automatically appended and passed to `tini`.
    resource_class: <your-resource-class-here>
    steps:
      - run:
          name: Who is PID 1?
...
=== Resource class pod template spec

To apply a custom entrypoint to all jobs using a resource class, configure it in the pod template spec within your Helm `values.yaml`. See <<resource-class-configuration-custom-pod>> for details on pod template spec configuration. The same entrypoint requirements apply.
