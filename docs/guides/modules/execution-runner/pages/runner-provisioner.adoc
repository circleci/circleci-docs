...
If the file is absent, VMs cannot be scheduled on that node regardless of how KubeVirt is configured. On cloud providers, nested virtualization is typically disabled by default. Explicitly enable it on the node pool or instance group before the nodes are created — it cannot be patched onto existing nodes.
...
On EKS, KVM support requires bare metal instances — regular EC2 instances, even Nitro-based ones, do not expose `/dev/kvm` to pods. Use a `.metal` instance type (for example, `m5.metal` or `c5.metal`). Nested virtualization support for EC2 instances has an active feature request, but the issue remains open.
...
Without `idleTimeout`, a pre-warmed VM that never receives a job waits indefinitely. Setting `idleTimeout` (for example, `"10m"`) causes VMs to shut down after that period of inactivity. Doing so is useful for:
...
* *RBAC misconfiguration*: The `ServiceAccount` used by the provisioner may lack permission to create or update `VirtualMachinePool` resources. Check events on the provisioner pod.
