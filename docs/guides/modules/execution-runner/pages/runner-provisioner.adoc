...
If the file is absent, VMs cannot be scheduled on that node regardless of how KubeVirt is configured. On cloud providers, nested virtualization is typically disabled by default. Explicitly enable it on the node pool or instance group before creating the nodes — it cannot be patched onto existing nodes.
...
On EKS, KVM support requires bare metal instances — regular EC2 instances, even Nitro-based ones, do not expose `/dev/kvm` to pods. Use a `.metal` instance type (for example, `m5.metal` or `c5.metal`). An open feature request tracks nested virtualization support on EC2 instances, but it remains unresolved.
...
* *RBAC misconfiguration*: The provisioner's `ServiceAccount` may lack permission to create or update `VirtualMachinePool` resources. Check events on the provisioner pod.
