...
. If the agent has been installed as a `systemd` service, run the following commands to stop and uninstall the service unit, and delete the service file. The new agent installs a new service unit:
+
```shell
