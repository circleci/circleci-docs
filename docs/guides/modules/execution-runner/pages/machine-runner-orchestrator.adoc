...
Set `min-nodes`/`num-nodes` (and the equivalent EKS/AKS fields) only as high as needed to host `minReplicas` VMs at your chosen VM size, and raise `max-nodes` to cover `maxReplicas`.
