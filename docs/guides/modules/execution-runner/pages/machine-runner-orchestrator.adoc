...
* On CircleCI Cloud, chart `1.0.0` polls task counts with that resource class token. `provisioner.circleToken` (a personal API token) is optional. Keep it only when you need the legacy `/tasks` endpoints. See the xref:toolkit:managing-api-tokens.adoc[Managing API Tokens] page.
...
NOTE: The node counts in the following examples (`min`/`desired`, and `max`) are illustrative only. Size the node pool to your actual workload.

Node pool sizing depends on two things: how many VMs you need running at once, and how many VMs fit on a single node.

* *Number of VMs*: driven by `scaling.minReplicas` and `scaling.maxReplicas` for the resource class (see <<scaling-behavior,Scaling Behavior>>). `minReplicas` is the pre-warmed floor that must always be schedulable, so set the node pool's `min`/`desired` count to at least enough nodes to host that many VMs. Set the pool's `max` to enough nodes to host `maxReplicas` VMs, since the autoscaler will not exceed it.
* *VMs per node*: determined by the VM `spec.domain.resources.requests` (see <<vm-specification-notes,VM Specification Notes>>) against the node's allocatable CPU and memory. A node only fits as many VMs as its allocatable resources allow; oversized VM requests relative to node capacity waste the remainder.

Two separate fields control this at node-pool creation: the autoscaler minimum (`--min-nodes`, `--min-count`, `minSize`) and the initial node count (`--num-nodes`, `--node-count`, `desiredCapacity`). Size both to host `minReplicas` VMs at your chosen VM size, and raise the autoscaler maximum (`--max-nodes`, `--max-count`, `maxSize`) to cover `maxReplicas`.
...
| `image.tag`
| Chart `appVersion` (currently `1.0.0`)
