...
:page-description: Scale self-hosted machine and container runners to match job demand.
:experimental:
...
* <<machine-runner-3-configuration-reference#runner-mode,Runner Mode>>
** Choosing `single-task` mode will cause machine runner to shut down after a single task. `single-task` mode suits completely ephemeral compute, because the resources can be automatically recycled upon machine runner exit.
** Choosing `continuous` mode will cause the machine runner to poll for new tasks after completing a task. Your scaling solution will need to monitor the task workload and actively shut down unused machine runners.
* <<machine-runner-3-configuration-reference#runner-idle-timeout,Runner Idle Timeout>>
