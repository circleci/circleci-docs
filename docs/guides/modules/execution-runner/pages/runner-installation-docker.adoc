...
This page describes how to install CircleCI's machine runner with the Docker executor on server. If you are looking to set up self-hosted runners in a private Kubernetes cluster, visit the xref:container-runner.adoc#[Container runner] page.

[#new-recommended-method-container-runner]
== Supported method for Docker installation (container runner)

The xref:container-runner.adoc#[container runner] is the recommended approach for running containerized jobs on self-hosted runners. Container runner offers the ability to seamlessly define, publish, and use custom Docker images during job execution, as well as the ability to easily manage dependencies or libraries through custom Docker images instead of enumerating dependencies as part of `steps` in the `.circleci/config.yml` file.
