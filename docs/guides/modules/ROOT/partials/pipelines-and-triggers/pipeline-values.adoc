[#pipeline-and-project-identity]
=== Pipeline and project information

These values provide core identifiers for the pipeline itself and the project it belongs to. They are available across all pipeline types and integration methods.

[.table-scroll]
--
[.datatable.no-sort-col-3,cols="6*", options="header"]
|===
| Variable
| Pipeline type
| Type
| Value
| Cloud
| Server

| `pipeline.id`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| A link:https://en.wikipedia.org/wiki/Universally_unique_identifier[globally unique identifier] for the pipeline.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.number`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| Integer
| A project-scoped integer identifier for the pipeline.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.definition.id`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin# +
_(except schedule workflows)_
| String
| The unique ID of the pipeline definition.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.project.git_url`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The URL where the current project is hosted. For example, `https://github.com/circleci/circleci-docs`.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.project.type`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The lower-case name of the VCS provider, for example, `github`, `bitbucket`.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.project.slug`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The slug of the project the pipeline definition belongs to.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.name`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#Cursor Origin# +
_(except schedule workflows)_
| String
| The name of the pipeline definition.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.integration_source`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The pipeline type, determined by the VCS integration used to check out code. Possible values: `github_app`, `github_server`, `github`, `bitbucket`, `bitbucket_dc`, `gitlab`.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

|===
--

[#checkout-information]
=== Checkout information

Values prefixed with `+pipeline.git.*+` describe the git state of the code CircleCI checks out for the pipeline. These values reflect the code CircleCI checks out and builds. For GitHub App and GitHub App Server pipelines, this may differ from the following:

* The commit or branch that triggered the event.
* The commit or branch used to fetch the configuration file.

[.table-scroll]
--
[.datatable.no-sort-col-3,cols="6*", options="header"]
|===
| Variable
| Pipeline type
| Type
| Value
| Cloud
| Server

| `pipeline.git.tag`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab#
| String
| The name of the git tag that CircleCI checks out. Empty if the pipeline checks out from a branch.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.git.branch`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The name of the git branch that CircleCI checks out. Empty if the pipeline checks out from a tag.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.git.branch.is_default`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| Boolean
| Whether CircleCI checks out the default branch.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**# (>= v4.7)

| `pipeline.git.revision`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The long (40-character) git SHA of the commit that CircleCI checks out.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.git.repo_url`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#Cursor Origin#
| String
| The URL of the repository.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.git.repo_owner`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Cursor Origin#
| String
| The VCS username of the repository owner.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.git.repo_name`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Cursor Origin#
| String
| The name of the repository, without the owner prefix.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.git.repo_id`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#Cursor Origin#
| String
| The unique identifier of the repository.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.git.commit.url`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin# +
_(except schedule workflows)_
| String
| The URL of the checkout commit.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.git.commit.subject`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The subject line of the checkout commit message.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.git.commit.body`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The body of the checkout commit message.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.git.commit.author_login`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The VCS username of the author of the checkout commit. Not available for Bitbucket Cloud because Bitbucket has link:https://developer.atlassian.com/cloud/bitbucket/bitbucket-api-changes-gdpr/[deprecated usernames].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.git.base_revision`
d| [.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud#
| String
| The long (40-character) git SHA of the build that ran before the current build. **Note:** While in most cases `pipeline.git.base_revision` will be the SHA of the commit that ran before your currently running pipeline, there are some caveats. When the build runs first for a branch, the variable will not appear. In addition, if you trigger the build via the API, the variable will not appear.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

|===
--

[#configuration-source]
=== Configuration source information

Values prefixed with `+pipeline.config.*+` describe where CircleCI fetched the configuration file from. These values currently only apply to GitHub App and GitHub App Server pipelines where the configuration file can reside in a separate repository from the one CircleCI builds. For all other pipeline types, the configuration file always lives in the same repository as the code CircleCI checks out, so you can use the corresponding `+pipeline.git.*+` values instead.

[.table-scroll]
--
[.datatable.no-sort-col-3,cols="6*", options="header"]
|===
| Variable
| Pipeline type
| Type
| Value
| Cloud
| Server

| `pipeline.config.repository.name`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#Cursor Origin#
| String
| The name of the repository containing the CircleCI configuration file, in the format `owner/repo-name`.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.config.repository.url`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#Cursor Origin#
| String
| The URL of the repository containing the CircleCI configuration file.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.config.file_path`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#Cursor Origin#
| String
| The file path to the CircleCI configuration file used by the pipeline.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.config.ref`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#Cursor Origin#
| String
| The full Git ref used to fetch the CircleCI configuration file, for example `refs/heads/main` or `refs/tags/v1.0`.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.config.sha`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#Cursor Origin#
| String
| The long (40-character) git SHA of the commit used to fetch the CircleCI configuration file.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

|===
--

[#trigger-and-event-information]
=== Trigger and event information

These values describe how and why the pipeline was triggered, including the event source, timing, and any event-specific data passed by the VCS provider.

[WARNING]
====
All `+pipeline.trigger_parameters.circleci.*+` and `+pipeline.trigger_parameters.github_app.*+` values are deprecated and their presence is not guaranteed after **2026-09-21**. Use the replacement values listed in the table below.
====

[.table-scroll]
--
[.datatable.no-sort-col-3,cols="6*", options="header"]
|===
| Variable
| Pipeline type
| Type
| Value
| Cloud
| Server

| `pipeline.trigger.received_at`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| The timestamp of when the pipeline was triggered, in ISO 8601 format.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.trigger.id`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#Cursor Origin#
| String
| The unique ID of the trigger that initiated the pipeline.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger.type`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#Cursor Origin#
| String
| The type of trigger that initiated the pipeline. Possible values: `github_app`, `github_server`, `github_oauth`, `bitbucket_cloud`, `bitbucket_dc`, `schedule`, `webhook`, `api`.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_source` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Bitbucket Data Center# +
[.nowrap]#GitLab# +
[.nowrap]#Cursor Origin#
| String
| Use `pipeline.trigger.type` instead.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.schedule.name`
d| [.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud#
| String
| The name of the schedule trigger if the pipeline was triggered via schedule. Empty if the pipeline was not triggered via schedule.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.schedule.id`
d| [.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud#
| String
| The unique ID of the schedule if the pipeline was triggered via schedule. Empty if the pipeline was not triggered via schedule.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#

| `pipeline.event.name`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Cursor Origin#
| String
| The name of the event that triggered the pipeline. Possible values: `custom_webhook`, `api`, `push`, `pull_request`, `schedule`.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.action`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth# +
[.nowrap]#Bitbucket Cloud# +
[.nowrap]#Cursor Origin#
| String
| The action associated with the event. This value is the same as `pipeline.event.name` except for events that have an `action` subtype: link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub `pull_request`] events and link:https://support.atlassian.com/bitbucket-cloud/docs/event-payloads/#Pull-request-events[Bitbucket Cloud `pullrequest:*`] events. Possible values: `custom_webhook`, `api`, `push`, `schedule`, `opened`, `synchronize`, `reopened`, `closed`, `ready_for_review`, `labeled`, `created`, `updated`, `fulfilled`, `rejected`. Read more about xref:guides:orchestrate:github-trigger-event-options.adoc#supported-trigger-options[GitHub Trigger Event Options].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.after`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth#
| String
| The SHA of the most recent commit on the branch after the push. Corresponds to the `after` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#push[GitHub push event payload]. Only populated for push events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.head_commit.url`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth#
| String
| The URL of the most recent commit on the branch. Corresponds to the `head_commit.url` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#push[GitHub push event payload]. Only populated for push events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.ref`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth#
| String
| The full Git ref that was pushed. Corresponds to the `ref` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#push[GitHub push event payload]. Only populated for push events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.context.github.pr_url`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The URL of the associated GitHub pull request, if one exists. If there are multiple associated pull requests, one URL is randomly chosen. This value matches environment variable `CIRCLE_PULL_REQUEST`.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.pull_request.base.ref`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The name of the base (or target) branch of the pull request. Corresponds to the `pull_request.base.ref` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.pull_request.base.sha`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The SHA of the base branch of the pull request. Corresponds to the `pull_request.base.sha` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.pull_request.head.ref`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The name of the head branch of the pull request. Corresponds to the `pull_request.head.ref` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.pull_request.head.sha`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The SHA of the head branch of the pull request. Corresponds to the `pull_request.head.sha` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.pull_request.draft`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| Boolean
| A boolean value indicating whether the pull request is a draft. Corresponds to the `pull_request.draft` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.pull_request.title`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The title of the pull request. Corresponds to the `pull_request.title` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.pull_request.number`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| Integer
| The numeric identifier of the pull request. Corresponds to the `pull_request.number` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.pull_request.merged`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| Boolean
| A boolean value indicating whether the pull request has been merged. Corresponds to the `pull_request.merged` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.pull_request.url`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The URL of the pull request. Corresponds to the `pull_request.url` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.label.name`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The name of the label applied to the pull request. Corresponds to the `label.name` field on the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads#pull_request[GitHub pull request event payload]. Only populated for pull request events with the `labeled` action.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.comment.body`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The body of the comment that triggered the pipeline, with leading and trailing whitespace removed. Corresponds to the `comment.body` field on `issue_comment` and `pull_request_review_comment` GitHub event payloads. Only populated for pipelines triggered by a PR comment.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.sender.type`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth#
| String
| The type of GitHub user that triggered the event. Corresponds to the `sender.type` field on GitHub event payloads. Possible values are `User`, `Organization`, and `Bot`.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.sender.avatar_url`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth#
| String
| The avatar URL of the GitHub user who triggered the event. Corresponds to the `sender.avatar_url` field on GitHub event payloads.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.sender.login`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth#
| String
| The VCS username of the GitHub user who triggered the event. Corresponds to the `sender.login` field on GitHub event payloads.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.repository.name`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth#
| String
| The name of the repository that triggered the event, without the owner prefix. Corresponds to the `repository.name` field on repository-related GitHub event payloads.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.repository.owner.login`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth#
| String
| The VCS username of the repository owner. Corresponds to the `repository.owner.login` field on repository-related GitHub event payloads.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.event.github.repository.url`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitHub OAuth#
| String
| The URL of the repository that triggered the event. Corresponds to the `repository.url` field on repository-related GitHub event payloads.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.webhook.body`
d| [.nowrap]#Custom webhook#
| String
| The body of the payload that was sent with a xref:guides:orchestrate:triggers-overview.adoc#trigger-a-pipeline-from-a-custom-webhook[Custom Webhook].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.circleci.trigger_type` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitLab#
| String
| Use `pipeline.trigger.type` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.circleci.event_time` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitLab#
| String
| Use `pipeline.trigger.received_at` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.circleci.event_type` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server# +
[.nowrap]#GitLab#
| String
| Use `pipeline.event.name` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.ref` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|For push events, use `pipeline.event.github.ref`. For pull request events, use `pipeline.event.github.pull_request.head.ref`.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.checkout_sha` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.git.revision` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.user_id` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|No direct replacement is available. If this value is critical to your workflows, contact CircleCI Support.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.user_name` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.event.github.sender.login` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.user_username` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.event.github.sender.login` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.user_avatar` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.event.github.sender.avatar_url` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.repo_name` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.event.github.repository.name` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.repo_url` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.event.github.repository.url` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.web_url` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.event.github.repository.url` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.commit_sha` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.event.github.after` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.commit_title` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.git.commit.subject` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.commit_message` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.git.commit.body` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.commit_timestamp` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|Use `pipeline.trigger.received_at` instead.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.commit_author_name` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|For the user who triggered the push event, use `pipeline.event.github.sender.*`. For the checkout commit author, use `pipeline.git.commit.author_login`.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.github_app.branch` +
icon:warning[Warning icon] *Deprecated* - support ends on 2026-08-01.
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
|
|For push events, use `pipeline.event.github.ref`. For pull request events, use `pipeline.event.github.pull_request.head.ref`.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.type`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.ref`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.checkout_sha`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.user_id`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.user_name`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.user_username`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.user_avatar`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.repo_name`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.repo_url`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.web_url`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.commit_sha`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.commit_title`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.commit_message`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.commit_timestamp`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.commit_author_name`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.commit_author_email`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.branch`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.default_branch`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.x_gitlab_event_id`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.trigger_parameters.gitlab.is_fork_merge_request`
d| [.nowrap]#GitLab#
|
| See GitLab documentation for link:https://docs.gitlab.com/ee/user/project/integrations/webhooks.html[webhooks] and link:https://docs.gitlab.com/ee/user/project/integrations/webhook_events.html[webhook events].
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

|===
--

[#deploy-information]
=== Deploy information

Values prefixed with `+pipeline.deploy.*+` are only available for deployment pipelines. See the xref:guides:deploy:set-up-rollbacks.adoc[Set up a Rollback Pipeline] and xref:guides:deploy:set-up-deploys.adoc[Set up a Deploy Pipeline] guides for more information.

[.table-scroll]
--
[.datatable.no-sort-col-3,cols="2,2,1,5,1,1", options="header"]
|===
| Variable
| Pipeline type
| Type
| Value
| Cloud
| Server

| `pipeline.deploy.component_name`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The name of the component that will be rolled back or deployed.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.deploy.environment_name`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The name of the environment in which the rollback or deploy will be performed.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.deploy.target_version`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The version of the component that needs to be rolled back to or deployed.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.deploy.current_version`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The current version of the component that CircleCI will roll back or deploy. Use this to verify that the component status matches expectations.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.deploy.namespace`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The namespace of the environment that was deployed to.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

| `pipeline.deploy.reason`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub App Server#
| String
| The reason for the deployment or rollback.
| [.circle-green]#**Yes**#
| [.circle-red]#**No**#

|===
--
