...
| `pipeline.id`
d| [.nowrap]#GitHub App# +
[.nowrap]#GitHub Enterprise Server# +
[.nowrap]#GitHub OAuth# +
...
Values prefixed with `+pipeline.git.*+` describe the git state of the code CircleCI checks out for the pipeline. These values reflect the code CircleCI checks out and builds. For GitHub App and GitHub Enterprise Server pipelines, this may differ from both the commit or branch that triggered the event and the commit or branch used to fetch the configuration file.
...
[.nowrap]#GitLab#
| String
| The name of the git tag being checked out. Empty if checking out from a branch.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#
...
[.nowrap]#GitLab#
| String
| The name of the git branch being checked out. Empty if checking out from a tag.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#
...
[.nowrap]#GitLab#
| Boolean
| Whether the branch being checked out is the default branch.
| [.circle-green]#**Yes**#
...
[.nowrap]#GitLab#
| String
| The long (40-character) git SHA of the commit being checked out.
| [.circle-green]#**Yes**#
| [.circle-green]#**Yes**#
...
Values prefixed with `+pipeline.config.*+` describe where the CircleCI configuration file was fetched from. These values are currently only available for GitHub App and GitHub Enterprise Server pipelines where the configuration file can be stored in a separate repository from the one being built. For all other pipeline types, the configuration file always lives in the same repository as the code being checked out so you can use the corresponding `+pipeline.git.*+` values instead.
