...
Values prefixed with `+pipeline.git.*+` describe the git state of the code CircleCI checks out for the pipeline. These values reflect the code CircleCI checks out and builds. For GitHub App and GitHub App Server pipelines, this may differ from both the commit or branch that triggered the event and the commit or branch used to fetch the configuration file.
