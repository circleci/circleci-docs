...
Values prefixed with `pipeline.git.*` describe the git state of the code CircleCI checks out for the pipeline. These values reflect the code CircleCI checks out and builds, which may differ from the commit or branch that triggered the pipeline event.
