...
Values prefixed with `pipeline.git.*` describe the git state of the code CircleCI checks out for the pipeline. These values reflect the code CircleCI checks out and builds, which may differ from both the commit or branch that triggered the pipeline event and the commit or branch from which the configuration file was fetched.
...
Values prefixed with `pipeline.config.*` describe where the CircleCI configuration file was fetched from. These values are only available for GitHub App pipelines using standalone configuration, where the config file is stored in a separate repository from the one being built.
...
These values describe how and why the pipeline was triggered. They are organized into three groups:

* *`pipeline.trigger.*`, `pipeline.trigger_source`, `pipeline.schedule.*`*: High-level trigger metadata available across pipeline types.
* *`pipeline.trigger_parameters.*`*: Raw webhook event parameters passed directly from the VCS provider. The prefix reflects the source — for example, `pipeline.trigger_parameters.github_app.*` for GitHub App and `pipeline.trigger_parameters.gitlab.*` for GitLab.
* *`pipeline.event.*`*: Normalized event values available for GitHub App pipelines only.
