...
* Check that the Deployment/Rollout is annotated with the required labels. More information is available in the xref:guides:deploy:configure-your-kubernetes-components.adoc[Configure Your Kubernetes Components] page. If the required labels are missing, add them to resolve the problem.
...
In either case, create a new token and reinstall the Release Agent with the new value to resolve the issue.
