...
* If you are using a Deployment, check whether the release agent restarted before all pods for the deployment could become ready. The restart behavior is a known limitation that will be addressed in future updates of the release agent. Restarting a release agent while a release is ongoing will cause the release agent to lose track of the release status and fail to update the CircleCI services accordingly.

[#no-releases]
=== Why are no new releases showing up, or why are component versions not being updated?
...
* Check whether tokens are being shared between multiple release environments. Sharing tokens between release environments is **not** supported. Check this by following these steps:
...
=== Why does `restore version` time out when using Helm?
...
Check if the error message is “invalid or missing project ID“. In this case the component is missing a valid xref:guides:deploy:configure-your-kubernetes-components.adoc#configure-release-management[Project ID].

[#old-releases-missing]
=== Why is my release or deployment history missing for older releases?

CircleCI automatically deletes deployment and release records once they exceed your plan's data retention period (Free: 90 days, Performance: 180 days, Scale: 2 years). The same retention policy applies to your pipeline, workflow, and job data. For more information, see the link:https://support.circleci.com/hc/en-us/articles/5645222646939-Cloud-Data-Retention-Policy-and-Settings[Cloud Data Retention Policy and Settings] support article.

Data retention rules on affect historical deployment and release records. The currently live version of any component is not affected.
