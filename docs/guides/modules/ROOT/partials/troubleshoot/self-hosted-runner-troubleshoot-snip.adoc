...
If the node is full, it will have a `node.kubernetes.io/disk-pressure` taint, which will prevent new task pods from being scheduled. If all valid nodes for the pod have the same taint, or other conditions that prevent scheduling, the task pod will wait in a pending state. It will remain there until an untainted valid node becomes available. This will show the job as stuck in the **Preparing Environment** step in the UI.
...
NOTE: Entrypoints must be commands that run forever without failing. If the entrypoint fails or terminates in the middle of a build, the build will also terminate. If you need to access logs or build status, consider using a background step instead of an entrypoint.
...
could not start task containers: pod failed to start: :
```
Correct the underlying architecture used for nodes with jobs to match the architecture for images being used by jobs.

[#bad-task-pod-configuration]
=== Bad task pod configuration

If the task pod for a resource class is incorrectly configured, the task will fail once claimed. In the UI the error will be in a `Runner Instance Failure` step with a message resembling:
...
If you see the following message on the job’s page, or if there is no content in the task lifecycle step (as shown), consider the potential causes described below:

----
Oops, there was an issue with your infrastructure. Verify your self-hosted runner infrastructure is operating and try re-running the job. If the issue persist, see our troubleshooting guide
----
...
- **Pod restart:** Check for any container agent pod restarts around the time the workflow ran. If the pod was restarted around that time it would have resulted in the job not being processed. In such a case, we recommend rerunning the job once again.
      ...
      spec:
        containers:
          - livenessProbe:
              ...
              failureThreshold: 5
```
If the liveness probe fails or the task pod terminates, a link:https://kubernetes.io/docs/concepts/containers/container-lifecycle-hooks/#container-hooks[pre-stop hook] attempts to kill any existing task agent by its provided PID. The hook ensures there are no orphaned task agent processes. However, if the PID on file does not map to an existing process, this will not throw an error and will instead log `PreStop hook: task agent appears never started or already stopped`.
...
NOTE: When `KUBE_DISABLE_POD_DELETION_ON_TASK_CLEANUP` is set to true, then task pods may dangle until they are manually cleaned or until garbage collection deletes these pods. By default, garbage collection removes resources after they have lived for 5 hours and 5 minutes. You may tweak these settings in your link:https://github.com/CircleCI-Public/container-runner-helm-chart/blob/main/values.yaml#L161-L175[container-agent `values.yaml`].

[#inter-container-communication]
=== Inter-container communication fails

Unlike the link:https://circleci.com/docs/reference/configuration-reference/#docker[Docker executor], container runner uses `localhost` for inter-container communication rather than container host names. See xref:guides:execution-runner:runner-faqs.adoc#how-do-containers-communicate-on-container-runner[How Do Containers Within a Job Communicate With Each Other?] for details.

If your job references a service container by name (for example, `postgres` or `redis`), update those connections to use `localhost` instead.
...
=== I installed my first self-hosted runner on macOS and the job is stuck in "Preparing Environment", but there are no errors, what do I do?
...
For example, with a concurrency limit of 20, submitting 20 jobs to resource class `X` fills the available concurrency. Jobs wait to run one at a time.
