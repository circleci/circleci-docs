...
In order to install self-hosted runners, you will need to create a namespace and resource class token. To create resource classes and tokens, you need to be an organization admin in the VCS provider. You can read about namespaces and resource classes on the xref:guides:execution-runner:runner-concepts.adoc#namespaces-and-resource-classes[Concepts] page.
