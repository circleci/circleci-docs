...
WARNING: Secrets masking will only prevent values from appearing in your job output. Invoking a bash shell with the `-x` or `-o xtrace` options may inadvertently log unmasked secrets (refer to xref:using-shell-scripts#[Using shell scripts]). If your secrets appear elsewhere, such as test results or artifacts, they will not be masked. Additionally, values are still accessible to users xref:ssh-access-jobs#[debugging builds with SSH].
