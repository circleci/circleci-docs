...
NOTE: For Datadog, the engine also accepts the monitor tags `app:<name>` and `service:<name>` as aliases for `component_name`, and `environment:<name>` as an alias for `env`. Prefer the canonical names.
