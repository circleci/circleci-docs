...
In general, CircleCI recommends jobs rely on as little state as possible to improve their reproducibility. An effective approach is to put cleanup steps at the start of a job. This ensures they run regardless of what happened to a previous job.
...
On self-hosted runners, `circleci-agent` is used for all commands in which you may use either `circleci-agent` or `circleci` on CircleCI Cloud (such as test splitting and step halt commands). Note, `circleci` is not to be confused with the xref:guides:toolkit:local-cli.adoc[Local CircleCI CLI], and is an alias of `circleci-agent`.

To use the local CircleCI CLI in your self-hosted runner jobs, install it via a job step. The CLI can proxy test commands to `circleci-agent`. Install it as a <<#how-do-i-install-dependencies-needed-for-my-jobs,dependency>> on your machine for machine runner, or include it in a Docker image for container runner.
...
No, container runner is meant to complement machine runners. CircleCI users can choose their execution environment (Container vs. Machine), just as on CircleCI’s cloud platform.
...
Like a machine runner, a container runner allows users to run arbitrary code in the hosted infrastructure. A bad actor could use this to gain knowledge of internal systems. Ensure you are following all best practices for security to mitigate this risk.
...
If you would like to run your CI job within a container, but do not want to use Kubernetes, you can use a xref:guides:execution-runner:install-machine-runner-3-on-docker.adoc[Machine Runner] with Docker installed.

[#how-do-containers-communicate-on-container-runner]
==== How do containers within a job communicate with each other?

On the link:https://circleci.com/docs/reference/configuration-reference/#docker[Docker executor], containers in the same task can reference each other by container name as a hostname. Container runner runs tasks as Kubernetes pods, where all containers in a job share a network namespace and communicate over `localhost` rather than by hostname. See the link:https://kubernetes.io/docs/concepts/services-networking/#the-kubernetes-network-model[Kubernetes network model] documentation for more details.
...
Note that you must check for greater than two processes. The `grep` process counts as one, and the `circleci-launch-agent` process counts as a separate process.
