...
=== Which orb type do I author?

The type of orb to author depends on what you need it for and how you want to share it.
...
Create a parameter for the environment variable name, even if it is a statically named environment variable the user _must not_ change. Then, assign it the correct default value. In the parameter description let the user know if this value must not be changed. Either way, consider instructing the user on how they can obtain their API key.
...
Bash is the preferred language as it is most commonly available among all available executors. Write Bash directly using the native xref:reference:ROOT:configuration-reference.adoc#run[Run] command. The default shell on macOS and Linux will be Bash.
...
This option is strongly discouraged wherever possible. Sometimes it may be necessary to fetch a remote binary file such as a CLI tool. Fetch these binaries from a package manager or host them in a VCS such as GitHub releases wherever possible. For example, installing Homebrew as a part of the link:https://circleci.com/developer/orbs/orb/circleci/aws-serverless#commands-install[AWS Serverless orb].
...
* Automatically checkout the user's code.
* Install a CLI.
