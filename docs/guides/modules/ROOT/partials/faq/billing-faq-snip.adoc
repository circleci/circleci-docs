...
If you notice that your account is receiving repeated refills, review your credit usage by logging in to the CircleCI web app, then select menu:Plan[Plan Usage]. In most cases, increasing your credit package will minimize repeat refills. You can manage your plan by clicking `Plan Overview`.
...
When your credit balance runs low and you have credit auto-refill (CAR) enabled, CAR provides additional credits to keep your builds running until the next scheduled allocation arrives. CAR is billed separately from your order's credit allocation schedule.
...
You can view your credit allocation interval schedule and settings by logging in to the CircleCI web app, then select menu:Plan[Plan Overview].
...
You will still have access to any balance credits previously purchased, and the 12 month expiry date will remain.
...
Storage usage is calculated once, at the moment an artifact, workspace, or cache is saved. Storage usage is not accrued day by day while it remains in storage. CircleCI multiplies the object's size by its configured retention period to get its total GB-days. This amount is added to your storage usage for the billing period in which it was saved.
...
- Approving xref:guides:orchestrate:workflows.adoc#holding-a-workflow-for-a-manual-approval[On-Hold Jobs] (approver will be considered the actor of all downstream jobs).
...
=== Will I be charged if my job is "Queued" or "Preparing"?
