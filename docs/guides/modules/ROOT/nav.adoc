...
** xref:orchestrate:use-a-github-actions-workflow-file-as-your-pipeline-configuration.adoc[Use a GitHub Actions workflow file as your pipeline configuration]
...
*** xref:orchestrate:cursor-origin-trigger-event-options.adoc[Cursor Origin trigger event options]
...
*** xref:permissions-authentication:pull-an-image-from-gcp-gar-with-oidc.adoc[Pull an image from GCP Artifact Registry with OIDC]
...
*** xref:execution-runner:machine-runner-orchestrator-performance-benchmarks.adoc[Machine Runner Orchestrator performance benchmarks]
...
*** xref:reference:ROOT:runner-api.adoc[Self-hosted runner API]
...
*** xref:deploy:set-up-release-validation-manually.adoc[Set up release validation manually]
*** xref:deploy:release-validation-reference.adoc[Release validation configuration reference]
** Deploys and rollbacks
*** xref:deploy:set-up-rollbacks.adoc[Set up a rollback pipeline]
...
*** xref:deploy:deploy-to-npm-registry.adoc[Publish to npm using OIDC trusted publishing]
*** xref:deploy:deploy-to-npm-registry-with-token.adoc[Publish to npm with an authentication token]
...
*** xref:optimize:build-tool-cache.adoc[Build tool cache]
...
** xref:insights:insights-tests.adoc[Test Insights]
** xref:insights:project-usage-dashboard.adoc[Project Usage Dashboard]
** xref:insights:insights-snapshot-badge.adoc[Generate an Insights snapshot badge]
** xref:reference:ROOT:insights-glossary.adoc[Insights glossary]
...
** xref:permissions-authentication:manage-email-addresses.adoc[Manage email addresses]
...
*** xref:security:site-to-site-connectivity.adoc[Site-to-site connectivity]
...
*** xref:execution-managed:ssh-access-jobs.adoc[Debug jobs with SSH rerun]
...
** xref:reference:ROOT:config-policy-reference.adoc[Config policy reference]
** How-to guides
...
* Integration
** xref:integration:add-ssh-key.adoc[Add SSH Keys]
** VCS integration
*** xref:integration:set-up-vcs-connections.adoc[Set up VCS connections]
*** xref:integration:version-control-system-integration-overview.adoc[VCS, pipeline types, and feature support]
*** xref:integration:github-enterprise-server-integration.adoc[GitHub Enterprise Server integration]
*** xref:guides:integration:using-the-circleci-github-app-in-an-oauth-org.adoc[Using the CircleCI GitHub App in an OAuth org]
*** xref:integration:status-updates.adoc[Status updates overview]
...
*** xref:integration:open-telemetry-integration.adoc[OpenTelemetry Integration]
** Cost management
*** xref:plans-pricing:integrate-with-finout-for-cost-management.adoc[Finout Integration]
*** xref:plans-pricing:integrate-with-vantage-for-cost-management.adoc[Vantage Integration]
** Project management
*** xref:integration:jira-plugin.adoc[Connect With Jira]

* Working with agents
** xref:toolkit:circleci-agent-skills.adoc[CircleCI Agent Skills]
** xref:toolkit:diagnose-failed-runs-with-failure-report.adoc[Fix broken builds efficiently with agents]
** MCP
...
** Environment CLI
*** xref:reference:ROOT:environment-cli-usage-guide.adoc[CircleCI environment CLI usage guide]
...
** APIs
*** xref:toolkit:api-intro.adoc[APIs overview]
*** xref:toolkit:api-v3-developers-guide.adoc[API v3 developers guide]
