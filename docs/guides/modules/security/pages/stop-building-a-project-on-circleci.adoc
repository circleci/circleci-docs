...
NOTE: To stop new pipelines without removing your VCS integration (webhook and deploy key), you can enable **Block all new work** in xref:orchestrate:skip-build.adoc#enable-block-new-work-for-a-project[Advanced project settings] instead. In-progress workflows are not cancelled automatically; cancel them from the UI if needed.
