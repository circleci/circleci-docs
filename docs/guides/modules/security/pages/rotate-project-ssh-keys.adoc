...
:page-vcs: github, github-enterprise, gitlab, gitlab-self-hosted, bitbucket
:page-server-min-version: "4.7"
:page-audience: developer
:page-description: How to rotate your project SSH keys.
:experimental:

To revoke usage of a project SSH key, rotate the key pairs by following the steps outlined below. Separate instructions exist for each supported version control system (VCS).

When using project SSH keys, CircleCI holds the private key. The target system (typically a VCS, machine, or another SSH-based system) holds the matching public key via its authorization system. To rotate SSH keys, both the private and public keys need to be removed.
...
This section is only relevant to projects in a `github` type organization:

. Take note of the current key information to rotate, including the fingerprint. You can also click the key name to open the related GitHub page that lists the public key.
. Delete the deploy key by clicking the **X**.
. Add a new deploy key by clicking **Add Deploy Key**.
. Go to GitHub's repository project settings to delete the matching public key.
+
The GitHub URL is typically `\https://github.com/<org>/<project>/settings/keys`, or you may already have the page open if you clicked on the key name in step 1. The keys are named `CircleCI`. Remove any key titled `CircleCI` created before the rotation. The new public SSH key will be automatically added once the old key is deleted.
...
. Go to GitHub's User settings to delete the matching public key.
+
The GitHub URL is typically `https://github.com/settings/keys` or you may already have the page open if you clicked on the key name in step 1. Remove any key titled `CircleCI` created before this rotation. The new public SSH key will be automatically added once the old key is deleted (page may need to be refreshed to view the key).
...
. Delete the deploy key by clicking the **X**.
. Add a new deploy key by clicking **Add Deploy Key**.
. Go to Bitbucket's project admin access-key settings to delete the matching public key.
+
The Bitbucket URL is typically `\https://bitbucket.org/<org>/<project>/admin/access-keys/`. The keys are named `CircleCI`. Since the fingerprint hash is not shown, remove any key titled `CircleCI` created before the rotation. The new public SSH key will be automatically added once the old key is deleted (page may need to be refreshed to view the key).
...
. Take note of the current key information to rotate.
. Delete the user key by clicking the **X**.
. Add a new user key following the xref:permissions-authentication:users-organizations-and-integrations-guide.adoc#create-a-user-key[Create a User Key] instructions.
. Go to Bitbucket's user account settings to delete the matching public key.
+
The Bitbucket URL is typically `\https://bitbucket.org/account/settings/ssh-keys/`. The user names the keys, therefore, CircleCI does not know if the key name contains the string `CircleCI`. Remove any key created before the rotation.
