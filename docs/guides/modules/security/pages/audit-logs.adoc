...
. Grant the role/principal permission to write objects to your bucket (required: `s3:PutObject` on `<your-bucket>/*`). For example:
+
[source,json]
----
...
(Remember to substitute your actual organization ID.)

* **OpenID Client ID (Audience):** Your organization ID (the `<ORG_ID>` value).
