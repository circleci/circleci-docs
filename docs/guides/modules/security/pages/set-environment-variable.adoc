...
IMPORTANT: To access environment variables in the same step where they are set, you **must** run `source "$BASH_ENV"` after writing to `$BASH_ENV`. Without sourcing, the variables will only be available in subsequent steps, not in the current shell.
