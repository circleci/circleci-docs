...
NOTE: Bitbucket repositories do *not* provide an API that allows CircleCI contexts to be restricted, only GitHub OAuth app accounts support security group restrictions. To find out which GitHub account type you have, refer to the xref:integration:github-integration.adoc#[GitHub OAuth integration] page.
...
include::guides:ROOT:partial$shared-sections/secrets-masking.adoc[]
