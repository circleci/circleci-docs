...
To invoke a job that uses a restricted context, every restriction type that is set on the context must pass. Security groups, project restrictions, and expression restrictions combine with logical AND.

[#how-restrictions-combine]
==== How restrictions combine

* *Security groups:* The user who triggered the pipeline must belong to at least one assigned group. The default group `All members` allows any organization member. If you assign more than one named group, membership in any one of those groups is enough (logical OR).
* *Project restrictions:* If you add any project restrictions, the job must run in one of the listed projects (logical OR). If you add none, every project in the organization can use the context.
* *Expression restrictions:* If you add any expression restrictions, every expression must evaluate to `true` (logical AND). If you add none, this check passes.

A context with both a security group and a project restriction is available only when both checks pass. Group membership does not bypass a project allowlist. A listed project does not bypass a security group.

If the user running the workflow does not have access to the context, or an expression evaluates to false, the workflow fails with the `Unauthorized` status.
...
. Select *Add Environment Variables* to add environment variables to the context. Fill out your desired name and value in the dialog box, then select *Add Environment Variables* again to finalize. Use of the environment variables for this context is now limited to members of the security groups.
...
+
--
// Vale merges consecutive AsciiDoc ordered list items into one sentence, because
// the `. ` marker reads as a period, so SentenceLength miscounts these short steps.
pass:[<!-- vale circleci-docs.SentenceLength = NO -->]
. In the https://app.circleci.com/[CircleCI web app], go to menu:Organization Settings[Contexts].
...
pass:[<!-- vale circleci-docs.SentenceLength = YES -->]
--
API::
+
--
...
NOTE: All operators are left associative. In practice, avoid operator chaining for anything other than `and` or `or`. Evaluation may cause type mismatches for other operators (see <<evaluation>>).
...
. Select the *X* icon for the context you want to delete. A confirmation dialog appears.
