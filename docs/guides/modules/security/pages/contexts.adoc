...
If you have existing contexts (or environment variables) and you would like to rename your organization or repository, follow the xref:rename-organizations-and-repositories.adoc[Rename organizations and repositories] guide to make sure you do not lose access to contexts or environment variables in the process.
...
Create a context and then add environment variables to it. You can use the xref:reference:ROOT:configuration-reference.adoc#context[`context` key] in the workflows section of a project's `.circleci/config.yml` file to give any job(s) access to the environment variables associated with the context, as shown in the image below.
