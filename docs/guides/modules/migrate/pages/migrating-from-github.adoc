...
TIP: **Not ready to convert your workflows?** A GitHub App or Cursor Origin pipeline can run an existing GitHub Actions workflow file as it is, without any conversion to CircleCI syntax. A subset of GitHub Actions features is available. See the xref:orchestrate:use-a-github-actions-workflow-file-as-your-pipeline-configuration.adoc[Use a GitHub Actions Workflow File as Your Pipeline Configuration] page.
