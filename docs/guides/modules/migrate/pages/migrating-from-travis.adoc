...
:page-description: An overview of how to migrate from Travis CI to CircleCI.
:experimental:
...
| `language:` +
`os:`
| xref:execution-managed:executor-intro.adoc[`docker`, `machine`, `macos`, `windows`]
| CircleCI does not assume dependencies or commands based on a language. Instead, choose an executor and use `run:` steps as shown below to execute commands you require (for example, install, build, test).
...
| `matrix:`
| xref:reference:ROOT:configuration-reference.adoc#matrix[`matrix`]
| CircleCI also offers matrix syntax under our workflows configuration.

| `stage:`
| xref:reference:ROOT:configuration-reference.adoc#requires[`requires`]
...
group of cache paths to defer to in the declared order. Refer to the xref:optimize:caching.adoc[caching
