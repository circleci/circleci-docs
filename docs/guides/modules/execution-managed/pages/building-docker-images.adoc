...
Using `setup_remote_docker` for a job means that all Docker commands execute locally on the same virtual machine that is used to spin up the primary container.
...
NOTE: Gen2/Gen3 does not currently support the xref:security:ip-ranges.adoc[IP Ranges].
...
xref:circleci-images.adoc[CircleCI Convenience Images] have the Docker CLI pre-installed. If you are using a third-party image that does not have the Docker CLI installed, link:https://docs.docker.com/install/#supported-platforms[you will need to install it] as part of your job.
