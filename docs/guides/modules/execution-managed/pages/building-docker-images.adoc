...
Using `setup_remote_docker` for a job means that all Docker commands execute locally on the same virtual machine that spins up the primary container.

In the CircleCI web app, jobs that run in the remote Docker environment have the **Remote Docker** label, as follows:

.Remote Docker label
image::guides:ROOT:remote-docker-label.png[Remote Docker label]
...
Gen2 resource classes are also available for remote Docker (for example, `arm.medium.gen2`).

NOTE: **Arm gen2 is currently in beta.** Performance and pricing are tentative and may change when this feature becomes generally available. Arm-based remote Docker also does not currently support the xref:security:ip-ranges.adoc[IP Ranges].
...
| Remote Docker (Arm Linux Machine) resource class
| `arm.medium.gen2`
| `arm.medium.gen2` (Arm VM)
| `arm.large.gen2`
| `arm.large.gen2` (Arm VM)
| `arm.xlarge.gen2`
| `arm.xlarge.gen2` (Arm VM)
| `arm.2xlarge.gen2`
| `arm.2xlarge.gen2` (Arm VM)
|===
--

- Full list of resource classes: xref:reference:ROOT:configuration-reference.adoc#resourceclass[Configuration Reference].
...
Below is a breakdown of what is happening during this build's execution:

* All commands execute in the xref:reference:ROOT:glossary.adoc#primary-container[Primary Container]. (line 5).
* Once you call `setup_remote_docker`, all Docker-related commands execute locally. (line 12).
* Enable xref:reference:ROOT:glossary.adoc#docker-layer-caching[Docker Layer Caching] (DLC) to speed up image building. (line 13).
* We use xref:security:set-environment-variable.adoc#set-an-environment-variable-in-a-project[Set an Environment Variable in a Project] to store credentials for Docker Hub. (line 19).

[#install-the-docker-cli]
== Install the Docker CLI

xref:circleci-images.adoc[CircleCI Convenience Images] have the Docker CLI pre-installed. If you are using a third-party image that does not have the Docker CLI installed, link:https://docs.docker.com/install/#supported-platforms[you need to install it] as part of your job.
...
CircleCI supports the tags listed below for remote Docker, as per our xref:remote-docker-images-support-policy.adoc#tagging[Remote Docker Tagging Policy].
...
The above tags resolve to Docker 28.

To use Docker 29, use the following tag:
...
To use Docker 24, use the following tag:
...
When you set `prefer_same_region` to `true`, CircleCI attempts to pull the image from the same region as the VM on a best-effort basis. If the same-region pull fails, it falls back to pulling from the image's original region.
