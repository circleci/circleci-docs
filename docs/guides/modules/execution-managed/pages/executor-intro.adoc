...
CAUTION: *Legacy images with the prefix "CircleCI/" are https://discuss.circleci.com/t/legacy-convenience-image-deprecation/41034[deprecated]* on December 31, 2021. For faster builds, upgrade your projects with link:https://circleci.com/blog/announcing-our-next-generation-convenience-images-smaller-faster-more-deterministic/[next-generation convenience images].
...
Find out more about the Linux VM execution environment on the xref:using-linuxvm.adoc[Using Linux Virtual Machines] page.
...
Find out more about the Windows execution environment on the xref:using-windows.adoc[Using the Windows Execution Environment] page. See link:https://circleci.com/developer/orbs/orb/circleci/windows[the Windows orb page on the developer hub] for the list of options available in the Windows orb.
...
Find out more about the Arm VM execution environment on the xref:using-arm.adoc[Using the Arm VM Execution Environment] page. You can also use xref:using-docker.adoc#arm[Arm on Docker].
...
When using a machine executor, if a port range is hard coded, avoid the range in `/proc/sys/net/ipv4/ip_local_reserved_ports`. Port range 32768 - 60999 is used by the TCP stack to allocate ephemeral ports for connections. Ports in that range may have already been allocated by a previous connection, so collisions are possible.
