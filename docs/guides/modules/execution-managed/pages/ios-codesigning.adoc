...
In your `Matchfile`, the `git_url` should be an *SSH* URL (in the `+git@github.com:...+` format), rather than an *HTTPS* URL. Otherwise you may see authentication errors when you attempt to use match. For example:
