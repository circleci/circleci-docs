...
This document describes how to authenticate with your Docker registry provider to pull images. Authenticated pulls allow access to private Docker images and may grant higher rate limits, depending on your registry provider.

CircleCI has partnered with Docker to ensure that our users can continue to access Docker Hub without rate limits. As of November 1st 2020, with few exceptions, you will not be affected by any rate limits when pulling images from Docker Hub through CircleCI. However, these rate limits may be implemented for CircleCI users in the future. We encourage you and your team to add Docker Hub authentication to your CircleCI configuration. Consider upgrading your Docker Hub plan, as appropriate, to prevent any impact from rate limits in the future.
...
If you have https://docs.docker.com/docker-hub/2fa/[two-factor authentication set up on Docker Hub], you can use https://docs.docker.com/docker-hub/access-tokens/[your personal access token] for the `password` key instead.
For example:
...
Both options are virtually the same. However, the second option enables you to specify the variable name you want for the credentials. This can be useful where you have different AWS credentials for different infrastructures.

For example, your SaaS app runs the speedier tests and deploys to staging infrastructure on every commit. For git tag pushes, we run the complete test suite before deploying to production:
...
workflows:
  main:
    jobs:
      - build:
          filters: pipeline.git.branch or (pipeline.git.tag matches /^\d{4}\.\d+$/) # run the build job on every branch, and on tags matching the pattern
      - deploy:
          requires:
            - build
          filters: pipeline.git.tag matches /^\d{4}\.\d+$/ # only run the deploy job on tags matching the pattern
----
...
CircleCI supports pulling private images from Google Cloud Artifact Registry (GAR) and Google Container Registry (GCR).

The best practice approach is to pull from GAR/GCR using OpenID Connect (OIDC). See the xref:permissions-authentication:pull-an-image-from-gcp-gar-with-oidc.adoc[Pull an Image From GCP Artifact Registry With OIDC] how-to guide for full details.

Some advantages to using OIDC over using GCP service account keys:
...
You can also authenticate using a GCP service account key stored as a CircleCI environment variable. Use it to configure Docker credentials before pulling the image in your job steps.

NOTE: Public images hosted on GAR or GCR do not require authentication configuration. CircleCI pulls them without credentials.
