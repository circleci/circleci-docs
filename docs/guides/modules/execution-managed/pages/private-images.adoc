...
workflows:
  main:
    jobs:
      - build:
          filters: pipeline.git.branch or pipeline.git.tag matches /^\d{4}\.\d+$/ # run the job on all branches and tags that match the pattern
      - deploy:
          requires:
            - build
          filters: pipeline.git.branch or pipeline.git.tag == /^\d{4}\.\d+$/ # run the job on all branches and tags that match the pattern
----
