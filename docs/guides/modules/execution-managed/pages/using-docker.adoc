...
You can use the Docker execution environment to run your xref:orchestrate:jobs-steps.adoc[Jobs] in Docker containers. You access the Docker execution environment using the xref:reference:ROOT:configuration-reference.adoc#docker[Docker executor]. Using Docker increases performance because it builds only what your application requires.

Specify a Docker image in your xref:reference:ROOT:configuration-reference.adoc[`.circleci/config.yml`] file to spin up a container. All steps in your job run in this container.
...
A container is an instance of a specified Docker image. CircleCI refers to the first image listed in your configuration for a job as the _primary_ container image, and this is where all steps in the job run. You can also specify _secondary_ containers to run alongside the primary container for services such as databases.
...
CircleCI supports nearly all public images on Docker Hub and other Docker registries by default when you specify the `docker:` key in your `config.yml` file. If you want to work with private images/registries, refer to xref:private-images.adoc[Using Docker Authenticated Pulls].
...
include::ROOT:partial$execution-resources/docker-arm-gen2-resource-table.adoc[]
...
NOTE: **Arm gen2 for Docker is currently in beta.** The performance figure above comes from benchmarks measured during the beta period, and both performance and pricing are tentative and may change when Arm gen2 becomes generally available.
...
* You distribute your application as a Docker image (requires using xref:building-docker-images.adoc[Remote Docker]).
...
* CircleCI has partnered with Docker to ensure that our users can continue to access Docker Hub without rate limits. Rate limits do not affect you when pulling images from Docker Hub through CircleCI. We encourage you to xref:private-images.adoc[Add Docker Hub Authentication] to your CircleCI configuration and consider upgrading your Docker Hub plan, as appropriate, to prevent any impact from rate limits.
...
When using a multi-container job setup, all containers run in a common network, and every exposed port is available on `localhost`. All containers can communicate with one another. You can also change this hostname using the `name` key. For a full list of options, see the xref:reference:ROOT:configuration-reference.adoc#docker[Configuration Reference].
...
You cannot control caching availability via settings or configuration. Choosing a popular image, such as link:https://circleci.com/developer/images[CircleCI convenience images], increases the chances of hitting cached layers in the "Spin Up Environment" step.
