...
Before we get to setting up the macOS executor, we will need to set up our example application.
...
will be far more complex. This app serves as an introduction to the macOS
...
TIP: If you would like to test running the code in the example configuration file (below) yourself, either fork or duplicate the example application from GitHub. The example configuration file is not guaranteed to work on any/all Xcode projects.
...
jobs: # a basic unit of work in a run
  ...
  build:
    ...
    steps:
      ...
      - store_artifacts: # store this build output. Read more: https://circleci.com/docs/guides/optimize/artifacts/
          path: app.zip
          destination: app
