...
Using more modern compute, and using gen1 CPU performance as a baseline (100%), gen2 instances deliver up to 125% faster multi-threaded CPU performance compared to gen1 equivalents.
...
Using gen1 CPU performance as a baseline (100%), gen3 instances deliver up to 167% faster multi-threaded CPU performance compared to gen1 equivalents. These are CPU benchmark figures; real-world performance gains for your workloads are typically more modest.

On a per-resource-class basis, gen3 can be priced either higher or lower than its gen1 or gen2 equivalent. See our link:https://circleci.com/pricing/price-list/[Pricing Page] for details.
