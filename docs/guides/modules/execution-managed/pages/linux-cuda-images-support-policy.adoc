...
include::guides:ROOT:partial$execution-resources/linux-cuda-deprecation-notice.adoc[]

[#overview]
== Overview

This document outlines the xref:using-gpu.adoc#linux-gpu[CircleCI Linux CUDA image] release, update, and deprecation policy and applies to our Linux CUDA images.
