...
We recommend using the latest stable versions of all required tools. Older versions may contain bugs that cause unexpected failures with CircleCI features.
...
* Install tools from source or official release binaries.
...
You will see how all commands specified in your Dockerfile are executed. If there are any errors they will be displayed and you will need to fix them before continuing. If the build is successful you will have something like this at the end:
...
NOTE: To use an image with the CircleCI xref:using-docker.adoc[Docker Executor] you must have a public repository.
If you want to keep your image private, refer to the xref:private-images.adoc[Using Docker Authenticated Pulls] document for instructions.
...
** `0.0.1` - tag (version) of the image. Always update the tag if you change something in a `Dockerfile` otherwise you may have unpredictable results.
...
After the image has been pushed, it is available for use in your `.circleci/config.yml`:
...
The Ruby image comes with git pre-installed so there is no reason to reinstall it. Next, add `sudo`, `python2.7`, and `postgresql-9.5` to the installation list. Then, install yarn with npm.
