...
In 2020, CircleCI introduced the next generation (next-gen) of convenience images. These new images are designed to replace the legacy convenience images released during the link:https://circleci.com/blog/say-hello-to-circleci-2-0/[announcement of CircleCI 2.0]. The next-gen CircleCI convenience images are designed from the ground up for a CI/CD environment. They are designed to be faster, more efficient, and most importantly, more reliable. You can learn more about all of the features link:https://circleci.com/blog/announcing-our-next-generation-convenience-images-smaller-faster-more-deterministic/[on our blog post]. As we begin to deprecate the legacy images, this document provides information on the migration process.
...
If you are using the `buildpack-deps` image, the suggestion is to use the new CircleCI Base image, `cimg/base`. For the other two images, you can install the software yourself in the base image or use a third-party image instead.
...
With legacy images, 4 different variant tags existed that you could use to get browser testing for a particular image. For example, if you did browser testing with the Python v3.7.0 image, you might have used Docker `image: circleci/python:3.7.0-browsers`. These 4 tags have now been consolidated into a single tag designed to be used together with the link:https://circleci.com/developer/orbs/orb/circleci/browser-tools[CircleCI Browser Tools orb].
...
Legacy images had variant tags for several different base operating systems. Some images used versions of Debian and Ubuntu, while other images provided several different bases. For next-gen images, this is no longer the case. All CircleCI next-gen images are based on the latest LTS release of Ubuntu.
...
* A library you used previously now has a different version.
* An apt package is no longer pre-installed. In this scenario, install that package using:
...
Each image has its own GitHub repository. You can find them in the link:https://github.com/CircleCI-Public?q=cimg-&type=&language=&sort=[CircleCI-Public GitHub organization]. These repositories are where you can learn more about an image, what makes up the image, open GitHub issues, and contribute PRs. If you are having an issue with an image, especially if it is a migration issue, you can open a GitHub issue and ask questions. You can also link:https://support.circleci.com/hc/en-us/requests/new[open a support ticket] or reach out on link:https://discuss.circleci.com/t/legacy-convenience-image-deprecation/41034[CircleCI Discuss].
