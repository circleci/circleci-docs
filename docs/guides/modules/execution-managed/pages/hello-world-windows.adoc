...
You can view the link:https://github.com/CircleCI-Public/circleci-demo-windows/blob/master/.circleci/config.yml[entire configuration file]. It also includes browser and UI testing, but we will focus on the `hello-world` workflow for now.
...
Next, we declare orbs that we will be using in our build. We will only use the link:https://circleci.com/developer/orbs/orb/circleci/windows[Windows orb] to help us get started. This example uses the `4.1.1` version of the orb, but you may consider using a more recent version.
