...
NOTE: When using an orb, check the link:https://circleci.com/developer/orbs[Orb Registry] to confirm you are using the most recent version. Alternatively, use the version that best fits your specific project.
...
For this step, we use commands that come preconfigured in the Go link:https://circleci.com/developer/orbs/orb/circleci/go[orb]. The `go/mod-download` command downloads Go modules and caches them automatically. Tests are run using the `go/test` command.
...
jobs:
  build_and_test: # this can be any name you choose
    executor:
      ...
      tag: '1.22' # Specify a version tag
    steps:
      - checkout # checkout source code
      - go/mod-download # Download and cache Go modules
...
* xref:optimize:parallelism-faster-jobs.adoc[Parallelism and Test Splitting]
