...
NOTE: *Using CircleCI Cloud?* We recommend the xref:guides:integration:slack-integration.adoc[Slack Integration] for workflow-status notifications instead of using the Slack orb. Set up once for your organization, no `config.yml` changes needed.
...
The sample code below shows the basics you need to set up Slack notifications in your `config.yml` file. You can copy and paste the whole thing or take the individual elements and incorporate them into your project `config.yml` file:
...
IMPORTANT: If you commit your `config.yml` file at this point and trigger a pipeline, your build will fail. You need to add your Slack credentials in the next step.
...
+
.Guides:Root:Slack Orb Name App
...
+
.Guides:Root:Slack Orb Permissions
image::guides:ROOT:slack-orb-permissions.png[Slack app Permissions]
+
. On the _OAuth & Permissions_ page, scroll down to **Scopes**. Create the permissions for your Slack app here.
+
.Guides:Root:Slack Orb Add Scope
...
* files:write
+
.Guides:Root:Slack Orb Bot Token Scopes
...
+
.Guides:Root:Slack Orb Install Workspace
...
. You see a *Bot User OAuth Token*. Copy this token to your clipboard, ready to add it to CircleCI. Make sure you keep this private.
+
.Guides:Root:Slack Orb Copy Token
...
. Select the **Org** page.
+
.Guides:Root:Slack Orb Organization Settings
...
+
.Guides:Root:Slack Orb Create Context
...
+
.Guides:Root:Slack Orb Environment Variable
...
+
.Guides:Root:Slack Orb Copy Link
...
+
You will then see a green **Success** badge and a green tick next to your `notify` job.
+
.Guides:Root:Slack Orb Success
image::guides:ROOT:slack-orb-success.png[Success]

. Select your job to see what just happened. You see the message body that was sent to Slack.

. Now open your Slack workspace. In the default channel you specified earlier, you see the alert triggered by your CircleCI pipeline.
+
.Guides:Root:Slack Orb Text Notification
...
* *Line 10* specifies the template to be used, in this case `success_tagged_deploy_1`.
...
Commit your updated `config.yml` file (and push it, if you are working remotely). Once the pipeline is complete, you see a more sophisticated alert in your Slack channel.

.Guides:Root:Slack Orb Deployment Successful
