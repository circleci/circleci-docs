...
Signing in this way still requires a browser at some point in the process. The `--no-browser` flag only changes where you see the sign-in URL; you still need a browser to open it and complete sign-in.
...
If you have an invite email from your team, you are _joining_ an existing CircleCI organization, whether you already have an account or are creating one for the first time.

. Follow the link in your invite email and follow the instructions in the web app.
. CircleCI takes you to your organization's dashboard in the web app. From here, you can follow projects or trigger pipelines in projects to which you have access in your organization, or xref:create-project.adoc[Create New Projects].

NOTE: If you are setting up a project for the first time, you may need to authenticate with your VCS provider. That authentication is a one-time step — subsequent projects do not require you to authenticate again. Refer to the xref:create-project.adoc[Creating a Project] guide for more information.
...
* If you sign up with **email and password** and add **GitHub**, **GitLab**, or **Bitbucket Data Center** to your org, you choose the VCS integration type when you create the org. You authenticate with your VCS provider as part of that setup. Team members you invite can join without authenticating with your VCS, but they authenticate the first time they create a project.
