...
* *Line 8:* The `name` attribute provides useful information when returning warnings, errors, and output. It is helpful if the `name` is meaningful to you as an action within your build process.
...
* On Line 5, change the image to `cimg/base:current`.
...
* *Line 5:* This line now specifies a Docker image that supports git. `cimg/base:current` is a small Ubuntu-based image for running basic jobs.
...
You can now see some additional steps on the CircleCI dashboard:
...
If you click on the _Using-Node_ job and then the _Running the Node Container_ step, you can see that the command `node -v` has printed the version of Node.
...
TIP: To increase your understanding, experiment with other xref:execution-managed:circleci-images.adoc[CircleCI Images], or add some more jobs to your workflow.

[#part-4-adding-a-manual-approval]
== Part 4: Adding a manual approval

The CircleCI workflow model is based on the orchestration of preceding jobs. As you saw in Part 3, the `requires` statement specifies that a job runs only if the previous job has been successfully executed.

In Part 3, an event triggering the pipeline caused the `Hello-World` job to run immediately. The remaining jobs ran automatically, once `Hello-World` had completed successfully.
...
Most of this code will look familiar to you, with a couple of important points:

* *Lines  52-56*: This creates a new job called _Hold-for-Approval_. The `type` specifies this is an _approval_, so you are required to manually approve this job in CircleCI. An approval job is useful if you want to see whether the previous jobs have been executed as expected. For example, you can check that a website looks correct on a test server before making it live. Or you might want a human to perform checks before you execute any expensive jobs.
* *Lines 57-59*: This final job - _Now-Complete_ - `requires` the successful completion of _Hold-for-Approval_, so it will execute only once you have approved that previous job in CircleCI.

As before, commit and push your updated `config.yml` file.

If you look at your pipeline in CircleCI, you will see a purple status badge of *Needs Approval*.

To approve the job, select the thumbs up icon to the right of the _Hold-for-Approval_ job in the _Actions_ column. In the pop-up message, click the blue btn:[Approve] button.

Now you will see a tick in the Actions column and your jobs will complete.

Select the _Now-Complete_ job, then the _Approval Complete_ step. You can see the output of your command: `The work is now complete`.
...
If you use VS Code, you might find the official xref:toolkit:vs-code-extension-overview.adoc[CircleCI Extension] useful when writing, editing, navigating and troubleshooting your YAML config files.
...
* xref:reference:ROOT:configuration-reference.adoc[Configuring CircleCI]
* xref:execution-managed:executor-intro.adoc[Executors and Images]
