...
Your `.circleci/config.yml` will start off with the version of CircleCI you wish to use. CircleCI Cloud and newer server versions use CircleCI `2.1`.
...
Some limitations exist depending on the execution environment in your configuration. Visit the xref:execution-managed:executor-intro.adoc[Introduction to Execution Environments] page to get started and find information on each execution environment.
...
The last thing we need for a basic CircleCI configuration is a job to actually run. Jobs are orchestrated using workflows, which are a set of rules defining a collection of jobs and their run order. Using the Docker configuration example, we can add steps to our job. Steps are a list of commands to run inside the Docker container.
...
If you have a CircleCI account, you can create a new project and add these examples to a `.circleci/config.yml` file. You can see the strings printed out in the job's build pipeline in the CircleCI web UI.

YAML can be quite picky about indentations. You can use a https://yaml-online-parser.appspot.com/[YAML checker] to parse your YAML and make sure it is valid.

If you would like a more complex configuration tutorial, visit the xref:config-intro.adoc[Configuration Tutorial] page. Before starting this tutorial, you must already have a CircleCI account set up, as you will follow along in the CircleCI web UI. You can also find a variety of configuration examples on the xref:toolkit:sample-config.adoc[Sample Configuration] page.
...
.VS Code extension showing definition on hover
