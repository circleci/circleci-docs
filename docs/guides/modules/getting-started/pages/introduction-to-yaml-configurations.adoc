...
Your `.circleci/config.yml` will start off with the version of CircleCI you wish to use. CircleCI Cloud and newer server versions must use CircleCI `2.1`.
...
Images are used by either a Docker or machine executor. In our example, we are using a Docker **convenience image** that spins up an Ubuntu environment. To use macOS as an execution environment, use a **machine image** instead. This spins up a dedicated virtual machine with Xcode preinstalled, as in the example below.
...
.VS Code extension go-to-definition example
