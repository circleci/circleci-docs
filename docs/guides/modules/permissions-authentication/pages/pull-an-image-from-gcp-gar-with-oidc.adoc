...
Either modify your attribute mapping or the incoming assertion to produce
a mapped attribute that is less than 127 bytes.
----

The CircleCI OIDC V2 token's `sub` claim includes VCS information and can exceed GCP's 127-byte limit for `google.subject`. For example:

[source,plaintext]
----
org/<org_id>/project/<project_id>/user/<user_id>/vcs-origin/github.com/<org>/<repo>/vcs-ref/refs/heads/<branch>
----

To resolve this, update the `google.subject` attribute mapping in your GCP Workload Identity pool to derive a shorter value. For more information on attribute mappings, see the https://docs.cloud.google.com/iam/docs/workload-identity-federation-with-other-clouds#mappings-and-conditions[GCP attribute mappings and conditions documentation]. The recommended mapping uses the last path segment of `sub` (the user UUID), prefixed with the org ID:

[source,plaintext]
----
google.subject=assertion.org_id + "/" + assertion.sub.split("/")[assertion.sub.split("/").size() - 1]
----

This produces a value like `<org_uuid>/<user_uuid>` (73 bytes), which is well within the limit and works consistently across all trigger types.

If your IAM conditions reference `google.subject`, update them to match the new format. To preserve the full `sub` for use in conditions, add it as a custom attribute:
