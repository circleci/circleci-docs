...
For alternative methods, see the xref:execution-managed:private-images.adoc#aws-ecr[AWS ECR] section of the Docker authenticated pulls page.
...
* A CircleCI account integrated with a supported VCS. See xref:getting-started:first-steps.adoc[Sign Up and Try CircleCI] for more information.
...
+
** The **Identity Provider** that you created for CircleCI. See xref:openid-connect-tokens.adoc#set-up-aws[Set Up AWS Instructions] for more information.
...
. Navigate back to the **Add Permissions** page and refresh to find your new policy, and carry on following the AWS instructions to complete the role creation.
...
. Replace `<your-iam-role-arn>` with the ARN of the IAM role you want to assume. Use the role you created in the last section.
