...
AWS STS now supports a dedicated condition key for CircleCI projects. Using this key is the most secure and direct way to ensure an IAM role can only be assumed by a specific project within your organization.
...
NOTE: The `oidc.circleci.com/project-id` claim is used for trust policy evaluation during the initial authentication step and is not available as a session variable after the role is assumed.
