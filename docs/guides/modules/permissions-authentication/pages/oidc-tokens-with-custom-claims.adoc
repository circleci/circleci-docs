...
Use OpenID Connect (OIDC) tokens to connect your pipelines to compatible cloud services without the need to manage long-lived secrets in CircleCI. Create CircleCI OIDC tokens, using the CircleCI task agent CLI, and customize the `aud` claim to meet the unique needs of different cloud services.
...
TIP: See also the full xref:toolkit:task-agent-cli-usage-guide.adoc[Task agent CLI usage guide].
...
Use the CircleCI task agent CLI in a `step` to create your customized token, as follows:
