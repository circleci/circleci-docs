...
[#create-oidc-token-with-root-iss]
== Create OIDC token with root issuer

Use the `--root-issuer` flag with the CircleCI environment CLI to generate an OIDC token with the issuer (`iss`) claim set to `\https://oidc.circleci.com`.

[#create-oidc-token-in-a-job-with-custom-claims]
== Create OIDC token in a job with custom claims

TIP: See also the full xref:reference:ROOT:environment-cli-usage-guide.adoc[Environment CLI Usage Guide].
