...
- A checkout source, which determines which repository will be checked out when running a xref:reference:configuration-reference.adoc#checkout[Checkout Step].
...
If you have an organization of type `github` or `bitbucket` with a corresponding VCS integration, GitHub OAuth app or Bitbucket cloud, a pipeline definition is created automatically for each of your projects when the project is set up.

If you have a `circleci` organizaiton, you must create pipeline definitions manually. See the xref:orchestrate:pipelines.adoc[Pipelines] page for more information.
