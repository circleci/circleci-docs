...
You can log in to CircleCI using one of the following methods. These methods cannot be used together. For example, if you have an email/password account and then use a social login, this will create a new account connected to your VCS provider:
...
|Mutually exclusive with other VCS authorizations except GitHub App.

|Cursor Origin
|Not applicable. Cursor Origin requires no user-level authorization.
|Installing the CircleCI app into your Origin codebase is the only action required. See the <<manage-vcs-integrations>> section.
|===
--

== Organizations

An organization in CircleCI is a top-level account that serves as a container for your team's projects, settings, permissions, and billing. Organizations are typically linked to your version control system (VCS) account(s) (GitHub, GitLab, Bitbucket, or Cursor Origin).
...
| `circleci`
a| * GitHub App (add-on, available for GitHub.com, GitHub Enterprise Cloud, and GitHub Enterprise Server).
* GitLab (add-on).
* GitLab self-managed (add-on).
* Bitbucket Data Center (add-on).
* Cursor Origin (add-on, beta).
...
NOTE: The GitHub App integration for GitHub Enterprise Server is currently in Preview. For more information, see the xref:integration:github-enterprise-server-integration.adoc[GitHub Enterprise Server Integration] page. The Cursor Origin integration is in beta and is available for `circleci` type organizations only.

The xref:guides:integration:version-control-system-integration-overview.adoc[VCS Integration Overview] page provides a full list of CircleCI features available for each organization and integration type.
...
* You could set up a trigger for your pipeline that runs when someone pushes to the repository itself. It could also trigger when there is a push to the default branch of a library your project depends on.
...
The VCS integration options available to you depend on your organization type. By _integration_ here we mean the way CircleCI checks out your code from your VCS provider. Options are as follows:

[.table-scroll]
--
[cols=7*, options="header"]
|===
| Organization type |GitHub App | GitHub OAuth | Bitbucket Cloud | Bitbucket Data Center | GitLab | Cursor Origin (beta)
...
* GitHub OAuth and Bitbucket Cloud are set up when logging in to CircleCI with your GitHub or Bitbucket social login. No additional action is required, but the installation can be viewed at menu:Org[VCS Connections].
* GitHub App, GitLab self-managed, Bitbucket Data Center, and Cursor Origin are managed from menu:Org[VCS Connections]. However, when creating a project (if using a `circleci` organization) or creating a new pipeline from menu:Project Settings[Project setup] you will also be walked through the installation process.
...
If you are using a self-hosted GitLab instance or Bitbucket Data Center instance, some additional configuration is required as follows. See the <<manage-vcs-integrations>> section for more information.
...
*Configure*: Set up an integration with Cursor Origin from menu:Org[VCS Connections]. Select btn:[Set up integration] next to Cursor Origin, then follow the instructions to install and authorize the CircleCI app in your Origin codebase. You are also walked through this installation when you create your first Cursor Origin project. See the xref:getting-started:create-project.adoc#create-a-project[Create a Project] page for those steps.

*View*: To view a Cursor Origin integration, navigate to menu:Org[VCS Connections].

*Configuration files*: CircleCI cannot commit files to a Cursor Origin repository. You add the `.circleci/config.yml` file to your repository yourself, rather than committing it from the CircleCI web app during project creation.
...
For a guide to inviting your team, see the xref:getting-started:invite-your-team.adoc[Invite Your Team] page. To understand the differences between registered users, unregistered users, and active users, and how they affect billing, see xref:plans-pricing:user-types-and-registration.adoc[User types and registration].
...
The pipelines you can create and configure are derived from the integration types available to you. You can find this information in the <<vcs-integration>> section.

If you have an organization of type `github` or `bitbucket` with a corresponding VCS integration, a pipeline definition is created automatically for each of your projects when the project is set up.

If you have a `circleci` organization, you must create pipeline definitions manually. See the xref:orchestrate:pipelines.adoc[Pipelines] page for more information.

== Triggers

The trigger options available to you depend on your pipeline type. These options are described in the following table:

#TODO Confirm whether manual and API triggering are supported for Cursor Origin pipelines. The Origin row currently asserts Yes, copied from the other pipeline types, and is unverified.#
...
+
--
When using the GitHub App integration, CircleCI uses HTTPS-based authentication to check out your code for both GitHub.com, GitHub Enterprise Cloud, and GitHub Enterprise Server. No SSH keys or deploy keys are required.
--
GitHub OAuth::
+
--
...
+
--
When using the Cursor Origin integration, CircleCI uses HTTPS-based authentication to check out your code. No SSH keys or deploy keys are required.
--
====
...
If you would rather use a custom checkout command, follow the steps below to ensure the authenticity of the host you are connecting to.
...
* Never use non-machine user keys. It is best practice for keys to be associated with the build, not with a specific person.
...
* Pushing commits back to your repository. If your pipeline needs to commit and push changes back to GitHub, you need the write permissions that a user key provides. Common examples include updating a changelog, bumping version numbers, generating documentation, or creating automated pull requests.
* Accessing multiple private repositories. When your build depends on multiple private repos (beyond just the one being built), a user key gives your pipeline access to all repos your GitHub user can access. A user key is useful for:
...
The trade-off with choosing a user key is that user keys grant broad access based on your GitHub account's permissions. User keys are a bigger security risk if compromised.
...
NOTE: To use the recommended machine-user approach, create a new GitHub/Bitbucket account with minimal necessary permissions. Then follow these steps while logged in as that machine user account.
...
. You will see a warning to use a machine user. Select btn:[Confirm User].
...
Remember that SSH keys must be shared only with trusted users. GitHub collaborators on projects employing user keys can access your repositories, therefore, only entrust a user key to someone with whom you would entrust your source code.
...
* Secure authentication to remote systems. SSH keys allow your pipeline to authenticate to servers, repositories, or services without storing plaintext passwords. That kind of authentication is crucial when you need to deploy code to production servers, pull from private repositories, or interact with remote infrastructure.
...
* If your motivation for moving is to **leverage new functionality that is only available to the GitHub App integration**, you do not need to migrate at all. Instead, install the GitHub App alongside your existing OAuth app integration. See the xref:guides:integration:using-the-circleci-github-app-in-an-oauth-org.adoc[Using the CircleCI GitHub App in an OAuth Organization] page.
...
+
NOTE: You can install the CircleCI GitHub App into the same GitHub organization that already uses the GitHub OAuth App integration. This works as long as your original CircleCI organization is not already connected to it.
...
. If the repository you connected is also connected to your previous CircleCI organization, CircleCI starts pipelines for both organizations on every push. If your test from step 8 was successful, go to **Project Settings** in your organization connected to the GitHub OAuth App (your "old" org) and select btn:[Stop Building]. This ensures that push events to your repository only trigger pipelines in the project connected to your GitHub App organization.
...
. Invite your teammates to the new organization (the one that is integrated with the CircleCI GitHub App) using the instructions on the xref:getting-started:invite-your-team.adoc[Invite Your Team] page.
...
.. Select the icon:cancel[Cancel icon] next to the "new" organization to remove the shared relationship between the "new" and "old" organizations.
...
* The application could use that OAuth token to act on behalf of the user via the API, with whatever permissions granted during the OAuth flow.
...
- Once you have approved CircleCI, you will see a checkmark next to your organization's name.

After access is granted, CircleCI will behave normally again.
