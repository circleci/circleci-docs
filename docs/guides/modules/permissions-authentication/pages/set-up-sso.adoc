...
. Select **Org** in the sidebar.
. Select **Single sign-on (SSO)** from the sidebar.
. On the SSO Settings page, select **Set up SSO**.
...
+
TIP: If your IdP requires a logo for the application (for example, Okta), you can download the CircleCI logo from our link:https://brand.circleci.com/613faff00/p/732047-logos[brand site].
. Add `https://app.circleci.com` to your IdP's trusted origins for iframe embedding. Adding this origin validates a user's active SSO session when they interact with an SSO organization in CircleCI.
. The IdP will generate the following information, which you will need to copy in order to complete the next section:
** SSO URL
** x509 signing certificate
. Set up user accounts in your IdP. Your users will be able to access CircleCI through your IdP once they have a CircleCI account that they sign into with an email and password. Users that joined your organization before SSO was set up will retain their assigned roles. New users are given a default role of xref:roles-and-permissions-overview.adoc#organization-role-permissions-matrix[Organization Viewer].
...
. Copy the provided TXT record and add it to your DNS. Your SSO Connection will be marked as **Pending** until this step is complete. Adding the TXT record to your DNS allows CircleCI to validate that you are the owner of the domain on which you are setting up SSO.
+
...
** New organization members must be invited to your organization. See the xref:manage-roles-and-permissions.adoc#add-people-to-your-organization[Manage Roles and Permissions] page for more information.
** Users must log into their email/password CircleCI account to authenticate with SSO. See the xref:sign-in-to-an-sso-enabled-organization.adoc#log-in-to-an-organization[Sign in to an SSO-Enabled Organization] page for more information.

Your organization is now using SSO to grant and deny user access. You can manage user roles by following the steps outlined on the xref:manage-roles-and-permissions.adoc[Manage Roles and Permissions] page.
