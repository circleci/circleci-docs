= Set up cross-repo triggers for library releases
:page-platform: Cloud
:page-description: Learn how to automatically trigger consumer repository pipelines when you tag a new library release.
:experimental:

This guide shows you how to set up cross-repo triggers so that when you tag a new release of your internal library, CircleCI automatically runs tests in the repositories that consume that library. This validates that your library changes work correctly with dependent applications before you publish the release.
...
**Why tag-based triggers?** Tags represent stable release points in your library's development. Testing consumers against tagged versions ensures that each library release is validated before being published to package registries or deployed to production.

NOTE: This guide focuses on tag-based triggers. If you need to trigger on other events like branch pushes or pull requests, see the xref:guides:orchestrate:triggers-overview.adoc[Triggers overview] for alternative approaches.
...
* The consumer repository must be set up as a project in CircleCI and have a configuration file that defines the tests or validation steps you want to run. You can have separate configuration files for your build pipeline and a pipeline you want to run when the library changes. See the xref:guides:getting-started:create-project.adoc[Create a Project in CircleCI] page for steps to get your project set up in CircleCI.
...
== 2. Add a trigger for library events
...
=== 2.a. Create the trigger in CircleCI
...
. From the **Event** dropdown menu, select **Tag created**.
+
This will trigger the consumer pipeline when a new tag is created in the library repository, which is ideal for testing against official library releases.
+
...
+
IMPORTANT: Changing the event source is what enables the cross-repo trigger. The trigger now watches for events in the library repository rather than the consumer repository.
...
=== 2.b. Configure tag filters in your workflow

The trigger is now configured in CircleCI, but you also need to update your consumer repository configuration file to run workflows for tags.

IMPORTANT: By default, CircleCI does not run workflows for tags unless you explicitly specify tag filters.

In your consumer repository's `.circleci/config.yml` file, add tag filters to your workflow:
...
== 3. Use library information in the consumer pipeline
...
== 4. Test your cross-repo trigger
...
If you have multiple consumer repositories that depend on your library, repeat sections 1-2 for each consumer repository. Each consumer can have its own pipeline configuration and trigger settings based on its specific testing requirements.
...
* xref:guides:orchestrate:triggers-overview.adoc[Learn about other trigger types] - explore triggering on pushes or pull requests instead of tags
* xref:guides:orchestrate:workflows.adoc#executing-workflows-for-a-git-tag[Learn about workflow filtering] - configure more advanced workflow filters for different scenarios
