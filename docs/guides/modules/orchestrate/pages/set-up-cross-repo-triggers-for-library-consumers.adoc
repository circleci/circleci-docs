...
* A CircleCI account connected to GitHub App or Cursor Origin. Cursor Origin is in beta.
* A library repository and a consumer repository in the same GitHub organization or Cursor Origin codebase. If you want to follow the centralized config options, you will need a third repository for your centralized config store.
* Admin access to both the library repository and the consumer repository.
...
CircleCI automatically populates the following pipeline values when a GitHub App cross-repo trigger fires:
...
NOTE: These `pipeline.event.github.*` values apply to GitHub App pipelines. Cursor Origin pipelines have `pipeline.event.name` and checkout values such as `pipeline.git.repo_name`. For the full list, see the xref:reference:ROOT:variables.adoc[Pipeline Values] guide.
...
. Select **GitHub trigger +** or **Cursor Origin trigger +** at the bottom of the pipeline box. Choose the button that matches your integration.
...
+
NOTE: You can also configure triggers for other events like **PR merged** or **All pushes**. See the xref:github-trigger-event-options.adoc[GitHub Trigger Event Options] and xref:cursor-origin-trigger-event-options.adoc[Cursor Origin Trigger Event Options] pages for more information.
...
. Navigate to your library repository in GitHub or Cursor Origin.
. Create a new tag using the command line, or the GitHub or Cursor Origin interface:
+
[source,console]
----
$ git tag v1.2.3
...
. You will see a new pipeline running with the name you configured in step 3.
