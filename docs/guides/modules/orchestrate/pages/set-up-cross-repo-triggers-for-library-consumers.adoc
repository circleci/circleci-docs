...
Here is an example consumer configuration file that uses the built-in pipeline values. This configuration works for both Option 1 (config in consumer repository) and Option 2 (centralized config repository), as CircleCI automatically populates the pipeline values with trigger information regardless of where the config file is stored.
...
.. Select **Save**.
.. In your central config repository, create or update the configuration file at the path you specified using the example config from <<1-create-configuration-file-for-your-consumer-pipeline,step 1>>.
+
