...
:page-platform: cloud
:page-vcs: github, github-enterprise, cursor-origin
...
|Use this option when you want a platform team to maintain consistent test configurations across multiple consumers. It also works when you want to reuse the same configuration file for multiple consumer repositories.
...
.. In the **Config filepath** field, leave the default to run your existing `circleci/config.yml` file. To run a different build on changes to the library, enter the path to that configuration file instead, for example `.circleci/library-integration-tests-config.yml`.
