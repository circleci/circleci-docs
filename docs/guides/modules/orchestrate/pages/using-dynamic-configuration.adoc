...
* **Line 8**: Invoke the `continuation` orb so we can use it.
* **Lines 11-20**: Define a job called `setup` that uses the default xref:execution-managed:executor-intro.adoc[`executor`] provided by the `continuation` orb as an . This job:
