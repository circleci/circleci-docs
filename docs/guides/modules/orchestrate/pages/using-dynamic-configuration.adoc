...
- <<pass-parameters-to-continuation-workflow>>: In this example, a parameter is passed from the setup workflow to the continuation workflow using the continuation orb. The continuation workflow uses conditional logic to execute based on the parameter value.
...
* **Line 4**: Add the line `setup: true` to the top-level of the config, to designate it for use of CircleCI's dynamic configuration feature.
* **Line 8**: Invoke the `continuation` orb so we can use it.
* **Lines 11-18**: Define a job called `set-param` that uses the continuation orb's `continue` command to pass a parameter called `further` with a value of `true` to the continuation workflow.
...
* **Lines 4-7**: Define a pipeline parameter called `further` with a type of `boolean` and a default value of `false`.
* **Line 20**: Use the xref:reference:ROOT:configuration-reference.adoc#using-when-in-workflows[`when` key] in the workflow to conditionally execute the workflow based on the value of the `further` parameter.
...
* **Line 8**: Invoke the `continuation` orb so we can use it.
* **Lines 11-20**: Define a job called `setup` that uses the default xref:execution-managed:executor-intro.adoc[`executor`] provided by the `continuation` orb. This job:
** Calls the xref:reference:ROOT:configuration-reference.adoc#checkout[`checkout`] step to checkout code from the configured repository.
** Calls the xref:reference:ROOT:configuration-reference.adoc#run[`run`] step to execute the existing `generate-config` script, so we can pass its output to the `continue` job of the `continuation` orb.
...
workflows:
  ...
  # triggered. see:
  # https://circleci.com/docs/configuration-reference/#using-expressions-in-your-configuration
  # for more information.
  run-integration-tests:
    when: pipeline.parameters.run-build-service-1-job or pipeline.parameters.run-build-service-2-job
    jobs:
      - maven/test:
...
** **Lines 22-28**: The `service-1` workflow triggers the `maven/test` job on the `service-1` directory, when the pipeline parameter value mapped to run-build-service-1-job is set to `true`. This will only happen if a change was made in the `service-1` directory in the commit, as determined based on the path filtering in the setup phase (`config.yml`).
** **Lines 31-37**: The `service-2` workflow triggers the `maven/test` job on the `service-2` directory, when the pipeline parameter value mapped to run-build-service-2-job is set to `true`. This will only happen if a change was made in the `service-2` directory in the commit, as determined based on the path filtering in the setup phase (`config.yml`).
...
* A setup configuration, `config.yml`.
* Separate configuration files for building the code and the docs, `code-config.yml` and `docs-config.yml`.
* A `shared/` configuration directory containing our shared jobs in separate YAML files. The `shared/` directory is used to generate `shared-config.yml` using the link:https://cli.circleci.com/reference/#circleci-config-pack-path[CircleCI CLI Command] `circleci config pack`.
...
The setup configuration has a single job, `setup`, which has the following steps:
...
One way to handle this scenario is to provide an alternative configuration using the `config-path` parameter (see line 27 <<setup-config-1,here>>). `config-path` is ignored if a mapping is in place, but will be used if no mapping matches the path to modified files. In this example, we use a `no-updates.yml` configuration file.
