...
* A setup configuration, `config.yml`.
* Separate configuration files for building the code and the docs, `code-config.yml` and `docs-config.yml`.
* A `shared/` configuration directory containing our shared jobs in separate YAML files. This is used to generate `shared-config.yml` using the link:https://cli.circleci.com/reference/#circleci-config-pack-path[CircleCI CLI Command] `circleci config pack`.
