...
#TODO In the Cursor Origin tab below, confirm whether the trigger form has an Event Source field that can point at a different repository, as the GitHub App form does.#

#TODO Confirm the button label in the Cursor Origin tab below. It is written as "Cursor Origin trigger +", based on the GitHub App tab's "GitHub trigger +".#
...
These steps also apply to GitHub App Server pipelines. See the xref:integration:github-enterprise-server-integration.adoc[GitHub Enterprise Server Integration] page for more information.
...
****
* The *Event Source* defaults to the repository associated with the pipeline. You can change the event source to a different repository if you wish. If the event source does not match the pipeline's config or checkout source, an option appears to specify which branch to use for the trigger.
. Select btn:[Save]
...
. Select btn:[Save]
+
CAUTION: When setting up a trigger you will see a modal titled **Webhook URL** requesting that you set up a webhook in GitLab. **You do not need to take action**. CircleCI sets the webhook up automatically. The modal is a known issue and will be fixed.
...
. Select the ellipsis next to your project icon:more[Ellipsis menu icon] and choose **Project Settings**.
. Select **Triggers** in the sidebar.
. Select btn:[Add Trigger].
. Select the same location in the "Connect to" dropdown menu that you selected for your pipeline (for example, Bitbucket Data Center).
. Select btn:[Next].
. Complete the form fields and options:
** Give your trigger a descriptive name.
** Authorize your connection if this is not already showing with a check mark image:guides:ROOT:icons/passed.svg[passed icon, role="no-border"] (Not required for custom webhooks).
...
When the CircleCI Origin app is installed in your Origin codebase, CircleCI receives events for the repositories it can access. When a trigger is created, CircleCI uses the event data to determine whether a pipeline is triggered.

include::guides:ROOT:partial$app-navigation/steps-to-project-settings.adoc[]
+
. Select *Project Setup* in the sidebar.
. Select btn:[Cursor Origin trigger +] at the bottom of the pipeline box.
. Choose when to run the pipeline by selecting an option from the *Event* menu. The default is *All pushes*.
+
For the full list of event options, see the xref:cursor-origin-trigger-event-options.adoc[Cursor Origin Trigger Event Options] page.
. Select btn:[Save].

To verify your trigger is set up correctly, push a commit to your repository. Your config file must already be committed there. See the xref:getting-started:create-project.adoc#create-a-project[Create a Project] page.
--
Scheduled::
+
--
...
+
--
GitHub OAuth triggers are set up automatically when a GitHub OAuth pipeline is set up. You can delete and re-add a trigger for a GitHub OAuth pipeline. If you do delete a GitHub OAuth trigger, you will see a line at the bottom of the pipeline section: "To re-add the default OAuth trigger click here". Select the link to re-add the default OAuth trigger.
--
====
