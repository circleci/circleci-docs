...
NOTE: Automatic reruns are only supported for `run` steps, not special steps like `checkout` or `setup_remote_docker`. You must configure the `command` key for the step. You cannot use the short form run step configuration with automatic reruns.
...
workflows:
  test-workflow:
    when: pipeline.trigger.type != "schedule" # run the workflow only if it is not triggered by a schedule trigger
    max_auto_reruns: 4
    jobs:
      - build
      - test
...
Automatic workflow reruns are indicated on the pipelines page in the CircleCI web app. In the Trigger event column you will see *Auto-rerun* followed by the rerun attempt number, as shown in the following screenshot.
