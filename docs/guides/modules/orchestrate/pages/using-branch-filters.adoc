...
The following example shows using the xref:pipeline-variables.adoc#pipeline-values[pipeline value] `pipeline.git.branch` to control `when` a step should run. In this case the step `run: echo "I am on main"` only runs when the commit is on the main branch:
