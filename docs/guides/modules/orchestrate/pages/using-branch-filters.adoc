...
NOTE: Step-level `when` conditions use the xref:reference:ROOT:configuration-reference.adoc#legacy-logic-statements[legacy logic statement syntax]. For workflow-level filtering, you can use the modern xref:reference:ROOT:configuration-reference.adoc#using-expressions-in-your-configuration[expression syntax] instead.
