...
* A GitHub App integration. This integration is optional but recommended. Only GitHub App pipelines can use a configuration file stored outside the project repository, which is what makes centralized config worthwhile. See the xref:permissions-authentication:users-organizations-and-integrations-guide.adoc[Users, Organizations, and Integrations Guide] for more information.
