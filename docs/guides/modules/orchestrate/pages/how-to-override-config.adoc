...
* A GitHub App integration or a Cursor Origin integration. These integrations are optional but recommended. Only GitHub App and Cursor Origin pipelines can store the configuration file outside the project repository. That is what makes centralized config worthwhile. Cursor Origin is in beta. See the xref:permissions-authentication:users-organizations-and-integrations-guide.adoc[Users, Organizations, and Integrations Guide] for more information.
...
The URL depends on where you store the orb:
...
+
--
`https://raw.githubusercontent.com/my-org/my-repo/path/to/file/orb.yml`
--
Cursor Origin::
+
--
`https://raw.cursorusercontent.com/my-org/my-repo/main/path/to/file/orb.yml`
--
====
...
The default `config-override-url` in this example is a GitHub raw URL. For Cursor Origin, use `https://raw.cursorusercontent.com/my-org/my-repo/main/.circleci/team-config.yml` instead.
...
This prefix allows importing configurations from any repository in your GitHub organization.
--
Cursor Origin::
+
--
[source,text]
----
https://raw.cursorusercontent.com/
----

This prefix allows importing configurations from any repository in your Origin codebase.
--
====
...
* Ensure the organization has the CircleCI GitHub App or the CircleCI Origin app installed. One of these integrations is required to use a config source outside the project repository. See the xref:permissions-authentication:users-organizations-and-integrations-guide.adoc[Users, Organizations, and Integrations Guide] for more information.
...
* For VCS triggers, ensure you are using pipeline values that are available at runtime. For example:
+
[tabs]
====
GitHub App::
...
[source,yaml]
----
https://raw.cursorusercontent.com/<< pipeline.git.repo_owner >>/<< pipeline.git.repo_name >>/main/path/to/file/orb.yml
----
--
====
+
These pipeline values compile to the team's repository that triggered the change, and fetch that team's override. This assumes all team repositories are in the same GitHub organization or Cursor Origin codebase.
...
* Verify that your project uses a GitHub App or Cursor Origin pipeline.
...
=== Do I need GitHub App or Cursor Origin?
GitHub App and Cursor Origin provide the best experience for centralized configuration management. Without one of these integrations, you would need to duplicate the centralized config across multiple repositories.
...
Config overrides only support job overrides at this time.
