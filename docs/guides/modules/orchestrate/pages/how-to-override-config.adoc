...
See the `override-with` key entry in the xref:reference:ROOT:configuration-reference.adoc#override-with[Configuration Reference] for more information.

== Prerequisites
Before following this how-to guide, ensure you have met the following prerequisites:

* A CircleCI account, you can xref:getting-started:first-steps.adoc#[Sign Up For Free].
...
* Learn about xref:config-policies:config-policy-management-overview.adoc[Config Policy Management].
* Explore other configuration reuse patterns in the xref:reference:ROOT:reusing-config.adoc[Reusable Configuration Reference].
* Understand xref:orbs:use:orb-concepts.adoc[Orbs Concepts] and design.