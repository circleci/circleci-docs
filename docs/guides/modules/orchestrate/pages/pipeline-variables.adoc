...
Use the xref:reference:ROOT:configuration-reference.adoc#using-when-in-workflows[`when` clause] (or the inverse clause `unless`) under a workflow declaration, along with xref:reference:ROOT:configuration-reference.adoc#using-expressions-in-your-configuration[expressions], to decide whether or not to run that workflow. Expressions in a `when` or `unless` clause should evaluate to a truthy or falsy value.
