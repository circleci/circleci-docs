...
When you create a trigger for a GitHub App pipeline, you can choose from a set of trigger options that are based on link:https://docs.github.com/en/webhooks/webhook-events-and-payloads[GitHub App events] and their properties. Using GitHub App trigger options you can trigger a pipeline on non-push events.

TIP: The trigger event options described on this page apply to both GitHub App pipelines for GitHub Cloud (GitHub.com and GitHub Enterprise Cloud) and GitHub Enterprise Server.
...
****
Regardless of your chosen trigger option and pipeline type (GitHub OAuth, GitHub App), CircleCI will **never** trigger a pipeline for the following:

* On branch or tag deletion push events.

GitHub App pipelines are also never triggered:
...
* *Pushes to merge queues*: Trigger a pipeline on pushes to branches that start with the prefix `gh-readonly-queue/`.
* *PR opened or pushed to, default branch and tag pushes*: Trigger a pipeline for open-PR branch pushes, PR opens, tag pushes, or default branch pushes, including re-opened PRs.
...
* *Non-draft PR opened*: Trigger a pipeline when a non-draft pull request is opened. Draft PRs will not trigger a pipeline. See the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads?actionType=opened#pull_request[GitHub docs for the pull request opened event].
...
* *"run-ci" label added to PR*: Trigger a pipeline when a label "run-ci" is added to a PR. See the link:https://docs.github.com/en/webhooks/webhook-events-and-payloads?actionType=labeled#pull_request[GitHub docs for pull request labeled].
...
The following config orchestration tools are available within your pipelines:
