...
* `pipeline.event.github.pull_request.draft`: a boolean value indicating whether the pull request is a draft.
  ** **Usage example**: if wanting to trigger a pipeline on *PR opened or pushed to, default branch and tag pushes* as well as on *PR marked ready for review*, but ensure that it never runs if the event is associateed with a PR that is marked as *draft*, use the following filter. The condition `pipeline.event.name == "api"` ensures that the workflow runs if triggered manually via the web-app, or via API. 
