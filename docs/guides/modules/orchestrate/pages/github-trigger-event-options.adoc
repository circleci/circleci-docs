...
If your trigger is not restricted to only pull request events, any logical statement using the following pipeline values must also include a condition to ensure that the pipeline runs when the event is a pull request.
