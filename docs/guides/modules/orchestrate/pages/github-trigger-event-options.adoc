...
* `pipeline.event.github.pull_request.draft`: a boolean value indicating whether the pull request is a draft.
  ** **Usage example**: To trigger a pipeline on _PR opened or pushed to, default branch and tag pushes__, but ensure that it never runs if the event is associated with a PR that is marked as draft, use the following filter. The condition pipeline.event.github.pull_request.draft == false excludes draft PRs from triggering the workflow. The condition pipeline.event.name == "api" ensures that the workflow runs when triggered manually via the web app or API.
