...
To use `pg_dump`, `pg_restore` and similar utilities requires some extra configuration to ensure that `pg_dump` invocations will also use the correct version. The PostgreSQL client tools installed in the primary container must match the PostgreSQL server version running in the service container.

For example, if you are using `cimg/postgres:15.0` as your service container, but your primary container only has PostgreSQL 14 client tools, you will see an error like:

[,shell]
----
pg_dump: error: server version: 15.0; pg_dump version: 14.x
pg_dump: error: aborting because of server version mismatch
----

To fix this, install the matching PostgreSQL client version from the official PostgreSQL APT repository. Add the following to your `config.yml` file (replacing `15` with your PostgreSQL server version):

[,yml]
----
    steps:
      - run:
          name: Install matching PostgreSQL client tools
          command: |
            # Add PostgreSQL APT repository
            sudo sh -c 'echo "deb http://apt.postgresql.org/pub/repos/apt $(lsb_release -cs)-pgdg main" > /etc/apt/sources.list.d/pgdg.list'
            wget --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo apt-key add -
            sudo apt-get update
            # Install the PostgreSQL client version that matches your server
            sudo apt-get install -y postgresql-client-15
      - run:
          name: Add PostgreSQL binaries to PATH
          command: echo 'export PATH=/usr/lib/postgresql/15/bin:"$PATH"' >> "$BASH_ENV"
----

NOTE: Replace `15` in both `postgresql-client-15` and `/usr/lib/postgresql/15/bin` with the major version number of your PostgreSQL service container (for example, `14`, `15`, or `16`).
...
If you are using the CircleCI MySQL Docker image, you can customize the configuration using the `command` key (See more in the xref:reference:ROOT:configuration-reference.adoc#docker[Docker Executor Reference]). For example:
...
Refer to the xref:toolkit:postgres-config.adoc[Database Configuration Examples] document for additional configuration file examples.
