...
Some common incident scenarios include the following:
...
[TIP]
====
Set and export `$CIRCLE_TOKEN` with a xref:toolkit:managing-api-tokens.adoc[Personal API Token] to keep it out of your shell history.
Set and export `$CIRCLE_HOST` with your CircleCI hostname (`https://circleci.com` for Cloud, or your Server domain name).
====
...
CAUTION: Presigned URLs expire in minutes but require no authentication — do not share them.
...
**Server:** The Cloud audit log page and Cloud streaming flow do not apply. Review your xref:server-admin:operator:troubleshooting-and-support.adoc#generate-support-bundle[Support Bundle], platform logs, and your deployment's required object storage backend used for build output, artifacts, and audit/event logs.
