...
:page-description: Investigate pipelines and step output, then review audit logs and assess credential exposure after an incident.
:experimental:

After you have paused or stopped pipelines, use this guide to investigate what happened. If you have not already done so, start with xref:orchestrate:pause-pipelines-during-an-incident.adoc[Pause or Stop Pipelines During an Incident]. This page supplements your organization's incident response process with CircleCI-specific API calls, audit log queries, and ready-to-run download scripts.
...
TIP: This script follows `next_page_token` for project pipelines so it can process more than the first page of results. Pipelines are returned in reverse chronological order.
...
Jobs may store files as artifacts. Use xref:optimize:artifacts.adoc[Storing Build Artifacts] and the link:https://circleci.com/docs/api/v2/#tag/Job/operation/getJobArtifacts[Get job artifacts] API reference for the latest retrieval options and examples.
...
**Cloud Scale:** Stream audit logs to your own storage (S3) for bulk analysis. See xref:security:audit-logs.adoc#stream-audit-logs[Stream Audit Logs].
...
For rotation steps, see the xref:orchestrate:pause-pipelines-during-an-incident.adoc#secrets-and-credentials[Secrets and Credentials] section of xref:orchestrate:pause-pipelines-during-an-incident.adoc[Pause or Stop Pipelines During an Incident].
