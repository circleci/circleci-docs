...
For the full list of event options per integration, see the xref:github-trigger-event-options.adoc[GitHub Trigger Event Options], xref:gitlab-trigger-options.adoc[GitLab Trigger Options], and xref:cursor-origin-trigger-event-options.adoc[Cursor Origin Trigger Event Options] pages.
...
#TODO Cursor Origin is missing from this section. Confirm whether Origin pipelines support manual triggering from the web app. If they do, add Cursor Origin to the checkout source step below, as Origin pipelines have a separate checkout source. The same question is tracked against the UI triggers row of the VCS integration overview feature table.#

NOTE: This feature is not available for GitLab integrations.

. In the link:https://app.circleci.com/home[CircleCI web app], select your organization from your homepage.
. Select *Pipelines* for your project. If you do not see your project listed, select *Projects* in the sidebar to get to a full list. You will now be on your project's pipelines page.
...
. If your code is integrated via the **GitHub App or Bitbucket Data Center**, select your checkout source and branch. This tells CircleCI which code to check out when running a xref:reference:ROOT:configuration-reference.adoc#checkout[`checkout` step]. If your code is integrated via **GitHub OAuth or Bitbucket Cloud**, you can skip this step, the same location is used for both config and checkout source.
...
.. Find your project.
.. Select the ellipsis menu icon:more[Ellipsis menu icon] and choose **Project Settings**.
.. Copy your project slug. Save this somewhere, you will need it later.

. Find your definition ID:
.. You are in project settings if you are following these steps in sequence. If not see previous steps to get there. Select *Project Setup* in the sidebar, or **Pipelines** if you are using Bitbucket Cloud.
.. Copy the definition ID for the correct pipeline. Save this somewhere, you will need it in the next step.
+
.Screenshot showing the pipeline definition ID copy button
image::guides:ROOT:orchestrate-and-trigger/pipeline-definition-id.png[screenshot showing location of pipeline definition ID copy button]
