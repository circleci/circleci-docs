...
workflows:
  ...
  nightly: `
    triggers:
      - schedule:
          cron: "0 0 * * *"
          filters: pipeline.git.branch == main or pipeline.git.branch matches /^release\/.*/
    jobs:
      - coverage
----
...
workflows:
  dev_stage_pre-prod:
    jobs:
      ...
      - test_stage:
          filters: pipeline.git.branch == stage
      - test_pre-prod:
          filters: pipeline.git.branch matches /^pre-prod(?:-.+)?$/
----
...
workflows:
  ...
  tagged-build:
    jobs:
      - build:
          filters: pipeline.git.tag matches /^v.*/
----
...
workflows:
  build-deploy:
    jobs:
      - build:
          filters: pipeline.git.branch or pipeline.git.tag
      - deploy:
          requires:
            - build
          filters: pipeline.git.branch or pipeline.git.tag matches /^v.*/
----
...
workflows:
  build-test-deploy:
    jobs:
      - build:
          filters: pipeline.git.branch or pipeline.git.tag matches /^config-test.*/
      - test:
          requires:
            - build
          filters: pipeline.git.branch or pipeline.git.tag matches /^config-test.*/
      - deploy:
          requires:
            - test
          filters: pipeline.git.tag matches /^config-test.*/
----
