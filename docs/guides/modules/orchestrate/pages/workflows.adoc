...
stateDiagram-v2
    ...
    state "Approval Required" as on_hold {
        [*] --> waiting_approval
        waiting_approval --> approved : approval job
        approved --> [*]
    }

    state "Serial Queue" as queued {
        [*] --> waiting_queue
        waiting_queue --> ready : queued as part of a serial group
        ready --> [*]
    }

    state "Failure Path" as failing {
        [*] --> failing_state
        failing_state --> failed
        failing_state --> canceled
    }
