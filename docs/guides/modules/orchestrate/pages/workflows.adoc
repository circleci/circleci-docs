...
Schedule workflows to run your jobs at specific times rather than on every commit. Scheduling is useful for resource-intensive jobs, nightly builds, or generating reports on a regular cadence without triggering on every code change.

NOTE: Scheduled workflows are not available for GitHub App, GitLab or Bitbucket Data Center pipelines.

IMPORTANT: For new scheduled pipelines, use xref:schedule-triggers.adoc[schedule triggers] instead of scheduled workflows. Schedule triggers offer more control (restricted contexts, dynamic config support, auto-cancel, and schedule changes without editing `.circleci/config.yml`). Scheduled workflows are documented here for reference and for teams that cannot yet migrate. See the xref:migrate-scheduled-workflows-to-schedule-triggers.adoc[Migration Guide] to move to schedule triggers.
