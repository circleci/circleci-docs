...
workflows:
  ...
  nightly: `
    triggers:
      - schedule:
          cron: "0 0 * * *"
          filters: pipeline.git.branch == "main" or pipeline.git.branch matches /^release\/.*/ # only run the nightly workflow on the main branch and any branch that starts with `release/`
    jobs:
      - coverage
----
...
----
    triggers:
      - schedule:
          ...
          filters: pipeline.git.branch == "main" # only run the workflow on the main branch
----
...
----
    triggers:
      - schedule:
          cron: "5 4 * * 1,3,4,5,6"
          filters:
            pipeline.git.branch == "main" # only run the workflow on the main branch
----
...
workflows:
  dev_stage_pre-prod:
    jobs:
      ...
      - test_stage:
          filters: pipeline.git.branch == "stage" # only run the workflow on the stage branch
      - test_pre-prod:
          filters: pipeline.git.branch matches /^pre-prod(?:-.+)?$/ # only run the workflow on any branch that starts with `pre-prod` including any suffix added to the branch name using a hyphen
----
...
workflows:
  ...
  tagged-build:
    jobs:
      - build:
          filters: pipeline.git.tag matches /^v.*/ # only run the workflow on any tag that starts with `v`
----
...
workflows:
  build-deploy:
    jobs:
      - build:
          filters: pipeline.git.branch or pipeline.git.tag # only run the workflow on any branch or tag
      - deploy:
          requires:
            - build
          filters: pipeline.git.branch or pipeline.git.tag matches /^v.*/ # only run the workflow on any branch or tag that starts with `v`
----
...
workflows:
  build-test-deploy:
    jobs:
      - build:
          filters: pipeline.git.branch or pipeline.git.tag matches /^config-test.*/ # only run the workflow on any branch or tag that starts with `config-test`
      - test:
          requires:
            - build
          filters: pipeline.git.branch or pipeline.git.tag matches /^config-test.*/ # only run the workflow on any branch or tag that starts with `config-test`
      - deploy:
          requires:
            - test
          filters: pipeline.git.tag matches /^config-test.*/ # only run the workflow on any tag that starts with `config-test`
----
...
workflows:
  build: # This workflow will run on all branches except 'main' and will not run on tags
    jobs:
      - test:
          filters: pipeline.git.branch != main # only run the workflow on any branch that is not the main branch
  staging: # This workflow will only run on 'main' and will not run on tags
    jobs:
      - test:
          filters: pipeline.git.branch == "main" &filters-staging # this yaml anchor is setting these values to "filters-staging"
      - deploy:
          requires:
            - test
          filters:
  ...
  production: # This workflow will only run on tags (specifically starting with 'v.') and will not run on branches
    jobs:
      - test:
          filters: pipeline.git.tag matches /^v.*/ &filters-production # this yaml anchor is setting these values to "filters-production"
      - deploy:
          requires:
            - test
          filters:
