...
TIP: For a full specification of the `workflows` key, see the xref:reference:ROOT:configuration-reference.adoc#workflows[Workflows] section of the configuration reference.
...
image::guides:ROOT:sequential_workflow.png[Sequential Job Execution Workflow]
...
image::guides:ROOT:fan-out-in.png[Fan-out and Fan-in Workflow]
...
image::guides:ROOT:workflow-using-requires-example-1.png[Flexible Job Execution Workflow - Example 1]
...
* Jobs downstream of an `approval` job can be restricted by adding a xref:security:contexts.adoc#approve-jobs-that-use-restricted-contexts[Restricted Context] to those downstream jobs.
...
.Workflow graph with "Needs approval" job, the approval popup, and the workflow graph
image::guides:ROOT:approval-workflow-map.png[A three section image showing workflow graph with "Needs approval" job, the approval popup, and the workflow graph]
...
. You need your workflow ID and the `approval_request_id` (which is the job ID for the job requiring approval) to make the API POST request. Depending on your use case, you might be configuring an approval job in a pipeline, or using an external service to approve a job. You can get the required parameters using xref:reference:ROOT:variables.adoc[Pipeline Values or Environment Variables], or by receiving xref:integration:outbound-webhooks.adoc[Webhooks] or polling the API. To approve a job, use the link:https://circleci.com/docs/api/v2/index.html#operation/approvePendingApprovalJobById[approve a job] endpoint, for example:
+
[,shell]
----
curl --request POST \
...
--
. Set up your API authentication. Steps are available in the xref:toolkit:api-developers-guide.adoc#add-an-api-token[API Developers Guide].
. You need the workflow ID so you can use the link:https://circleci.com/docs/api/v2/index.html#operation/listWorkflowJobs[get a workflow's jobs] endpoint to get the user ID. You can get the workflow ID using xref:reference:ROOT:variables.adoc[Pipeline Values or Environment Variables], or by receiving xref:integration:outbound-webhooks.adoc[Webhooks] or link:https://circleci.com/docs/api/v2/index.html#tag/Pipeline/operation/listWorkflowsByPipelineId[polling the API].
+
[,shell]
----
curl --request GET \
...
. In the JSON response, take note of the value associated with the `approved_by` key. The value is the user ID of the user who approved the job.
+
[,json]
----
 "name" : "hold",
...
IMPORTANT: For new scheduled pipelines, use xref:schedule-triggers.adoc[Schedule Triggers] instead of scheduled workflows. Schedule triggers offer improved control. Scheduled workflows are documented here for reference and for teams that cannot yet migrate to schedule triggers. See the xref:migrate-scheduled-workflows-to-schedule-triggers.adoc[Migration Guide] to move to schedule triggers.
...
By default, a workflow runs on every push, or based on the event rules you have set up for your trigger (menu:Project settings[Triggers] or menu:Project settings[Project Setup]). To trigger a workflow on a schedule, add the `triggers` key to the workflow and specify a `schedule` including cron syntax and branch filters. Scheduled workflows use the `cron` syntax to represent Coordinated Universal Time (UTC). See <<nightly-example>> for an basic example.

Scheduled workflows are *not* available for GitHub App or GitLab pipelines.
...
For more details, see the `branches` section of the xref:reference:ROOT:configuration-reference.adoc#schedule-branches[Configuration Reference].
...
Scheduled workflows do not support xref:dynamic-config.adoc[Dynamic Configuration]. If you use dynamic configuration you will need to use xref:schedule-triggers.adoc[Schedule Triggers]. More information can be found in this link:https://support.circleci.com/hc/en-us/articles/360060833331-Support-for-Scheduled-Workflows-in-Dynamic-Configurations-Setup-Workflows[support article].

=== Scheduled workflows and billing

When you schedule a workflow, the workflow will be counted as an individual active user. For more information, see the xref:plans-pricing:user-types-and-registration.adoc#active-users[User Types and Registration] guide.
...
The following example shows a workflow with four sequential jobs that each use a context to access environment variables. See the xref:security:contexts.adoc[Contexts] page for detailed instructions on this setting in the application.
...
The `test1` and `test2` jobs access environment variables in the `org-global` context when the pipeline meets every restriction type set on the context. Security groups, project restrictions, and expression restrictions combine with logical AND. For details, see the xref:security:contexts.adoc#how-restrictions-combine[How Restrictions Combine] section.

* Was the pipeline triggered by a user who has access (is in the relevant org/security group etc.)? See the xref:security:contexts.adoc#security-group-restrictions[Security Group Restrictions] page for more information.
* Does the project have access to the context? By default all projects in an organization have access to contexts set for that organization, but restrictions on project access can be configured. See the xref:security:contexts.adoc#project-restrictions[Project Restrictions] guide for more information.
* Does the pipeline meet the requirements of any expression restrictions set up for the context? See the xref:security:contexts.adoc#expression-restrictions[Expression Restrictions] guide for more information.

== Use conditional logic in workflows

You may use a `when` clause (the inverse clause `unless` is also supported) under a workflow declaration with xref:reference:ROOT:configuration-reference.adoc#using-expressions-in-your-configuration[Expression Syntax] to determine whether or not to run that workflow.
...
* xref:set-up-multiple-configuration-files-for-a-project.adoc[Set Up Multiple Configuration Files for a Project].
* xref:github-trigger-event-options.adoc[GitHub Trigger Event Options].
****
...
NOTE: Workflows ignore `branches` keys used in the `jobs` declaration. If you use the deprecated xref:reference:ROOT:configuration-reference.adoc#branches[Job-level Branches Key], replace them with workflow filters.
...
workflows:
  dev_stage_pre-prod:
    jobs:
      - test_dev:
          filters: pipeline.git.branch == "dev" or pipeline.git.branch matches /^user-.*/ # using regex requires the entire branch to match
      - test_stage:
          filters: pipeline.git.branch == "stage" # only run the test_stage job on the stage branch
      - test_pre-prod:
          filters: pipeline.git.branch matches /^pre-prod(?:-.+)?$/ # only run the test_pre-prod job on any branch that matches the pattern
----

This setup can be illustrated as follows:

.Branch-Level Job Execution
image::guides:ROOT:branch_level.png[Branch-Level Job Execution]
...
workflows:
  ...
  tagged-build:
    jobs:
      - build:
          filters: pipeline.git.tag matches /^v.*/ # only run the build job on tags starting with `v`
----
...
workflows:
  build-deploy:
    jobs:
      - build:
          filters: pipeline.git.branch or pipeline.git.tag # run the build job on every branch and every tag
      - deploy:
          requires:
            - build
          filters: pipeline.git.branch or (pipeline.git.tag matches /^v.*/) # run the deploy job on every branch, and on tags starting with `v`
----
...
workflows:
  build-test-deploy:
    jobs:
      - build:
          filters: pipeline.git.branch or (pipeline.git.tag matches /^config-test.*/) # run the build job on every branch, and on tags starting with `config-test`
      - test:
          requires:
            - build
          filters: pipeline.git.branch or (pipeline.git.tag matches /^config-test.*/) # run the test job on every branch, and on tags starting with `config-test`
      - deploy:
          requires:
            - test
          filters: pipeline.git.tag matches /^config-test.*/ # only run the deploy job on tags starting with `config-test`
----
...
workflows:
  build: # This workflow will run on all branches except 'main' and will not run on tags
    jobs:
      - test:
          filters: pipeline.git.branch != "main" # only run the test job on any branch other than main
  staging: # This workflow will only run on 'main' and will not run on tags
    jobs:
      - test:
          filters: &filters-staging pipeline.git.branch == "main" # this yaml anchor is setting these values to "filters-staging"
      - deploy:
          requires:
            - test
          filters: *filters-staging # this is calling the previously set yaml anchor
  production: # This workflow will only run on tags (specifically starting with 'v.') and will not run on branches
    jobs:
      - test:
          filters: &filters-production pipeline.git.tag matches /^v.*/ # this yaml anchor is setting these values to "filters-production"
      - deploy:
          requires:
            - test
          filters: *filters-production # this is calling the previously set yaml anchor
----
...
Each workflow has an associated xref:reference:ROOT:configuration-reference.adoc#persisttoworkspace[Workspace] for transferring files to downstream jobs as a workflow progresses.

Configuration options are available to:

* Persist files to the workspace using the xref:reference:ROOT:configuration-reference.adoc#persisttoworkspace[Workspace] key.
+
[,yaml]
----
...
* Attach a workflow's xref:reference:ROOT:configuration-reference.adoc#attachworkspace[Workspace] to a container.
+
[,yaml]
----
- attach_workspace:
...
For further information on workspaces and their configuration see the xref:workspaces.adoc[Using Workspaces to Share Data Between Jobs] doc.
...
+
--
.Rerun a workflow from failed from the pipelines page
...
+
--
.Rerun a workflow from failed from the workflows page
...
NOTE: If you rerun a workflow containing a job that was previously re-run with SSH, the new workflow runs with SSH enabled for that job. This applies even after SSH capability is disabled at the project level.
...
| QUEUED
| The workflow is queued due to being part of a serial group. For more information, see the xref:controlling-serial-execution-across-your-organization.adoc[Controlling Serial Execution Across Your Organization] guide.
| No

|===
--

NOTE: After 90 days non-terminal workflows are automatically canceled by CircleCI.
