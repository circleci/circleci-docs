...
By default, a workflow runs on every push, or based on the event rules you have set up for your trigger (menu:Project settings[Triggers] or menu:Project settings[Project Setup]). To trigger a workflow on a schedule, add the `triggers` key to the workflow and specify a `schedule` includingh CRON syntax and branch filters. Scheduled workflows use the `cron` syntax to represent Coordinated Universal Time (UTC). See <<nightly-example>> for an basic example.
...
You may use a `when` clause (the inverse clause `unless` is also supported) under a workflow declaration with a xref:reference:ROOT:configuration-reference.adoc#logic-statements[Logic Statements] to determine whether or not to run that workflow.
