...
You can use flexible requires to run a specific job only when the previous job is cancelled. This is useful for notifications, cleanup tasks, or logging when a deployment or long-running job is manually cancelled.
