...
You may use a `when` clause (the inverse clause `unless` is also supported) under a workflow declaration with xref:reference:ROOT:configuration-reference.adoc#using-expressions-in-your-configuration[expressions] to determine whether or not to run that workflow.
