...
For a concise order of operations during an incident (cancel running work, block new pipelines, stop building, and coordinating followers), see xref:orchestrate:pause-pipelines-during-an-incident.adoc[Pause or stop pipelines during an incident].
...
NOTE: **Block all new work** prevents new pipelines (and reruns) at CircleCI while your VCS integration remains. If you need to fully disconnect a **GitHub OAuth** or **Bitbucket Cloud** project—removing the webhook and deploy key from the repository—see xref:security:stop-building-a-project-on-circleci.adoc[Stop building a project on CircleCI].
