...
* Update your CircleCI config to include a workflow that will only run for specific branches. You can configure the workflow using an expression.
+
.The nightly-run-workflow only runs on the main and staging branches
[,yaml]
----
...
* You can also choose to prevent the workflow from running for specific branches using the `not` operator with `matches` for pattern matching:
+
[,yaml]
----
include::ROOT:example$expression-examples/workflow-when/exclude-branch-dynamic.yml[]
----
...
. Update your workflow config to use a when statement so the workflow will only run when the pipeline parameter is true.
+
[,yaml]
----
include::ROOT:example$expression-examples/workflow-when/boolean-dynamic-syntax.yml[]
----
...
[,yaml]
----
include::ROOT:example$expression-examples/workflow-when/boolean-short-syntax.yml[]
----

* You can create complex expressions that involve multiple parameters using `and`, `or`, and parentheses for grouping:
+
[,yaml]
----
include::ROOT:example$expression-examples/workflow-when/boolean-complex.yml[]
----
...
+
.The my-workflow workflow runs when: the event is a push or a pull request that is not a draft or the deploy parameter is true.
[,yaml]
----
workflows:
  my-workflow:
...
== Run a step based on a nested condition

=== Goal

You may want to configure a step that will run depending on the output of a nested condition using logic statement syntax.

NOTE: Expressions are not supported for step-level `when` conditions. Use logic statement syntax for the `condition` field of xref:reference:ROOT:configuration-reference.adoc#the-when-step[The When Step]. For workflow-level conditions, you can use xref:reference:ROOT:configuration-reference.adoc#using-expressions-in-your-configuration[Expression Syntax] instead.

=== Steps

* Update your CircleCI config to use xref:reference:ROOT:configuration-reference.adoc#the-when-step[The When Step] with a nested logic statement.
+
[,yaml]
----
include::ROOT:example$logic-statement-examples/step-when/when-step-nested.yml[]
----

=== Notes

* Logic statements can be nested to a maximum depth of 100 levels.
...
* Update your CircleCI config to use the `pipeline.trigger.type` and `pipeline.schedule.name` xref:pipeline-variables.adoc#pipeline-values[Pipeline Values].
+
.The nightly-run-workflow only runs when the trigger is a schedule trigger AND the name of that trigger is `nightly_build`
[,yaml]
----
include::ROOT:example$expression-examples/workflow-when/and-equal-dynamic.yml[]
----
...
[,yaml]
----
include::ROOT:example$expression-examples/workflow-when/pr-synced-or-labeled.yml[]
----

=== Notes

* The `pipeline.event.name` and `pipeline.event.action` variables are not available for GitLab pipelines.
