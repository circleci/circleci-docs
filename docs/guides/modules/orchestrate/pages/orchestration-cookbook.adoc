...
You may want to configure a workflow that runs when a specific event occurs (such as a push or pull request) or when a pipeline parameter is set to `true`. To combine pipeline parameters with event filters in a workflow `when` clause, you need to use filter expression syntax without template variables.
...
. Update your workflow `when` clause to use pipeline parameters directly in filter expressions (without template variable syntax `<< >>`):
+
[,yaml]
----
workflows:
...
* When using pipeline parameters in filter expressions, use the parameter name directly (for example, `pipeline.parameters.deploy`) without template variable syntax (`<< >>`). Replace `deploy` with your own parameter name.
* Do not mix template variable syntax with filter expressions. If a `when` clause contains template variables (`<< >>`), the entire clause is treated as a template replacement rather than a filter expression, which causes the workflow to always be created regardless of the actual condition.
+
[source,yaml]
----
...
----
+
* For simple parameter checks, you can use template variable syntax alone. Remember to replace `deploy` with your own parameter name:
+
[source,yaml]
----
workflows:
...
----
+
* For more details, see the xref:reference:ROOT:configuration-reference.adoc#using-when-in-workflows[Using `when` in workflows] section in the configuration reference.
