...
For example, consider a project with a `build` job that builds a `.jar` file and saves it to a workspace. The `build` job fans-out into concurrently running test jobs: `integration-test`, `unit-test`, and `code-coverage`, each of which can have access to the jar by attaching the workspace.
...
.Screenshot showing the workspaces data flow
image::guides:ROOT:workspaces.png[workspaces data flow]
...
When using self-hosted runners, there is a network and storage usage limit included in your plan. Some actions related to workspaces will accrue network and storage usage. Once your usage exceeds your limit, charges will apply.
...
You can customize storage usage retention periods for workspaces on the link:https://app.circleci.com/[CircleCI web app] by navigating to menu:Plan[Usage Controls]. For information on managing network and storage usage, see the xref:optimize:persist-data.adoc#managing-network-and-storage-usage[Persisting Data] page.
...
* For conceptual and usage information on Workflows, see the xref:workflows.adoc[Using Workflows to Orchestrate Jobs] page.
* xref:optimize:persist-data.adoc[Persisting Data]
* xref:optimize:caching.adoc[Caching Dependencies]
* xref:optimize:caching-strategy.adoc[Caching Strategies]
* xref:optimize:artifacts.adoc[Artifacts]
* xref:optimize:optimizations.adoc[Optimizations Overview]
