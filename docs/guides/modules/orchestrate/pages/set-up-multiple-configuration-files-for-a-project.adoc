...
A CircleCI project integrated with GitHub App or Cursor Origin can have one or more pipelines, each defined by a separate configuration file. CircleCI configuration files can be stored in the same repository as your code, or in a different repository.
...
For a full list, see the xref:github-trigger-event-options.adoc[GitHub Trigger Event Options] and xref:cursor-origin-trigger-event-options.adoc[Cursor Origin Trigger Event Options] pages.
...
* A CircleCI account connected to GitHub App or Cursor Origin. You can link:https://circleci.com/signup/[sign up for free]. Cursor Origin is in beta. GitHub OAuth projects cannot use multiple configuration files. See the xref:permissions-authentication:users-organizations-and-integrations-guide.adoc[Users, Organizations, and Integrations Guide] for more information.
...
In this first step, create the configs you need to build your project. Config files can be stored anywhere you choose within your GitHub organization or Cursor Origin codebase. This can be in the same repository, or a different repository. Your configuration files can have any name you choose, but must have the `.yml` extension.
...
Next, add a new pipeline for each config you have. Create and edit a pipeline in your menu:Project Settings[Project Setup].
...
. Following from the previous step, select btn:[GitHub trigger +] or btn:[Cursor Origin trigger +] at the bottom of the pipeline box. Choose the button that matches your integration.
