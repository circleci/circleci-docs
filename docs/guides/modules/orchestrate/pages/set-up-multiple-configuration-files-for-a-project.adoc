...
NOTE: **Splitting up an existing configuration?** If you have an existing project building on CircleCI and want to split your configuration into multiple files, create the config files you need. Then, move over parts of your existing configuration file to your new YAML files.
...
** Specify where your config file is located using the *Config File Path* field.
...
. Select the *Event Source* repository from the dropdown. This defaults to the config source repository. If you change it to a repository that does not match the pipeline's config or checkout source, you are prompted to specify a fixed branch. This branch is used for fetching config and checking out code.
