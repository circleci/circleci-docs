...
.Workspaces workflow illustration
...
For `circleci` type organizations, roles are set at the organization and project level. These roles are separate from permissions and roles in the version control system where your code is stored. The available roles are:
