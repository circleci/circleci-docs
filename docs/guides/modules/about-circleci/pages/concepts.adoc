...
An event is the inbound occurrence that prompts CircleCI to act — a push to a branch, a pull request opened, a scheduled time, or an API call. Events are the starting point of the entity chain: when CircleCI receives an event, it checks whether any <<triggers,triggers>> match and, if so, initiates a <<runs,run>>.
...
For GitHub App, GitLab SaaS and self-managed, Bitbucket Data Center, and Cursor Origin users, a _project_ in CircleCI is standalone. You create a project and then connect your code (stored in your GitHub, GitLab, Bitbucket Data Center, or Cursor Origin repository) to that project.
...
You can enable auto-cancel in *Advanced* project settings in the web app to automatically terminate an in-progress run when a new one is triggered on the same branch.

NOTE: Consider the impact of enabling auto-cancel if you have configured automated deployment jobs on non-default branches.
...
Triggers can respond to events from VCS activity such as pushes and pull requests, scheduled times, API calls, or actions in the web app or CLI. You can also initiate a run programmatically using the link:https://circleci.com/docs/api/v2/#operation/triggerPipeline[trigger a pipeline] API endpoint.
...
=== GitHub App, GitLab, Bitbucket Data Center, and Cursor Origin users
