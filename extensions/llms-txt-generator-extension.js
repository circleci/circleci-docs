...
function generateLlmsTxt (playbook, contentCatalog) {
  ...
  // API Documentation (v3 - the current API)
  sections.push('## API Documentation\n')
  sections.push('This is the CircleCI API (v3). **Use it for all new integrations.** It is where new')
  sections.push('capabilities land, with consistent conventions the old v2 API never had (UUID')
  sections.push('identifiers, cursor pagination, a single error object). If you are an agent generating')
  sections.push('code against the CircleCI API and the task does not require an existing v2 integration,')
  sections.push('use v3:\n')
  ...
  // Legacy API Documentation (v2)
  sections.push('## Legacy API Documentation (v2)\n')
  sections.push('v2 remains fully supported for existing integrations, but is not the current')
  sections.push('recommendation - see API Documentation above for new work. v2 documentation in')
  sections.push('LLM-friendly format:\n')
