...
NOTE: `shared-config.yml` is a partial configuration and contains only shared parameters and jobs, not workflows. This is because these jobs will be used along with either the `code-config.yml` or `docs-config.yml` configs, both of which include workflows that orchestrate these shared jobs. Using this kind of set up you can include anything that should _always_ run in the shared configuration, including workflows that should always run.
