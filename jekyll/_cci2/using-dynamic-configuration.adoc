...
NOTE: `shared-config.yml` is designed to be used either on it's own or along with other partial configurations. `code-config.yml` or `docs-config.yml` include workflows that orchestrate the shared jobs defined in `shared-config.yml`. Using this kind of set up you can include anything that should _always_ run in the shared configuration.
