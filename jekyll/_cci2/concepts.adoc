...
CircleCI roles or user types are set up differently depending on how you xref:version-control-system-integration-overview#[integrate your code].
...
Roles are set at the organization and project level and are separate to permissions and roles set in the version control system where your code is stored. The available roles are:
