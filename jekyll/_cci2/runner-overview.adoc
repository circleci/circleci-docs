...
When setting up self-hosted runner, you have the option of private Kubernetes clusters with container runner, a virtual machine execution environment with machine runner, as well as a Docker executor. The diagrams below illustrate how CircleCI's self-hosted container runner and machine runner extends our existing systems.
...
Machine runner runs jobs in a virtual machine (VM), launched in the same environment as the self-hosted runner binary. CircleCI's machine runner can be run in a VM on Linux, Windows, or macOS. Machine runner maps every job to a VM, so it is a solution if you have fewer jobs that need to be run, and do not need to scale.
...
The Docker executor is a container solution for projects that do not require much overhead, or for when more manual control over scaling is desired. 

The Docker executor for self-hosted runner is not compatible with CircleCI's convienience images or custom Docker images.
