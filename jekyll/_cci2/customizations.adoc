...
+
WARNING: It is important to use the IDs listen in the example below for the `machine` resources classes. IDs can, however, be changed for Docker resource classes.
. Run:
+
```shell
...
A second machine type (`large` in the example above) was introduced in CircleCI v2.18. A further step is required to enable this for your organization. Following are the required steps:
