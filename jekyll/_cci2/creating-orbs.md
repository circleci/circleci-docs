...
* By default, any orb created outside of the `circleci` namespace is an "uncertified" orb. To use uncertified orbs, you will need to enable the "Allow Uncertified Orbs" setting in the Security section of CircleCI's Organization Settings page for your organization (`https://circleci.com/[vcs]/organizations/[org-name]/settings#security`).
* If you are creating an orb, you can use [Development Versions](#using-development-versions) to avoid having your orb publicly/permanently listed in CircleCI's orb registry before it is ready.
