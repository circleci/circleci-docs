...
The following tables show feature support per VC for **CircleCI cloud**.

For **CircleCI server**, see the tags on the individual feature pages. On the whole features supported by GitHub OAuth are supported for CircleCI server.
