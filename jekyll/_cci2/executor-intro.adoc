...
When you configure an executor it is recommended that you also specify a resource class to Specify compute resource size (CPU and RAM). For an overview of available resource classes per Plan, see the link:https://circleci.com/pricing/[Resource class web page].
