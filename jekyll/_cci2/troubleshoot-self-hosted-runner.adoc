...
[#This job was claimed but has not started in over 10 minutes]
=== This job was claimed but has not started in over 10 minutes

If you see the message "This job was claimed but has not started in over 10 minutes" on the job’s page or if there is no content in the Task lifecycle step.

.Task lifecycle with no content
image::task-lifecycle.png[Screenshot showing the task lifecycle with no content.]

You should consider the potential causes below:

-   **Pod restart**  
    Check if there were any container agent pod restarts around the time the workflow ran. If the pod was restarted around that time it would have resulted in the job not being processed. In such a case, we recommend rerunning the job once again. 
...
-   **Network connectivity issue**
    Check the network connectivity of the container agent, especially if the issue is intermittent. The issue can be seen when the container agent has lost network connectivity after claiming the tasks.  
...
We also recommend checking the connection to the links on our FAQ includes a section about the [connectivity required for CircleCI’s self-hosted runners](https://circleci.com/docs/runner-faqs/#what-connectivity-is-required).

-   **Resources exhaustion**
    Check if your pods are reaching their resources limits in the cluster, as the pod could end the job to free up resources.  
    We recommend setting resource limits either within your [values.yaml](https://kubernetes.io/docs/concepts/configuration/manage-resources-containers/) or within your [config.yaml](https://circleci.com/docs/container-runner/#resource-class-configuration-custom-pod).  
    There are also external tools for monitoring resource usage on the [Kubernetes documentation](https://kubernetes.io/docs/tasks/debug/debug-cluster/resource-usage-monitoring/).
