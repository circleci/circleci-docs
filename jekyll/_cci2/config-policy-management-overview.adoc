...
If you already have a link:/docs/first-steps[CircleCI account] hooked up with your VCS, and you would like to get started with config policy management right away, check out the steps in the link:/docs/use-the-cli-and-vcs-for-config-policy-management/#create-and-upload-a-policy[Use the CLI and VCS for Config Policy Management] how-to guide.
