...
The following is an example of a policy that blocks SSH reruns on sensitive branches.
...
disallow_ssh_rerun = "SSH rerun is disallowed if any job is accessing sensitive contexts" {
    data.meta.ssh_rerun
    some wf_name, workflow in input.workflows
    some job_name, job_info in workflow.jobs[_]
    count(utils.to_set(job_info.context) & sensitive_contexts) > 0
}
----
