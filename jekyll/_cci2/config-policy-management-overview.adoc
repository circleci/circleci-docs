...
CAUTION: **Using server?** The ability to write policies that inspect compiles config (`input._compiled_`) discussed in this section is currently unsupported on CircleCI server
...
This is problematic becaise once the configuration is compiled, the job `test` will have a `resource_class == "large"`.
