...
Writing policies for CircleCI `version 2.1` configuration introduces some challenges due to the parameterizable and reusable configuration options. To read more about reusable configuration, see the xref:reusing-config#[Reusable config reference guide].

Before executing any pipelines, config `version 2.1` is compiled into config `version 2.0`. This compilation expands all parameters and reusable config blocks (executors, commands, orbs) into workflows and jobs.

To write highly effective policies, it is essential to reference the compiled version of the config as `input._compiled_` in your policies.
...
In the above example, the policy would **not trigger**. The policy inspects `jobs` and does not find a `resource_class == "large"`. The policy, in this case, is bypassed.

However, once the config is compiled, the job test will have a resource_class of large.

Another way this policy could be unintentionally bypassed is by using parameters. Consider the following `config.yml`, which uses a parameter to set the resource class for an executor:
...
To resolve both of these issues, it is important to acknowledge that we want to apply the policy to all jobs, which is a configuration `version: 2.0` construct, and write the policy to target the compiled version accordingly.
...
Notice that we are now validating `input._compiled_.jobs`. Regardless of parameters or reusable blocks (executors in this example), the policy is applied to all compiled jobs, and functions as intended.

[#policing-2.1-constructs]
=== Policing config 2.1 constructs

Often, we need to write policies . Writing policies against config version 2.1 constructs (orbs, executors, and commands) introduces the same parameterization challenges as described in the previous section.

However, we cannot rely on writing policies against the compiled input because these constructs _do not exist_ in configuration version 2.0.
...
The best approach here is to detect if an orb reference is a parameterized expression and raise a violation accordingly.
...
* **Banlists**: Assert that the input must not match a particular value or be within a set of prohibited values
...
The rule (`restrict_resource_classes`) raises a violation because `<< parameters.size >>` does not conform to the allowlist values of small or large. It is worth noting that even if this configuration would compile to a job that uses the correct `small` resource class, the violation is still triggered.
