...
Next, set the `circleci-launch-agent` version. On cloud, we select the latest version by
querying for our releases for it and on server, we must select the version
appropriate for our version of server. A table of server `circleci-launch-agent` versions
