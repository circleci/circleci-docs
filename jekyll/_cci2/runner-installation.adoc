...
Currently, the following platforms are supported with CircleCI Runner:
...
CAUTION: Running in a container is not currently supported.

==== Authentication

In order to complete this process you will need to create a namespace, resource-class and authentication token by performing the steps listed below:
