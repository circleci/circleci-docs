...
Next, set the launch-agent version. On cloud, we select the latest version by
querying for our releases for it and on server we must select the version
appropriate for our version of server. A table of server launch-agent versions
can be found https://circleci.com/docs/2.0/runner-server/[here].
...
# On server (using launch agent for server 3.1 for example)
