...
Replace `AUTH_TOKEN` with the token created in the <<#authentication,Authentication step>>. `RUNNER_NAME` can be anything you'd like though should be unique among all other runners used by the same resource class.
...
In some cases it may be required to fully remove a stopped Runner container from the system, such as when recreating the container using the same name.
