...
Unlike with the CircleCI cloud offering, server does not automatically update the `circleci-launch-agent` and requires manual intervention to update.
 
The process for manually updating the `circleci-launch-agent` binary can be broken down into three steps
...
==== Upgrading Runners for Kubernetes and Docker

When updating the `circleci-launch-agent` in a containerized environment that uses the CirlceCI Runner docker image, the container only needs to be restarted to update the `circleci-launch-agent`

==== Installing New `circleci-launch-agent-binary`

Following the same process to [[download]] the `circleci-launch-agent` binary, update the `agent_version` variable to the new version of the `circleci-launch-agent` to be installed

```bash
export agent_version="<launch-agent-version>"
```

Then following the installation instructions replace the old binary with the newly updated version.
