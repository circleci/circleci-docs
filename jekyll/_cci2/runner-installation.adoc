...
This page describes how to install **machine self-hosted runners** through the CircleCI web app (container runner coming soon). If you are interested in container runner on Kubernetes, please see the <<container-runner#,Container runner>> page. If you are interested in self-hosted runner on server, see the <<runner-installation-cli#,Self-hosted runners with the CLI>> page.
