...
+
. If this is your organization's first time using self-hosted runners, you'll need to create a namespace as the first part of this step. *Each organization can only create a single namespace*. While not required, we suggest using a lowercase representation of your CircleCI account name.  The UI will populate by default your org name as the suggested namespace.

. If your organization already use orbs, this namespace should be the same namespace orbs use.  Use the namespace that is used for orbs instead of a brand new one.  
 
. The second part of this step is to create a name for your resource class for your organization's self-hosted runners. 
