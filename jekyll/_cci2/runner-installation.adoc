...
When starting the docker container on server the `agent_version` environment variable will need to be passed in using the `--env` flag as well.
...
To upgrade Runner in a Docker environment, stop any currently running Runner containers and restart them following the <<Docker Installation, Docker>> instructions, using the `--env` flag to specify the new `circleci-launch-agent` version to pin.
