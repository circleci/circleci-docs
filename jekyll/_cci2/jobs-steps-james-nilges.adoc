...
The job names shown in this diagram are just examples. You can name your jobs anything you want.
...
See the xref:executor-intro#[Introduction to Execution Environments] document for a comparison of the different executor types and links to more information about each option.

[#steps-overview]
== Steps overview

Steps are collections of executable commands, that run during a job.

The `checkout` key is required under `steps` to checkout your code. The `run` key enables the addition of arbitrary, multi-line shell command scripting.

In addition to the `run` key, keys for `save_cache`, `restore_cache`, `store_artifacts`, `store_test_results`, and `add_ssh_keys` are nested under steps. For a full list of the step options see the xref:configuration-reference#steps[Steps key] section in xref:configuration-reference#[Configuration Reference].
...
Orbs are packages or reusable configurations that you can use in your projects. Orbs usually contain commands that you can use in your jobs, as well as whole jobs that you can schedule in your workflows.
