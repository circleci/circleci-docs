...
- Server Admin
---
= CircleCI Server v3.x Install - Prerequisites
...
* An existing Kubernetes cluster (see xref:server-3-install-creating-your-first-cluster[our guide] if you need help creating one), for example:
...
*** Your cluster must have outbound access to pull Docker containers and verify your license. If you don’t want to give open outbound access, see our https://help.replicated.com/community/t/customer-firewalls/55[list of ports] that will need access.
...
** Note that the `kubectl` tool must also be configured to have access your cluster
...
** xref:server-3-install-hardening-your-cluster.adoc#kubernetes-nodes[Kubernetes Loads]
