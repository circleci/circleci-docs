...
Configure triggers under menu:Project settings[Triggers]. You can configure triggers to initiate pipelines on a variety of events

* Non-push events, for example, a pull request being opened
* Push events, for example, only pushes to a default branch, or pushes to a tag.

For a full list see <<supported-trigger-options>>.
...
If you have a **GitHub App** account (see tip above) with CircleCI, you will see trigger options when creating a project in the CircleCI web app, and you have the option to change the trigger options for your first trigger. You can also set up additional triggers during project creation, or setup/edit later in your menu:Project settings[Triggers].
...
If you have a **GitHub OAuth** integration (see tip above) with CircleCI, an org admin can install the CircleCI GitHub App to enable GitHub app pipeline functionality, including the trigger options described on this page. Installing the CircleCI GitHub App is a one-time action. For more information, see the xref:github-integration#installing-github-app-alongside-github-oauth-app[GitHub OAuth integration overview].
