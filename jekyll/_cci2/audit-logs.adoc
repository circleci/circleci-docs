...
CircleCI customers can access the audit log feature from the UI in the link:https://app.circleci.com/[web app]. Go to the **Audit Log** page within the **Admin** section of the application to download audit logs as a CSV file. Audit log fields with nested data contain JSON blobs.
