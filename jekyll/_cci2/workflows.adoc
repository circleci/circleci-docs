...
This example shows a workflow with four sequential jobs. Each job waits until the "required" job attains a specified status.

This configuration snippet is an example of a workflow configured for flexible sequential jobs:
...
A job must wait until all upstream jobs in the dependency graph have attained either of the statuses specified under the corresponding `requires` key. In this example, the `deploy` job runs when the `build` job completes successfully:
