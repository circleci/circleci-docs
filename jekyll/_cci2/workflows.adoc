...
NOTE: After 90 days non-terminal workflows will are automatically by CircleCI.
...
To run a set of concurrent jobs, add a new `workflows:` section to your existing CircleCI configuration file. Give the workflow a unique name.
...
* Nest the `jobs` key with a list of job names that are defined in the configuration file. In this example the jobs have no dependencies defined, so they will run concurrently.
...
Use an `approval` job to configure a workflow to wait for manual approval before continuing. Anyone who has push access to the repository can approve the job to continue the workflow. Approve either by using the *Approve* button in the CircleCI web app, or via the API, to continue the workflow.
...
* The name of the approval job is arbitrary. For example, the name could be `wait` or `pause`.
...
Scheduling a workflow will prevent commits to your code from triggering the workflow for the specified branches, and instead the workflow will run on a schedule.
...
Workflows can be configured increase your ability to rapidly respond to failures. To rerun only a workflow's _failed_ jobs, follow these steps:
...
If you do not see your workflows trigger, a common cause is a configuration error preventing the workflow from starting. As a result, the workflow does not start any jobs. Navigate to your project's pipelines and select your workflow name to discern what might be failing.
...
NOTE: You cannot rerun jobs and workflows that are 90 days or older.
