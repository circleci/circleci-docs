...
* `hold` is a special job type that is configured when listing jobs under the `workflow` key. You do not need to define a `hold` type job in the `jobs` section of your configuration. If you do configure steps for a job that is given the `hold` type in the workflows section, the steps for that job will not be run. A `hold` job is _only_ used to hold the workflow for approval, not to run any work.
