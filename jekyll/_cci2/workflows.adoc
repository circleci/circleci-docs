...
* `approval` is a special job type that is configured when listing jobs under the `workflow` key.
* The approval job name must be a unique and not used by any other job in your configuration. For example, your configured jobs, such as `build` or `test1` in the example below, would not be set as `type: approval`.
...
A valid `schedule` requires a `cron` key and a `filters` key and you must specify which `branches` to run the workflow on.
...
You can filter workflows by branch, git tag, both, or neither. Workflow filters for branches and tags have the keys `only` and `ignore`:
...
If **both branch and tag** filtering is configured, the **branch** filters take precedence.

The following table outlines what will happen for different tag/branch filter configurations:

[.table.table-striped]
[cols=7*, options="header", stripes=even]
|===
|Push to tag?
|Config includes `tag: only`
|Config includes `tag: ignore`
|Push to branch?
|Config includes `branches: only`
|Config includes `branches: ignore`
|Outcome
...
| icon:thumbs-up[role="circle-green"]
| -
| Run workflow if push is to configured `only` branch(es).
...
| -
| icon:thumbs-up[role="circle-green"]
| Run workflow if push is **not** to `ignore` branch(es).
...
| -
| -
| Run workflow because there has been a push to a branch.
...
| -
| -
| Run workflow if push is to configured `only` tag(s).
...
| -
| -
| Run workflow if **not** to configured `ignore` tag(s).
...
| -
| -
| Do **not** run workflow because there is no push to a branch.
...
| icon:thumbs-up[role="circle-green"]
| -
| Run workflow if push is to configured `only` branch(es). Ignore all tag filters, branches take precedence.
...
| -
| icon:thumbs-up[role="circle-green"]
| Run workflow if push is **not** to configured `ignore` branch(es). Ignore tag information, branches take precedence and no configured tag filters.
...
| -
| -
| Run workflow if push is **not** to configured `ignore` tag(s).
