...
While the `resource_class` key only specifically supports configuring CPU/RAM, the existing classes _do_ provide some level of additional control over the hard disk space available to a particular job. For example, a job that runs with a larger `resource_class` option will occupy a greater share of its host instance's resources. Thus, that instance will be able to accommodate fewer other running jobs, meaning the job in question will _also_ have access to a greater share of its host instance's hard disk space. Similarly, a job run with `resource class: small` may have access to less disk space, as it will share its host instance's disk space with a greater number of other running jobs.
