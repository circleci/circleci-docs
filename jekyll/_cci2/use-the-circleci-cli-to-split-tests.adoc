...
== 2a. Split and run tests

There are two commands that you can use to split your tests. We recommend using `circleci tests run` as this enables you to also take advantage of the xref:rerun-failed-tests#[rerun failed tests] feature.

* See `circleci tests run` examples <<tests-run-examples,here>>
* See `circleci tests split` examples <<tests-split-examples,here>>
...
== 2b. Split tests using `circleci tests split`
