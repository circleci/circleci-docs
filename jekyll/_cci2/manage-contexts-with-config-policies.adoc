...
There are many reasons why you may want to only allow the use of certain contexts to specific projects. For example, you might want to only allow contexts that have EKS-related deployment keys to projects architected to be deployed on EKS using those keys. To achieve this, use the xref:config-policy-reference#contexts-allowed-by-project-ids[`contexts_allowed_by_project_ids`] helper to create a policy to apply this restriction.

This helper function accepts project ids (`PROJECTS`) and context names (`ALLOWED_CONTEXTS`), and prevents the usage of **any** context **not in** `ALLOWED_CONTEXTS` for **all** projects that are **in** `PROJECTS`. `PROJECTS` and `ALLOWED_CONTEXTS` can be supplied as one of the following types:
...
This helper function accepts project ids (`PROJECTS`) and context names (`BLOCKED_CONTEXTS`), and prevents the usage of **any** context **in** `BLOCKED_CONTEXTS` for **all** projects that are **in** `PROJECTS`. `PROJECTS` and `BLOCKED_CONTEXTS` can be supplied as one of the following types:
...
This helper function accepts project ids (`PROJECTS`) and context names (`RESERVED_CONTEXTS`), and prevents the usage of **any** context **in** `RESERVED_CONTEXTS` for **all** projects that are **not in** `PROJECTS`. `PROJECTS` and `RESERVED_CONTEXTS` can be supplied as one of the following types:
