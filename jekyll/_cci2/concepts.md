...
Jobs are a collection of steps and each job must declare an executor that is either `docker`, `machine`, `windows` or `macos`. Machine includes a default image if not specified, for Docker and Ubuntu, you must also declare an image.
