...
* How to use Docker containers to run in a specific environment.
* How to use workflows to orchestrate your build.
* How to add approval steps to your workflow.

CircleCI believes in *configuration as code*. Consequently, the entire delivery process from build to deploy is orchestrated through a single file called `config.yml`. The `config.yml` file is located in a folder called `.circleci` at the top of your repo project. CircleCI uses the YAML syntax for config. See the <<writing-yaml#,Writing YAML>> document for guideance on the basics.

== Part 1: Using the shell

CircleCI provides an on-demand shell to run commands. In this first example, you will set up your first build and execute a shell command.

. If you have not done so already, <<first-steps#,sign up with CircleCI>> and select either GitHub or BitBucket.
. In your Version Control System (VCS), make sure you have authorized access to the project you want to manage with CircleCI (in GitHub, you have the option to block CircleCI from accessing your private repositories).
...
. Create a `config.yml` file inside the `.circleci` folder.
. Add the following code to your `config.yml` file:
+
[source,yaml]
----
version: 2.1
...
* *Line 4:* The specifies that you are using a Docker image for the container where your job's commands are run.
...
+
. Commit your config.yml file (and push the file if you are working remotely). CircleCI detects the change and your job runs. You can see the output of the job in the CircleCI dashboard. A successful job is denoted by a green tick; a failed job by a red exclamation mark. Click on the job for more details. You should see your step - The First Step - with the the output of the commands.
...
If you already have a `config.yml` file from the example above, add another run step (lines 13-17) at the end. Make sure you observe the indentation.
...
You should now see some additional steps:
...
In Parts 1 and 2, you ran your job in a basic Linux-based Docker container.

With CircleCI, you can run different jobs in different environments, such as macOS, virtual machines, or Docker images. By changing the Docker image, you can quickly upgrade your environment version or change languages. 
...
* *Lines 5-12:* The first job is _Hello-World_. As in Part 1, it runs two commands in the Alpine Docker image.
* *Line 14:* The second job is _Fetch-Code_. It is indented to align with the _Hello-World_ job.
* *Lines 15-16:* The _Fetch-Code_ job uses a CircleCI convenience image. This is a small Ubuntu-based image for running basic jobs.
* *Lines 17-22:* This code is repeated from Part 2, but now it is a separate job.
* *Line 24:* The third job is _Using-Node_.
* *Lines 25-26:* This _Using-Node_ job uses a Docker image called `cimg/node:17.2`. This image contains version 17.2 of Node, along with a browser and other useful tools.
* *Lines 27-31:* As in the previous jobs, there is a _run_ step. This time, the command `node -v` prints the version of Node running in the container.
* *Lines 32-33:* This line creates a Workflow called _Example-Workflow_. Workflows define a list of jobs and their run order.
* *Lines 34-35:* These lines specify the first job, _Hello-World_.
* *Lines 36-38:* The syntax for the _Fetch-Code_ job is slightly different. The job name is followed by a colon, then a _requires_ statement. This line specifies that the _Hello-World_ job must run successfully before the _Fetch-Code_ job is executed.
* *Lines 39-41:* The final job is _Using-Node_. As before, this job requires the successfully completion of the previous job, _Fetch-Code_. 

As before, commit and push your updated `config.yml` file.

In CircleCI, your pipeline will look different. Your workflow is now called _Example-Workflow_ and you have three jobs, rather than just one. If you click on the _Using-Node_ job and then the _Running the Node Container_ step, you should see that the command `node -v` has printed the version of Node.
...
* Created multiple jobs in different Docker containers.
...
In Part 3, an event triggering a build caused the `Hello-World` job to run immediately. The remaining jobs ran automatically, once `Hello-World` had completely successfully.
