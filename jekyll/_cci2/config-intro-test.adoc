...
. Create a blank `config.yml` file inside the `.circleci` folder. Then commit it (and push, if you are working remotely).
. On the Projects page in CircleCI, click the blue *Set Up Project* button.
...
. In the pop-up window, choose the default *Fastest* option for selecting your config.yml file. Then type the name of the branch on which you want to work.

// screenshot

. Your config.yml file is currently empty, so this initial build will fail. 
...
* *Lines 15-16:* The _Fetch-Code_ job uses basic image. 
