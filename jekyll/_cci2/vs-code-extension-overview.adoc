...
NOTE: The ability to trigger a pipeline from VS Code with a _unversioned_ config is currently in **open preview**.
...
[WARNING]
====
Please be mindful when running pipelines with _unversioned_ configuration, and take care to ensure no jobs with unintended effects are included in the run.

Test run pipelines triggered from VS Code have access to contexts and project environment variables, based on which branch they are run on and on author permissions. We therefore recommend using this feature **only** on feature branches.

This feature is enabled by default on all projects. You can opt-out of this feature per project. For more details, see <<feature-controls>>. 

The following issues will be addressed in the coming weeks: 
- Audit logs do not mention that a pipeline was run with unversioned config.
- Pipelines run with unversioned configuration are not differentiated from other pipelines in the WebUI.
- PR status checks may be altered as a result of unversioned config overrides
====
...
The ability to trigger pipelines from VS Code is enabled by default on all projects. You can opt-out of this feature **at the project level**, as follows:

. Navigate to Project Settings[VS Code integration]
. Toggle the "Run pipelines with unversioned config" option to **Off** to opt-out

NOTE: We plan on adding organization-level controls for this feature soon. 
