...
[WARNING]
====
Use caution when running pipelines with _unversioned_ configuration. Take care to ensure no jobs with unintended effects are included in the run.

Test run pipelines triggered from VS Code have access to contexts and project environment variables based on branch and author permissions. We recommend using this feature **only** on feature branches.

The ability to trigger pipelines from VS Code is enabled by default on all projects. You can opt-out of this feature per project. For more details, see <<feature-controls>>. 

The following issue fixes are in development: 
- Audit logs do not mention that a pipeline was run with unversioned config.
- Pipelines run with unversioned configuration are not differentiated from other pipelines in the CircleCI web app.
