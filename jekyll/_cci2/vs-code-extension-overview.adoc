...
[#trigger-a-config-test-run]
== Trigger a config test run

NOTE: The config test run feature is currently in **closed preview**. Config test runs are available on the **Free Plan**, for accounts integrated through the **GitHub OAuth app**, or **Bitbucket**. To find out which GitHub account type you have, see the xref:github-integration#[GitHub OAuth app integration] page.

Use config test runs to iterate on your pipeline configuration without committing your trial and error changes to your version control system. Run and validate your full pipeline, or select jobs and workflows to validate individually. View the results of your config test runs in the CircleCI web app, just the same as any other pipeline.
...
Config test run pipelines have access to contexts, and project environment variables, based on which branch they are run on, and user permissions
====

NOTE: Config test runs are disabled for projects that make use of xref:dynamic-config#[dynamic configuration]

The following steps show how to trigger a config test run pipeline from the VS Code extension:
...
. For each project, there is a button to trigger the config test run, and check boxes for each workflow and job for you to choose which parts of your config you want to run. Select your options, and click icon:play[].

image::{{site.baseurl}}/assets/img/docs/vscode-ext-config-test-run.png[Screenshot showing the run pannel]

The pipeline is now visible in your Dashboard in the CircleCI web app, at `https://app.circleci.com/pipelines/github/`
