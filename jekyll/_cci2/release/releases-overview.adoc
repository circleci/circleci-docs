...
CircleCI looks for the word `deploy` in your job names to detect when a release may be in progress. CircleCI generates a release marker for each job that contains the word `deploy` in its name _unless_ any of the following conditions are true:
