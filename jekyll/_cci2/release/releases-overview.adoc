...
CircleCI integrates with Kubernetes deployments and Argo Rollouts to provide visibility and control over your release process through the CircleCI web app.
...
A command is a user-initiated action CircleCI performs on the user's behalf to manipulate a specific component. These actions are run asynchronously and CircleCI reports the results back to the CircleCI web app releases dashboard, as you see for an individual job step.

Some commands which are available for all components. These are:
