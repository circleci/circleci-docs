...
* The `target-version` param should match the name of the version being released (same as the value of the `circleci.com/version` label)
* The name of the "release plan" (`my-service-release` in the example above) can be any arbitrary value you'd like; it will be used to reference the "release plan" as part of the release job config
