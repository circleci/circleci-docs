...
If you want to only monitor the `default` namespace, you do not need to supply a list with `--set manageNamespaces=[YOUR_LIST_OF_NAMESPACES_TO_MONITOR]`. The `default` namespace is monitored by default.
