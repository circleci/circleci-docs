...
* In certain cases where there are multiple workflows running within a pipeline, there is a known bug where the status badge will show the result (Red or Green) of the latest workflow that has completed running instead of waiting for all of the workflows in a pipeline to finish terminating and then updating the badge (Red or Green) to use the result of the last workflow.
