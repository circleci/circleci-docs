...
CircleCI 2.0 uses Nomad as the primary job scheduler. Refer to the <<nomad#_introduction_to_nomad_cluster_operation,Introduction to Nomad Cluster Operation>> to learn more about the job scheduler and how to perform basic client and cluster operations.
...
To ensure enough Nomad clients are running to handle all builds, track the queued builds and increase the number of Nomad Client machines as needed to balance the load. For more on tracking metrics see <<monitoring#,Monitoring Your Installation>>.
