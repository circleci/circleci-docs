...
Alternatively, you can use the dynamic syntax to achieve the same result.
...
Leverage the `pipeline.scheduled_source` and `pipeline.schedule.name` xref:pipeline-variables#pipeline-values[pipeline values].
