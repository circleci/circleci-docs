...
Update your CircleCI config to leverage xref:configuration-reference#the-when-step[the `when` step].
...
Update your CircleCI config to leverage the `pipeline.scheduled_source` and `pipeline.schedule.name` xref:pipeline-variables#pipeline-values[pipeline values].
