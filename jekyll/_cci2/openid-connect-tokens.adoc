...
NOTE: At this time, CircleCI's built-in AWS ECR authentication for pulling images does not support OIDC.
...
Below is an example configuration adding GCP to a job and demonstrating that authentication works. The example should be adapted for your specific use case.
...
jobs:
  gcp:
    ...
    environment:
      GCP_CREDENTIAL_CONFIGURATION_FILE: gcp-circleci-provider-config.json
