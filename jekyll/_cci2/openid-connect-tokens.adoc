...
1. Save the downloaded configuration file in your repo. This file will be referenced in your CircleCI configuration
...
You will also need to add the following environment variables to a link:/contexts[context].
...
Below is an example configuration adding GCP to a job and demonstrating that authentication works. This example uses the the link:https://circleci.com/developer/orbs/orb/circleci/gcp-cli[circleci/gcp-cli] orb.
...
You can use multiple service accounts from the same GCP project as well as using multiple service accounts from multiple GCP projects. You can read about these methods and find an example on CircleCI's link:https://github.com/jtreutel/circleci-gcp-oidc-test#usage[example repository].