...
. Configure your job to run the `aws-cli/setup` command before interacting with any AWS services. You will need to provide the `aws-cli/setup` command with the `role_arn` associated with the role you have created in the step above along with your `region`.
+
[source,yaml]
----
jobs:
...
+
Additionally, if you do not provide a `role_session_name` or `session_duration`, their default values are `${CIRCLE_JOB}` (your job’s name) and 3600 seconds respectively.
