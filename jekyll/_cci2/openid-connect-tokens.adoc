...
In CircleCI jobs that use at least one context, the OpenID Connect ID tokens are available in the environment variables: `$CIRCLE_OIDC_TOKEN` and `$CIRCLE_OIDC_TOKEN_2`. The `$CIRCLE_OIDC_TOKEN_2` token includes a different format for the `sub` claim, which is highlighted in the table below.
...
| `sub`
a| The subject. This identifies who is running the CircleCI job and where. `$CIRCLE_OIDC_TOKEN_2` also includes information about the source of change.
