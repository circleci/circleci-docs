...
In CircleCI jobs that use at least one context, the OpenID Connect ID verson 1 token is available in the environment variable `$CIRCLE_OIDC_TOKEN`. A OpenID Connect ID version 2 token is available in the environment variable `$CIRCLE_OIDC_TOKEN_2`. The second version includes a different format for the sub claim which is highlighted in the format below. 
...
== Format of the OpenID Connect ID token V1
...
| The URL of the repo that triggered the pipeline. Its value is a string similar to 'github.com/orgnanization-123/repo-1'.
...
[#format-of-the-openid-connect-id-token_2]
== Format of the OpenID Connect ID token V2

The OpenID Connect ID token V2 contains the following standard https://openid.net/specs/openid-connect-core-1_0.html#IDToken[claims]:
...
| `sub`
| The subject. This identifies who is running the CircleCI job and where. It also includes information about the source of change. Its value is `"org/ORGANIZATION_ID/project/PROJECT_ID/user/USER_ID/vcs-origin/VCS_ORIGIN/vcs-ref/VCS_REF"`, a string, where `ORGANIZATION_ID`, `PROJECT_ID`, and `USER_ID` are UUIDs that identify the CircleCI organization, project, and user, respectively. The user is the CircleCI user that caused this job to run.
