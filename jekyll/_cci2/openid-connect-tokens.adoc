...
In CircleCI jobs that use at least one context, the OpenID Connect ID tokens are available in the following environment variables:
...
You will need to allow your AWS account to trust CircleCI's OpenID Connect tokens. To do this, create an Identity and Access Management (IAM) identity provider, and an IAM role in AWS. This is a one-time configuration to get set up.

. Visit the https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_providers_create_oidc.html[Creating OpenID Connect (OIDC) identity providers] page of the AWS docs and follow the instructions. There are multiple setup options covered on this page, including using the management console, or CLI.

. When asked for the **Provider URL**, enter: `\https://oidc.circleci.com/org/<your-organization-id>`, where `your-organization-id` is the ID of your CircleCI organization. Provide the same CircleCI Organization ID for the **Audience**.
+
{% include snippets/find-organization-id.adoc %}

. To create an IAM role, visit the https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-idp_oidc.html#idp_oidc_Create[Creating a role for web identity or OIDC] section of the AWS docs.
+
** Select the **Identity Provider** that you just created.
** For **Audience**, choose the only option. #THIS NEEDS CHECKING#
** Click **NEXT**. You will now be on the **Add Permissions** page. Here you can specify what your CircleCI jobs _can_ and _cannot_ do. Choose only permissions that your job will need. This is an https://docs.aws.amazon.com/IAM/latest/UserGuide/best-practices.html#grant-least-privilege[AWS best practice].
...
. Choose the CircleCI jobs you want to set up to use OIDC. Make sure each of these jobs use a valid xref:contexts#[context]. The OpenID Connect token is available only to jobs that use at least one context. The context used does not need to contain any environment variables.
...
. Next, run the `aws-cli/setup` command in your job before interacting with any AWS services. You will need to provide the `aws-cli/setup` command with the `role-arn` associated with the role you have created in the step above along with your `aws-region`.
