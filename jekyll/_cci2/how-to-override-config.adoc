...
Add your URL orb to the `orbs` section of your configuration.

Use the `override-with` key to replace the test job with a job from your orb. When the `main` workflow runs, CircleCI will use the custom-test job from your URL orb instead of your locally defined test job. If the `custom-test` job the orb does not exist, the `test` job configured in your main config will be used.
