...
You can set this parameter in several ways, but one simple option is to rely on CircleCI project variables. For example you could define it as
`target-version="1.0.${CIRCLE_BUILD_NUM}-${CIRCLE_SHA1:0:7}"`, which would yield a value with the following format `1.0.28853-ffdbeb1`.
