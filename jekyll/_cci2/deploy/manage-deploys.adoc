...
The _restore version_ command restores a previously deployed version of a component.
...
To understand the _restore version_ command, it may be helpful to understand the equivalent commands for Kubernetes Deployments, Helm, and Argo Rollouts. Executing the _restore version_ command from the CircleCI web app is the equivalent of using the following:
...
To understand the _restore version_ command, it may be helpful to understand the equivalent commands for Kubernetes Deployments, Helm, and Argo Rollouts. Executing the _scale component_ command is the equivalent of the following:
...
To understand the _restore version_ command, it may be helpful to understand the equivalent commands for Kubernetes Deployments, Helm, and Argo Rollouts. Executing the _restart component_ command is the equivalent of the following:
...
Use the _retry Rollout_ command to retry a Rollout that has failed or been cancelled. For more information about this command, see link:https://argo-rollouts.readthedocs.io/en/latest/generated/kubectl-argo-rollouts/kubectl-argo-rollouts_retry/[Argo Rollout retry].
...
Executing the _retry Rollout_ command is the equivalent to running:
...
Use the _promote_ commands to promote a deployment that is paused at a step. You have the following options:
...
. You will now see the btn:[Promote] and btn:[Promote All] options. Select btn:[Promote] to promote the deployment as required. The command is run immediately with no intermediary checks.
...
Use the _cancel deployment_ command to stop a deployment from progressing, and revert all steps. For more information see the link:https://argo-rollouts.readthedocs.io/en/stable/generated/kubectl-argo-rollouts/kubectl-argo-rollouts_abort/[Argo Rollouts docs for Rollouts Abort].
...
Executing the _cancel deployment_ command is the equivalent to running:
