...
The feature is available to customers on a link:https://circleci.com/pricing/[Performance or Scale plan]. This feature consumes 450 credits from your account for each GB of data used for jobs with IP ranges enabled. If you are looking to predict the cost, see CircleCI's link:https://circleci.com/blog/network-transfer-ip-ranges/[blog post] for further information.
...
CircleCI _does not recommend_ configuring an IP-based firewall based on the AWS or GCP IP addresses, as the vast majority are not CircleCI's machines. Additionally, there is _no guarantee_ that the addresses in the AWS or GCP endpoints persist from day-to-day, as these addresses are reassigned continuously.
