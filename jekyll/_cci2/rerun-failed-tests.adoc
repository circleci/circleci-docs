...
The snippet above uses the built-in xref:caching[`.Revision`] key to keep all coverage reports from the same git commit (sha1) together.  On a rerun, both the original job run's coverage report and the rerun's coverage report will be uploaded as an artifact to `cover.html`. It can then be used in a downstream job for aggregation or analysis.
