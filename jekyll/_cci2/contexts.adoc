...
Restrict contexts by setting rules that xref:pipeline-variables#pipeline-values[pipeline values] must match. These rules are called _expression restrictions_. Set expression restrictions using the context restriction API or on the Context page (menu:Organization Settings[Contexts>Expression Restrictions]) in the CircleCI web app.
