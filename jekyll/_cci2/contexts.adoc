...
Restrict contexts by setting rules that xref:pipeline-variables#pipeline-values[pipeline values] must match. These rules are called _expression restrictions_. Set up expression restrictions using the context restriction API or on the Context page (menu:Organization Settings[Contexts>Expression Restrictions]) in the CircleCI web app.
...
xref:pipeline-variables#pipeline-values[Pipeline values] can be used as variables in an expression restriction. There are also special job-specific variables available to use in expressions. You can not use the `pipeline.trigger_parameters.*` pipeline values in expression restrictions.
...
If a variable is referenced in an expression but there is no value set for that variable in the pipeline, the expression will _fail closed_ and prevent the context being used.

[#errors]
=== Errors

Any errors evaluating an expression will _fail closed_ and prevent the context being used. Errors include the following:
...
Restrict a context to the `main` branch, disallow use in an SSH rerun, and disallow use with pipelines xref:vs-code-extension-overview#test-run-your-config-from-vs-code[triggered with unversioned config]:
