...
You must be an organization administrator to complete the following task. Note: this is not supported for orgs that integrate with xref:github-apps-integration/[CircleCI's GitHub App].
...
. Click the *Add Security Group* (GitHub OAuth App users) or *Add Project Restriction* (GitLab users) button to view the dialog box.
