...
* Testing job uses `circleci tests run` (see below for details) to execute tests
  ** Note: If your current job is using test splitting, you must switch your `.circleci/config.yml` definition from `circleci tests split` to `circleci tests run`
