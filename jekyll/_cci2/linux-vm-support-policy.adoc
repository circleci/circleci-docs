...
This document outlines the CircleCI Linux VM (Machine) Image release, update and deprecation policy. This policy applies to all CircleCI Linux VM (Machine) Images built for the Machine executor. Note that there is a separate link:https://circleci.com/docs/linux-cuda-images-support-policy/[CUDA image support policy].
...
- `current` version of image will receive updates approximately every 3 months.
