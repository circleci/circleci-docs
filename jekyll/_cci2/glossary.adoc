...
[#resource-class]
== Resource class
A configuration option for specifying compute resource sizing for a job. Also used to identify a self-hosted runner, and configure a job to run on that self-hosted runner. Read more on the link:/docs/resource-class-overview[resource class overview] page.
