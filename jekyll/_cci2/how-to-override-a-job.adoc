...
* Familiarity with the concepts of URL and registry orbs, and the URL orb allow-list. #INSERT LINKS WHEN AVAILABLE#
...
#ADD LINK TO ALLOW-LIST HOW-TO WHEN READY#

=== 4. Override a job using the orb

Add your URL orb to the `orbs` section of your configuration. Use the `override-with` key to replace the test job with a job from your orb. When the `main` workflow runs, CircleCI will use the custom-test job from your URL orb instead of your locally defined test job. If the `custom-test` job the orb does not exist, the `test` job configured in your main config will be used.
...
* Does NOT include `orbs:` or `wor kflows:` sections.
* Contains only the `jobs:` section with the specific job definitions the team wants to override.
* Will be imported as an orb by the centralized configuration.

=== 3. Configure your URL allow list
To use URL-based orbs for team configurations, configure your organization's allow-list settings to include the URL of the orb file:
...
* Configure the project to use GitHub App integration. The CircleCI GitHub App integration is required for using a config source outside the project repo. See the xref:github-apps-integration#[GitHub App integration] guide for more information.
...
* When the pipeline runs, the centralized template created and managed by the platform team is used.
...
* For the test job, If it exists, the configured override job is used instead of the template's standard job.
* The standard security-scan and deploy jobs continue to run as defined in the template by the platform team.
...
* For VCS triggers, ensure you are using pipeline values that are available at runtime.
...
The override mechanism will work for GitHub OAuth projects, but there is no way to enforce a single config across multiple OAuth projects within the CircleCI platform.

=== Are scheduled pipelines supported?
Scheduled pipelines are not supported for decoupled projects (GitHub App/GitLab), so they cannot be triggered from a centralized config.
