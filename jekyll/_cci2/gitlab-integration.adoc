...
* Your GitLab self-managed instance URL, e.g. `https://test-gitlab.circleci.com``.
+
Your self-managed instance must already contain at least one project. The authorization attempt will be unsuccessful if your instance does not have any projects.  Note that the self-managed instance must be accessible via the public internet, **it cannot be behind a firewall**.
...
. Enter your self-managed instance URL. The field will automatically append `/api/v4`, so the URL that CircleCI uses to connect to your account looks something like `https://test-gitlab.circleci.com/api/v4`.
...
. Navigate to the **Integrations** section within **Organization Settings** to add a new instance. 

. You will need to enter the instance URL and the SSH host keys as described in the xref:#sign-up[Sign up] section earlier in this document. You have successfully set up the integration when you see a "Known hosts file verified" message.
+
More details on verifying self-managed instances are available in the <<#establish-the-authenticity-of-an-ssh-host,Establish the authenticity of an SSH host>> section below.
