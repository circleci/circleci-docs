...
. Navigate to the **Integrations** within **Organization Settings** to add a new instance. 

. You will need to enter the instance URL, and the SSH host keys if you have not already done so, as described in the xref:#sign-up[Sign up] section above. You have successfully set up the integration when you see a "Known hosts file verified" message.
+
