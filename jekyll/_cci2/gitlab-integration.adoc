...
:page-liquid:
:page-description: Learn how to integrate GitLab with CircleCI for your GitLab CI/CD projects with our GitLab step-by-step tutorial.
:icons: font
:experimental:

This page walks you through integrating a GitLab CI project with CircleCI. The sections below introduce you to concepts and ways to manage your GitLab CI/CD pipelines. CircleCI features that are in development for GitLab CI projects are detailed in the <<coming-soon>> section.

[#overview]
== What is GitLab CI/CD?

If you use GitLab (SaaS or Self-managed) to and manage version control and store your code, GitLab CI/CD (continuous integration and continuous delivery) is enabled for your project by default. GitLab CI/CD is the part of GitLab that allows you to build, test (continuous integration) and deploy (continuous deployment, continuous delivery) your projects each time a developer pushes a change to the codebase, and/or when a merge request is created. As an alternative, you can integrate with CircleCI to manage your CI/CD pipelines.

Integrate with CircleCI to access features for every aspect of your CI/CD pipelines:
...
GitLab CI/CD support is open to both new and existing users of CircleCI. Follow the steps on the xref:first-steps#gitlab-signup[Sign up and try CircleCI] page to connect your GitLab account to create your organization.
...
[#project-settings]
== GitLab CI/CD project settings

A project can have one or more **configurations**, which are pipeline definitions. _Configurations_ include, but are not limited to, a `.circleci/config.yml` file in your repo.
...
When creating a GitLab CI/CD project in CircleCI, an SSH key is created, which is used to check out code from your repo. Each configuration you create generates a new SSH key to access the code in the repo associated with that configuration. At this time, only **Additional SSH Keys** are applicable to GitLab projects.
...
== GitLab CI/CD organization settings

For GitLab CI/CD projects, organizations and users are managed independently from your VCS. Organizations and users are considered CircleCI organizations and users, with their own roles and permissions that do not rely on those defined in your VCS.
