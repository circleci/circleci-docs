...
. You will need to enter the instance URL and the SSH host keys as described in the xref:#sign-up[Sign up] section earlier in this document. The new instance is successfully integrated when you see a "Known hosts file verified" message.
+
...
SSH keys for remote servers can be fetched by running `ssh-keyscan <host>`, for example, `ssh-keyscan example.circleci.com`. 

When retrieving the host keys, you can confirm that you have the correct key by checking its fingerprints. You can check the fingerprints found in the **Instance Configuration** section of your self-managed instance's Help pages (link:https://gitlab.com/help/instance_configuration#ssh-host-keys-fingerprints shows an example).
