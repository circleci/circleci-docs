...
| `deploy:`
| xref:configuration-reference#run[`run`]
| Use a `run` step to run needed commands for deployment. See our xref:deploy/how-to/deployment-overview#[Deployment overview page] for links to examples.
...
group of cache paths to defer to in the declared order. Refer to the link:{{ site.baseurl }}/caching/[caching
