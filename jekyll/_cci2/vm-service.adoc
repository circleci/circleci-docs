...
Creating a Windows image and specifying it under the VM Service settings lets your users run jobs on dedicated Windows VMs. To create your Windows image run through these steps. 
...
. Copy the contents of https://github.com/CircleCI-Public/circleci-server-windows-image-builder into your new repo.
. Open up your installation of CircleCI Server and set up your AWS keys as described in the image builder https://github.com/CircleCI-Public/circleci-server-windows-image-builder#setting-up-on-circleci[READMEand].
. Next, https://circleci.com/docs/2.0/getting-started/#setting-up-your-build-on-circleci[connect your new repo] by clicking Follow Project from the Add Projects page.
. Click Start Building to run the Windows image buider.
. Your will find your new Windows AMI ID at the end of the `Summarize results` step in the job output.
. Copy the AMI ID and paste into the Custom Windows VM AMI field in your Management Console settings, under VM Provider (for example, `<your-hostname.com:8800/settings>`).
