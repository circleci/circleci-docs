...
// Here I need to tell people to enable the new machine large resource, and then enter instance types to use for default and large - but how does instance type correspond to the CPU/RAM stuff we have defined in picard-dispatcher for this release? Are "Instance type" and "CPU/RAM requirements" totally separate in this context?
...
WARNING: If https://circleci.com/docs/2.0/docker-layer-caching/[Docker Layer Caching (DLC)] is to be used, VM Service instances must be spun up on-demand. To ensure this can happen, either you need to ensure any preallocated instances are in use, or you should set both remote Docker and `machine` preallocated instance fields to `0`.
