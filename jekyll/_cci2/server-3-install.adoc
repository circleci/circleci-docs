...
Key used by VM Service differs from the policy used by object storage in the previous section. VM Service and Object Storage are kept distinct to allow organizations can utilize different cloud and on-premise providers within the same installation. 
