...
. **Once you understand the config, modify the executor and the individual steps to your needs.** For example, a https://circleci.com/docs/2.0/language-ruby/[Rails project] may use a Ruby container and run `bundler` commands. Consult our https://circleci.com/docs/2.0/tutorials/#section=configuration[Tutorials page] for various examples in different languages and frameworks.
...
. **Next, https://circleci.com/docs/2.0/optimizations/[optimize your jobs] with our advanced features** to see how much faster your builds could run on our platform.
. **Throughout this process, consult our https://circleci.com/docs/2.0/configuration-reference/#section=configuration[Config Reference]** for a list of all available configuration on our platform.
...
* TeamCity's Build Configurations are equivalent to https://circleci.com/docs/2.0/concepts/#jobs[CircleCI's jobs], which are atomic elements of work consisting of many steps or commands to achieve some sort of goal (e.g. run unit tests).
...
[cols="50%,50%"]
|===
| TeamCity _settings.kts_ | CircleCI _config.yml_
...
[cols="50%,50%"]
|===
| TeamCity Build Chain | CircleCI Workflow
