...
. Assuming the repository you connected is also connected to your previous CircleCI organization, CircleCI will start pipelines when a push event happens to the repository in both the old and new organizations. If your test from step 7 above was successful, go to **Project Settings** in your organization connected to the GitHub OAuth App (your "old" org) and click btn:[Stop Building].  This will ensure that push events to your repository only trigger pipelines in the project connected to your GitHub App organization.
. Repeat steps 5-8 by clicking menu:Projects[Create a Project] for each project in your previous organization.
...
. Submit a request to link:https://support.circleci.com/[Support at CircleCI] to delete the "old" organization.  At this point, you will be left with a GitHub App integrated organization that has the same payment plan and projects as your previous organization.

If you need to transfer private orbs or self-hosted runner resource classes, contact link:https://support.circleci.com/[Support at CircleCI] before following step 14.  Data from xref:insights#[Insights] and historical pipeline runs will not be present in your new organization.  Contexts will also not be present until you recreate them.
