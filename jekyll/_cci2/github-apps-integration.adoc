...
. If you are on a paid pricing plan, navigate back to the organization that is connected to the GitHub OAuth App and go to "Plan" in the CircleCI web app. Click on the "Share and Transfer" tab. Click "Add shared organization" and select the new organization that you just created that integrates with CircleCI's GitHub App. 
...
. If you followed step 5, navigate back to the "old" organization and go to "Plan" > "Share and Transfer".  Remove the shared relationship between the "new" organization and the "old" organization.  Then, click "Transfer Plan" and follow the instructions to transfer the plan from the "old" organization to the "new" organization.  
