...
CircleCI's GitHub App integration provides fine-grained permissions, uses short-lived tokens, and gives you more control over which repositories CircleCI has access to.  It also enables functionality like link:https://discuss.circleci.com/t/trigger-pipelines-from-anywhere-inbound-webhooks-now-in-preview/49864[inbound webhooks] and link:https://discuss.circleci.com/t/circleci-config-suggestions-bot/47918[CircleCI's config suggestions bot]. 

There is no automated way to migrate your organization that is connected with CircleCI's GitHub OAuth App integration to an organization that is connected with CircleCI's GitHub App integration.  Before attempting to move your information from an org integrated with the GitHub OAuth App to an org integrated with CircleCI's GitHub App, **confirm that you do not immediately need any of the functionality listed in the "currently not supported" section below**.  If you would like to switch from the OAuth App integration to the GitHub App integration, follow these steps:

1. From your existing CircleCI organization in the CircleCI web UI, click the organization dropdown in the top-left corner. 
2. At the bottom of the drop-down, click "Create a new organization".
3. On the "Connect your code" page, Click "GitHub".
4. You will be redirected to GitHub to install the CircleCI GitHub App into your GitHub organization. Note: you can install the CircleCI GitHub App into the same GitHub organization that already uses the GitHub OAuth App integration.
5. Follow the instructions in the web UI to create a project that is connected to your GitHub repository.
...
7. Perform a test push of code to your repository to ensure that the pipeline triggered and is working as expected in your new CircleCI organization.
8. Assuming the repository you connected is also connected to your previous CircleCI organization, CircleCI will start pipelines when a push event happens to the repository in both the previous and new organizations. If your test from step 7 above was successful, go to Project Settings in your organization connected to the GitHub OAuth App and click "Stop Building".  This will ensure that push events to your repository only trigger pipelines in the project connected to your GitHub App organization.
9. Repeat steps 5-8 by clicking "Projects" >> "Create a Project" for each project in your previous organization.
10. If you are using xref:contexts, you will need to recreate the contexts in your new organization.
11. Invite your teammates to the new organization that is integrated with GitHub App using the instructions on xref:invite-your-team[this page].
12. If you are on a Performance pricing plan, go to "Plan" in the CircleCI UI and click on the "Share and Transfer" tab. Click "Add shared organization" and follow the instructions to link your new organization (use "/circleci" as the VCS and the organization ID that is found in Organization Settings as the <org> if prompted).  
13. If you followed step 12, your pricing plan will now have a "parent" organization that is your "old" organization that uses the GitHub OAuth App integration and a "child" oranization that is your "new" organization that uses the GitHub App integration. 
14. Submit a request to link:https://support.circleci.com/[Support at CircleCI] to delete the "old" organization.  At this point, you will be left with a GitHub App integrated organization that has the same payment plan and projects as your previous organization.

If you need to transfer private orbs or self-hosted runner resource classes, contact link:https://support.circleci.com/[Support at CircleCI] before following step 14.  Data from xref:insights and historical pipeline runs will not be present in your new organization.  Contexts will also not be present
