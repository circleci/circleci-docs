...
GitLab-based triggers provide access to additional pipeline values and parameters. For more information on using pipeline values and parameters in CircleCI, refer to the <<pipeline-values#,Pipeline Values and Parameters>> document.
...
GitLab-based projects do not have the following built-in environment variables available. If your pipelines need these environment variables, we recommend you use suitable replacements from the available <<gitlab-vcs-support#pipeline-values-and-parameters,pipeline values>>.
