...
Every plan with CircleCI includes the ability to run jobs concurrently. However, the number of jobs you can run concurrently differs between plans. Please refer to the link:https://circleci.com/pricing/[pricing page] for up-to-date information on the number of concurrent job runs included in your plan.
...
Please note, if you are still using CircleCI Server 2 with `version: 2`, you will need to specify the version in the `workflows` section, as mentioned in the xref:#concurrency-in-workflows[Concurrency in workflows] section.
