...
This function accepts VCS branch names (`BRANCHES`) as an array-of-strings and 
context names (`RESERVED_CONTEXTS`) as one of string/set-of-strings/array-of-strings types.
Branch names NOT IN `BRANCHES` are NOT allowed to use the contexts IN the `RESERVED_CONTEXTS`, however other contexts can be used.
