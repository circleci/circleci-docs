...
TIP: For **projects integrated with GitHub OAuth app or Bitbucket Cloud**, you can also trigger a pipeline using the deprecated link:https://circleci.com/docs/api/v2/index.html#operation/triggerPipeline[Trigger a New Pipeline] endpoint, which is described in the **CircleCI server** tab.
