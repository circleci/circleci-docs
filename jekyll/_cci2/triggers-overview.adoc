...
NOTE: Triggering a pipeline via the CircleCI v2 API is not currently supported for GitLab, GitHub App or Bitbucket Data Center projects. To find out if you authorized through the GitHub OAuth app or the CircleCI GitHub App, see the xref:github-apps-integration#[GitHub App integration] page.  Alternatively, for projects using GitHub App, the <<trigger-a-pipeline-from-a-custom-webhook,custom webhooks>> functionality provides similar capabilities.
...
. To find the **Definition ID** for the **Pipeline** you want to trigger, navigate to your project's  **Project Settings**, and select **Pipelines** in the sidebar. In the column **Definition ID**, click btn:[Copy] for the Pipeline you want to run via API. 
