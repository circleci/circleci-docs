...
To trigger this webhook from using `curl`, copy and paste this sample request and replace the URL and secret with the URL and secret that you generated in the prevoius step:
...
Note that if triggering via `cURL`, you must use a `POST` request with `content-type: application/json`.
