...
The application exposes four services using load balancers, three of these load balancers are VPC-internal for connecting to the Nomad cluster and VMs. The frontend load balancer can be made private, separating it from the public internet if required. For further information see the <<server-3-operator-load-balancers#,Load Balancers>> doc.
