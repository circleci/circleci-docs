...
NOTE: SAML Single Sign-On for CircleCI is currently in preview for Scale customers. Please see <<prerequisites-and-limitations>>
...
- SSO for CircleCI is supported for organizations using integrations through GitHub App, GitLab, and Bitbucket Data Center. SSO is not supported for organizations that integrate their code through the GitHub OAuth app Bitbucket Cloud. See the xref:../version-control-system-integration-overview#[VCS integration overview] for more information on integration.
