...
* `--listTests` grabs all tests which get fed into `stdin` for `xargs yarn tests`.
 * `--listTests` can sometimes be too agressive depending on your setup.  You may need to be more specific with a regex and a glob commmand to only get the intended tests.
