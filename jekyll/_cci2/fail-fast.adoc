 ...
 ** *Answer:* Follow the same instructions as the example above and add append an additional parameter to your `circleci tests run` command: `--split-by=name` to split by filename OR  `--split-by=timing` to split by timing.  See the Known Limitations section above for constraints at this time with splitting by timing.
