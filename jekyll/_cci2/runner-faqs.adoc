...
[#can-i-run-multiple-machine-runners-on-a-single-host]
== Can I run multiple machine runners on a single host?

Yes, by running multiple replicas of the machine runner with unique names, it is possible to run as many agents (and therefore jobs) on a single host as you want. However, care must be taken to ensure that these jobs are sufficiently isolated from each other that they do not conflict if run at the same time.
