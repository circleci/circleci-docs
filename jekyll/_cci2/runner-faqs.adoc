...
== I installed my first self-hosted runner on macOS and the job is stuck in a "Preparing Environment" step but there are no errors, what should I do?
...
```
Cancel the job and re-run it.  If your job is still not running, file a [support ticket](https://support.circleci.com/hc/en-us).
