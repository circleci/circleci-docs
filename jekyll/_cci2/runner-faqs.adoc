...
Note that you must check to see if there are greater than two processes because the ```grep``` process itself will count as one process and the [launch agent](https://circleci.com/docs/2.0/runner-concepts/#launch-agent-and-task-agent) process will count as a separate process. 
