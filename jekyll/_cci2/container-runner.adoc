...
`launch-agent`, the equivalent to container-agent for jobs that use the machine executor and is in charge of polling CircleCI for work, can be installed on Kubernetes. It runs `task-agent` (the component in charge of executing work) within the same pod, as though it is running on a VM. 
