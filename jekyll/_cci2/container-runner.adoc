...
=== What is the difference between launch-agent on Kubernetes and container agent?

**launch-agent on Kubernetes**

`launch-agent` (the component in charge of polling CircleCI for work) can be installed on Kubernetes and is the equivalent to container-agent for jobs that use the machine executor. `launch-agent` runs `task-agent` (the component in charge of executing work) within the same pod, as though it is running on a VM.

The `task-agent` is not aware that it is running on Kubernetes.

This set-up uses a 1:1 ratio of `launch-agent` : `task-agent`.  

**Container agent**

Container agent, which polls CircleCI for work, is aware of Kubernetes and uses it to schedule `task-agent`. They run in separate pods and there is a 1:Many ratio between container agent and associated task agents.
