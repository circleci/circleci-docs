...
3. A 3rd party tool like Biuldah.

Note: 3rd party tools should be used at your own discretion.

While jobs that run with container agent cannot use CircleCI's <<building-docker-images#,setup_remote_docker>> feature, it is possible to use a 3rd party tool to build docker images in your container agent job without using the Docker daemon.

A simple option is to use a tool called [Buildah](https://github.com/containers/buildah).  Buildah can be used in your `.circleci/config.yml` syntax:
...
Buildah relies on the [fuse-overlay](https://github.com/containers/fuse-overlayfs) program inside of the container which means that a fuse device plugin must be configured in order to use it. `/dev/fuse` is required to use `fuse-overlayfs` inside of the container as this option tells Buildah on the host to add `/dev/fuse` to the container for Buildah's use. Kubernetes has a device plugin system to enable secure sharing of host devices with pods.  

This is one example of a Device Plugin for `dev/fuse`: https://github.com/kuberenetes-learning-group/fuse-device-plugin/blob/main/fuse-device-plugin-k8s-1.16.yml
...
If you plan to use a CircleCI [convenience image](https://circleci.com/developer/images), ensure you add the repository for installation to your job's `steps`:
