...
Once your configuration file is updated, validate whether the job ran successfully by triggering it and ensuring a green build using the CircleCI web app. See the <<runner-faqs#container-runner-specific-faqs,runner FAQ page>> for a full sample config if you are starting from scratch.
