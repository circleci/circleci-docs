...
. You will also need an https://velero.io/docs/v1.6/supported-providers/[storage bucket] created with the cloud storage provider of your choice.

First, we will go over the steps to use Velero in AWS using S3 as storage. Further down you will find instructions for GCP with GCS.

== Installation on AWS with S3
...
== Installation on GCP with GCS

To follow the instruction steps for Google Cloud Platform, it is necessary that you have `gcloud` and `gsutil` installed and configured locally.
Should that not be the case, you can follow Google's https://cloud.google.com/sdk/docs/[user guide] to set them up.
...
do not forget to re-set variables as necessary before proceeding. In the step below, for example, we will define a variable for your bucket name. Replace
...
. First, we will set a shell variable for your project ID. To do so, first make sure that your `gcloud` points to the correct project by looking at the current config:
+
[source,bash]
----
gcloud config list
...
----
NOTE: If you run several clusters with velero, you might want to consider using a more specific name for the Service Account than `velero` as suggested here
...
You can simply pass a JSON credentials file to Velero to authorize it performing actions as the Service Account. To do this, we first need to create a key:
[source,bash]
----
...
If you are already using Workload Identities in your cluster, you can bind
the GCP Service Account you just created to Velero's Kubernetes service account. In this case, though, the GCP Service Account will need the
...
For more options on customizing your installation refer to the https://github.com/vmware-tanzu/velero-plugin-for-gcp#install-and-start-velero[Velero docs].
