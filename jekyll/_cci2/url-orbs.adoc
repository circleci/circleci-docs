...
* Auth - The authentication method to use (`none`, `github-oauth`, or `github-app`)
...
. In the *Allow-List URLs*  section, select btn:[Add].
. Fill out the name, URL, and VCS fileds.
. Select btn:[Add URL].
...
[source, shell]
----
curl -X POST https://circleci.com/docs/api/private/organization/<organization-id/url-orb-allow-list \
...
. Scroll to the *Allow-List URLs* section.
...
[source, shell]
----
curl -X GET https://circleci.com/docs/api/private/organization/<organization-id/url-orb-allow-list \
  --header "Circle-Token: ${CIRCLE_TOKEN}"
----
