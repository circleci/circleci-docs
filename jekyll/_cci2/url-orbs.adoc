...
The ability for an organization to use private orbs is no longer dependent on publishing orbs. URL orbs offer a way to create orbs as well as control access within the organization.

The allow-list acts as a safe-list that restricts orb URL references to only approved URL prefixes, providing security and access control for organizations using private or external orbs.
...
Because there is no publishing step, URL orbs are expanded when the config is processed at the beginning of a pipeline. Registry orbs are expanded when the orb is published.
====
