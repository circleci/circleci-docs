...
* **Answer:** Both [commands](https://circleci.com/docs/2.0/reusing-config/#the-commands-key) and [jobs](https://circleci.com/docs/2.0/reusing-config/#authoring-parameterized-jobs) are elements that can be used within orbs. _Commands_ contain one or many [steps](https://circleci.com/docs/2.0/configuration-reference/#steps), which contain the logic of the orb. Commands generally execute some shell code (bash). _Jobs_ are a definition of what steps/commands to run and within what [executor](https://circleci.com/docs/2.0/reusing-config/#executors) to run them. _Commands_ are invoked at the job level. _Jobs_ are invoked at the _[Workflow](https://circleci.com/docs/2.0/workflows/#workflows-configuration-examples)_ level.
