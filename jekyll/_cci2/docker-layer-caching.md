...
Docker Layer Caching is a great feature to use if building Docker images is a regular part of your CI/CD process. DLC caches the individual layers of any Docker images built during your CircleCI jobs, and then reuses these image layers on subsequent CircleCI runs, rather than rebuilding the entire image from scratch every time. In short, the less your Dockerfiles change from commit to commit, the faster your image-building steps will run.
