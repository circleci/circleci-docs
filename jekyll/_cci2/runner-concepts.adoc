...
The machine-runner-3 replaces launch-agent on cloud, brining improvments to network resliencey, installation, and upgrading process. As with launch-agent any user who is able to execute a job will be able to gain the same privileges as the task-agent which when installed using the provided packages runs as the `circleci` user.
