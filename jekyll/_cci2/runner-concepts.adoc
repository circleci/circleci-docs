...
The machine runner 3.0 replaces launch-agent on cloud, bringing improvements to network resiliency, installation, and the upgrade process. As with launch-agent, any user who is able to execute a job will be able to gain the same privileges as the task-agent, which when installed using the provided packages runs as the `circleci` user.
...
Launch-agent is currently supported on Server v4.x and v3.x. Cloud users should instead refer to <<machine-runner-3,Machine Runner 3>>.
