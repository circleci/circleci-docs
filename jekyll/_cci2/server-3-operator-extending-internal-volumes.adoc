...
If you find that the new pods do show the resized volumes as expected, it is still worth running the following commands to check. In some instances the resize will fail, but the only error source is an event in the output from the following commands.
