...
- **Scaling Concurrency**: You can run up to 80 concurrent jobs on our monthly Performance Plan or even more on an [annual plan](https://circleci.com/pricing/). Travis CI has capped concurrencies of 1, 2, 5, and 10 on each of their plans.
...
|-------------------|------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------|
| language:,<br>os: | [docker, machine, macos, windows]({{ site.baseurl }}/2.0/executor-types/)          | CircleCI doesn't assume dependencies or commands based on a language. Instead, CircleCI allows you to use any of the major operating systems or [Docker images]({{ site.baseurl }}/2.0/circleci-images/#next-gen-language-images) as executors. Specify a Docker image with your needed prerequisites or use a VM and install other needed libraries (see pre-installed software for [Windows]({{ site.baseurl }}/2.0/hello-world-windows/), [macOS]({{ site.baseurl }}/2.0/testing-ios/#supported-xcode-versions), and [Linux]({{ site.baseurl }}/2.0/docker-to-machine/#pre-installed-software) images). Then, use `run:` steps as shown below to execute commands you require (e.g., install, build, test). |
