...
NOTE: Selecting a custom webhook as a pipeline trigger is available in "Preview" for orgs that integrate with GitHub via the CircleCI GitHub OAuth App, see our link:ttps://discuss.circleci.com/t/trigger-pipelines-from-anywhere-custom-webhooks-now-in-preview/49864[community forum] to request access.  To find out which GitHub account type you have, see the xref:github-apps-integration#[GitHub App integration] page.  Orgs that integrate with CircleCI's GitHub App do not need to request access.
