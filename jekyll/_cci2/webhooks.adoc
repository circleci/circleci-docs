...
The inbound webhook body is available by using the `pipeline.trigger_parameters.webhook.body` pipeline value.

The following example shows how you can use `jq` to extract values you want to use in your configuration into environment variables. In this example the webhook body contains a property called `branch`. `jq` is installed and used to extract the `branch` value into an environment variable, which is then used in the following GitHub clone command.
