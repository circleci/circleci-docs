...
NOTE: If your services are externalized then you may run `./migrate.sh --skip-databases` to skip Postgres and Mongo. In the event Postgres, Mongo, and Vault are all externalized, you simply need to scp your keys from `/data/circle/circleci-encryption-keys` on the services box. You may `cat` these files and upload their contents to the 3.x configuration page.
