...
[NOTE]
====
The link:https://circleci.com/developer/orbs/orb/circleci/evals[CircleCI evals orb] supports integrations with Braintrust and LangSmith.
...
Test cases are composed of a name and an assertion. When configuring test cases, we suggest assigning a name based on the test scenario. The examples above, use a name based on an input metric: “`correctness`” , “`toxicity`” and "`labels`".
