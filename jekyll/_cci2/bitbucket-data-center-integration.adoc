...
. If you have not already done so, **add a `.circleci` directory to the root of the repository, then add a `config.yml` file in that directory**. When you commit this change in your repository, you should see the pipeline trigger for the first time on the CircleCI dashboard.
+
[TIP]
====
...
* **Verify the addition**: After running `ssh-keyscan`, you can verify that the keys have been added to the `known_hosts` file by checking the file's content, as follows:
+
[,source]
----
cat ~/.ssh/known_hosts
