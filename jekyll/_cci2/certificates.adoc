...
=== Adding the certificate to Replicated
 ...
 Replicated Management Console(`<circleci-hostname>.com:8800`) runs on different web server, so you need to apply the above certificate for Replicated as well.

. To do so, navigate to `https://hostname:8800/console/settings#tls-key-cert`

. Upload your newly generated certificate and key

. Click "Save" at the bottom of the page and restart the Replicated UI when prompted

Even if you can't access the Replicated Page with the hostname, you still can access by IP address.

Also, as alternative option, you can upload the certificate and key by following command on the ServiceBox.

```
$ sudo replicated console cert set <domain> /path/to/key /path/to/cert
