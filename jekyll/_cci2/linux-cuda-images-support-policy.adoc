...
This document outlines the xref:#[CircleCI Linux CUDA image] release, update, and deprecation policy and applies to our Linux CUDA Images.
...
Tagging for these images will be categorized with the following image slugs: 

* `linux-cuda-<CUDA-version>:<year.month.patch>`
** For example, `linux-cuda-11:2023.02.1`
** The “default” slugs will simply remove the tag if users want to keep up-to-date:
*** For example, `linux-cuda-11` or `linux-cuda-12`
* `linux-cuda-<CUDA-version>:edge`
** This will be available a week prior to general releases. These images incorporate any new changes for customers who want the latest version as fast as possible.
