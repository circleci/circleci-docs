...
A CircleCI project can have any number of pipelines, each defined in a separate configuration file. The configuration files can all be stored in the same repo, or even in different repos if you wish. 
Each pipeline can be configured to run on one or many specific trigger events.
If your existing configuration file is getting too large to manage, and it includes several unrelated workflows, consider splitting it into multiple files.
