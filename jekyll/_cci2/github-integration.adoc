...
Once the GitHub App is installed, it is recommend that each user authorize the GitHub App connection. To do this, users can navigate to **Project Settings > Pipelines > Add Pipeline > connections** and clicking the **"Authorize"** button.
Authorizing ensure that users can:
1. Trigger pipelines link:https://circleci.com/docs/triggers-overview/#run-a-pipeline-from-the-circleci-web-app[from the CircleCI web app].
2. Run builds, if the flag link:https://circleci.com/docs/prevent-unregistered-users-from-spending-credits/[Prevent unregistered user spend] is turned on.
3. Run builds, if they require access to link:https://circleci.com/docs/contexts/[contexts].
