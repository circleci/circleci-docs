...
The GitHub App and OAuth app **co-exist side-by-side in the same organization** without needing to sacrifice any functionality that is xref:version-control-system-integration-overview#feature-support-for-each-integration-type[not yet available to either of the two integrations] or needing to migrate to a new organization.
...
* The possibility to set up your pipelines so that the the **config file and the application code are stored in different repositories**. Refer to the link:https://circleci.com/changelog/unlocking-any-cross-repo-pipeline-and-trigger-setups-including-central/[changelog entry] for more information.
