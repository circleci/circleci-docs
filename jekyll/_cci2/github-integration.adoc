...
_Note: Deleting the OAuth webhook will prevent the xref:variables/#built-in-environment-variables[built-in environment variable] `CIRCLE_PULL_REQUEST` from being populated in all pipelines. This is a side effect we are working on, and will be resolved in the near future._

This action is reversible: to re-enable the OAuth trigger, go to menu:Project Settings[Overview], select “Stop building” at the bottom of the page, then select "Follow". This will create a new CircleCI webhook in your repository.
