...
Some existing and future functionality on CircleCI can only be enabled through the more secure xref:github-apps-integration#[CircleCI’s GitHub App integration].

Until October 2024, the CircleCI GitHub App Integration was available exclusively to organizations created after September 2023. **Now, all organizations can install the CircleCI GitHub App to access new functionality. This includes organizations that currently integratie with CircleCI's GitHub OAuth App.**
The GitHub App and OAuth app **co-exist side-by-side in the same organization** without needing to sacrifice any functionality that is xref:github-apps-integration#currently-not-supported[not yet available] to either of the two integrations (such as xref:scheduled-pipelines#[scheduled pipelines]) or needing to set up and migrate to a new organization.

Some key functionality enabled through the GitHub App integration is:

- The option to have **multiple pipelines in the same project**, each defined in its own YAML file
- The possibility to set up your pipelines so that the the **config file and the application code are stored in different repositories**
- A **more flexible trigger system**, with each pipeline having any number of VCS or non-VCS triggers. This includes:
    ...
    - [Coming soon] **More GitHub events as triggers**: Pipelines can be set up to run on events different than “push”, with more powerful customization of trigger conditions. 
...
To get started, go to Project Settings for one of your projects, select the "Pipelines" tab, and link:https://circleci.com/docs/pipelines/#add-or-edit-a-pipeline[add a new pipeline]. One of the key steps in creating yoru first pipeline will be installing your GitHub App and connecting to it.
Once you will have set up your first pipeline, you will be able to add triggers to it through the **Project Settings > Triggers** page.
...
Compared to the GitHub OAuth App, the xref:github-apps-integration#[CircleCI’s GitHub App integration] offers increased control over which resources in your VCS CircleCI can access, and lets CircleCI access those resources in a more secure manner. Specifically:
