...
[CAUTION]
====
Having both a GitHub App trigger *and* a GitHub OAuth trigger configured in the same project could result in duplicate builds. To prevent this behavior, make sure to disable the GitHub OAuth trigger. To do so:

. Navigate to the GitHub repository associated with the project.
. Go to "Settings", then "Webhooks".
. Locate the CircleCI webhook (`https://circleci.com/hooks/github`) and click "Delete".
====
