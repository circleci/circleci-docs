...
1. Run an SSH build using the Rerun Job with SSH button on the {% comment %} TODO: Jobs {% endcomment %}Builds page of the CircleCI app. The command to log into the container over SSH apears, as follows:

    ssh -p 64625 ubuntu@54.221.135.43
    

1. To add port-forwarding to the command, use the `-L` flag. The following example forwards requests to `http://localhost:3000` to port `8080` on the CircleCI container. This would be useful, for example, if your {% comment %} TODO: Job {% endcomment %}build runs a debug Ruby on Rails app, which listens on port 8080.
