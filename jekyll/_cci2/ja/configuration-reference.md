...
| タイプ | データ寿命 | 用途 | 参照先 | |\---\---\-----|\---\---\---\---\-----|\---\---\---\---\---\---\---\---\---\---\---\---|\---\---\--- | Artifacts | 1カ月単位 | artifacts の長期間に渡る保管 | **JOBS ページ**の Artifacts タブから参照可能。`tmp/circle-artifacts.<hash>/container` などの配下に格納される | | Workspaces | Workflow に従う | `attach_workspace:` ステップを用いて下流のコンテナで Workspace をアタッチする | `attach_workspace` を実行すると、Workspace の内容全体をコピー・再構築する | | Caches | 1カ月単位 | ジョブ実行の高速化に役立つ変化の少ないデータの保存に用いる。npm や Gem パッケージなど。 | The `save_cache` job step with a `path` to a list of directories to add and a `key` to uniquely identify the cache (for example, the branch, build number, or revision). Restore the cache with `restore_cache` and the appropriate `key`. |
{: class="table table-striped"}
