...
8. 2.0 Builders クラスタをインストールするには、`m4.xlarge` または `m4.4xlarge` を使用し、適切な AWS リージョンで、各 Builder に対し以下のパブリック CircleCI AMI のいずれかを起動します (希望する同時実行数によって起動する数を調整します)。 {#builder} **Note:**  CircleCI allocates 4GB per build container by default, consider using spot instances as Builder components.
...
1. [Get Started (開始)] ボタンをクリックして登録またはサインインします。 最初にログインしたユーザーなので、管理者になります。
1. [Hello World]({{site.baseurl}}/2.0/hello-world/) ページを使用してプロジェクトを追加します。
