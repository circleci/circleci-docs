...
CPUs | Processor | RAM | HD \-----|\---\---\---\---\---\---\---\---\---|\---\---\---\--- 2 | Intel(R) Xeon(R) @ 2.3GHz | 8GB | 100GB {: class="table table-striped"}
...
Using the `macos` executor allows you to run your {% comment %} TODO: Job {% endcomment %}build in a macOS environment on a VM (4CPU 8GB RAM). You can also specify which version of Xcode should be used. See the [Supported Xcode Versions section of the Testing iOS]({{ site.baseurl }}/2.0/testing-ios/#supported-xcode-versions) document for the complete list of version numbers.
...
{:.no_toc} - `name:tag` - `circleci/node:7.10-browsers` - `name@digest` - `redis@sha256:34057dd7e135ca41...`
