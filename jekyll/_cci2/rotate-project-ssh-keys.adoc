...
. Go to GitLab’s project settings for the repository and expand deploy keys section. The keys are named `circleci-pipeline-triggers` and you can match the fingerprint. It is recommended to remove any key created before the rotation.
