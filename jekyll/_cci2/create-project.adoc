...
+
TIP: If you cannot see your project, select the top bar to navigate to your user homepage and check you have selected the correct organization.
+
...
+
For guidance on creating a `config.yml` file, see the <<config-intro#,Configuration Introduction>>.
+
...
To make changes to your pipeline, either edit the `.circleci/config.yml` file in your repository, or select the ellipsis next your project in the Dashboard and choose **Configuration File**. This opens the CircleCI configuration editor, from where you can edit and commit your `config.yml` file.
