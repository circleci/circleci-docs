...
Please note: if using setup workflows, all pipeline parameters must be present in the subsequent configuration, even if calling with the API 
...
NOTE: When using setup workflows, all pipeline parameters must be present in the subsequent configuration, including in API calls. More information on setup workflows can be found in the xref:dynamic-config#how-dynamic-config-works[How dynamic config works] section of the Dynamic Configuration overview.
