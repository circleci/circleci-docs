...
include::../_includes/snippets/runner/container-runner-prereq.adoc[]
...
[source,shell]
----
helm upgrade --wait --timeout=5m container-agent container-agent/container-agent -n circleci -f values.yaml
----
...
[source,shell]
----
kubectl wait gateway --timeout=5m --all --for=condition=Programmed -n circleci
----
+
