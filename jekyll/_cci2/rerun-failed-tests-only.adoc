...
* If a shell script is invoked to run tests, `circleci tests run` should be placed **in the shell script** itself, and not `.circleci/config.yml`. Alternatively, see the xref:#output-test-files-only[section above] to pipe the list of test files to be run to a txt file and then pass the list of test file names to your shell script.
...
**Question:** What is the oldest job that can use "rerun failed tests only"

**Answer:** Rerunning failed tests only is currently available only for workflows that are less than 15 days old.
