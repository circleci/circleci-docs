...
```yaml
     -run:
        ...
        command: | 
          ...
          npm ci 
          npm run start &
          circleci tests glob "cypress/**/*.cy.js" | circleci tests run --command="xargs npx cypress run --reporter circleci-cypress-reporter --spec --test-results-path="test_results"" --verbose
