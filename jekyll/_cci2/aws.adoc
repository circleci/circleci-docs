...
. Next, to provision your instances, run:
+
```shell
terraform apply
```
...
. If you wish to use AWS Cloudwatch or Datadog for collating metrics for your installation, set this up here:

.Metrics
image:metrics_setup.png[]]

. Artifacts persist data after a job is completed, and may be used for longer-term storage of your build process outputs. By default, CircleCI Server only allows whitelisted artifact types to be served. This is to protect users from uploading, and potentially executing malicious content. The **Artifacts** setting here allows you to override this protection. For more information on safe/unsafe types see the Build Artifacts chapter in the https://circleci.com/docs/2.0/build-artifacts/[CircleCI Server Operations Guide]
...
. You can use https://github.com/circleci/realitycheck[our realitycheck repo] to check basic CircleCI functionality.
//<!--Is this what we hope they will do? Could use stronger language. Also would be good to have some help for what to do if jobs fail, or do we just want people to tell us so we can help?-->
...
== Teardown
If your wish to destroy you installation of CircleCI Server, please let us know first in case there are any specific, supplementary steps required for your installation. Below is our basic step by step guide to tearing down an installation of CircleCI Server that was made with Terraform:
...
. Navigate to the S3 dashboard, locate the S3 bucket associated with your CircleCI cluster and delete it/its contents (If you set `force_destroy_s3_bucket = "true"` in your `terraform.tfvars` file, skip this step).
