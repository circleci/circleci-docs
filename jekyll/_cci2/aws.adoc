...
Following is a step by step guide to installing CircleCI Server v{{site.server_version}} with Terraform.
...
+
CAUTION: If Docker Layer Caching (DLC) is to be used VM preallocation must either be set to `0` – on-demand, or all preallocated instances must be in use fro DLC to work.
