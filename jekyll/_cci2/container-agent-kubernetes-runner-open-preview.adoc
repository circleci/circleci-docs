...
To run a job with no custom configuration, simply add the following configuration to the helm chart `values.yaml`. Skip to <<Running a Job>> to run your first job, or keep reading to learn how to provide custom configuration to your pods
...
The pod config takes all fields that a normal [Kubernetes pod does](https://kubernetes.io/docs/reference/kubernetes-api/workload-resources/pod-v1/#debugging). If service containers are used in a CircleCI job the first `container` spec is used for all containers within the task pod, there is currently no way to provide different container configuration between service containers & the main task container.
