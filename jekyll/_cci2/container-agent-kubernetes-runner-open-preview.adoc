...
Just like the existing self-hostd runner, Container Agent allows users to run arbitrary code in the infrastructure where Container Agent is hosted meaning that a bad actor can use it as a method to gain knowledge of internal systems.  Ensure you are following all best practices for security to mitigate this risk.
