...
* Better handling of non-alphanumeric characters in authentication passwords for Docker executors. Fixed a bug that was preventing the use of `~` and `?` in the [Docker ID password field under the `auth` key]({{ '/2.0/private-images/#docker-executor' | docs_url }}).
* Improved the way commit statuses are handled to reduce instances of them being published out of order in certain situations.
* Security vulnerability patch for the `VM-scaler` service.
