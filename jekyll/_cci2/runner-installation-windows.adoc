...
The page below walks you through installing a machine runner and its dependencies (ie. Chocolatey, Git and Gzip) on your Windows Server.
...
By default, Windows machine runners run in <<runner-config-reference#runner-mode,single task mode>> in order to ensure high reliablity of the underlying technology that the self-hosted runner uses to execute jobs. This is the **recommended mode** for Windows self-hosted runners. 
