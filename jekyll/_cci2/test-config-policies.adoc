...
This guide assumes you are familiar with writing policies for use with the config policy management feature. If you would like more information or information on configuration policy management in general, or the processes around writing policies, see the xref:config-policy-management-overview.adoc[Config policy management overview] and xref:create-and-manage-config-policies#[Create and manage config policies] pages.

[#prerequisites]
== Prerequisites

The steps in this guide assume you have followed and completed the xref:/docs/use-the-cli-and-vcs-for-config-policy-management/#create-a-policy#[Create a policy] guide. In that guide you will have done the following:

* Enabled config policy management for your organization
* Created a simple policy to check CircleCI config versions within your org, so you have the following file structure: `./config-policies/version.rego`

[#write-a-policy-test]
== Write a policy test

In this how-to guide you will add a test for the `version.rego` policy to test how the policy will operate in practice, and check the decisions it will provide are valid.
...
----
+
NOTE: If you already have other policies in your `./config-policies` directory, you will probably see a different output, possibly including failures, at this stage.
...
[source,shell]
----
ok    test_absent_version        0.000s
ok    test_inferior_version      0.000s
ok    test_valid_version         0.000s
ok    test_version_wrong_type    0.000s
ok    config-policies            0.000s

4/4 tests passed (0.000s)
----
...
[source,shell]
----
circleci policy test ./config-policies --verbose --run "test_absent_version"
----
+
Expected output:
+
...
[source,shell]
----
circleci policy test ./config-policies --verbose --run "test_absent_version" --debug
----
+
Expected output:
+
...
[source,shell]
----
FAIL    test_absent_version    0.000s
   {
     "enabled_rules": [
-      "check_min_remote_docker_version",
+      "check_version",
...
FAIL    test_inferior_version    0.001s
   {
     "enabled_rules": [
-      "check_min_remote_docker_version",
+      "check_version",
...
FAIL    test_valid_version    0.000s
   {
     "enabled_rules": [
-      "check_min_remote_docker_version",
+      "check_version",
...
FAIL    test_version_wrong_type    0.000s
   {
     "enabled_rules": [
-      "check_min_remote_docker_version",
+      "check_version",
...
fail    config-policies    0.001s

0/7 tests passed (0.001s)
...
As an example, suppose we want to exclude certain projects from the version rule above. We can disable the rule for a specific project, using the `project_id`, by modifying the `enable_rule` statement in the `version.rego` file, as follows:

[source.rego]
----
exempt_project := "a944e13e-8217-11ed-8222-cb68ef03c1c6"

enable_rule["check__config_version"] { data.meta.project_id != exempt_project }
----

Add a test for this to the `test_version.yaml` file, specifying metadata to test the exemption. Add the following to the end of the file:
...
[source,shell]
----
circleci policy test ./config-policies/... -v
----
...
Using this method, OPA evaluates rules that start with `test_` and expects the output to be truthy. The `circleci policy test` command runs the OPA tests and reports them as `<opa.tests>`.

As an example, take the helper function below. This function takes a job value and returns the job name, and the corresponding tests:
...
When `circleci policy test` is run, any opa tests that the policy contains are run:
...
In verbose mode you can see the OPA tests that were run by name:
