...
----
+
NOTE: If you already have other policies in your `./config-policy` directory, you will probably see a different output, possibly including failures, at this stage.
...
Next, add a second policy and test to your `config-policies` directory. The steps below show hot to add a policy that specifies the minimum Docker version for xref:building-docker-images.adoc[remote Docker], writing tests for that policy, and running those tests.
