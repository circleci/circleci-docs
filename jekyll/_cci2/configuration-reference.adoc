...
Configure a job type. Options are `release`, `approval`, `no-op`, `build` (default). If a type is not specified, the job defaults to a `build` type.
...
The `no-op` type is used to configure a job that performs no actions and consumes no credits. This is commonly used to organise the order of operations within a workflow and make it easier to maintain. No `job` configuration is required for a `no-op` type job.
