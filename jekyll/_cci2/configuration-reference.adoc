...
| Y
| String
| `2`, `2.0`, or `2.1` See the xref:reusing-config#[Reusing Config] doc for an overview of 2.1 keys available to simplify your `.circleci/config.yml` file, reuse, and parameterized jobs.
|===
...
You can use the CircleCI CLI to split your test suite across parallel containers so the job completes in a shorter time. Learn more on the link:{{ site.baseurl }}/parallelism-faster-jobs/[Test splitting and parallelism] page.
...
See link:{{ site.baseurl }}/reusing-config/#parameter-syntax[Parameter Syntax]
// In this reference, it's not mentioned which types are allowed for job-parameters.
...
Learn more about execution environments and executors in the link:{{ site.baseurl }}/executor-intro/[Introduction to Execution Environments].
...
| N
| String or List
| The command used as executable when launching the container. `entrypoint` overrides the image's https://docs.docker.com/engine/reference/builder/#entrypoint[`ENTRYPOINT`].

| command
| N
| String or List
| The command used as pid 1 (or args for entrypoint) when launching the container. `command` overrides the image's `COMMAND`. It will be used as arguments to the image `ENTRYPOINT` if it has one, or as the executable if the image has no `ENTRYPOINT`.
...
For a link:{{ site.baseurl }}/glossary/#primary-container[primary container] (the first container in the list) if neither `command` nor `entrypoint` is specified in the config, then any `ENTRYPOINT` and `COMMAND` in the image are ignored.
...
A link:{{ site.baseurl
}}/custom-images/#adding-an-entrypoint[custom image] may disable this behavior and force the `ENTRYPOINT` to run.

You can specify image versions using tags or digest. You can use any public images from any public Docker registry (defaults to Docker Hub). Learn more about specifying images on the link:{{ site.baseurl }}/using-docker[Using the Docker Execution Environment] page.
...
Some registries, Docker Hub, for example, may rate limit anonymous docker pulls. It is recommended you authenticate in such cases to pull private and public images. The username and password can be specified in the `auth` field.  See link:{{ site.baseurl }}/private-images/[Using Docker Authenticated Pulls] for details.
...
For steps to get set up with OIDC to pull images from AWS ECR, see the link:/docs/pull-an-image-from-aws-ecr-with-oidc/[Pull and image from AWS ECR with OIDC] page.
...
More information on what software is available in each image can be found in our https://discuss.circleci.com/tag/machine-images[Discuss forum].

The machine executor supports link:{{ site.baseurl }}/docker-layer-caching[Docker Layer Caching] which is useful when you are building Docker images during your job or Workflow.
...
When using the Linux link:{{ site.baseurl }}/using-gpu[GPU executor], the available images are:
...
To use the https://circleci.com/developer/machine/image/android[Android image] directly with the machine executor, add the following to your job:
...
The Android image can also be accessed using the https://circleci.com/developer/orbs/orb/circleci/android[Android orb].

For examples, refer to the link:/docs/android-machine-image[Using Android Images with the Machine Executor] page.
...
* https://circleci.com/developer/machine/image/windows-server-2022-gui[windows-server-2022-gui]
* https://circleci.com/developer/machine/image/windows-server-2019[windows-server-2019]

More information on what software is available in each image can be found in our https://discuss.circleci.com/c/ecosystem/circleci-images/[Discuss forum].

Alternatively, use the https://circleci.com/developer/orbs/orb/circleci/windows[Windows orb] to manage your Windows execution environment. For examples, see the link:{{site.baseurl}}/using-windows/[Using the Windows Execution Environment] page.
...
When using the Windows link:{{ site.baseurl }}/using-gpu[GPU executor], the available image is:

* https://circleci.com/developer/machine/image/windows-server-2019-cuda[`windows-server-2019-cuda`]
...
CircleCI supports running jobs on https://developer.apple.com/macos/[macOS], to allow you to build, test, and deploy apps for macOS, https://developer.apple.com/ios/[iOS], https://developer.apple.com/tvos/[tvOS] and https://developer.apple.com/watchos/[watchOS]. To run a job in a macOS virtual machine, add the `macos` key to the top-level configuration for your job and specify the version of Xcode you would like to use.
...
| Y
| String
| The version of Xcode that is installed on the virtual machine, see the link:{{ site.baseurl }}/using-macos/#supported-xcode-versions[Supported Xcode Versions section of the Testing iOS] document for the complete list.
|===
...
We implement soft concurrency limits for each resource class to ensure our system remains stable for all customers. If you are on a Performance or custom plan and experience queuing for certain resource classes, it is possible you are hitting these limits. https://support.circleci.com/hc/en-us/requests/new[Contact CircleCI support] to request a raise on these limits for your account.
...
Use the `resource_class` key to configure a https://circleci.com/docs/runner-overview/[self-hosted runner instance].
...
*Arm on Docker* For pricing information, and a list of CircleCI Docker convenience images that support Arm resource classes, see the https://circleci.com/product/features/resource-classes/[Resource classes page].
...
If you are working on CircleCI server v3.1 and up, you can access the macOS execution environment using link:/docs/runner-overview[self-hosted runner].
...
In this case, the `checkout` step will checkout project source code into the job's <<jobs,`working_directory`>>.
...
NOTE: the `run` step replaces the deprecated `deploy` step. If your job has a parallelism of 1, the deprecated `deploy` step can be swapped out directly for the `run` step. If your job has parallelism >1, see link:{{site.baseurl}}/migrate-from-deploy-to-run[Migrate from deploy to run].
...
You can also configure commands to run <<background-commands,in the background>> if you don't want to wait for the step to complete before moving on to subsequent run steps.
...
For more information about which files are executed when bash is invocated, https://linux.die.net/man/1/bash[see the `INVOCATION` section of the `bash` manpage].
...
| Y
| Logic
| link:{{site.baseurl}}/configuration-reference/#logic-statements[A logic statement]
...
The checkout command automatically adds the required authenticity keys for interacting with GitHub and Bitbucket over SSH, which is detailed further in our link:/docs/github-integration#establish-the-authenticity-of-an-ssh-host[integration guide] -- this guide will also be helpful if you wish to implement a custom checkout command.
...
Allows Docker commands to be run locally. See link:{{ site.baseurl }}/building-docker-images/[Running Docker Commands] for details.
...
| N
| boolean
| Set this to `true` to enable link:{{ site.baseurl }}/docker-layer-caching/[Docker Layer Caching] in the Remote Docker Environment (default: `false`)

| version
| N
| String
| Version string of Docker you would like to use (default: `20.10.17`). View the list of supported docker versions link:{{site.baseurl}}/building-docker-images/#docker-version[here].
|===

*Notes*:

* `setup_remote_docker` is not compatible with the `machine` executor. See link:{{ site.baseurl }}/docker-layer-caching/#machine-executor[Docker Layer Caching in Machine Executor] for information on how to enable DLC with the `machine` executor.
...
Generates and stores a cache of a file or directory of files such as dependencies or source code in our object storage. Later jobs can <<restore_cache,restore this cache>>. Learn more on the link:{{site.baseurl}}/caching/[Caching Dependencies] page.

Cache retention can be customized on the https://app.circleci.com/[CircleCI web app] by navigating to menu:Plan[Usage Controls].
...
| The environment variable `variableName` (supports any environment variable link:{{site.baseurl}}/env-vars[exported by CircleCI] or added to a specific link:{{site.baseurl}}/contexts[context]--not any arbitrary environment variable).
...
* Wildcards are not currently supported in `save_cache` paths. Please visit the https://ideas.circleci.com/cloud-feature-requests/p/support-wildcards-in-savecachepaths[Ideas board] and vote for this feature if it would be useful for you or your organization.
...
Restores a previously saved cache based on a `key`. Cache needs to have been saved first for this key using the <<save_cache,`save_cache` step>>. Learn more in link:{{ site.baseurl }}/caching/[the caching documentation].
...
See <<run,run>> for current processes. If you have menu:parallelism[1] in your job, see the link:{{site.baseurl}}/migrate-from-deploy-to-run[Migrate from deploy to run] guide.
...
Step to store artifacts (for example logs, binaries, etc) to be available in the web app or through the API. See the link:{{ site.baseurl }}/artifacts/[Uploading Artifacts] document for more information.
...
Artifact storage retention can be customized on the https://app.circleci.com/[CircleCI web app] by navigating to menu:Plan[Usage Controls].
...
Special step used to upload and store test results for a build. Test results are visible on the CircleCI web application under each build's *Test Summary* section. Storing test results is useful for timing analysis of your test suites. For more information on storing test results, see the link:{{site.baseurl}}/collect-test-data/[Collecting Test Data] page.

It is also possible to store test results as a build artifact; to do so, please refer to <<storeartifacts,the *store_artifacts* step>>.
...
Special step used to persist a temporary file to be used by another job in the workflow. For more information on using workspaces, see the link:{{site.baseurl}}/workspaces/[Using Workspaces to Share Data Between Jobs] page.
...
Special step used to attach the workflow's workspace to the current container. The full contents of the workspace are downloaded and copied into the directory the workspace is being attached at. For more information on using workspaces, see the link:{{site.baseurl}}/workspaces/[Using Workspaces to Share Data Between Jobs] page.
...
Special step that adds SSH keys from a project's settings to a container. Also configures SSH to use these keys. For more information on SSH keys see the link:{{site.baseurl}}/github-integration/#create-additional-github-ssh-keys[Create additional GitHub SSH keys] page.
...
Pipeline values are available to all pipeline configurations and can be used without previous declaration. For a list of pipeline values, see the link:/docs/pipeline-variables/[Pipeline values and parameters] page.
...
Enables jobs to go through a set of well-defined IP address ranges. See link:{{ site.baseurl }}/ip-ranges/[IP ranges] for details.
...
* A paid account on a https://circleci.com/pricing/[Performance or Scale plan] is required to access IP ranges.
...
The scheduled workflows feature is set to be deprecated. Using *scheduled pipelines* rather than scheduled workflows offers several benefits. Visit the scheduled pipelines link:{{site.baseurl}}/migrate-scheduled-workflows-to-scheduled-pipelines[migration guide] to find out how to migrate existing scheduled workflows to scheduled pipelines. If you would like to set up scheduled pipelines from scratch, visit the link:{{site.baseurl}}/scheduled-pipelines[Scheduled pipelines] page.
...
| Y
| String
| See the http://pubs.opengroup.org/onlinepubs/7908799/xcu/crontab.html[crontab man page].
|===
...
The `branches` key controls whether the _current_ branch should have a schedule trigger created for it, where _current_ branch is the branch containing the `config.yml` file with the `trigger` stanza. That is, a push on the `main` branch will only schedule a link:{{ site.baseurl }}/workflows/#using-contexts-and-filtering-in-your-workflows[workflow] for the `main` branch.
...
You may use a `when` clause (the inverse clause `unless` is also supported) under a workflow declaration with a link:{{site.baseurl}}/configuration-reference/#logic-statements[logic statement] to determine whether or not to run that workflow.
...
Refer to the link:{{ site.baseurl }}/workflows[Workflows] for more examples and conceptual information.
...
A job may have a `type` of `approval` indicating it must be manually approved before downstream jobs may proceed. For more information see the link:{{site.baseurl}}/workflows/#holding-a-workflow-for-a-manual-approval[Using Workflows to Orchestrate Jobs] page.
...
You can read more about using regex in your config in the link:{{ site.baseurl }}/workflows/#using-regular-expressions-to-filter-tags-and-branches[Using Workflows to Schedule Jobs] page.
...
For more information, see the link:{{ site.baseurl }}/workflows/#executing-workflows-for-a-git-tag[Executing Workflows For a Git Tag] section of the Workflows document.
...
The `matrix` stanza allows you to run a parameterized job multiple times with different arguments. For more information see the how-to guide on link:{{site.baseurl}}/using-matrix-jobs[Using Matrix Jobs]. In order to use the `matrix` stanza, you must use parameterized jobs.
...
`matches` uses https://docs.oracle.com/javase/8/docs/api/java/util/regex/Pattern.html[Java regular
expressions]
