...
**Example** of a job with a `build` type. This is the default type and does not need to be configured:
...
The `no-op` type is used to configure a job that performs no actions and consumes no credits. This is commonly used to organise the order of operations within a workflow and make it easier to maintain. Only the `type` is required for a `no-op` type job, no further job configuration is required.
...
This behavior occurs because the primary container is typically only used for running the `steps` and not for its `ENTRYPOINT`, and an `ENTRYPOINT` may consume significant resources or exit prematurely.
...
A value of `always` means that the step will run regardless of the exit status of previous steps. This behaviour is useful if you have a task that you want to run regardless of whether the previous steps are successful or not. For example, you might have a job step that needs to upload logs or code-coverage data somewhere.
...
Pre and post steps allow you to execute steps in a given job without modifying the job. For example, to run custom setup steps before job execution.
