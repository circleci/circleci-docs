...
Using expression-based job filters is one way to optimize your pipelines to lower costs, decrease time to feedback, or run specific jobs based on context of the source of change.
...
Allow the job to run only on the project's `main` branch, or branches starting with `integration-test`:
...
Allow the job to run only on the `main` branch, and disallow use with pipelines xref:vs-code-extension-overview#test-run-your-config-from-vs-code[triggered with unversioned configuration]:
