...
When the `assignPublicIP` option is set to true, all EC2 instances created by pachine provisioner are assigned **public** IPv4 addresses, and, as such, all services communicating with them do so via their public addresses.

When hardening an installation where the machine provisioner uses public IPs, the following rules can be created:
