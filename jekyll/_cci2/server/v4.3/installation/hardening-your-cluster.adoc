...
When the `assignPublicIP` option is set to true, all EC2 instances created by Machine Provisioner are assigned **public** IPv4 addresses, and, as such, all services communicating with them do so via their public addresses.
