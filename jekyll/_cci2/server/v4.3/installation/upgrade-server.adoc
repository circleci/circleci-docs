...
* We have moved away from Vault to Tink for encryption. The process for migration is link:https://github.com/CircleCI-Public/server-scripts/tree/main/vault-to-tink[documented here] and includes a convenience script to move existing secrets. You should complete the migration to Tink on your 4.2 install before backing up your server installation, in preperation for upgrading to 4.3. Customers that don't perform this step may have issues restoring Vault from backup in 4.3. 
