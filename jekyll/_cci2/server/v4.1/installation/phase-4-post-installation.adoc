...
Before you begin with the CircleCI server v4.x post installation phase, ensure you have run through xref:/docs/server/v4.1/installation/phase-1-prerequisites#[Phase 1 – Prerequisites], xref:/docs/server/v4.1/installation/phase-2-core-services#[Phase 2 - Core Services Installation] and xref:/docs/server/v4.1/installation/phase-3-execution-environments#[Phase 3 - Execution Environments Installation].
...
xref:/docs/server/v4.1/operator/backup-and-restore/#troubleshooting[troubleshooting] section.
...
For more information and steps to complete these tasks, see the xref:/docs/server/v4.1/operator/managing-orbs#[Orbs on server guide].

ifndef::pdf[]
[#next-steps]
== Next steps

* xref:/docs/server/v4.1/installation/hardening-your-cluster#[Hardening Your Cluster]
* xref:/docs/server/v4.1/installation/migrate-from-server-3-to-server-4#[Server 4.x Migration]
* xref:/docs/server/v4.1/operator/backup-and-restore#[Backup & Restore]
endif::[]