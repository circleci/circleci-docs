...
Backups of CircleCI server can be created through link:https://velero.io/[Velero]. You installed Velero in your cluster during the prerequisites installation phase.
...
----
+
NOTE: `us-east-1` does not support a link:https://docs.aws.amazon.com/AmazonS3/latest/API/API_CreateBucket.html#API_CreateBucket_RequestBody[LocationConstraint]. If your region is `us-east-1`, omit the bucket configuration.
...
+
NOTE: Backups require link:https://restic.net/[restic] to operate. When installing Velero, ensure that you have the `--use-restic` flag set, as shown below:
+
[source, bash]
----
velero install \
...
. **Set up permissions for Velero**
+
CAUTION: If your server installation runs within a GKE cluster, ensure that your current IAM user is a cluster admin for this cluster, as RBAC objects need to be created. More information can be found in the link:https://cloud.google.com/kubernetes-engine/docs/how-to/role-based-access-control#iam-rolebinding-bootstrap[GKE documentation].
...
+
If you are already using link:https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity[Workload Identity] in your cluster, you can bind the GCP Service Account you just created to Velero's Kubernetes service account. In this case, the GCP Service Account needs the
...
+
NOTE: Backups require link:https://restic.net/[restic] to operate. When installing Velero, ensure that you have the `--use-restic` flag set.
...
* xref:migrate-from-server-3-to-server-4#[Server 4.x Migration]
