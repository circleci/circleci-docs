...
* You have taken a backup of the v3.x instance, following the instructions at the xref:server/v4.1/operator/backup-and-restore#[Backup and Restore] guide.
...
Execute the link:https://github.com/CircleCI-Public/server-terraform[Nomad Terraform] to re-create nomad client where `server_endpoint` value is set to be `<domain>:4647`. You can follow the steps mentioned xref:server/v4.1/installation/phase-3-execution-environments#nomad-clients[here].
...
* xref:server/v4.1/operator/operator-overview#[Server 4.x Operator Overview]
endif::[]
