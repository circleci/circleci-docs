...
Before you begin with the CircleCI server v4.x execution environment installation phase, ensure you have run through xref:/docs/server/v4.1/installation/phase-1-prerequisites#[Phase 1 – Prerequisites] and xref:/docs/server/v4.1/installation/phase-2-core-services#[Phase 2 - Core Services Installation].
...
* *Server_endpoint* - This is the domain name of the CircleCI application appended with port number `4647`. For example, `server.example.com:4647`.
...
The CircleCI link:https://github.com/CircleCI-Public/server-terraform/tree/main/nomad-gcp[nomad module] can create a service account using xref:/docs/server/v4.1/installation/phase-1-prerequisites#enable-workload-identity-in-gke[Workload Identity] and send out the email. Set the variables `nomad_auto_scaler = true` and `enable_workload_identity = true`.
--
...
+
External VMs need the networking rules described in xref:/docs/server/v4.1/installation/hardening-your-cluster/#external-vms[Hardening your Cluster]
...
CircleCI runner does not require any additional server configuration. Server ships ready to work with runner. However, you need to create a runner and configure the runner agent to be aware of your server installation. For complete instructions for setting up runner, see the xref:/docs/runner-overview#[runner documentation].
...
* xref:/docs/server/v4.1/installation/phase-4-post-installation#[Phase 4: Post Installation]
endif::pdf[]
