...
Before you begin with the CircleCI server v4.x execution environment installation phase, ensure you have run through xref:server/v4.1/installation/phase-1-prerequisites#[Phase 1 – Prerequisites] and xref:server/v4.1/installation/phase-2-core-services#[Phase 2 - Core Services Installation].
...
The CircleCI link:https://github.com/CircleCI-Public/server-terraform/tree/main/nomad-gcp[nomad module] can create a service account using xref:server/v4.1/installation/phase-1-prerequisites#enable-workload-identity-in-gke[Workload Identity] and send out the email. Set the variables `nomad_auto_scaler = true` and `enable_workload_identity = true`.
--
...
+
External VMs need the networking rules described in xref:server/v4.1/installation/hardening-your-cluster/#external-vms[Hardening your Cluster]
...
* xref:server/v4.1/installation/phase-4-post-installation#[Phase 4: Post Installation]
endif::pdf[]
