...
Before you begin with the CircleCI server v4.x core services installation phase, ensure all xref:/docs/server/v4.1/installation/phase-1-prerequisites#[prerequisites] are met.
...
The application requires a Kubernetes Secret containing signing and encrpytion keysets. These keysets are used to encrypt and sign artifacts generated by CircleCI. These keys were created during the xref:/docs/server/v4.1/installation/phase-1-prerequisites#encryption-signing-keys[prerequisites phase]. CircleCI will not be able to recover the values if lost. Depending on how you prefer to manage Kubernetes Secrets, there are two options.
...
Enter the domain name you specified when creating your xref:/docs/server/v4.1/installation/phase-1-prerequisites#frontend-tls-certificates[Frontend TLS key and certificate].
...
[#github]
==== GitHub
These instructions are for the GitHub.com, **not** GitHub Enterprise. Use the client ID and secret you created with your Github OAuth application in the xref:/docs/server/v4.1/installation/phase-1-prerequisites#create-a-new-github-oauth-app[prerequisites phase].
...
In the case of GitHub Enterprise add the `defaultToken` created in the xref:/docs/server/v4.1/installation/phase-1-prerequisites#create-a-new-github-oauth-app[prerequisite phase] to the `GitHub` section. The hostname should not include the protocol, ex: `github.exampleorg.com`.
...
Regardless of your storage provider, the bucket name you created during the xref:/docs/server/installation/phase-1-prerequisites#object-storage-and-permissions[prerequisites phase] will need to be included.
...
* configure the `no_proxy` hosts and subnets. This should include localhost, your GitHub Enterprise host (optional), the hostname of your CircleCI installation (see xref:/docs/server/v4.1/installation/installing-server-behind-a-proxy#known-limitations[Known Limitations] for an explanation), and the CIDRs of both vm-service and Nomad.
...
* xref:/docs/server/v4.1/installation/phase-3-execution-environments#[Phase 3: Execution Environments Installation]
endif::[]
