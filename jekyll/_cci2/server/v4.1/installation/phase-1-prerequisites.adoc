...
CircleCI server v4.x is installed as a helm chart. The installation process is broken down into four phases. There is a validation step at the end of each phase, allowing you to confirm success before moving to the next phase. Depending on your requirements, phases 3 and 4 may include multiple steps. This installation guide assumes you have already read the xref:server/v4.1/overview/circleci-server-v4-overview#[CircleCI Server v4.x Overview].
...
* xref:server/v4.1/installation/phase-3-execution-environments/#nomad-autoscaler-gcp[Nomad Autoscaler]
* xref:server/v4.1/installation/phase-3-execution-environments/#gcp-3[VM]
* xref:server/v4.1/installation/phase-1-prerequisites/#configuring-google-cloud-storage[Object-Storage]
...
[#next-steps]
== Next steps
* xref:server/v4.1/installation/phase-2-core-services#[Phase 2: Core Services Installation].
endif::[]
