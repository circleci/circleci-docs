...
Now you are ready to access the REPL. From a system with access to the private Docker registry and `kubectl` access to the cluster, run the below script. Please be sure to modify it to match your namespace if it is not `circleci-server`.
