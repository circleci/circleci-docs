...
CircleCI server can run Docker jobs on the Nomad clients, and also in dedicated virtual machines. These VM jobs are controlled by the Nomad clients, therefore the Nomad clients must be able to access the VMs on port 22 for SSH and port 2376 for xref:/docs/configuration-reference#setupremotedocker[remote Docker jobs].
...
CircleCI server v4.x uses Nomad as the primary job scheduler. Refer to our xref:/docs/server/v4.1/operator/introduction-to-nomad-cluster-operation#[Introduction to Nomad Cluster Operation] to learn more about the job scheduler and how to perform basic client and cluster operations.
