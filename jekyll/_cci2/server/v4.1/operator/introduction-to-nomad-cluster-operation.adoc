...
- **Nomad jobs:** A Nomad job is a specification, provided by a user, that declares a workload for Nomad. A Nomad job corresponds to an execution of a CircleCI job. If the job uses link:/docs/parallelism-faster-jobs[parallelism], for example `parallelism: 10`, then Nomad runs 10 jobs.
...
* Read the link:/docs/server/v4.1/operator/managing-user-accounts[Manging user accounts] guide.
endif::[]
