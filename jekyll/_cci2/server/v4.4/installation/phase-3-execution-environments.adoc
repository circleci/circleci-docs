...
Before you begin with the CircleCI server v4.4 execution environment installation phase, ensure you have run through xref:phase-1-prerequisites#[Phase 1 – Prerequisites] and xref:phase-2-core-services#[Phase 2 - Core Services Installation].
...
** Nomad Server is installed and configued within your Kubernetes cluster and is used for scheduling jobs
...
* <<machine-provisioner,Machine provisioner>> is used in AWS/GCP installation to provision resources and run jobs that are configured to use the `machine` execution environment, including Linux and Windows VMs. If you install outside AWS or GCP (that is, a local installation) you will use self-hosted runners to run VM-configured jobs.

* xref:../../../runner-overview#[Runner] can be used for all execution environments.
...
helm upgrade circleci-server oci://cciserver.azurecr.io/circleci-server -n $namespace --version {serverversion44} -f <path-to-values.yaml>
----
