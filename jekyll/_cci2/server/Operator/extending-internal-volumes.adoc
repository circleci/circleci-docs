...
- Server Admin
---
= CircleCI Server v4.x Internal Database Volume Expansion
...
== Overview
_Persistent volume expansion of MongoDB and Postgres is available for server v4.2.0 and higher._
...
NOTE: As a precaution, it is always a good idea to https://circleci.com/docs/2.0/server-4-operator-backup-and-restore/?section=server-administration[create a backup of your cluster] first.


=== Step 0 - Confirm current volume size
By default, the persistent volume claims used by our internal databases have a capacity of 8Gi. However, this initial value can be set at the time of first deployment via  helm-value file. You can confirm the size of your persistent volume claim capacity using the command: `kubectl get pvc <pvc-name>`.
