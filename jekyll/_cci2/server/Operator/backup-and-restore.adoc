...
- Server Admin
---
= CircleCI Server v4.x Backup and Restore
...
== Overview
_Backup and restore is available for server v4.1.0 and up._
...
To enable backup support you need to install and configure https://velero.io/[Velero] on your cluster.
...
NOTE:  When installing Velero, ensure that you have the `--use-restic` flag set, as shown below:
...
== Optional - Scheduling backups

See https://velero.io/docs/main/backup-reference/[Velero's documention] on creating scheudled backupts
