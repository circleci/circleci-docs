...
* You have taken a backup of the v3.x instance, following the instructions on the link:https://circleci.com/docs/2.0/server-4-operator-backup-and-restore[Server v4.x Backup and Restore Page]. 
...
-------------------------------------------------------------------------
helm diff upgrade <release-name> -n <namespace> -f <path>/server-kots-exporter/output/helm-values.yaml --show-secrets --context 5 <chart-directory>

Helm upgrade command -
-------------------------------------------------------------------------
cd <chart-directory>
helm dep update
helm upgrade <release-name> -n <namespace> -f <path>/server-kots-exporter/output/helm-values.yaml <chart-directory> --force
...
[source,shell]
----
 helm diff upgrade <release-name> -n <namespace> -f <path>/server-kots-exporter/output/helm-values.yaml --show-secrets --contexts 5 <chart-directory>
----
...
[source,shell]
----
helm upgrade <release-name> -n <namespace> -f <path>/server-kots-exporter/output/helm-values.yaml <chart> --force
----
...
=== Step 7 - Update DNS setting
Server 4.x migration is a destructive change for your DNS configuration. There is only one `load-balancer/external-ip` service `circleci-proxy` which you need to update in the DNS configuration.
...
=== Step 8 - Execute Nomad Terraform
Execute the link:https://github.com/CircleCI-Public/server-terraform[Nomad Terraform] to re-create nomad client where `server_endpoint` value is set to be `<domain>:4647`. 
