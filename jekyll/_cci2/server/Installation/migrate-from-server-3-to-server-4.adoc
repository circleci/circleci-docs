...
* You have taken a backup of the v3.x instance, following the instructions on the link:https://circleci.com/docs/2.0/server-3-operator-backup-and-restore[Server v3.x Backup and Restore Page]. 
...
Helm Diff command (only changes) - - helm diff upgrade <release-name> -n <namespace> -f <path>/server-kots-exporter/output/helm-values.yaml --show-secrets --context 5 <chart>
...
=== Step 7 - Update DNS setting
Server 4.x migration is a destructive change for your DNS configuration. There is only one load-balancer/external-ip. You now need to update this in your DNS configuration.
...
=== Step 8 - Execute Nomad Terraform
Execute the link:https://github.com/CircleCI-Public/server-terraform[Nomad Terraform] to re-create nomad client where `server_endpoint` value to be `<domain>:4647`. 
