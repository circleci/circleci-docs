...
+
We recommend using https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity[workload identities] to allows workloads/pods in your GKE clusters to impersonate Identity and Access Management (IAM) service accounts to access Google Cloud services. Use the following command to provision a simple cluster:
+
```shell
...
If you already have a GKE cluster and need to enable workload identities on the cluster and the node pools.
...
* *Kubernetes Namespace* - The Kubernetes namespace for CircleCI server installation.

* You can choose one of the following options for workload/pod authentication:
** *Service Account Email* - Service Account Email id if using Google Workload Identity. (**Recommended**)
...
+
```shell
gcloud iam service-accounts create circleci-storage --description="Service account for CircleCI cloud storage" --display-name="circleci-storage"
```
...
+
This is required so that the workloads/pods can access the storage bucket from the cluster using the kubernetes service account `"<K8S_NAMESPACE>/object-storage"`.
+
```shell
...
. **JSON Key File**
+
This step is NOT required if using link:https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity[Workload Identities] which is the recommended method for workload/pod authentication.
+
Use the following command to generate static credentials:
+
```shell
gcloud iam service-accounts keys create <KEY_FILE> \
