...
Before you begin with the CircleCI server v4.7 core services installation phase, ensure all prerequisites are met, as described in the xref:phase-1-aws-prerequisites#[Phase 1 - AWS Prerequisites] or xref:phase-1-gcp-prerequisites#[Phase 1 - GCP Prerequisites] sections.
...
Regardless of your storage provider, the bucket name you created during the prerequisites phase (xref:phase-1-aws-prerequisites#object-storage-and-permissions[AWS], xref:phase-1-gcp-prerequisites#object-storage-and-permissions[GCP])  will need to be included.
