...
Complete audit logs may be downloaded as a CSV file from the audit log page within the admin section of the application. Audit log fields with nested data contain JSON blobs. Note that the audit log download can take a very long time to start; we recommend clicking the **Download** button once and leaving it to run.
