...
==== VPC & Cluster Sizing Recommandations
While there are no strict requirements for VPC setup or disk size, the following practices are recommended for optimal performance and stability in production environments.

1. VPC Selection
  - It's advised to create a new VPC specifically for CircleCI Server and its components, rather than using an existing one.
