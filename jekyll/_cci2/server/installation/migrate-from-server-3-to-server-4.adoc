...
helm registry login cciserver.azurecr.io --username <image-registry-username> --password <image-registry-password>
...
If job does not exists then check the pod log `circle-migrator.log` in `log` directory and pod log can be is look like below -
[source,shell]
----
...
Else, contact with your circleci point of contact for further instructions.
...
helm registry login cciserver.azurecr.io -u $USER_NAME -p $PASSWORD
