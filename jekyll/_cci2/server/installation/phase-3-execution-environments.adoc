...
[source, shell]
----
cat the_above_config_file | base64
----

Then, using that additional base64 encoded config, and your AWS access key and secret, create the secret.

[source, shell]
----
# Base64 encoded additional configuration field
ADDITIONAL_CONFIG=dGhlIGJhc2U2NCBlbmNvZGVkIGNvbmZpZwo=

kubectl create secret generic nomad-autoscaler-secret \
  --from-literal=awsAccessKey=<access-key> \
  --from-literal=awsSecretKey=<secret-key> \
...
This key may be added directly to the values.yaml file, or the secret can be created manually. When creating the secret manually, an additional field is required - see the below example.
...
* <<phase-4-post-installation#,Phase 4: Post Installation>>
endif::pdf[]
