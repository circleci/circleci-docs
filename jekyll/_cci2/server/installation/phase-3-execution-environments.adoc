...
----
# Fetch VPC ID
aws eks describe-cluster --name=<YOUR_CLUSTER_NAME> --query "cluster.resourcesVpcConfig.vpcId" -output text | xargs

# Fetch Subnet IDs
aws eks describe-cluster --name=<YOUR_CLUSTER_NAME> --query "cluster.resourcesVpcConfig.subnetIds" --output text | xargs
----
+
...
[source,shell]
----
aws ec2 describe-vpcs --filters Name=vpc-id,Values=<YOUR_VPCID> --query "Vpcs[].CidrBlock" --output text | xargs
----
+
...
The CircleCI link:https://github.com/CircleCI-Public/server-terraform/tree/main/nomad-gcp[nomad module] can create a service account using link:/docs/server/installation/phase-1-prerequisites#enable-workload-identity-in-gke[Workload Identity] and send out the email. Set the variables `nomad_auto_scaler = true` and `enable_workload_identity = true`.
--
...
# Fetch CIDR Block
aws eks describe-cluster --name=<YOUR_CLUSTER_NAME> --query "cluster.kubernetesNetworkConfig.serviceIpv4Cidr" --output text | xargs
----
...
----
# find CIDR block
aws ec2 describe-subnets --subnet-ids=<NOMAD_SUBNET_ID> --query "Subnets[*].[SubnetId, CidrBlock]" | xargs
----
+
[source,shell]
----
