...
Web アプリの左上に、組織の切り替えメニューがあります。 You can view workflows or single jobs by expanding a pipeline and clicking in on any workflow or job descriptor.
...
Clicking the three dots (meatball menu) in the project row will allow you to open an individual project's configuration file, Insights, and settings. フォローしているプロジェクトに対しては、このメニューにフォローを解除するオプションも表示されます。 There is also a **Project Settings** button found on individual project pages, which you can view by clicking on a project's name in the main **Projects** dashboard.
