...
For CircleCI Enterprise installations, you can use the `+image.tag+` to set a pinned version of launch agent. 互換性のあるバージョンタグについては、xref:runner-installation-cli.adoc#self-hosted-runners-for-server-compatibility[セルフホストランナーのインストール] を参照して下さい。
