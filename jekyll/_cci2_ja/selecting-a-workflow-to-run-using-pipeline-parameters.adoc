...
以下の例ではデフォルトで `build` ワークフローを実行し、API を使って他にどのワークフローを実行するかを制御することができます。 次に、API v2 の link:https://circleci.com/docs/api/v2/#operation/triggerPipeline[新しいパイプラインのトリガー] エンドポイントを使って別のワークフローを選択し、`action` に別の値を指定する例を示します。
...
この `action` パラメーターは、プロジェクトへのプッシュ時にデフォルトで `build` されます。