...
Windows Executor を使用している場合は、SSH 接続を行うシェルを渡す必要があります。 たとえば、ビルド内で `powershell` を実行するには、`ssh -p <remote_ip> -- powershell.exe` とします。 Consider reading the [Hello World on Windows]({{site.baseurl}}/hello-world-windows) document to learn more.
...
[GitHub と Bitbucket の連携]({{ site.baseurl }}/gh-bb-integration/)
