...
CircleCI's [Reusable Configuration]({{site.baseurl}}/ja/2.0/reusing-config/) features allow you to define parameterizable configuration elements and re-use those elements throughout a project config file. It is recommended you become familiar with the full [Configuration Reference]({{site.baseurl}}/ja/2.0/configuration-reference/) features before moving on to the [Reusable Configuration Reference]({{site.baseurl}}/ja/2.0/reusing-config/).

### Commands
{: #commands }

Commands contain one or more steps in which [parameters]({{site.baseurl}}/ja/2.0/reusing-config/#using-the-parameters-declaration) can be used to modify behavior. Commands are the logic of orbs and are responsible for executing steps such as [checking out code](https://circleci.com/docs/ja/2.0/configuration-reference/#checkout), or running shell code, for example, running bash or CLI tools. For more information see the [Authoring Reusable Commands]({{site.baseurl}}/ja/2.0/reusing-config/#authoring-reusable-commands) guide.
...
Executors are parameterized execution environments in which [jobs]({{site.baseurl}}/ja/2.0/orb-concepts/#jobs) can be run. CircleCI provides multiple [executor options]({{site.baseurl}}/ja/2.0/configuration-reference/#docker--machine--macos--windows-executor):
...
For more information, see the guide to [Authoring Reusable Executors]({{site.baseurl}}/ja/2.0/reusing-config/#authoring-reusable-executors) and the registry page for the [Node Orb](https://circleci.com/developer/orbs/orb/circleci/node#executors-default).

### Jobs
{: #jobs }

[Jobs]({{site.baseurl}}/ja/2.0/reusing-config/#authoring-parameterized-jobs) define a collection of [steps](https://circleci.com/docs/ja/2.0/configuration-reference/#steps) to be run within a given [executor]({{site.baseurl}}/ja/2.0/orb-concepts/#executors), and are orchestrated using [Workflows]({{site.baseurl}}/ja/2.0/workflows/). Jobs will also individually return their status via [GitHub Checks](https://circleci.com/docs/ja/2.0/enable-checks/).
...
_[See: Writing Inline Orbs]({{site.baseurl}}/ja/2.0/reusing-config/#writing-inline-orbs) for more information on inline orbs._
...
In order to _pack_ an orb, an [@orb.yml]({{site.baseurl}}/ja/2.0/orb-author/#orbyml) file must be present. The `@` signifies the _root_ of our orb project. Within the same directory, you can include additional directories for each orb component's type, such as [commands]({{site.baseurl}}/ja/2.0/reusing-config/#authoring-reusable-commands), [jobs]({{site.baseurl}}/ja/2.0/reusing-config/#authoring-parameterized-jobs), [executors]({{site.baseurl}}/ja/2.0/reusing-config/#authoring-reusable-executors), and [examples]({{site.baseurl}}/ja/2.0/orb-concepts/#usage-examples). Any additional files or folders will be safely ignored.

Additionally, the _pack_ command provides a special pre-processor for orb developers that allows you to import code from external files using the [file include syntax]({{site.baseurl}}/ja/2.0/orb-concepts/#file-include-syntax) (`<<include(file)>>`).
...
View more about including bash scripts in the [Orb Author]({{site.baseurl}}/ja/2.0/orb-author/#scripts) guide.
...
- Refer to [Orb Introduction]({{site.baseurl}}/ja/2.0/orb-intro/) for a high-level overview of CircleCI orbs.
- Refer to [Orbs Reference]({{site.baseurl}}/ja/2.0/reusing-config/) for detailed reference information about Orbs, including descriptions of commands, jobs and executors.
- Refer to [Orbs FAQs]({{site.baseurl}}/ja/2.0/orbs-faq/) for information on frequent issues encountered when using orbs.
