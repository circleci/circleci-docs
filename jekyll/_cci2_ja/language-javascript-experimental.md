...
This guide uses a sample Node.JS application to describe configuration best practices for Node.JS applications building on CircleCI. The application is [hosted on GitHub]({{site.gh_public_org_url}}/circleci-demo-javascript-react-app) and is [building on CircleCI]({{site.cci_public_org_url}}/circleci-demo-javascript-react-app){:rel="nofollow"}.

Consider [forking the repository]({{site.gh_help_articles_url}}/fork-a-repo/) and rewriting [the configuration file]({{site.gh_public_org_url}}/circleci-demo-javascript-react-app/blob/master/.circleci/config.yml) as you follow this guide.
...
完了です。 You just set up a Node.js app to build on CircleCI. Check out your project’s [pipeline page]({{site.baseurl}}/2.0/project-build/#overview) to see how this looks when building on CircleCI.
