...
[NOTE]
====
**CircleCIでのGitHub認証が変更されます**。 2023年8月より、CircleCI アカウントを GitHub で認証する際、**GitHub OAuth** アプリではなく、CircleCI の **GitHub App** から認証されるようになります。どのアカウントタイプかは、CircleCI Web アプリにアクセスし、ブラウザで URL を確認することで確認できます：

* このスタイルの URL は、**GitHub App** で認証されたことを示します: `https://app.circleci.com/pipelines/circleci/UTxCZDiJ9MLGLC8hR1ZDmg`
* このスタイルの URL は、**GitHub OAuth app** で認証されたことを示します: `https://app.circleci.com/github/<your GitHub username>`

GitHub Apps と OAuth　の違いに関しては、こちらの link:https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/differences-between-github-apps-and-oauth-apps[GitHub Apps と OAuth アプリの違い]をご覧ください。
====

＊お客様の CircleCI　アカウントが **GitHub OAuth アプリ**で認証されている場合は、このページが適応されますので引き続きお読みください。

[#introduction]
== GitHub 連携に関する概要

CircleCI にプロジェクトを追加すると、新規アカウント登録時に CircleCI に与えた権限を使用して、以下の設定がリポジトリに追加されます。
...
NOTE: CircleCI は絶対に必要なアクセス許可しか要求しません。 また、CircleCI が要求できるアクセス許可は GitHub OAuth アプリ が提供すると決めたアクセス許可のみに制限されます。 たとえば、全ユーザーのリポジトリ (パブリックとプライベートの両方) の一覧を GitHub から取得するには、 https://developer.github.com/apps/building-oauth-apps/understanding-scopes-for-oauth-apps/#available-scopes[`repo` スコープ] の権限が必要で、これは書き込みアクセス許可に相当します。 GitHub はユーザーのリポジトリの一覧に対して読み取りのみの権限は提供していないため、このような設定が必要になります。
