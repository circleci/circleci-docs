...
* フロントエンドのロードバランサーが廃止され、代わりに Ingress リソースとTraefik Ingress コントローラが使用されるようになりました。 この互換性を損なう変更に伴い、DNSの再設定が必要になります。 詳細と手順については、https://circleci.com/docs/ja/2.0/server-3-whats-new/#release-3-1-0[CircleCI Server の新機能に関するページ]を参照してください。
