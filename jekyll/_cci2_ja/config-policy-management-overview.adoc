...
このフォーマットを使用すると、ポリシー作成者 は、CircleCI のポリシー評価の出力を解析する能力に影響を与えることなく、カスタムヘルパー関数を作成できます。 ポリシーはすべて `input` 変数を使って設定データにアクセスできます。 `input` は評価されるプロジェクトの設定です。  'input' は CircleCI の設定ファイルと一致するので、'jobs' や 'workflows' などの設定ファイルの任意の要素にご希望の措置を適用するルールを記述することができます。
