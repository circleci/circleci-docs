...
CAUTION: 設定ファイルのポリシー管理機能は現在オープンプレビュー段階であるため、コンプライアンスの用途では **使用しないでください**。 The feature might become unavailable occasionally during **open preview**, during which build configurations may **not** be evaluated against policies.
