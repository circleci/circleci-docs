...
ステップで環境変数を設定するには、 xref:configuration-reference#run[`environment` キー] を使用します。 これは、 `PATH` を変更するときや、他の変数を参照する環境変数を設定するときに便利です。
...
NOTE: すべての `run` ステップが新しいシェルであるため、環境変数はステップ間で共有されません。
