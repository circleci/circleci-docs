...
CircleCI では環境変数の設定時の挿入はサポートしていませんが、xref:env-vars#parameters-and-bash-environment[`BASH_ENV` を使用して] 現在のシェルに変数を設定することは可能です。
...
NOTE: シェルによっては、`~/.tcshrc` や `~/.zshrc` などのシェルスタートアップファイルに新しい変数を付加しなければならない場合があります。 複数のステップで環境変数にアクセスできるようにする必要がある場合は、xref:env-vars#parameters-and-bash-environment[`BASH_ENV` を使用して] 値をエクスポートします。
