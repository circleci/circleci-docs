...
==== 暗号化された EBS ボリューム
AWS EC2 のダッシュボードで、Account Attributes を常に https://aws.amazon.com/premiumsupport/knowledge-center/ebs-automatic-encryption/[encrypt new EBS volumes] するように変更します。 このオプションを有効にすると、VM サービスにより作成されたすべての新しい EBS ボリュームが暗号化されるようになります。 このオプションは、リージョンごとの設定であり、CircleCI Server を実行しているリージョンで有効化する必要があります。
