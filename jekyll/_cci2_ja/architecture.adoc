...
This document outlines the containerized services that run on the Services machine within a CircleCI server v2.x installation. このドキュメントは、サービス運用の概要を理解し、サービス停止時のトラブルシューティングに役立てていただくことを目的としています。 以下の表に、補足と凡例を合わせて示しています。
