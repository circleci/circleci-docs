...
| `domain-service`
| CircleCI ドメイン モデルに関する情報を保存、提供します。
| ワークフローを開始できなくなります。 一部の REST API 呼び出しが失敗し、CircleCI UI で `500` エラーが発生する可能性があります。 LDAP 認証を使用している場合、すべてのログインに失敗するようになります。
| icon:fire-extinguisher[]
...
| メトリクスが動作しなくなりますが、ジョブは引き続き実行できます。
| icon:check-circle[]
| なし
...
| HashiCorp 社の Vault (キー管理、セキュアなストレージ、その他の暗号化関連サービスを提供する暗号化サービス) のインスタンスです。 `contexts-service` の暗号化とキー保存の処理に使用されます。
