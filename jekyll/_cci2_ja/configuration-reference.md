...
**注: ** `run` ステップは、廃止予定の `deploy` ステップに代わるものです。 ジョブの並列処理が 1 つの場合、廃止予定の `deploy` ステップは、 `run` ステップに直接スワップアウトできます。 ジョブの並列処理が > 1 つの場合は、[ワークフロー](#workflows)と関連付けられている[フィルタリング]({{site.baseurl}}/2.0/configuration-reference/#jobfilters)および/ または[パイプラインのスケジュール実行]({{site.baseurl}}/2.0/scheduled-pipelines/)を使用します。 詳細は、

ファンアウトとファンインの例を参照して下さい。 </p> 
...
CircleCI のオブジェクトストレージにある、依存関係やソースコードのようなファイル、ディレクトリのキャッシュを生成し、保存します。 キャッシュはその後のジョブで[復元](#restore_cache)することができます。 詳細については、[キャッシュに関するドキュメント]({{ site.baseurl }}/ja/2.0/caching/)を参照してください。
...
この例では、`v1-myapp-cache-new` で示したキャッシュがリストアされます。最初のキー（`v1-myapp-cache`）にもマッチはしていますが、後の方のキーが `v1-myapp-cache` というプレフィックスで検索したときに一番最後にマッチしたものになるからです。

キーの詳しい書式については、[`save_cache` ステップ](#save_cache)の `key` セクションをご覧ください。

CircleCI が `keys` のリストを処理するときは、最初にマッチした既存のキャッシュをリストアします。 通常は、より特定度の高いキー (たとえば、`package-lock.json` ファイルの正確なバージョンに対応するキー) を最初に記述し、より汎用的なキー (たとえば、プロジェクトの任意のキャッシュが対象となるキー) をその後に記述します。 キーに該当するキャッシュが 1 つもない場合は、警告とともにステップはスキップされます。
...
Web アプリケーションや API を通じて使うアーティファクト（ログ、バイナリなど）を保存するステップです。 詳細については、[アーティファクトに関するドキュメント]({{ site.baseurl }}/2.0/artifacts/)を参照してください。
...
root キーは、ワークスペースのルートディレクトリとなるコンテナ内のディレクトリを指します。 一方、paths の値は必ずルートへの相対ディレクトリとなります。
...
ワークフローで使用しているワークスペースを現在のコンテナにアタッチするのに利用する特殊なステップです。 ワークスペース内のコンテンツは完全な状態でダウンロードされ、ワークスペースがアタッチされているディレクトリにコピーされます。
...
ワークフロー1 つ 1 つは、それぞれに一時的なワークスペースが関連付けられています。  ワークスペースは、ジョブの実行中にビルドした固有のデータを、同じワークフローの他のジョブに渡すために使用されます。 ジョブ内では `persist_to_workspace` ステップでワークスペースにファイルを追加でき、さらに `attach_workspace` ステップを呼び出すと、ワークスペース内のファイルをアタッチしたファイルシステムにダウンロードできます。 ワークスペースで可能なのはファイルの追加のみです。ジョブからワークスペースにファイルは追加できても、ワークスペースからファイルを削除することは不可能になっています。 ファイルを受け取る側のジョブは、ファイルを渡す側のジョブによってワークスペースに追加されたファイルしか参照できないことになります。 ワークスペースをアタッチするとき、上流にある各ジョブの「レイヤー」は、ワークフローグラフで表示される順序通りに上流のジョブから適用されます。 しかし、2 つのジョブを同時に実行すると、そのレイヤーの適用順序は確定できません。 同時に実行した複数のジョブが同じファイル名でデータを保持し ワークスペースにアタッチするようなケースでは、エラーが発生しますのでご注意ください。
...
| タイプ      | 存続期間               | 用途                                                           | 例                                                                                                                                             |
| -------- | ------------------ | ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------- |
| アーティファクト | 1 か月               | 長期アーティファクトを保存します。                                            | **[Job]** ページの [Artifacts] タブから参照できます。 `tmp/circle-artifacts.<hash>/container` などのディレクトリの下に格納されています。                                    |
...
ワークスペースや キャッシュ、アーティファクトに関する詳細は、[ワークフローにおけるデータの永続化: キャッシュ、アーティファクト、ワークスペースの使用](https://circleci.com/blog/persisting-data-in-workflows-when-to-use-caching-artifacts-and-workspaces/)を参照してください。
...
プロジェクト設定でコンテナに対して SSH キーを登録する特殊なステップです。 指定するキーを使用するように SSH を設定します。
...
**注 :** CircleCI は登録された全ての SSH キーに対して `ssh-agent` を通じて署名を行います。ただし、コンテナにキーを実際に登録するには、コンフィグファイル内で `add_ssh_keys` を**必ず**使用する必要があります。
...
あらゆるジョブの自動化に用います。 各ワークフローは、キーとなるワークフロー名と、値となるマップで構成します。 Workflow の名前は `config.yml` ファイル内でユニークでなければなりません。 ワークフローの設定でトップレベルに置くべきキーは、`version` と `jobs` になります。
...
ワークフローの実行契機となるトリガーを指定します。 デフォルトの動作では、ブランチにプッシュされたときにワークフローがトリガーされます。
...
ジョブは、各組織で設定したグローバル環境変数を使えるように設定ことも可能です。設定画面で コンテキストを追加する方法については[コンテキスト]({{ site.baseurl }}/2.0/contexts)を参照してください。
...
CircleCI は明示的にタグフィルターを指定しない限り、タグに対して Workflows は実行しません。 さらに、ジョブが (直接的または間接的に) 他のジョブを必要とする場合は、それらのジョブにタグ フィルターを指定する必要があります。

tags では `only` キーと `ignore` キーが使えます。 スラッシュで囲むことで正規表現でタグに一致させたり、そのような文字列のリストでマップさせたりできます。 正規表現は、文字列**全体**に一致させる必要があります。 CircleCI では軽量版と注釈付き版のどちらのタグにも対応しています。
