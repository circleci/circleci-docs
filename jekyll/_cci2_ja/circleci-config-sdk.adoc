...
* CLI ツールとのビルドインテグレーションまたはブラウザーベースの操作を実現する

たとえば、生成した静的な設定ファイルを、CLI ツールで CircleCI 設定を作成する際に使用したり、 link:https://github.com/CircleCI-Public/visual-config-editor/[ビジュアル設定エディター] などのブラウザーベースのツール環境で利用できます。
