...
|Constraint Checker を有効にするかどうかの指定
|false
...
コンテナエージェントを使用すると、Kubernetes のすべての設定を含むタスクポッドを設定できます。 つまり、制約によりスケジュール実行できない方法でポッドを設定できる可能性があります。 これを支援するために、コンテナエージェントには、ポッドをスケジュールできるように、クラスタの現在の状態と照らし合わせて各リソースクラスの設定を定期的に検証する Constraint Checker が用意されています。 これにより、コンテナエージェントがスケジュールできないジョブを要求し、失敗するのを防ぐことができます。

Constraint Checker によるチェックの失敗が多すぎた場合、再びチェックに合格するようになるまでそのリソースクラスの要求は無効になります。

このクラスタの状態に対して、現在以下の制約がチェックされています。
