...
コンテナエージェントは、他のワークロードがない状態で、Kubernetes 名前空間で実行されていることを前提としています。 エージェントまたはガベージコレクション (GC) が、同じ名前空間のポッドを削除してしまう可能性があります。
...
<<#running-a-job,Running a job>> に進んで最初のジョブを実行するか、このまま読み進んで、ポッドへのカスタム設定の適用方法をご覧ください。
...
- 具体的には、クラスタ内のコンテナエージェントを使用して実行されるようにジョブをルーティングするため、コンテナエージェントのジョブ用に作成したリソースクラスを使用するようにリソースクラスのスタンザを更新します。
+
```YAML
...
コンテナエージェントは、クラスタに残ったままの、 `app.kubernetes.io/managed-by=circleci-container-agent` というラベルが付いたポッドやシークレットを削除するガベージコレクタを備えています。 デフォルトでは、これによって、5 時間 5 分を経過したジョブがすべて削除されます。 この時間は `agent.kubeGCThreshold` パラメーターを使って短縮または延長できます。 ただし、ガベージコレクション (GC) の頻度を下げると、 `agent.maxRunTime` パラメーターで指定するタスクの最大実行時間も短くなり、GC 頻度より小さい値になります。 そうでない場合、実行中のタスクポッドが GC によって削除される可能性があります。

コンテナエージェントは、終了シグナルを送信すると、ドレインして再起動します。 オープンプレビューの現時点では、コンテナエージェントが、起動に失敗したタスクを自動的に再起動することはありません。 これは、CircleCI Web アプリで行うことができます。

現時点では、コンテナエージェントがクラッシュしても、処理中またはキューで待機中のタスクが適切に扱われることは期待できません。 オープンプレビューの過程で、クラッシュ時の対処方法が追加され、文書化される予定です。

[#cost-and-availability]
== コストと可用性

コンテナエージェントのジョブは、<<persist-data#managing-network-and-storage-use,ランナーネットワーク通信>> に対応できます。 これは、セルフホストランナーの既存の料金モデルとも合致し、CircleCI の他のネットワーク/ストレージ料金体系にも近づく予定です。 ご不明な点があれば、CircleCI の担当者にお問い合わせください。
