...
=== オープンプレビューの間に、コンテナエージェントの機能が更新される予定はありますか。

はい。この製品では現在も開発が進んでいます。 コンテナエージェント自体への更新は、デプロイされているコンテナエージェントに自動的に適用されるはずです。 ご利用中のお客様に行っていただく操作はありません。
...
大幅な機能変更があった場合は、このページの内容を更新してお知らせいたします。
