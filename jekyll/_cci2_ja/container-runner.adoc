...
**** CircleCI 設定で、ビルドイメージジョブを作成します。 `setup_remote_docker` を使用せずに、イメージをビルドするための Docker コマンドを列挙し、前の手順で作成したビルドイメージのリソースクラスを指定します。 ビルドイメージジョブが、ビルドされたイメージを使用するジョブより先に実行されるようにしてください。 At the end of your build image job, push the image up and subsequently use container agent to pull that image and run your Docker job(s).
