...
* Access to a clean build environment with every job which downloads an arbitrary container image and executes the job in a container, taking full advantage of lightweight container-based technologies.

Container runner is a technology now in open preview that a user can install in a private Kubernetes (k8s) cluster which enables them to run containerized CI jobs on self-hosted compute, similar to how jobs that use the native Docker executor run on CircleCI’s cloud platform.
...
Before using any self-hosted runner on CircleCI, <<runner-installation#circleci-web-app-installation,create>> a <<runner-concepts#namespaces-and-resource-classes,resource class>> and its associated resource class token. これは、CircleCI Web アプリまたは <<runner-installation-cli#,CircleCI CLI>> を使って行えます (CircleCI Web アプリを推奨)。 The resource class token will be used in the `agent.resourceClasses` parameter below.
...
Container runner uses a Helm chart for installation. `container-agent` というリリース名で Helm チャートをインストールするには、次を実行します。
...
*** `agent.resourceClasses` is the only default value that must be overridden to run your first job.
...
カスタマイズなしでジョブを実行するには、次の設定を Helm チャートの `values.yaml` に追加します。 `MY_TOKEN` is your runner resource class token. Update `namespace/my-rc` with your namespace and runner resource class.
...
** 指定したイメージのエントリポイントが無効な場合、ジョブは以下のエラーとともに失敗します。 `could not run task: launch circleci-agent on "container-0" failed: command terminated with exit code 139`  Specify an entrypoint using the <<custom-images#adding-an-entrypoint,Adding an entrypoint>> documentation to mitigate this error.
