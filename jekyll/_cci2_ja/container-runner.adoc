...
リソースクラスとは、CircleCI ジョブを、そのジョブを処理するために識別された特定のランナー (またはコンテナエージェント) と対応付けるためのラベルです。 The first part of the resource class is your organization’s namespace. たとえば、 `circleci/documentation` などです。
