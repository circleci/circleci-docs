...
1. Run `kubectl create namespace circleci` to create the `circleci` Kubernetes namespace
1. Create a file called `values.yaml` file containing the following:
+
```yaml
agent:
  resourceClasses:
...
```
1. Run `helm install container-agent container-agent/container-agent -n circleci -f values.yaml`
