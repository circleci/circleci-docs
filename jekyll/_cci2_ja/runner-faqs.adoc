...
セルフホストランナーでは、すべてのコマンドで `circleci-agent` を使用しますが、クラウド版 CircleCI では、テスト分割やステップ停止などのために `circleci-agent` または `circleci` のいずれかを使用します。 `circleci` を <<local-cli#,CircleCI のローカル CLI>> と混同しないようご注意ください。`circleci` は、単に `circleci-agent` の別名です。
