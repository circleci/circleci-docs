...
クラウド版 CircleCI では `circleci-agent` か `circleci` のいずれかを使用できるすべてのコマンド (テスト分割コマンドやステップ停止コマンドなど) に対して、セルフホストランナーでは `circleci-agent` が使用されます。 `circleci` を <<local-cli#,CircleCI のローカル CLI>> と混同しないようご注意ください。`circleci` は、単に `circleci-agent` の別名です。
