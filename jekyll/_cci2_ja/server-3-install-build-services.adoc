...
```
+
オプションで、vm-service では AWS キーの代わりに https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[サービスアカウントの役割]の使用もサポートしています。 役割を使用する場合は、以下のステップ 6 のポリシーを使って次のhttps://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[手順]を実行します。
