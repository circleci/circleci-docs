...
===== AWS
.
 IAM ユーザーまたは Nomad Autoscaler のロールとポリシーを作成します。 以下のいずれか方法で実行します。:
  `nomad_auto_scaler = true` を設定すると、 * Our link:https://github.com/CircleCI-Public/server-terraform/tree/main/nomad-aws[Nomad モジュール] が IAM ユーザーを作成し、キーを出力します。
...
.
 KOTS 管理者コンソールで Nomad Autoscaler を `enabled` に設定します。 最大の Node 数を設定します* : ASG の最大値として現在設定されている値を上書きします。
