...
CircleCI Server の設定とデプロイが完了したら、VM サービスが適切に動作しているか確認する必要がありあます。 You can rerun the https://github.com/circleci/realitycheck[realitycheck] project within your CircleCI installation and you should see the VM Service Jobs complete. この時点で、すべてのテストが合格しているはずです。
