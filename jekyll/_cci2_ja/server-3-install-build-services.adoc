...
===== AWS
. IAM ユーザーまたは Nomad Autoscaler の役割とポリシーを作成します。 次のいずれかの方法で作成します。
  ...
  * Nomad Autoscaler 用の https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[サービスアカウントの役割]を作成し、次の IAM ポリシーを添付します。
[source, json]
{
    "Version": "2012-10-17",
    "Statement": [
...
.. または、Nomad Autoscaler の役割の ARN を追加する。
...
. Nomad Autoscaler のサービスアカウントを作成します。
  ...
  * Nomad GCP サービスアカウントを手動で作成することも可能です。 サービスアカウントには `compute.admin`の役割が必要です。
...
```
+
vm-service では、オプションで AWS キーの代わりに https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[サービスアカウントの役割]の使用もサポートしています。 役割を使用する場合は、以下のステップ 6 のポリシーを使って以下のhttps://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[手順]を実行します。
