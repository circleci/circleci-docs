...
専有ホストリソースの識別子は、`macos.x86.metal.gen1` で、他の macOS リソースクラスと同じ Xcode イメージをサポートしています (Xcode 10.3 を除く)。 すべてのバージョンのリストは 「iOS のテスト」の「サポートされている Xcode のバージョン」のセクション (https://circleci.com/docs/2.0/testing-ios/#supported-xcode-versions) をご覧ください。
