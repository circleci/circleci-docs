...
専有ホストリソースの識別子は、`macos.x86.metal.gen1` で、他の macOS リソースクラスと同じ Xcode イメージをサポートしています (Xcode 10.3 を除く)。 すべてのバージョンのリストは 「iOS のテスト」の https://circleci.com/docs/ja/2.0/using-macos/#supported-xcode-versions[サポートされている Xcode のバージョンのセクション] をご覧ください。
