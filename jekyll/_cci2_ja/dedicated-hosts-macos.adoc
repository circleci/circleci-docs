...
この専有ホストリソースクラスは、Xcode IDE を使って iOS、iPadOS、macOS、WatchOS 、および tvOS アプリケーションの開発、ビルド、テスト、署名を行なうお客様にご利用いただけます。

このリソースクラスの貸し出しは 24 時間以上で、 Intel ベースの Mac ハードウェア上で動作します。 すべてのバージョンのリストは 「iOS のテスト」の https://circleci.com/docs/ja/using-macos/#supported-xcode-versions[サポートされている Xcode のバージョンのセクション] をご覧ください。
