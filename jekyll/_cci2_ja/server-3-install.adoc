...
TIP: CircleCI Server 用の TLS 証明書がない場合は、任意のツールを使用して生成できます。 See the <<オプション: TLS 証明書の生成>> section.
...
. *[Manually Set Service Ports (手動でサービス ポートを設定)]*: Nomad、出力プロセッサ、VM サービス用に構成されたデフォルトのポートを編集する場合に選択します。 このボックスをオンにすると、各サービス用のポート フィールドが表示されます。
...
CircleCI Server 3.x では、オブジェクト ストレージにビルド アーティファクト、テスト結果、その他の状態をホストします。 CircleCI Server 3.x では、以下をサーポートしています。
...
NOTE: CircleCI では、`blocked_cidrs`を使ってジョブ内から特定のCIDRへのネットワークアクセスをブロックしています。 これは Nomad クライアントによる任意のコードでアクセスするべきではないサービスを使っての通信 (VMサービスを使ってVMをスピンアップするなど) を防ぐためのセキュリティー機能です。 Normad クライアントへの、そしてNormad クライアントからのアクセスをブロックしないでください。ブロックすると `setup_remote_docker` に失敗します。
