...
If you see the message "This job was claimed but has not started in over 10 minutes" on the job’s page, or if there is no content in the task lifecycle step (as shown):
...
ジョブをキャンセルし、再実行します。 If your job is still not running, file a link:https://support.circleci.com/hc/en-us[support ticket].
