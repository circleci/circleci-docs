...
- **Pod の再起動:** ワークフローが実行される前後のタイミングでコンテナエージェントの Pod が再起動していないか確認してください。 Pod がその前後のタイミングで再起動されていた場合、ジョブは処理されません。 その場合、そのジョブをもう一度再実行することをお勧めします。

 `kubectl logs -n circleci <full pod name> --previous` コマンドをつsかって、それまでのすべての実行のログを確認することができます。
