...
```
* クラスタの VPC 設定やディスク サイズに関する要件はありません。 It is recommended that you set up a new VPC rather than use an existing one.

ifndef::env-gcp[]

==== EKS
Amazon EKS クラスタ の作成に関する詳細は https://aws.amazon.com/quickstart/architecture/amazon-eks/[こちら] をご覧ください。 We recommend using https://docs.aws.amazon.com/eks/latest/userguide/getting-started-eksctl.html[`eksctl`] to create your cluster, which creates a VPC and selects the proper security groups for you.
...
. 完了すると、*クライアントID* が表示されます。 *[Generate a new Client Secret (新しいクライアント シークレットを生成する]* を選択し、新しい OAuth アプリ用のクライアントシークレットを生成します。
 You need these values when you configure CircleCI server.
+
.クライアント ID とシークレット
...
=== フロントエンド TLS 証明書
By default, CircleCI server creates self-signed certificates to get you started. 本番環境では、信頼できる認証局の証明書を指定する必要があります。 The link:https://letsencrypt.org/[Let's Encrypt] certificate authority, for example, can issue a free certificate using their link:https://certbot.eff.org/[certbot] tool. The sections below cover using Google Cloud DNS and AWS Route 53.
...
=== 暗号化/署名キー
CircleCI で生成されるアーティファクトの暗号化と署名には、以下のキーセットを使用します。 You need these values to configure server.
...
ニーズに最適なストレージを選んでください。 [Storage Bucket Name (ストレージ バケット名)] は必須です。 AWS と GCP のどちらを使用しているかに応じて、以下のフィールドも入力してください。 Before proceeding, ensure the bucket name you provide exists in your chosen object storage provider.
