...
| tar.gz (`tar zxvf kots_linux_amd64.tar.gz`) から`kots` を抽出したら、`sudo mv kots /usr/local/bin/kubectl-kots`を実行し、インストールします。 Mac users will need to grant a security exception.
