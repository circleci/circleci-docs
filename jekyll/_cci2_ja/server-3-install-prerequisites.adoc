...
== ステップ 1: 前提条件
CircleCI Server のインストールには 4つのステップがあります。 各ステップの最後に、そのステップが成功していることを確認してから次のステップに進みます。 Depending on your requirements, phases 3 and 4 may include multiple steps. なお、このインストールガイドは、https://circleci.com/docs/ja/2.0/server-3-overview[[CircleCI Server 3.x の概要]] を先に読んでいることを前提としています。
