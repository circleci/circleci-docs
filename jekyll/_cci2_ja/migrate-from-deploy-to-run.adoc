...
廃止予定の `deploy` ステップ_ が使用されている設定ファイルは、変更する必要があります_。ジョブで xref:parallelism-faster-jobs#[parallelism (並列処理)] が含まれているかどうかに関わらず、`deploy` ステップの _すべての_ インスタンスを削除する必要があります。
