...
- *Does your job have parallelism of 1?* Swap out the `deploy` key for the xref:configuration-reference#run[`run`] key. 移行に必要な処理はこれだけです。
- *Does your job have parallelism > 1?* There is no direct replacement for the `deploy` step if you are using `parallelism > 1` in your job. The recommendation is to create two separate jobs within one workflow - a test job, and a deploy job. テストジョブではテストをが並列で実行され、デプロイジョブはテストジョブに依存します。 The test job has `parallelism > 1`, and the deploy job will have the command from the previous `deploy` step replaced with `run` and no parallelism. Please see examples in this guide.
