...
廃止予定の `deploy` ステップが使用されている設定ファイルは、変更する _必要があります。_ジョブで xref:parallelism-faster-jobs#[parallelism (並列実行)] が含まれているかどうかに関わらず、`deploy` ステップの _すべての_ インスタンスを削除する必要があります。
