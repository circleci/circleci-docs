...
まず、CircleCI のビルド設定ファイルを作成します。 ソースコードリポジトリのルートディレクトリに、`.circleci` というフォルダを作成し、その中に `config.yml` という名前のファイルを作成します。 次に、CircleCI のドキュメント ( https://circleci.com/docs/ja/2.0/config-intro/[こちら] ) に沿って、 `.config.yml` ファイルの設定方法を確認します。
...
* https://circleci.com/docs/ja/2.0/configuration-reference/[CircleCI YML のリファレンス]
...
2+|マルチプラットフォームでのジョブの実行:  GitLab では、タグを使ってビルドエージェントを識別します。 CircleCI では主要な OS と Docker はすべてサポートしており、設定で明示的に定義する必要があります。 詳しくは、 https://circleci.com/docs/2.0/executor-intro/#section=configuration[実行環境に関するドキュメント] </a> をご覧ください。
...
. https://circleci.com/docs/ja/2.0/executor-intro/[Executor の選択]

. https://circleci.com/docs/ja/2.0/configuration-reference/#checkout[コードのチェックアウト]
. https://circleci.com/docs/ja/2.0/env-vars/[環境変数] と https://circleci.com/docs/ja/2.0/contexts/[コンテキスト]
. 依存関係のインストール、 https://circleci.com/docs/ja/2.0/caching/[依存関係のキャッシュ] も参照してください。
. https://circleci.com/docs/ja/2.0/configuration-reference/#docker[サービスコンテナ]
. テストコマンドの実行
. https://circleci.com/docs/ja/2.0/custom-images/[カスタム CircleCI イメージ]

. https://circleci.com/docs/ja/2.0/configuration-reference/#resource_class[リソースクラス]
. https://circleci.com/docs/ja/2.0/workflows/[ワークフロー]
. https://circleci.com/docs/ja/2.0/collect-test-data/[テスト結果] / test splitting / https://circleci.com/docs/ja/2.0/parallelism-faster-jobs/[並列実行]
