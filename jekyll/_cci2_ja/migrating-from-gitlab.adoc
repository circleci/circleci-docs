...
2+| マルチプラットフォームでのジョブの実行:  GitLab では、タグを使ってビルドエージェントを識別します。 CircleCI では主要な OS と Docker はすべてサポートしており、設定で明示的に定義する必要があります。 詳しくは、 https://circleci.com/docs/ja/2.0/executor-types/#section=configuration[Executor に関するドキュメント] をご覧ください。
...
. https://circleci.com/docs/ja/2.0/executor-types/[Executor の選定]
...
. https://circleci.com/docs/ja/2.0/configuration-reference/#docker[サービス コンテナ]
. テストコマンドの実行
. https://circleci.com/docs/ja/2.0/custom-images/[カスタムイメージ]
. https://circleci.com/docs/ja/2.0/configuration-reference/#resource_class[リソース クラス]
. https://circleci.com/docs/ja/2.0/workflows/[ワークフロー]
. https://circleci.com/docs/ja/2.0/collect-test-data/[テスト結果]/ テストの分割 / https://circleci.com/docs/ja/2.0/parallelism-faster-jobs/[並列実行]
