...
. ワークフローの最後で、link:https://circleci.com/developer/orbs/orb/circleci/continuation[`continuation` orb]  からジョブを呼び出す必要があります。 注: xref:using-dynamic-configuration#[ダイナミックコンフィグの使用] で説明されているように、 指定されたファイルの更新 に基づいてワークフローまたはステップを条件付きで実行する場合 ("パスフィルタリング")、これは適用 されません 。
