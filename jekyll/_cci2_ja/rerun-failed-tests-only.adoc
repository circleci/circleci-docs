...
**回答:** はい、`circleci tests split --split-by=timings --timings-type=` と同様に、`test-selector` フラグを使用してタイミングタイプを指定することができます。  `file`、`classname`、`name` を渡すことができます（ `name` はテスト名で分割されます）。
