...
+
CircleCI の xref:troubleshoot-test-splitting#video-troubleshooting-globbing[glob コマンド]を使用して、テストファイルのリストを作成しています。 今回は、名前の先頭が `test_` で末尾が `.py` のテストファイルを探しました。
* `echo $TEST_FILES |`
+
