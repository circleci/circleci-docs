...
デフォルトでは、内部のデータベースが使用する永続ボリューム要求の容量は 8Gi です。 ただし、この初期値は最初のデプロイ時に KOTS 管理者コンソールから設定できます。 We can confirm the size of our persistent volume claim capacity using `kubectl get pvc <pvc-name>`.
...
=== ステップ 4: KOTS 管理者を新しい PVC サイズに更新する
...
When running through these steps, if you find, as expected, that the new pods **do** show the resized volumes, it is still worth running the `kubectl describe` commands shown below to check. In some instances the resize will fail, but the only way to know is by viewing an event in the output from `kubectl describe`.
...
これらのステップを実行しても、データディレクトリに割り当てられたディスクサイズが拡張しない場合は、データベースポッドを再起動する必要があります。 This will cause downtime of 1-5 mins as the databases restart. 以下のコマンドでデータベースを再起動することができます。
