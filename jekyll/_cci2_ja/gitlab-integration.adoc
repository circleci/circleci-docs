...
GitLab ベースのトリガーでは、追加のパイプライン値にアクセスできます。 CircleCI でのパイプライン値とパラメーターの使用に関する詳細は、
<<pipeline-variables#,パイプラインの値とパラメーター>> を参照して下さい。 **パイプラインのスケジュール実行機能は現時点では GitLab ユーザーはご利用いただけません。**
