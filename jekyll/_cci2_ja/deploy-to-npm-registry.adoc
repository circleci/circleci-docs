...
1. パッケージのパブリッシュに使用するアカウント用に npm authToken を取得します。 You can do that by logging in to npm (`npm login`). This will save the authToken to the `~/.npmrc` file. 次の行を探します。
+
```shell
