...
+
. If this is your organization's first time using self-hosted runners, you'll need to create a namespace as the first part of this step. *作成できる名前空間は、各組織 1 つのみです。* 必須ではありませんが、 CircleCI のアカウント名を小文字で使用することを推奨します。 CircleCI will populate your org name as the suggested namespace by default in the UI.
+
