...
NOTE: セルフホストランナーは、https://app.circleci.com/[CircleCI Web アプリ]から直接インストールできるようになりました！ このオプションを UI で使用できるようにするには、 <<#self-hosted-runner-terms-agreement,Runner Terms>>ランナー規約</a> に同意する必要があります。
