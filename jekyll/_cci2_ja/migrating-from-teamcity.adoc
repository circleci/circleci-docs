...
. *この設定を理解したら、必要に応じて Executor や各ステップの変更を行います。 * 例えば、 https://circleci.com/docs/2.0/language-ruby/[Rails プロジェクト] では Ruby のコンテナ イメージを使用して `bundler` コマンドを実行することができます。 Consult our <<examples-and-guides-overview#, Examples and Guides Overview>> for various examples in different languages and frameworks.
...
TeamCity では、必要なOSとツールがインストールされたビルドエージェントと、それに対応するビルド設定をセットアップする必要があります。 CircleCI では、すべてのジョブ設定に Executor が定義されており、CircleCI がそのエージェントのスピンアップを代行します。 https://circleci.com/docs/2.0/executor-intro/[利用可能な Executor のリスト]を参照してください。
...
[#resources]
=== 関連資料
* CircleCI のその他の設定例は、 https://circleci.com/docs/ja/2.0/example-configs/#section=configuration[サンプルプロジェクト] をご覧ください。
