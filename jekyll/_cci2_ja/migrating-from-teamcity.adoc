...
**自動スケーリング アーキテクチャ**:: All execution environments are dynamically created according to your config and terminated on completion of tasks. 1つのジョブから数百のジョブまで、手間や追加費用をかけずに同時に実行することができます。
...
その他にも、CircleCI ソリューションを特徴づける様々な機能があります。 https://circleci.com/signup/[今すぐ無料アカウントを作成し、]お試しください。 チームでのご利用をお考えでしたら、https://circleci.com/talk-to-us/?source-button=MigratingFromGithubActionsDoc[弊社セールス チームにご連絡をいただき]、トライアルを設定してください。
