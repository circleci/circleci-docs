...
**オプション 2:** 値を入力せず、ご自身でシークレットを作成します。 CircleCI はこの値があるものと想定します。 `github.clientID` で指定した同じ場所から取得します。 ご自身のホームページを `{{ .Values.global.scheme }}://{{ .Values.global.domainName }}` に設定し、コールバックを `{{ .Value.scheme }}://{{ .Values.global.domainName }}/auth/github` に設定します。
...
|文字列
|`nil`
|GitHub Enterprise のインスタンスを直接 `ssh-keyscan` できない場合に要求されます。

|`github.hostname`
|文字列
|`"ghe.example.com"`
|GitHub のホスト名です。 GitHub.com では無視されます。
...
|ブール値
|`false`
|GitHub で自己署名証明書を使用している場合は 'true' に設定します。 GitHub.com では無視されます。 インストールした GitHub Enterprise で TLS を使用していない場合は 'http' に設定します。

|`github.unsafeDisableWebhookSSLVerification`
|ブール値
|`false`
|Webhook での SSL 検証を無効にします。

|`global.container.org`
|文字列
|`"circleci"`
|すべてのイメージをプルするレジストリ組織で、デフォルトでは CircleCI です。 これは安全でないため本番シナリオではおやめください。 GitHub インストール環境が CircleCI Server の証明書に署名した認証局を信頼しない場合 (自己署名の場合など) に要求されます。
...
|文字列
|`"notice"`
|Kong のデバッグレベルを指定します。 設定できるレベルは、 `debug` 、 `info` 、 `warn` 、`error` 、 `crit` です。 デフォルトの設定は "notice" です。  その後、これを true に設定し、必要に応じて更新された設定で `helm upgrade` を実行します。
...
|ブール値
|`false`
|⚠️ 警告: 有効化するとロードバランサーを再作成するフロントエンドのサービスが再作成されます。

|`nginx.loadBalancerIp`
|文字列
|`false`
|ロードバランサーの IP です。GCP を使用してプロビジョニングされたロードバランサーの静的 IP を使用するには、予約済みの静的 IPv4 アドレスに設定します. デプロイした設定を更新する場合は、フロントエンドのドメインを新しいロードバランサーにルーティングする必要があります。 また、`service.beta.kubernetes.io/aws-load-balancer-ssl-cert: <acm-arn>` を `nginx.annotations` ブロックに追加する必要があります。
...
|文字列
|`{"CACertificate":"","certificate":"","enabled":false,"privateKey":""}`
|Base64 でエンコードした Nomad mTLS 認証局です。 mTLS はトラフィックを暗号化し、またクライアントを認証し、認証されていないクライアントはワーカーとしてクラスタに入れなくなります。 Base64 でエンコードした PEM エンコード形式の証明書が想定されています。
...
----
|アーティファクトが期限切れになるまでの日数です。 チャートが機能するには、`object_storage.s3.enabled` または `object_storage.gcs.enabled` のいずれかが true である必要があります。
...
|整数
|`1`
|orb-service のデプロイ用にデプロイするレプリカの数です。 たとえば AWS `us-west-2` の場合は、リージョンのエンドポイント (`\http://s3.us-west-2.amazonaws.com`)  になります。 S3 互換ストレージを使用している場合は、オブジェクトストレージサーバーの API エンドポイントを指定します。
...
|ブール値
|`false`
|`certificatesSecret` で使用されている CA 証明書のファイル名です。
...
|リスト
|`{"auth":{"enabled":false,"password":null,"username":null},"host":"proxy.example.com","port":3128}`
|ホスト名のリストです。IP CIDR ブロックはプロキシから除外されます。
...
|ブール値
|`false`
|⚠️ 警告: これを true に変更すると、HTML アーティファクトがダウンロードされるのではなく提供されるようになります。
...
|整数
|`25`
|ポート 25 のアウトバウンド接続が、ほとんどのクラウドプロバイダーでブロックされます。
...
|整数
|`1`
|soketi のデプロイ用にデプロイするレプリカの数です。 トラフィックの機密性を保証できない場合は、このオプションを無効化しないでください。
