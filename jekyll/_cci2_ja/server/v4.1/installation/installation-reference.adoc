...
|S3 用 API エンドポイントです。 If in AWS `us-west-2`, for example, this would be the regional endpoint `\http://s3.us-west-2.amazonaws.com`. S3 互換ストレージを使用している場合は、オブジェクトストレージサーバーの API エンドポイントを指定します。
