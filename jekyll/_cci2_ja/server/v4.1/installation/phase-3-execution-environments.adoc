...
CircleCI Server v4.x 実行環境のインストールステップを開始する前に、 xref:phase-1-prerequisites#[ステップ 1: 前提条件] と xref:phase-2-core-services#[ステップ 2: コアサービスのインストール] が実行済みであることを確認してください。
...
* *Server_endpoint* - CircleCI アプリケーションのドメイン名です。
...
----
+
次に、見つけた VPCID を使用して次のコマンドを実行し、クラスタの CIDR ブロックを取得します。 AWS の場合、 DNS サーバーは CIDR Block (`CidrBlock`) の３番目の IP です。たとえば、CIDR ブロックが `10.100.0.0/16` の場合、 3 番目の IP は `10.100.0.2` になります。
+
[source,shell]
----
...
* Nomad Autoscalerの link:https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[Role for Service Accounts]を作成し、<<iam-policy-for-nomad-autoscaler,IAM policy below>>を添付することができます：
...
CircleCI link:https://github.com/CircleCI-Public/server-terraform/tree/main/nomad-gcp[Nomad モジュール] により、 xref:phase-1-prerequisites#enable-workload-identity-in-gke[Workload Identity] を使ってサービスアカウントを作成し、メールを送信できます。 変数 `nomad_auto_scaler = true` と `enable_workload_identity = true` を設定します。
--
...
CircleCI では、CircleCI Server のインストールをテストできる link:https://github.com/circleci/realitycheck[realitycheck] というプロジェクトを作成しました。 今後このプロジェクトを監視し、システムが期待どおりに動作しているかを確認します。 引き続き次のステップを実行すると、 realitycheck のセクションが赤から緑に変わります。
...
レポジトリのクローンに成功したら、CircleCI Server 内からフォローすることができます。 以下の変数を設定する必要があります。 詳細は、 link:https://github.com/circleci/realitycheck#prerequisites-1[リポジトリ README] を参照してください。
...
Nomad クライアントを正常にデプロイし、権限リソースを取得したので、Nomad サーバーを設定できるようになりました。 スケーリングルールなど、さまざまなオプションの設定が可能です。 VM サービスは、 AWS および GCP インストール環境に固有のものです。これは、VMサービスがこれらのクラウドプロバイダーの特定の機能に依存しているためです。

NOTE: サーバーのインストールプロセスが完了したら、VM サービスの詳細設定が行えます。これには、開発者が Windows 実行環境にアクセスできるように Windows イメージをビルドおよび指定したり、代替の Linux マシンイメージを指定したり、スピンアップ状態を常に維持するための事前割り当てインスタンスの数の指定などが含まれます。
 詳細については、 xref:../operator/manage-virtual-machines-with-vm-service[VM サービスを使った仮想マシンの管理] を参照してください。
...
----
+
Nomad クライアントが使用する各 link:https://github.com/CircleCI-Public/server-terraform/blob/main/nomad-aws/variables.tf#L1-L11[サブネット] のサブネット cidr ブロックを見つけ、次のコマンドを使って 2 つのルールを追加します。 
+
[source,shell]
----
# find CIDR block
...
[.tab.vmauthaws.IRSA]
--
以下は link:https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[IRSA についての AWS のドキュメント] の概要です。CircleCI での VM サービスの設定には十分です。
...
----
+
vm-service では、オプションで AWS キーの代わりに link:https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[サービスアカウントのロール]の使用もサポートしています。 ロールを使用する場合は、以下のステップ 6 のポリシーを使って以下の link:https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[手順]を実行します。
...
`values.yaml` ファイルへの変更を適用します。 次のコマンドを実行します。
...
----
. *サービスアカウントで Workload Identity を有効化、または JSON キーファイルを取得します*
+
Workload Identity を使用しているかどうかにより、以下のいずれかのオプションを選択してください。
+
[.tab.workloadorjson.Enable_Workload_Identity_for_Service_Account]
--
この手順は、GKE で link:https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity[Workload Identity] を使用している場合のみ実行する必要があります。 Steps to enable Workload Identities are provided in xref:../../../server-3-install-prerequisites/index.html#enabling-workload-identities-in-gke-optional[Phase 1 - Prerequisites].
...
--
GKE で link:https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity[Workload Identity] を使用している場合は、この手順は不要です。
...
values.yaml に VM サービスの設定を追加します。
...
CircleCI では、CircleCI Server のインストールをテストできる link:https://github.com/circleci/realitycheck[realitycheck] というプロジェクトを作成しました。
...
CircleCI のランナーには、追加のサーバー設定は不要です。 CircleCI Server はランナーと連携する準備ができています。 ただし、ランナーを作成し、CircleCI Server のインストールを認識するようにランナーエージェントを設定する必要があります。 ランナーの設定についての詳細は、xref:../../../runner-overview#[ランナーに関するドキュメント] をご覧ください。
