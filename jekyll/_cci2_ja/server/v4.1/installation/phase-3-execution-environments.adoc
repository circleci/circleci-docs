...
The CircleCI link:https://github.com/CircleCI-Public/server-terraform/tree/main/nomad-gcp[nomad module] can create a service account using xref:phase-1-prerequisites#enable-workload-identity-in-gke[Workload Identity] and send out the email. 変数 `nomad_auto_scaler = true` と `enable_workload_identity = true` を設定します。
--
...
CircleCI has created a project called link:https://github.com/circleci/realitycheck[realitycheck] which allows you to test your server installation. 今後このプロジェクトを監視し、システムが期待どおりに動作しているかを確認します。 引き続き次のステップを実行すると、 realitycheck のセクションが赤から緑に変わります。
...
レポジトリのクローンに成功したら、CircleCI Server 内からフォローすることができます。 以下の変数を設定する必要があります。 For full instructions please see the link:https://github.com/circleci/realitycheck#prerequisites-1[repository readme].
...
VM service is used to configure virtual machines for jobs that run in Linux VM, Windows amd Arm execution environments, and those that are configured to use xref:../../../configuration-reference#setupremotedocker[remote Docker]. スケーリングルールなど、さまざまなオプションの設定が可能です。 VM サービスは、 AWS および GCP インストール環境に固有のものです。これは、VMサービスがこれらのクラウドプロバイダーの特定の機能に依存しているためです。
...
NOTE: サーバーのインストールプロセスが完了したら、VM サービスの詳細設定が行えます。これには、開発者が Windows 実行環境にアクセスできるように Windows イメージをビルドおよび指定したり、代替の Linux マシンイメージを指定したり、スピンアップ状態を常に維持するための事前割り当てインスタンスの数の指定などが含まれます。
 For more information, see the xref:../operator/manage-virtual-machines-with-vm-service[Managing Virtual Machines with VM Service] page.
...
CircleCI のランナーには、追加のサーバー設定は不要です。 CircleCI server ships ready to work with runner. ただし、ランナーを作成し、CircleCI Server のインストールを認識するようにランナーエージェントを設定する必要があります。 For complete instructions for setting up runner, see the xref:../../../runner-overview#[runner documentation].

NOTE: ランナーには組織ごとに１つ名前空間が必要です。 CircleCI server can have many organizations. CircleCI Server 内に複数の組織が存在する場合、各組織につき１つランナーの名前空間を設定する必要があります。
