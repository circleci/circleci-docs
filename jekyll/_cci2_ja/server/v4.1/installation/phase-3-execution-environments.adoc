...
VM サービスにより、Linux VM、Windows、Arm の各実行環境で実行されるジョブや xref:../../../configuration-reference#setupremotedocker[リモート Docker] を使用するように設定されたジョブの仮想マシンを設定できます。 スケーリングルールなど、さまざまなオプションの設定が可能です。 VM サービスは、 AWS および GCP インストール環境に固有のものです。これは、VMサービスがこれらのクラウドプロバイダーの特定の機能に依存しているためです。
