...
* *Server_endpoint*: ポート番号 `4647` を付加した CircleCI アプリケーションのドメイン名です。  (例: server.example.com:4647)。
...
* Nomad Autoscaler 用の link:https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html[サービスアカウントのロール] を作成し、<<iam-policy-for-nomad-autoscaler,下記の IAM ポリシー>> を添付します。
...
+
NOTE: 自動モードを使用した場合は、 link:https://cloud.google.com/vpc/docs/vpc#ip-ranges[こちらの表]を参照して、リージョンに基づいて Nomad クライアントの CIDR を見つけることができます。
+
[source,shell]
----
...
--
この手順は、GKE で link:https://cloud.google.com/kubernetes-engine/docs/how-to/workload-identity[Workload Identity] を使用している場合のみ実行する必要があります。 Workload Identity を有効化する手順は、xref:../../../server-3-install-prerequisites#enabling-workload-identities-in-gke-optional[ステップ 1: 前提条件] を参照してください。
...
* xref:phase-4-post-installation#[ステップ 4: ポストインストール]
+
endif::pdf[]