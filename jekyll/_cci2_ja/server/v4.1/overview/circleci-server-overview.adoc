...
:page-liquid: CircleCI Server v4.x は、GCP または AWS の Kubernetes クラスタにインストールしていただける継続的インテグレーションおよび継続的デリバリー (CI/CD) プラットフォームです。
:page-description:
:icons: font
:toc: macro
...
* xref:../overview/release-notes[CircleCI Server v4.x のリリースノート]
* xref:../installation/phase-1-prerequisites[Server 4.x のインストールの前提条件]
* xref:../installation/migrate-from-server-3-to-server-4[Server v3 から Server v4 への移行]
+
endif::pdf[]