...
VM サービスにより、 xref:../../../configuration-reference/#machine[`Machine`] Executor と xref:../../../building-docker-imagess[リモート Docker] ジョブの実行方法を制御できます。

VM サービスにより、 xref:../../../configuration-reference/#machine[`Machine`] Executor と xref:../../../building-docker-imagess[リモート Docker] ジョブの実行方法を制御できます。 仮想マシンを事前に拡張する方法の詳細については、デフォルトの `values.yaml` ファイルを参照してください。
