...
CAUTION: CircleCI Server のコアサービスとビルドサービスの設定と検証が完了するまで、これらのオプションはデフォルトのままにしておくことをお勧めします。 Steps to set up VM service are provided in the installation guide for xref:../installation/phase-3-execution-environments/#aws-vm-service[AWS] and xref:../installation/phase-3-execution-environments/#gcp-authentication[GCP].
...
`values.yaml` ファイルに、AWS EC2 と連携するように VM サービスを設定するセクションがあります。 インストールの間に、セキュリティグループと認証のセットアップを行います。 See the xref:../installation/phase-3-execution-environments/#aws-vm-service[Installation Phase 3 - Execution Environments] page for more information.
...
Windows Executor が必要な場合は、`values.yaml` ファイルに AMI ID を記述します。 To create a Windows image, use the link:https://github.com/CircleCI-Public/circleci-server-windows-image-builder[CircleCI server Windows image builder].
...
Linux Machine Executor にカスタム AMI を指定する場合、`values.yaml` ファイルに AMI ID を記述します。 To create a Linux image, use the link:https://github.com/CircleCI-Public/circleci-server-linux-image-builder[CircleCI server Linux image builder].
...
`values.yaml` ファイルに、GCP と連携するように VM サービスを設定するセクションがあります。 インストールの間に、セキュリティグループと認証のセットアップを行います。 See the xref:../installation/phase-3-execution-environments/#aws-vm-service[Installation Phase 3 - Execution Environments] page for more information.
...
WARNING: If xref:../../../docker-layer-caching/[Docker Layer Caching (DLC)] is used, VM Service instances need to be spun up on demand. これを実現する方法は 2 つあります。 **1 つは**、事前割り当てインスタンスを使用中にする、**もう 1 つは**、リモート Docker と `マシン` 用の事前割り当てインスタンスのフィールドの両方を 0 に設定する方法です。
