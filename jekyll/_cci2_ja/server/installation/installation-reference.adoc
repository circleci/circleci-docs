...
|文字列型
|`""`
|API トークン で、2 つのオプションがあります。 オプション 1: ここに値を設定し、CircleCI が自動的にシークレットを作成する。 オプション 2: 値を入力せず、ご自身でシークレットを作成する。 CircleCI はこの値があるものと想定します。
...
|文字列型
|`""`
|Github 経由の OAuth ログイン用のクライアント ID で、2 つのオプションがあります。 オプション 1: ここに値を設定し、CircleCI が自動的にシークレットを作成する。 オプション 2: 値を入力せず、ご自身でシークレットを作成する。 CircleCI はこの値があるものと想定します。 Settings > Developer Settings > OAuth Apps に移動して作成します。 ご自身のホームページを `{{ .Values.global.scheme }}://{{ .Values.global.domainName }}` に設定し、コールバックを `{{ .Value.scheme }}://{{ .Values.global.domainName }}/auth/github` に設定します。

|github.clientSecret
|文字列型
|`""`
|Github 経由の OAuth ログイン用のクライアントシークレットで、2 つのオプションがあります。 オプション 1: ここに値を設定し、CircleCI が自動的にシークレットを作成する。 オプション 2: 値を入力せず、ご自身でシークレットを作成する。 CircleCI はこの値があるものと想定します。 github.clientID で指定した同じ場所から取得します。
...
|ブール値
|`false`
|Webhook での SSL 検証を無効にします。 これは安全でないため本番シナリオではおやめください。 This is required if your Github installation does not trust the certificate authority that signed your Circle server certificates (e.g they were self signed).
