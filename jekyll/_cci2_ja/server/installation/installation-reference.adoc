...
|RPC 通信には mTLS の使用を強く推奨します。 mTLS はトラフィックを暗号化し、またクライアントを認証して、認証されていないクライアントがワーカーとしてクラスタに入れないようにします。 Base64 エンコード PEM エンコード形式の証明書が想定されています。
...
|整数
|`1`
|orb-service をデプロイするためにデプロイするレプリカの数です。

|output_processor.replicas
|整数
|`2`
|output-processor をデプロイするためにデプロイするレプリカの数です。

|permissions_service.replicas
|整数
|`1`
|permissions-service をデプロイするためにデプロイするレプリカの数です。
