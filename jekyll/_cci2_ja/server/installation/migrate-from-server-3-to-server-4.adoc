...
=== 1. CircleCI イメージのレジストリ用 Docker シークレットの作成
...
=== 2. リポジトリのクローンと kots-exporter スクリプトの実行
