...
VM service is used to configure virtual machines for jobs that run in Linux VM, Windows amd Arm execution environments, and those that are configured to use link:/docs/configuration-reference/#setupremotedocker[remote Docker]. スケーリングルールなど、さまざまなオプションを設定することができます。 VM サービスは、 AWS および GCP インストール環境に固有のものです。これは、VMサービスがこれらのクラウドプロバイダーの機能に特に依存しているためです。
