...
NOTE: このオプションは、パイプラインに 5GB を超えるアーティファクトが保存されている場合は推奨しません。 A limitation with the MinIO backend causes S3 uploads with objects greater than 5GB to fail when using IAM keys. For 5GB+ object support, use IRSA (documented below).
