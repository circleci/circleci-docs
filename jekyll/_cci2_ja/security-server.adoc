...
完全な監査ログは、アプリケーションの管理者セクション内にある [Audit Log (監査ログ)] ページから CSV ファイル形式でダウンロードできます。  ネストされたデータを含む監査ログフィールドには JSON BLOB が含まれます。終了まで時間かかるですが、ダウンロードボタンを押してからしばらく休んで頂ければ幸いです。
