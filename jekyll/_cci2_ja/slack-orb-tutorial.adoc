...
== Slack Orb の概要 config.yml ファイルを作成する
...
まだ作成していない場合は、リポジトリのルートに `.circleci` フォルダーを作成します。
...
Slack Orb には、CircleCI の各種イベントをチャンネルに通知するために使用できる通知テンプレートがいくつか含まれています： また、基本的な通知を作成し、Slack アプリを作成・認証し、テンプレートも使用しました。
