...
* Docker Executor の認証パスワードで、英数字以外の文字をより適切に処理できるようになりました。 Fixed a bug that was preventing the use of `~` and `?` in the https://circleci.com/docs/private-images/#docker-executor[Docker ID password field under the `auth` key].
* 全体的なセキュリティーのアップデート
