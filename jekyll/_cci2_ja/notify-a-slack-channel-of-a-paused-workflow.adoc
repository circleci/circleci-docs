...
Every CircleCI project requires a configuration file called <<introduction-to-yaml-configurations#,`.circleci/config.yml`>>. 以下の手順に従って、完全な `config.yml` ファイルを作成してください。
