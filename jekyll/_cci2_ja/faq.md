...
{:.no_toc}
CircleCI の従業員が許可なくコードを覗き見ることはありません。CircleCI のサポートが必要になったとき、サポートエンジニアがお客様の許可を得たうえで、問題解消のためにコードを確認することがあります。
...
{:.no_toc}
新たに作成しなくても、CircleCI であらかじめ用意しているビルド済みイメージが使えます。ただし、それらのイメージは将来的には廃止になる可能性があります。
...
{:.no_toc}
Enterprise という呼び名は「ファイアウォール環境」の追加オプションを意味するものでした。しかしながらこの用語はお客様や CircleCI スタッフの混乱を招くものでした。
...
**run キュー** は、CircleCI 自体が高負荷状態のときに使われるものです。この場合、ユーザーのビルドは run キューに置かれ、マシンが利用可能になり次第処理されます。
...
### Docker イメージの指定の仕方は？ どこにあるイメージを使えますか？
{:.no_toc}
...
{:.no_toc}
環境変数 `TZ` を使うことで Docker イメージ内のタイムゾーンを設定できます。`.circleci/config.yml` ファイルで下記のように記述してください。
...
{:.no_toc}
CircleCI は実行タイミングの精度については保証していません。スケジュールされた Workflow は、指定した時刻にコミットがプッシュされたかのような形で実行される仕組みとなっています。

<a name="billing"></a>
## 料金・支払

<a name="how-do-i-upgrade-my-plan-with-more-containers-to-prevent-queuing"></a>
...
また、IPv6 環境でサービスのテストをするのに、コンテナに IPv6 アドレスを割り当てるよう Docker をカスタマイズすることも可能です。下記のように Docker デーモンを設定することで、グローバルな IPv6 アドレスを有効化できます。
