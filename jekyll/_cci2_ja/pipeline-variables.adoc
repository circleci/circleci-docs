...
Web アプリでスケジュールされたパイプラインを設定する際にも、パラメーターを呼び出すことができます。 The parameters are part of the trigger form in menu:Project Settings[Triggers]. Any parameter set up as a part of a scheduled pipeline will also need to be declared in the configuration file, otherwise the pipeline will fail with the error `Unexpected argument(s)`.
