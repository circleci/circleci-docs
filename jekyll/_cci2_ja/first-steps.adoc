...
. GitLab リポジトリから新しいプロジェクトを作成するオプション画面が表示されます。
 画面の指示に従って GitLab.com または GitLab Self-managed のアカウントと連携します。 リポジトリを選択し、新しいプロジェクトを作成すると、CircleCI Web アプリのダッシュボードにリダイレクトされます。
--
...
. GitLab.com オプションの隣にある **Connect** をクリックします。 GitLab アカウントの承認、プロジェクトの作成、および GitLab.com または GitLab Self-managed アカウントからのリポジトリの接続についての説明が表示されます。 その後、組織のダッシュボードに自動的に移動します。
