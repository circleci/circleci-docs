...
一般的に求められるのは、パイプラインを夜間にトリガーする設定です。 下記のコード例では、パイプラインを毎日深夜に実行するように設定する方法を紹介します。 廃止予定のワークフローのスケジュール実行とは異なり、基本的なパイプラインのスケジュール実行は設定ファイルでは設定しません。 スケジュールは、link:https://app.circleci.com/[CircleCI Web アプリ] から設定するか、ご希望の場合は link:https://circleci.com/docs/api/v2/index.html[API] を使って設定することも可能です。
