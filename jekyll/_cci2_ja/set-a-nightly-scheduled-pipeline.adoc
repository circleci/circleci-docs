...
設定したい一般的なシナリオは、パイプラインを夜間にトリガーすることです。 下記のコード例では、パイプラインを毎日深夜に実行するように設定する方法を紹介します。 廃止予定のワークフローのスケジュール実行とは異なり、基本的なパイプラインのスケジュール実行は設定ファイル内で設定しません。 スケジュールは、link:https://app.circleci.com/[CircleCI Web アプリ] から設定するか、ご希望に応じて、link:https://circleci.com/docs/api/v2/index.html[API] を使って設定することも可能です。

[#set-a-in-the-web-app]
=== Web アプリでスケジュールを設定する

お好きなプロジェクトで、 **Project Settings > Triggers** に移動すると、トリガーのスケジュールを指定するフォームが表示されます。
