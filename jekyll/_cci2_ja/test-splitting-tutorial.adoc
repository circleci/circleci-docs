...
テスト分割は通常、一つのジョブ内で設定されます。 In this tutorial, you will modify the `build-and-test` job to define the number of parallel test runs, where the test suites are located, and how tests should be split (in this case, by timing).
