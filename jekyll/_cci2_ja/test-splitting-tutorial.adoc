...
:page-liquid:
:page-description: このチュートリアルでは、テスト分割とは？ CircleCI でテストを分割し、実行する方法などについてわかりやすく説明します。
:icons: font
:toc: macro
...
* CircleCI アカウントに関連付けられたバージョン管理システム (VCS) プロバイダー (GitHub、Bitbucket、GitLab) : まだお済みでない場合は、xref:github-integration#[GitHub] 、xref:bitbucket-integration#[Bitbucket の連携]、xref:gitlab-integration#[GitLabの連携] のページの手順に従って VCS プロバイダーの関連付けをしてください。

[#about-the-sample-app]
== サンプルアプリについて

このチュートリアルでは基本的な React アプリを使用します。 このプロジェクトのリポジトリは、 link:https://github.com/CircleCI-Public/circleci-react-test-splitting-tutorial[GitHub] でご確認ください。 このアプリは、 link:https://create-react-app.dev/[React アプリの作成] を使って作成され、 link:https://jestjs.io/[Jest] のテストフレームワークを使用するように設定されています。 link:https://github.com/jest-community/jest-junit[jest-junit] Reporter を使ってテスト結果を JUnit XML ファイルとしてエクスポートします。
...
--
. GitHub で link:https://github.com/CircleCI-Public/circleci-react-test-splitting-tutorial/fork[リポジトリをフォーク] します。

. link:https://app.circleci.com[CircleCI Web アプリ] を開きます。 お客様個人の組織であることを確認し (`\https://app.circleci.com/pipelines/<vcs-provider><your-vcs-username>`)、 **Projects** に移動します。
...
--
. リポジトリの URL を使って、プロジェクトのコードをBitbucketにインポートします。: `https://github.com/CircleCI-Public/circleci-react-test-splitting-tutorial`

. link:https://app.circleci.com[CircleCI Web アプリ]を開いて、お客様個人の組織であることを確認し (`\https://app.circleci.com/pipelines/bitbucket/<your-vcs-username>`)、 **Projects** に移動します。
...
. アプリ内の設定エディターに移動します。このプロジェクトでは実際にYarnを使っているので、以下のステップを編集して、`npm`の代わりに`yarn`を使うようにします：
+
[source,yaml]
----
steps:
...
image::test-splitting-first-pipeline.png[Successful pipeline run]
+
緑色の Success ステータスを展開し、`build-and-test` ジョブを開いて、パイプラインで実行されたステップを自由に見てください。
+
image::test-splitting-first-setup-steps.png[Steps run successfully within the job]
--
...
--
. プロジェクトのコードを GitLab にインポートするには、こちらのリポジトリの URL を使用してください。: `https://github.com/CircleCI-Public/circleci-react-test-splitting-tutorial`

. link:https://app.circleci.com[CircleCI Web アプリ] を開いて、お客様個人の組織であることを確認し (`\https://app.circleci.com/pipelines/circleci/<project-slug>`)、 **Projects** に移動します。

. btn:[Create Project]をクリックします。

. ドロップダウンからサンプルプロジェクトリポジトリを選択します。

. config.yml ファイルを選択するよに求められます。そのウィンドウで、"Fast: Take me to a config.yml template that I can edit" オプションを選択します。**Set Up Project** をクリックします。

. アプリ内設定エディターに移動します。btn:[Change: Hello World]をクリックして設定テンプレートを変更します。すると、サンプル設定のリストが表示されます。ウィンドウを下にスクロールし、**Node (Advanced)**と表示されたら、btn:[Select]をクリックします。
+
image::test-splitting-sample-configs.png[Sample config for Node]

. このプロジェクトは実際にはYarnを使用しているので、`npm` の代わりに `yarn` を使用するように設定内の `build-and-test` ジョブの定義を編集する必要があります。アプリ内のエディタを使って、次のようにステップを編集します：
+
[source,yaml]
----
steps:
...
コードリポジトリのローカルコピーをダウンロードしたら、テキストエディターで下記の手順を実行し、`.circleci/config.yml` に変更を加えます。 また、GitHub または Bitbucket を使用している場合は、CircleCI ウェブアプリでブランチを選択し、**Edit Config** ボタンを選択すると、プロジェクトの設定ファイルを編集できます。
...
+
例えば、`circleci/node@5.0.2` のような新しいバージョンの Node orb にアップグレードしてみてください。あるいは、GitHub や Bitbucket を使っている場合は、Web アプリでプロジェクトの **ダッシュボード** にアクセスし、プロジェクトダッシュボードの btn:[Trigger Pipeline] をクリックして、パイプラインをトリガーすることもできます。
