...
CircleCI believes in *configuration as code*. Consequently, the entire delivery process from build to deploy is orchestrated through a single file called `config.yml`. The `config.yml` file is located in a folder called `.circleci` at the top of your repo project. CircleCI の設定には YAML 構文が使われます。 See the <<writing-yaml#,Writing YAML>> document for guidance on the basics.
...
. If you have not done so already, <<first-steps#,sign up with CircleCI>> and select either your Version Control System (VCS). You can also sign up with email.
...
* *Lines 32-33:* This line creates a Workflow called _Example-Workflow_. ワークフローは、ジョブのリストとその実行順序を定義します。
...
* *Lines 39-41:* The final job is _Using-Node_. As before, this job requires the successful completion of the previous job, _Fetch-Code_.
