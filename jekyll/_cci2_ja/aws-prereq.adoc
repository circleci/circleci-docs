...
- https://github.com[github.com] と連携する場合、ネットワークのアクセス制御リスト (ACL) で GitHub Webhook のポート80 および 443 が許可されていることを確認します。 GitHub と連携する場合は、CircleCI をパブリック サブネット内に準備するか、パブリック ロード バランサーを使用して github.com トラフィックを転送するように設定します。
