...
For the https://circleci.com/docs/2.0/using-docker[Docker executor], specify a username and password in the `auth` field of your https://circleci.com/docs/2.0/configuration-reference[config.yml] file. パスワードを保護したい場合は、 https://circleci.com/docs/ja/2.0/contexts/[コンテキスト] を作成するか、プロジェクトごとの環境変数を使用します。
