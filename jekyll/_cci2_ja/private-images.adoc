...
https://circleci.com/docs/ja/2.0/using-docker[Docker Executor] を使用する場合は、 https://circleci.com/docs/ja/2.0/configuration-reference[config.yml ファイル] の `auth` フィールドにユーザー名とパスワードを指定します。 パスワードを保護したい場合は、 https://circleci.com/docs/ja/2.0/contexts/[コンテキスト] を作成するか、プロジェクトごとの環境変数を使用します。
