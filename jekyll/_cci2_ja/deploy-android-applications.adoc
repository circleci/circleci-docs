...
- CircleCI での Android プロジェクトのセットアップCircleCI で初めてプロジェクトをセットアップする場合は、 <<getting-started#,クイックスタートガイド>> で詳細を確認してください。
...
. CircleCI で安全にアクセスできるようキーストアを Base64 に変換する必要があります。CircleCI で安全にアクセスできるようキーストアを Base64 に変換する必要があります。+ 
+
```shell
...
+
Open your Android project in the link:https://app.circleci.com/[CircleCI web app] and select *Project Settings*.*Environment Variables* に移動し、以下の変数を追加します。
+
...
- CircleCI を使ったデプロイの概要 xref:deployment-overview.adoc[Deployment overview] for a general introduction on deployment with CircleCI, as well as examples for specific deployment targets such as Google Cloud Platform, AWS, and Heroku. 
