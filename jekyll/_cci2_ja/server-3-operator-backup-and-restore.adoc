...
_Backup and Restore is not yet available on the current version of Server 3.x; however, it will be available for preview before the next release._
...
IMPORTANT: The restore will create new load balancers for CircleCI's services. その結果、  KOTS 管理者コンソールで DNS レコードまたはホスト名と IP の設定を更新する必要があります。 You may also need to consider updating the `nomad server endpoint` provided to your Nomad clients.
