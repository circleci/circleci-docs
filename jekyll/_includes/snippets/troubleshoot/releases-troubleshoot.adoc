...
=== Why all buttons (Promote, Promote All and Cancel) are disabled on a Release 

Check whether the Release is a Rollback. If this is the case, then this is a known issue that will be solved in one of the upcoming updates to the Release Agent.

[#commands-failing]
=== Why are all actions on a Component or Release for a given Component failing?

Check if the error message is “invalid or missing project ID“. In that case the Component is missing a valid Project ID.
