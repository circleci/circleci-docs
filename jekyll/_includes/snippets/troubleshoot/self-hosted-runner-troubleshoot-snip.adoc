...
NOTE: when `` is set to true, then task pods may dangle until they are manually cleaned or until garbage collection deletes these pods. By default, garbage collection removes resources after they have lived for 5 hours and 5 mins. You may tweak these settings however in your link:https://github.com/CircleCI-Public/container-runner-helm-chart/blob/main/values.yaml#L161-L175[container-agent values.yaml].
...
NOTE: The `Rerun job with SSH` feature is disabled by default. To enable this feature, see the xref:machine-runner-3-configuration-reference.adoc#runner-ssh-advertise-addr[machine runner configuration reference] or the xref:container-runner-installation.adoc#enable-rerun-job-with-ssh[container runner installation guide].
