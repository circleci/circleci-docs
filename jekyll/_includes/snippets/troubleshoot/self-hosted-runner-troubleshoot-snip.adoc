...
You can check the logs for any of the previous runs using the command `kubectl logs $(kubectl get pods -l app=container-agent -o jsonpath='{.items[0].metadata.name}' -n <namespace>) --previous -n <namespace>  >> container-runner.logs
`.
