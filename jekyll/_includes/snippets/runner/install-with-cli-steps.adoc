...
+
ifdef::container[]
NOTE: *If you are installing container runners on server, continue with steps 3 and 4 below.* Otherwise, skip to the next section.
endif::[]
ifdef::windows[]
NOTE: *If you are installing Windows machine runners on server, continue with steps 3 and 4 below.* Otherwise, skip to the next section.
endif::[]
