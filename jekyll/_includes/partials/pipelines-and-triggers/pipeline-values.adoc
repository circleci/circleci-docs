...
| GitHub App
| String
| Only populated for pipelines triggered xref:github-trigger-event-options/#supported-trigger-options[by pull request events] (`pipeline.event.name = "pull_request"`). It contains the name of the *head* branch of the PR - that is the branch containing the changes to be merged. 
| icon:check[]
| icon:times[]
