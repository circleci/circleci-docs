$(() => {
  ...
  window.OptimizelyClient.getVariationName({
    ...
    if (variation === 'treatment') {
      ...
      $('#orb-content-swap').replaceWith(
        '<a id=orb-kit href=https://circleci.com/docs/2.0/creating-orbs/#orb-development-kit>' +
          'Orb Development Kit' +
          '</a>',
      );

      $('#orb-intro, #orb-concepts, #orb-publish, #orb-faq, #orb-kit').click(
        function () {
          window.AnalyticsClient.trackAction('docs-orb-bullet', {
            link: this.id,
          });
        },
      );
