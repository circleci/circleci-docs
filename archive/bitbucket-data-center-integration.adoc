...
CircleCI CI/CD allows you to build, test and deploy your Bitbucket Data Center projects each time a developer pushes a change and when a pull request is merged.
...
The VCS Connections page under Org allows you to connect your organization with either Bitbucket Data Center or a GitLab self-managed instance. Steps to integrate with Bitbucket Data Center are described on the xref:getting-started:create-project.adoc[Create a project] page.
