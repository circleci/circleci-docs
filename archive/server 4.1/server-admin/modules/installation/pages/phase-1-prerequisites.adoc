...
CircleCI server v4.x is installed as a Helm chart. The installation process is broken down into four phases. There is a validation step at the end of each phase, allowing you to confirm success before moving to the next phase. Depending on your requirements, phases 3 and 4 may include multiple steps. This installation guide assumes you have already read the xref:overview:circleci-server-overview.adoc[CircleCI server v4.1 overview].
...
[.table.table-striped]
[cols="1,2,3,3", options="header", stripes=even]
|===
| Tool
| Version
| Used for
...
. In your browser, navigate to menu:Your-GitHub-instance[User Settings>Developer Settings>OAuth Apps] and select btn:[New OAuth App].
+
...
You can authenticate CircleCI with S3 in one of two ways:
