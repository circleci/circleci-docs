...
NOTE: An NGINX reverse proxy is placed in front of https://github.com/Kong/charts[Kong] and exposed as a Kubernetes service named `circleci-proxy`. NGINX is responsible routing the traffic to the following services: `kong`, `vm-service`, `output-processor` and `nomad`.
...
* https://kubernetes.io/docs/concepts/services-networking/network-policies/[Kubernetes Network Policy Overview]
...
ifndef::pdf[]
## Next steps
* link:/docs/server/installation/migrate-from-server-3-to-server-4[Migrate from Server v3.x to v4.0.x]
